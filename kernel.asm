
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 70 c6 10 80       	mov    $0x8010c670,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 ff 36 10 80       	mov    $0x801036ff,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 34 89 10 	movl   $0x80108934,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
80100049:	e8 04 4f 00 00       	call   80104f52 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 90 05 11 80 84 	movl   $0x80110584,0x80110590
80100055:	05 11 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 94 05 11 80 84 	movl   $0x80110584,0x80110594
8010005f:	05 11 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 b4 c6 10 80 	movl   $0x8010c6b4,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 94 05 11 80    	mov    0x80110594,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c 84 05 11 80 	movl   $0x80110584,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 94 05 11 80       	mov    0x80110594,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 94 05 11 80       	mov    %eax,0x80110594

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 84 05 11 80 	cmpl   $0x80110584,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate a buffer.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
801000bd:	e8 b1 4e 00 00       	call   80104f73 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 94 05 11 80       	mov    0x80110594,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	83 c8 01             	or     $0x1,%eax
801000f6:	89 c2                	mov    %eax,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
80100104:	e8 cc 4e 00 00       	call   80104fd5 <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 80 c6 10 	movl   $0x8010c680,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 34 4a 00 00       	call   80104b58 <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 84 05 11 80 	cmpl   $0x80110584,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
  }

  // Not cached; recycle some non-busy and clean buffer.
  // "clean" because B_DIRTY and !B_BUSY means log.c
  // hasn't yet committed the changes to the buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 90 05 11 80       	mov    0x80110590,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
8010017c:	e8 54 4e 00 00       	call   80104fd5 <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
  }

  // Not cached; recycle some non-busy and clean buffer.
  // "clean" because B_DIRTY and !B_BUSY means log.c
  // hasn't yet committed the changes to the buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 84 05 11 80 	cmpl   $0x80110584,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 3b 89 10 80 	movl   $0x8010893b,(%esp)
8010019f:	e8 96 03 00 00       	call   8010053a <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 c9 25 00 00       	call   801027a1 <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 4c 89 10 80 	movl   $0x8010894c,(%esp)
801001f6:	e8 3f 03 00 00       	call   8010053a <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	83 c8 04             	or     $0x4,%eax
80100203:	89 c2                	mov    %eax,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 8c 25 00 00       	call   801027a1 <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 53 89 10 80 	movl   $0x80108953,(%esp)
80100230:	e8 05 03 00 00       	call   8010053a <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
8010023c:	e8 32 4d 00 00       	call   80104f73 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 94 05 11 80    	mov    0x80110594,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c 84 05 11 80 	movl   $0x80110584,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 94 05 11 80       	mov    0x80110594,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 94 05 11 80       	mov    %eax,0x80110594

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	83 e0 fe             	and    $0xfffffffe,%eax
80100290:	89 c2                	mov    %eax,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 92 49 00 00       	call   80104c34 <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 80 c6 10 80 	movl   $0x8010c680,(%esp)
801002a9:	e8 27 4d 00 00       	call   80104fd5 <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	83 ec 14             	sub    $0x14,%esp
801002b6:	8b 45 08             	mov    0x8(%ebp),%eax
801002b9:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002bd:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
801002c1:	89 c2                	mov    %eax,%edx
801002c3:	ec                   	in     (%dx),%al
801002c4:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801002c7:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
801002cb:	c9                   	leave  
801002cc:	c3                   	ret    

801002cd <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002cd:	55                   	push   %ebp
801002ce:	89 e5                	mov    %esp,%ebp
801002d0:	83 ec 08             	sub    $0x8,%esp
801002d3:	8b 55 08             	mov    0x8(%ebp),%edx
801002d6:	8b 45 0c             	mov    0xc(%ebp),%eax
801002d9:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002dd:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002e0:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002e4:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002e8:	ee                   	out    %al,(%dx)
}
801002e9:	c9                   	leave  
801002ea:	c3                   	ret    

801002eb <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002eb:	55                   	push   %ebp
801002ec:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002ee:	fa                   	cli    
}
801002ef:	5d                   	pop    %ebp
801002f0:	c3                   	ret    

801002f1 <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002f1:	55                   	push   %ebp
801002f2:	89 e5                	mov    %esp,%ebp
801002f4:	56                   	push   %esi
801002f5:	53                   	push   %ebx
801002f6:	83 ec 30             	sub    $0x30,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
801002f9:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801002fd:	74 1c                	je     8010031b <printint+0x2a>
801002ff:	8b 45 08             	mov    0x8(%ebp),%eax
80100302:	c1 e8 1f             	shr    $0x1f,%eax
80100305:	0f b6 c0             	movzbl %al,%eax
80100308:	89 45 10             	mov    %eax,0x10(%ebp)
8010030b:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010030f:	74 0a                	je     8010031b <printint+0x2a>
    x = -xx;
80100311:	8b 45 08             	mov    0x8(%ebp),%eax
80100314:	f7 d8                	neg    %eax
80100316:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100319:	eb 06                	jmp    80100321 <printint+0x30>
  else
    x = xx;
8010031b:	8b 45 08             	mov    0x8(%ebp),%eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100321:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100328:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010032b:	8d 41 01             	lea    0x1(%ecx),%eax
8010032e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100331:	8b 5d 0c             	mov    0xc(%ebp),%ebx
80100334:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100337:	ba 00 00 00 00       	mov    $0x0,%edx
8010033c:	f7 f3                	div    %ebx
8010033e:	89 d0                	mov    %edx,%eax
80100340:	0f b6 80 04 90 10 80 	movzbl -0x7fef6ffc(%eax),%eax
80100347:	88 44 0d e0          	mov    %al,-0x20(%ebp,%ecx,1)
  }while((x /= base) != 0);
8010034b:	8b 75 0c             	mov    0xc(%ebp),%esi
8010034e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100351:	ba 00 00 00 00       	mov    $0x0,%edx
80100356:	f7 f6                	div    %esi
80100358:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010035b:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010035f:	75 c7                	jne    80100328 <printint+0x37>

  if(sign)
80100361:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100365:	74 10                	je     80100377 <printint+0x86>
    buf[i++] = '-';
80100367:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010036a:	8d 50 01             	lea    0x1(%eax),%edx
8010036d:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100370:	c6 44 05 e0 2d       	movb   $0x2d,-0x20(%ebp,%eax,1)

  while(--i >= 0)
80100375:	eb 18                	jmp    8010038f <printint+0x9e>
80100377:	eb 16                	jmp    8010038f <printint+0x9e>
    consputc(buf[i]);
80100379:	8d 55 e0             	lea    -0x20(%ebp),%edx
8010037c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010037f:	01 d0                	add    %edx,%eax
80100381:	0f b6 00             	movzbl (%eax),%eax
80100384:	0f be c0             	movsbl %al,%eax
80100387:	89 04 24             	mov    %eax,(%esp)
8010038a:	e8 c1 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
8010038f:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100393:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100397:	79 e0                	jns    80100379 <printint+0x88>
    consputc(buf[i]);
}
80100399:	83 c4 30             	add    $0x30,%esp
8010039c:	5b                   	pop    %ebx
8010039d:	5e                   	pop    %esi
8010039e:	5d                   	pop    %ebp
8010039f:	c3                   	ret    

801003a0 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a0:	55                   	push   %ebp
801003a1:	89 e5                	mov    %esp,%ebp
801003a3:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a6:	a1 14 b6 10 80       	mov    0x8010b614,%eax
801003ab:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003ae:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b2:	74 0c                	je     801003c0 <cprintf+0x20>
    acquire(&cons.lock);
801003b4:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
801003bb:	e8 b3 4b 00 00       	call   80104f73 <acquire>

  if (fmt == 0)
801003c0:	8b 45 08             	mov    0x8(%ebp),%eax
801003c3:	85 c0                	test   %eax,%eax
801003c5:	75 0c                	jne    801003d3 <cprintf+0x33>
    panic("null fmt");
801003c7:	c7 04 24 5a 89 10 80 	movl   $0x8010895a,(%esp)
801003ce:	e8 67 01 00 00       	call   8010053a <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d3:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003d9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e0:	e9 21 01 00 00       	jmp    80100506 <cprintf+0x166>
    if(c != '%'){
801003e5:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003e9:	74 10                	je     801003fb <cprintf+0x5b>
      consputc(c);
801003eb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ee:	89 04 24             	mov    %eax,(%esp)
801003f1:	e8 5a 03 00 00       	call   80100750 <consputc>
      continue;
801003f6:	e9 07 01 00 00       	jmp    80100502 <cprintf+0x162>
    }
    c = fmt[++i] & 0xff;
801003fb:	8b 55 08             	mov    0x8(%ebp),%edx
801003fe:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100402:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100405:	01 d0                	add    %edx,%eax
80100407:	0f b6 00             	movzbl (%eax),%eax
8010040a:	0f be c0             	movsbl %al,%eax
8010040d:	25 ff 00 00 00       	and    $0xff,%eax
80100412:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100415:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100419:	75 05                	jne    80100420 <cprintf+0x80>
      break;
8010041b:	e9 06 01 00 00       	jmp    80100526 <cprintf+0x186>
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4f                	je     80100477 <cprintf+0xd7>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0xa0>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13c>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xaf>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x14a>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 57                	je     8010049c <cprintf+0xfc>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2d                	je     80100477 <cprintf+0xd7>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x14a>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8d 50 04             	lea    0x4(%eax),%edx
80100455:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100458:	8b 00                	mov    (%eax),%eax
8010045a:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80100461:	00 
80100462:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100469:	00 
8010046a:	89 04 24             	mov    %eax,(%esp)
8010046d:	e8 7f fe ff ff       	call   801002f1 <printint>
      break;
80100472:	e9 8b 00 00 00       	jmp    80100502 <cprintf+0x162>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100477:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010047a:	8d 50 04             	lea    0x4(%eax),%edx
8010047d:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100480:	8b 00                	mov    (%eax),%eax
80100482:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100489:	00 
8010048a:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
80100491:	00 
80100492:	89 04 24             	mov    %eax,(%esp)
80100495:	e8 57 fe ff ff       	call   801002f1 <printint>
      break;
8010049a:	eb 66                	jmp    80100502 <cprintf+0x162>
    case 's':
      if((s = (char*)*argp++) == 0)
8010049c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049f:	8d 50 04             	lea    0x4(%eax),%edx
801004a2:	89 55 f0             	mov    %edx,-0x10(%ebp)
801004a5:	8b 00                	mov    (%eax),%eax
801004a7:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004aa:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004ae:	75 09                	jne    801004b9 <cprintf+0x119>
        s = "(null)";
801004b0:	c7 45 ec 63 89 10 80 	movl   $0x80108963,-0x14(%ebp)
      for(; *s; s++)
801004b7:	eb 17                	jmp    801004d0 <cprintf+0x130>
801004b9:	eb 15                	jmp    801004d0 <cprintf+0x130>
        consputc(*s);
801004bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004be:	0f b6 00             	movzbl (%eax),%eax
801004c1:	0f be c0             	movsbl %al,%eax
801004c4:	89 04 24             	mov    %eax,(%esp)
801004c7:	e8 84 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004cc:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 e1                	jne    801004bb <cprintf+0x11b>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x162>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x162>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 bf fe ff ff    	jne    801003e5 <cprintf+0x45>
      consputc(c);
      break;
    }
  }

  if(locking)
80100526:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052a:	74 0c                	je     80100538 <cprintf+0x198>
    release(&cons.lock);
8010052c:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
80100533:	e8 9d 4a 00 00       	call   80104fd5 <release>
}
80100538:	c9                   	leave  
80100539:	c3                   	ret    

8010053a <panic>:

void
panic(char *s)
{
8010053a:	55                   	push   %ebp
8010053b:	89 e5                	mov    %esp,%ebp
8010053d:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100540:	e8 a6 fd ff ff       	call   801002eb <cli>
  cons.locking = 0;
80100545:	c7 05 14 b6 10 80 00 	movl   $0x0,0x8010b614
8010054c:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
8010054f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100555:	0f b6 00             	movzbl (%eax),%eax
80100558:	0f b6 c0             	movzbl %al,%eax
8010055b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010055f:	c7 04 24 6a 89 10 80 	movl   $0x8010896a,(%esp)
80100566:	e8 35 fe ff ff       	call   801003a0 <cprintf>
  cprintf(s);
8010056b:	8b 45 08             	mov    0x8(%ebp),%eax
8010056e:	89 04 24             	mov    %eax,(%esp)
80100571:	e8 2a fe ff ff       	call   801003a0 <cprintf>
  cprintf("\n");
80100576:	c7 04 24 79 89 10 80 	movl   $0x80108979,(%esp)
8010057d:	e8 1e fe ff ff       	call   801003a0 <cprintf>
  getcallerpcs(&s, pcs);
80100582:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100585:	89 44 24 04          	mov    %eax,0x4(%esp)
80100589:	8d 45 08             	lea    0x8(%ebp),%eax
8010058c:	89 04 24             	mov    %eax,(%esp)
8010058f:	e8 90 4a 00 00       	call   80105024 <getcallerpcs>
  for(i=0; i<10; i++)
80100594:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059b:	eb 1b                	jmp    801005b8 <panic+0x7e>
    cprintf(" %p", pcs[i]);
8010059d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a0:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801005a8:	c7 04 24 7b 89 10 80 	movl   $0x8010897b,(%esp)
801005af:	e8 ec fd ff ff       	call   801003a0 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b4:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005b8:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bc:	7e df                	jle    8010059d <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005be:	c7 05 c0 b5 10 80 01 	movl   $0x1,0x8010b5c0
801005c5:	00 00 00 
  for(;;)
    ;
801005c8:	eb fe                	jmp    801005c8 <panic+0x8e>

801005ca <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005ca:	55                   	push   %ebp
801005cb:	89 e5                	mov    %esp,%ebp
801005cd:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d0:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005d7:	00 
801005d8:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005df:	e8 e9 fc ff ff       	call   801002cd <outb>
  pos = inb(CRTPORT+1) << 8;
801005e4:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005eb:	e8 c0 fc ff ff       	call   801002b0 <inb>
801005f0:	0f b6 c0             	movzbl %al,%eax
801005f3:	c1 e0 08             	shl    $0x8,%eax
801005f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005f9:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100600:	00 
80100601:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100608:	e8 c0 fc ff ff       	call   801002cd <outb>
  pos |= inb(CRTPORT+1);
8010060d:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100614:	e8 97 fc ff ff       	call   801002b0 <inb>
80100619:	0f b6 c0             	movzbl %al,%eax
8010061c:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
8010061f:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100623:	75 30                	jne    80100655 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100625:	8b 4d f4             	mov    -0xc(%ebp),%ecx
80100628:	ba 67 66 66 66       	mov    $0x66666667,%edx
8010062d:	89 c8                	mov    %ecx,%eax
8010062f:	f7 ea                	imul   %edx
80100631:	c1 fa 05             	sar    $0x5,%edx
80100634:	89 c8                	mov    %ecx,%eax
80100636:	c1 f8 1f             	sar    $0x1f,%eax
80100639:	29 c2                	sub    %eax,%edx
8010063b:	89 d0                	mov    %edx,%eax
8010063d:	c1 e0 02             	shl    $0x2,%eax
80100640:	01 d0                	add    %edx,%eax
80100642:	c1 e0 04             	shl    $0x4,%eax
80100645:	29 c1                	sub    %eax,%ecx
80100647:	89 ca                	mov    %ecx,%edx
80100649:	b8 50 00 00 00       	mov    $0x50,%eax
8010064e:	29 d0                	sub    %edx,%eax
80100650:	01 45 f4             	add    %eax,-0xc(%ebp)
80100653:	eb 35                	jmp    8010068a <cgaputc+0xc0>
  else if(c == BACKSPACE){
80100655:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065c:	75 0c                	jne    8010066a <cgaputc+0xa0>
    if(pos > 0) --pos;
8010065e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100662:	7e 26                	jle    8010068a <cgaputc+0xc0>
80100664:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100668:	eb 20                	jmp    8010068a <cgaputc+0xc0>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066a:	8b 0d 00 90 10 80    	mov    0x80109000,%ecx
80100670:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100673:	8d 50 01             	lea    0x1(%eax),%edx
80100676:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100679:	01 c0                	add    %eax,%eax
8010067b:	8d 14 01             	lea    (%ecx,%eax,1),%edx
8010067e:	8b 45 08             	mov    0x8(%ebp),%eax
80100681:	0f b6 c0             	movzbl %al,%eax
80100684:	80 cc 07             	or     $0x7,%ah
80100687:	66 89 02             	mov    %ax,(%edx)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x11c>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 90 10 80       	mov    0x80109000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 90 10 80       	mov    0x80109000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 df 4b 00 00       	call   80105296 <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	8d 14 00             	lea    (%eax,%eax,1),%edx
801006c6:	a1 00 90 10 80       	mov    0x80109000,%eax
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 c8                	add    %ecx,%eax
801006d2:	89 54 24 08          	mov    %edx,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 04 24             	mov    %eax,(%esp)
801006e1:	e8 e1 4a 00 00       	call   801051c7 <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 d3 fb ff ff       	call   801002cd <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 ba fb ff ff       	call   801002cd <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 a6 fb ff ff       	call   801002cd <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 90 fb ff ff       	call   801002cd <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 c0 b5 10 80       	mov    0x8010b5c0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 87 fb ff ff       	call   801002eb <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 fb 67 00 00       	call   80106f76 <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 ef 67 00 00       	call   80106f76 <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 e3 67 00 00       	call   80106f76 <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 d6 67 00 00       	call   80106f76 <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 1f fe ff ff       	call   801005ca <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 a0 07 11 80 	movl   $0x801107a0,(%esp)
801007ba:	e8 b4 47 00 00       	call   80104f73 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 37 01 00 00       	jmp    801008fb <consoleintr+0x14e>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 64                	je     8010083a <consoleintr+0x8d>
801007d6:	e9 91 00 00 00       	jmp    8010086c <consoleintr+0xbf>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 55                	je     8010083a <consoleintr+0x8d>
801007e5:	e9 82 00 00 00       	jmp    8010086c <consoleintr+0xbf>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 eb 44 00 00       	call   80104cda <procdump>
      break;
801007ef:	e9 07 01 00 00       	jmp    801008fb <consoleintr+0x14e>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 5c 08 11 80       	mov    0x8011085c,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 5c 08 11 80       	mov    %eax,0x8011085c
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 5c 08 11 80    	mov    0x8011085c,%edx
80100816:	a1 58 08 11 80       	mov    0x80110858,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	74 16                	je     80100835 <consoleintr+0x88>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
8010081f:	a1 5c 08 11 80       	mov    0x8011085c,%eax
80100824:	83 e8 01             	sub    $0x1,%eax
80100827:	83 e0 7f             	and    $0x7f,%eax
8010082a:	0f b6 80 d4 07 11 80 	movzbl -0x7feef82c(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100831:	3c 0a                	cmp    $0xa,%al
80100833:	75 bf                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100835:	e9 c1 00 00 00       	jmp    801008fb <consoleintr+0x14e>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083a:	8b 15 5c 08 11 80    	mov    0x8011085c,%edx
80100840:	a1 58 08 11 80       	mov    0x80110858,%eax
80100845:	39 c2                	cmp    %eax,%edx
80100847:	74 1e                	je     80100867 <consoleintr+0xba>
        input.e--;
80100849:	a1 5c 08 11 80       	mov    0x8011085c,%eax
8010084e:	83 e8 01             	sub    $0x1,%eax
80100851:	a3 5c 08 11 80       	mov    %eax,0x8011085c
        consputc(BACKSPACE);
80100856:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
8010085d:	e8 ee fe ff ff       	call   80100750 <consputc>
      }
      break;
80100862:	e9 94 00 00 00       	jmp    801008fb <consoleintr+0x14e>
80100867:	e9 8f 00 00 00       	jmp    801008fb <consoleintr+0x14e>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100870:	0f 84 84 00 00 00    	je     801008fa <consoleintr+0x14d>
80100876:	8b 15 5c 08 11 80    	mov    0x8011085c,%edx
8010087c:	a1 54 08 11 80       	mov    0x80110854,%eax
80100881:	29 c2                	sub    %eax,%edx
80100883:	89 d0                	mov    %edx,%eax
80100885:	83 f8 7f             	cmp    $0x7f,%eax
80100888:	77 70                	ja     801008fa <consoleintr+0x14d>
        c = (c == '\r') ? '\n' : c;
8010088a:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
8010088e:	74 05                	je     80100895 <consoleintr+0xe8>
80100890:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100893:	eb 05                	jmp    8010089a <consoleintr+0xed>
80100895:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089a:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
8010089d:	a1 5c 08 11 80       	mov    0x8011085c,%eax
801008a2:	8d 50 01             	lea    0x1(%eax),%edx
801008a5:	89 15 5c 08 11 80    	mov    %edx,0x8011085c
801008ab:	83 e0 7f             	and    $0x7f,%eax
801008ae:	89 c2                	mov    %eax,%edx
801008b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008b3:	88 82 d4 07 11 80    	mov    %al,-0x7feef82c(%edx)
        consputc(c);
801008b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008bc:	89 04 24             	mov    %eax,(%esp)
801008bf:	e8 8c fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c4:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008c8:	74 18                	je     801008e2 <consoleintr+0x135>
801008ca:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008ce:	74 12                	je     801008e2 <consoleintr+0x135>
801008d0:	a1 5c 08 11 80       	mov    0x8011085c,%eax
801008d5:	8b 15 54 08 11 80    	mov    0x80110854,%edx
801008db:	83 ea 80             	sub    $0xffffff80,%edx
801008de:	39 d0                	cmp    %edx,%eax
801008e0:	75 18                	jne    801008fa <consoleintr+0x14d>
          input.w = input.e;
801008e2:	a1 5c 08 11 80       	mov    0x8011085c,%eax
801008e7:	a3 58 08 11 80       	mov    %eax,0x80110858
          wakeup(&input.r);
801008ec:	c7 04 24 54 08 11 80 	movl   $0x80110854,(%esp)
801008f3:	e8 3c 43 00 00       	call   80104c34 <wakeup>
        }
      }
      break;
801008f8:	eb 00                	jmp    801008fa <consoleintr+0x14d>
801008fa:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
801008fb:	8b 45 08             	mov    0x8(%ebp),%eax
801008fe:	ff d0                	call   *%eax
80100900:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100903:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100907:	0f 89 b7 fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
8010090d:	c7 04 24 a0 07 11 80 	movl   $0x801107a0,(%esp)
80100914:	e8 bc 46 00 00       	call   80104fd5 <release>
}
80100919:	c9                   	leave  
8010091a:	c3                   	ret    

8010091b <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
8010091b:	55                   	push   %ebp
8010091c:	89 e5                	mov    %esp,%ebp
8010091e:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
80100921:	8b 45 08             	mov    0x8(%ebp),%eax
80100924:	89 04 24             	mov    %eax,(%esp)
80100927:	e8 7d 10 00 00       	call   801019a9 <iunlock>
  target = n;
8010092c:	8b 45 10             	mov    0x10(%ebp),%eax
8010092f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
80100932:	c7 04 24 a0 07 11 80 	movl   $0x801107a0,(%esp)
80100939:	e8 35 46 00 00       	call   80104f73 <acquire>
  while(n > 0){
8010093e:	e9 aa 00 00 00       	jmp    801009ed <consoleread+0xd2>
    while(input.r == input.w){
80100943:	eb 42                	jmp    80100987 <consoleread+0x6c>
      if(proc->killed){
80100945:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010094b:	8b 40 24             	mov    0x24(%eax),%eax
8010094e:	85 c0                	test   %eax,%eax
80100950:	74 21                	je     80100973 <consoleread+0x58>
        release(&input.lock);
80100952:	c7 04 24 a0 07 11 80 	movl   $0x801107a0,(%esp)
80100959:	e8 77 46 00 00       	call   80104fd5 <release>
        ilock(ip);
8010095e:	8b 45 08             	mov    0x8(%ebp),%eax
80100961:	89 04 24             	mov    %eax,(%esp)
80100964:	e8 f2 0e 00 00       	call   8010185b <ilock>
        return -1;
80100969:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010096e:	e9 a5 00 00 00       	jmp    80100a18 <consoleread+0xfd>
      }
      sleep(&input.r, &input.lock);
80100973:	c7 44 24 04 a0 07 11 	movl   $0x801107a0,0x4(%esp)
8010097a:	80 
8010097b:	c7 04 24 54 08 11 80 	movl   $0x80110854,(%esp)
80100982:	e8 d1 41 00 00       	call   80104b58 <sleep>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100987:	8b 15 54 08 11 80    	mov    0x80110854,%edx
8010098d:	a1 58 08 11 80       	mov    0x80110858,%eax
80100992:	39 c2                	cmp    %eax,%edx
80100994:	74 af                	je     80100945 <consoleread+0x2a>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
80100996:	a1 54 08 11 80       	mov    0x80110854,%eax
8010099b:	8d 50 01             	lea    0x1(%eax),%edx
8010099e:	89 15 54 08 11 80    	mov    %edx,0x80110854
801009a4:	83 e0 7f             	and    $0x7f,%eax
801009a7:	0f b6 80 d4 07 11 80 	movzbl -0x7feef82c(%eax),%eax
801009ae:	0f be c0             	movsbl %al,%eax
801009b1:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(c == C('D')){  // EOF
801009b4:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009b8:	75 19                	jne    801009d3 <consoleread+0xb8>
      if(n < target){
801009ba:	8b 45 10             	mov    0x10(%ebp),%eax
801009bd:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009c0:	73 0f                	jae    801009d1 <consoleread+0xb6>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009c2:	a1 54 08 11 80       	mov    0x80110854,%eax
801009c7:	83 e8 01             	sub    $0x1,%eax
801009ca:	a3 54 08 11 80       	mov    %eax,0x80110854
      }
      break;
801009cf:	eb 26                	jmp    801009f7 <consoleread+0xdc>
801009d1:	eb 24                	jmp    801009f7 <consoleread+0xdc>
    }
    *dst++ = c;
801009d3:	8b 45 0c             	mov    0xc(%ebp),%eax
801009d6:	8d 50 01             	lea    0x1(%eax),%edx
801009d9:	89 55 0c             	mov    %edx,0xc(%ebp)
801009dc:	8b 55 f0             	mov    -0x10(%ebp),%edx
801009df:	88 10                	mov    %dl,(%eax)
    --n;
801009e1:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009e5:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009e9:	75 02                	jne    801009ed <consoleread+0xd2>
      break;
801009eb:	eb 0a                	jmp    801009f7 <consoleread+0xdc>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009ed:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f1:	0f 8f 4c ff ff ff    	jg     80100943 <consoleread+0x28>
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
  }
  release(&input.lock);
801009f7:	c7 04 24 a0 07 11 80 	movl   $0x801107a0,(%esp)
801009fe:	e8 d2 45 00 00       	call   80104fd5 <release>
  ilock(ip);
80100a03:	8b 45 08             	mov    0x8(%ebp),%eax
80100a06:	89 04 24             	mov    %eax,(%esp)
80100a09:	e8 4d 0e 00 00       	call   8010185b <ilock>

  return target - n;
80100a0e:	8b 45 10             	mov    0x10(%ebp),%eax
80100a11:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a14:	29 c2                	sub    %eax,%edx
80100a16:	89 d0                	mov    %edx,%eax
}
80100a18:	c9                   	leave  
80100a19:	c3                   	ret    

80100a1a <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a1a:	55                   	push   %ebp
80100a1b:	89 e5                	mov    %esp,%ebp
80100a1d:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a20:	8b 45 08             	mov    0x8(%ebp),%eax
80100a23:	89 04 24             	mov    %eax,(%esp)
80100a26:	e8 7e 0f 00 00       	call   801019a9 <iunlock>
  acquire(&cons.lock);
80100a2b:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
80100a32:	e8 3c 45 00 00       	call   80104f73 <acquire>
  for(i = 0; i < n; i++)
80100a37:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a3e:	eb 1d                	jmp    80100a5d <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a40:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a43:	8b 45 0c             	mov    0xc(%ebp),%eax
80100a46:	01 d0                	add    %edx,%eax
80100a48:	0f b6 00             	movzbl (%eax),%eax
80100a4b:	0f be c0             	movsbl %al,%eax
80100a4e:	0f b6 c0             	movzbl %al,%eax
80100a51:	89 04 24             	mov    %eax,(%esp)
80100a54:	e8 f7 fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a59:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a60:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a63:	7c db                	jl     80100a40 <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a65:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
80100a6c:	e8 64 45 00 00       	call   80104fd5 <release>
  ilock(ip);
80100a71:	8b 45 08             	mov    0x8(%ebp),%eax
80100a74:	89 04 24             	mov    %eax,(%esp)
80100a77:	e8 df 0d 00 00       	call   8010185b <ilock>

  return n;
80100a7c:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a7f:	c9                   	leave  
80100a80:	c3                   	ret    

80100a81 <consoleinit>:

void
consoleinit(void)
{
80100a81:	55                   	push   %ebp
80100a82:	89 e5                	mov    %esp,%ebp
80100a84:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a87:	c7 44 24 04 7f 89 10 	movl   $0x8010897f,0x4(%esp)
80100a8e:	80 
80100a8f:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
80100a96:	e8 b7 44 00 00       	call   80104f52 <initlock>
  initlock(&input.lock, "input");
80100a9b:	c7 44 24 04 87 89 10 	movl   $0x80108987,0x4(%esp)
80100aa2:	80 
80100aa3:	c7 04 24 a0 07 11 80 	movl   $0x801107a0,(%esp)
80100aaa:	e8 a3 44 00 00       	call   80104f52 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100aaf:	c7 05 0c 12 11 80 1a 	movl   $0x80100a1a,0x8011120c
80100ab6:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ab9:	c7 05 08 12 11 80 1b 	movl   $0x8010091b,0x80111208
80100ac0:	09 10 80 
  cons.locking = 1;
80100ac3:	c7 05 14 b6 10 80 01 	movl   $0x1,0x8010b614
80100aca:	00 00 00 

  picenable(IRQ_KBD);
80100acd:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ad4:	e8 c3 32 00 00       	call   80103d9c <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ad9:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100ae0:	00 
80100ae1:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae8:	e8 70 1e 00 00       	call   8010295d <ioapicenable>
}
80100aed:	c9                   	leave  
80100aee:	c3                   	ret    

80100aef <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100aef:	55                   	push   %ebp
80100af0:	89 e5                	mov    %esp,%ebp
80100af2:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  begin_op();
80100af8:	e8 13 29 00 00       	call   80103410 <begin_op>
  if((ip = namei(path)) == 0){
80100afd:	8b 45 08             	mov    0x8(%ebp),%eax
80100b00:	89 04 24             	mov    %eax,(%esp)
80100b03:	e8 fe 18 00 00       	call   80102406 <namei>
80100b08:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b0b:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b0f:	75 0f                	jne    80100b20 <exec+0x31>
    end_op();
80100b11:	e8 7e 29 00 00       	call   80103494 <end_op>
    return -1;
80100b16:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1b:	e9 e8 03 00 00       	jmp    80100f08 <exec+0x419>
  }
  ilock(ip);
80100b20:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b23:	89 04 24             	mov    %eax,(%esp)
80100b26:	e8 30 0d 00 00       	call   8010185b <ilock>
  pgdir = 0;
80100b2b:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b32:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b39:	00 
80100b3a:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b41:	00 
80100b42:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b48:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4c:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b4f:	89 04 24             	mov    %eax,(%esp)
80100b52:	e8 11 12 00 00       	call   80101d68 <readi>
80100b57:	83 f8 33             	cmp    $0x33,%eax
80100b5a:	77 05                	ja     80100b61 <exec+0x72>
    goto bad;
80100b5c:	e9 7b 03 00 00       	jmp    80100edc <exec+0x3ed>
  if(elf.magic != ELF_MAGIC)
80100b61:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b67:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6c:	74 05                	je     80100b73 <exec+0x84>
    goto bad;
80100b6e:	e9 69 03 00 00       	jmp    80100edc <exec+0x3ed>

  if((pgdir = setupkvm()) == 0)
80100b73:	e8 4f 75 00 00       	call   801080c7 <setupkvm>
80100b78:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b7b:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b7f:	75 05                	jne    80100b86 <exec+0x97>
    goto bad;
80100b81:	e9 56 03 00 00       	jmp    80100edc <exec+0x3ed>

  // Load program into memory.
  sz = 0;
80100b86:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b8d:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b94:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100b9a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100b9d:	e9 cb 00 00 00       	jmp    80100c6d <exec+0x17e>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba2:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100ba5:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bac:	00 
80100bad:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb1:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bb7:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bbb:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bbe:	89 04 24             	mov    %eax,(%esp)
80100bc1:	e8 a2 11 00 00       	call   80101d68 <readi>
80100bc6:	83 f8 20             	cmp    $0x20,%eax
80100bc9:	74 05                	je     80100bd0 <exec+0xe1>
      goto bad;
80100bcb:	e9 0c 03 00 00       	jmp    80100edc <exec+0x3ed>
    if(ph.type != ELF_PROG_LOAD)
80100bd0:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bd6:	83 f8 01             	cmp    $0x1,%eax
80100bd9:	74 05                	je     80100be0 <exec+0xf1>
      continue;
80100bdb:	e9 80 00 00 00       	jmp    80100c60 <exec+0x171>
    if(ph.memsz < ph.filesz)
80100be0:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be6:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100bec:	39 c2                	cmp    %eax,%edx
80100bee:	73 05                	jae    80100bf5 <exec+0x106>
      goto bad;
80100bf0:	e9 e7 02 00 00       	jmp    80100edc <exec+0x3ed>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bf5:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bfb:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100c01:	01 d0                	add    %edx,%eax
80100c03:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c0e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c11:	89 04 24             	mov    %eax,(%esp)
80100c14:	e8 7c 78 00 00       	call   80108495 <allocuvm>
80100c19:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c1c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c20:	75 05                	jne    80100c27 <exec+0x138>
      goto bad;
80100c22:	e9 b5 02 00 00       	jmp    80100edc <exec+0x3ed>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c27:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c2d:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c33:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c39:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c3d:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c41:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c44:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c48:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c4c:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c4f:	89 04 24             	mov    %eax,(%esp)
80100c52:	e8 53 77 00 00       	call   801083aa <loaduvm>
80100c57:	85 c0                	test   %eax,%eax
80100c59:	79 05                	jns    80100c60 <exec+0x171>
      goto bad;
80100c5b:	e9 7c 02 00 00       	jmp    80100edc <exec+0x3ed>
  if((pgdir = setupkvm()) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c60:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c64:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c67:	83 c0 20             	add    $0x20,%eax
80100c6a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c6d:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c74:	0f b7 c0             	movzwl %ax,%eax
80100c77:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c7a:	0f 8f 22 ff ff ff    	jg     80100ba2 <exec+0xb3>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c80:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c83:	89 04 24             	mov    %eax,(%esp)
80100c86:	e8 54 0e 00 00       	call   80101adf <iunlockput>
  end_op();
80100c8b:	e8 04 28 00 00       	call   80103494 <end_op>
  ip = 0;
80100c90:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c97:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c9a:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c9f:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100ca4:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100ca7:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100caa:	05 00 20 00 00       	add    $0x2000,%eax
80100caf:	89 44 24 08          	mov    %eax,0x8(%esp)
80100cb3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cb6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cba:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cbd:	89 04 24             	mov    %eax,(%esp)
80100cc0:	e8 d0 77 00 00       	call   80108495 <allocuvm>
80100cc5:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cc8:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100ccc:	75 05                	jne    80100cd3 <exec+0x1e4>
    goto bad;
80100cce:	e9 09 02 00 00       	jmp    80100edc <exec+0x3ed>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cd3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cd6:	2d 00 20 00 00       	sub    $0x2000,%eax
80100cdb:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cdf:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ce2:	89 04 24             	mov    %eax,(%esp)
80100ce5:	e8 db 79 00 00       	call   801086c5 <clearpteu>
  sp = sz;
80100cea:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ced:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100cf0:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100cf7:	e9 9a 00 00 00       	jmp    80100d96 <exec+0x2a7>
    if(argc >= MAXARG)
80100cfc:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100d00:	76 05                	jbe    80100d07 <exec+0x218>
      goto bad;
80100d02:	e9 d5 01 00 00       	jmp    80100edc <exec+0x3ed>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100d07:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d0a:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100d11:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d14:	01 d0                	add    %edx,%eax
80100d16:	8b 00                	mov    (%eax),%eax
80100d18:	89 04 24             	mov    %eax,(%esp)
80100d1b:	e8 11 47 00 00       	call   80105431 <strlen>
80100d20:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100d23:	29 c2                	sub    %eax,%edx
80100d25:	89 d0                	mov    %edx,%eax
80100d27:	83 e8 01             	sub    $0x1,%eax
80100d2a:	83 e0 fc             	and    $0xfffffffc,%eax
80100d2d:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d30:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d33:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100d3a:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d3d:	01 d0                	add    %edx,%eax
80100d3f:	8b 00                	mov    (%eax),%eax
80100d41:	89 04 24             	mov    %eax,(%esp)
80100d44:	e8 e8 46 00 00       	call   80105431 <strlen>
80100d49:	83 c0 01             	add    $0x1,%eax
80100d4c:	89 c2                	mov    %eax,%edx
80100d4e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d51:	8d 0c 85 00 00 00 00 	lea    0x0(,%eax,4),%ecx
80100d58:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d5b:	01 c8                	add    %ecx,%eax
80100d5d:	8b 00                	mov    (%eax),%eax
80100d5f:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d63:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d67:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d6a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d6e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d71:	89 04 24             	mov    %eax,(%esp)
80100d74:	e8 11 7b 00 00       	call   8010888a <copyout>
80100d79:	85 c0                	test   %eax,%eax
80100d7b:	79 05                	jns    80100d82 <exec+0x293>
      goto bad;
80100d7d:	e9 5a 01 00 00       	jmp    80100edc <exec+0x3ed>
    ustack[3+argc] = sp;
80100d82:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d85:	8d 50 03             	lea    0x3(%eax),%edx
80100d88:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d8b:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d92:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d96:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d99:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100da0:	8b 45 0c             	mov    0xc(%ebp),%eax
80100da3:	01 d0                	add    %edx,%eax
80100da5:	8b 00                	mov    (%eax),%eax
80100da7:	85 c0                	test   %eax,%eax
80100da9:	0f 85 4d ff ff ff    	jne    80100cfc <exec+0x20d>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100daf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100db2:	83 c0 03             	add    $0x3,%eax
80100db5:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100dbc:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100dc0:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100dc7:	ff ff ff 
  ustack[1] = argc;
80100dca:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dcd:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100dd3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dd6:	83 c0 01             	add    $0x1,%eax
80100dd9:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100de0:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100de3:	29 d0                	sub    %edx,%eax
80100de5:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100deb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dee:	83 c0 04             	add    $0x4,%eax
80100df1:	c1 e0 02             	shl    $0x2,%eax
80100df4:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100df7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dfa:	83 c0 04             	add    $0x4,%eax
80100dfd:	c1 e0 02             	shl    $0x2,%eax
80100e00:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100e04:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100e0a:	89 44 24 08          	mov    %eax,0x8(%esp)
80100e0e:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100e11:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e15:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100e18:	89 04 24             	mov    %eax,(%esp)
80100e1b:	e8 6a 7a 00 00       	call   8010888a <copyout>
80100e20:	85 c0                	test   %eax,%eax
80100e22:	79 05                	jns    80100e29 <exec+0x33a>
    goto bad;
80100e24:	e9 b3 00 00 00       	jmp    80100edc <exec+0x3ed>

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e29:	8b 45 08             	mov    0x8(%ebp),%eax
80100e2c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e32:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e35:	eb 17                	jmp    80100e4e <exec+0x35f>
    if(*s == '/')
80100e37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e3a:	0f b6 00             	movzbl (%eax),%eax
80100e3d:	3c 2f                	cmp    $0x2f,%al
80100e3f:	75 09                	jne    80100e4a <exec+0x35b>
      last = s+1;
80100e41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e44:	83 c0 01             	add    $0x1,%eax
80100e47:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e4a:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e4e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e51:	0f b6 00             	movzbl (%eax),%eax
80100e54:	84 c0                	test   %al,%al
80100e56:	75 df                	jne    80100e37 <exec+0x348>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e58:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e5e:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e61:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e68:	00 
80100e69:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e6c:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e70:	89 14 24             	mov    %edx,(%esp)
80100e73:	e8 6f 45 00 00       	call   801053e7 <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e78:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e7e:	8b 40 04             	mov    0x4(%eax),%eax
80100e81:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e84:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e8a:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e8d:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e90:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e96:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e99:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e9b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100ea1:	8b 40 18             	mov    0x18(%eax),%eax
80100ea4:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100eaa:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100ead:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100eb3:	8b 40 18             	mov    0x18(%eax),%eax
80100eb6:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100eb9:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100ebc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100ec2:	89 04 24             	mov    %eax,(%esp)
80100ec5:	e8 ee 72 00 00       	call   801081b8 <switchuvm>
  freevm(oldpgdir);
80100eca:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ecd:	89 04 24             	mov    %eax,(%esp)
80100ed0:	e8 56 77 00 00       	call   8010862b <freevm>
  return 0;
80100ed5:	b8 00 00 00 00       	mov    $0x0,%eax
80100eda:	eb 2c                	jmp    80100f08 <exec+0x419>

 bad:
  if(pgdir)
80100edc:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100ee0:	74 0b                	je     80100eed <exec+0x3fe>
    freevm(pgdir);
80100ee2:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ee5:	89 04 24             	mov    %eax,(%esp)
80100ee8:	e8 3e 77 00 00       	call   8010862b <freevm>
  if(ip){
80100eed:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100ef1:	74 10                	je     80100f03 <exec+0x414>
    iunlockput(ip);
80100ef3:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ef6:	89 04 24             	mov    %eax,(%esp)
80100ef9:	e8 e1 0b 00 00       	call   80101adf <iunlockput>
    end_op();
80100efe:	e8 91 25 00 00       	call   80103494 <end_op>
  }
  return -1;
80100f03:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100f08:	c9                   	leave  
80100f09:	c3                   	ret    

80100f0a <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100f0a:	55                   	push   %ebp
80100f0b:	89 e5                	mov    %esp,%ebp
80100f0d:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100f10:	c7 44 24 04 8d 89 10 	movl   $0x8010898d,0x4(%esp)
80100f17:	80 
80100f18:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100f1f:	e8 2e 40 00 00       	call   80104f52 <initlock>
}
80100f24:	c9                   	leave  
80100f25:	c3                   	ret    

80100f26 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f26:	55                   	push   %ebp
80100f27:	89 e5                	mov    %esp,%ebp
80100f29:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f2c:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100f33:	e8 3b 40 00 00       	call   80104f73 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f38:	c7 45 f4 94 08 11 80 	movl   $0x80110894,-0xc(%ebp)
80100f3f:	eb 29                	jmp    80100f6a <filealloc+0x44>
    if(f->ref == 0){
80100f41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f44:	8b 40 04             	mov    0x4(%eax),%eax
80100f47:	85 c0                	test   %eax,%eax
80100f49:	75 1b                	jne    80100f66 <filealloc+0x40>
      f->ref = 1;
80100f4b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f4e:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f55:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100f5c:	e8 74 40 00 00       	call   80104fd5 <release>
      return f;
80100f61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f64:	eb 1e                	jmp    80100f84 <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f66:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f6a:	81 7d f4 f4 11 11 80 	cmpl   $0x801111f4,-0xc(%ebp)
80100f71:	72 ce                	jb     80100f41 <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f73:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100f7a:	e8 56 40 00 00       	call   80104fd5 <release>
  return 0;
80100f7f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f84:	c9                   	leave  
80100f85:	c3                   	ret    

80100f86 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f86:	55                   	push   %ebp
80100f87:	89 e5                	mov    %esp,%ebp
80100f89:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f8c:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100f93:	e8 db 3f 00 00       	call   80104f73 <acquire>
  if(f->ref < 1)
80100f98:	8b 45 08             	mov    0x8(%ebp),%eax
80100f9b:	8b 40 04             	mov    0x4(%eax),%eax
80100f9e:	85 c0                	test   %eax,%eax
80100fa0:	7f 0c                	jg     80100fae <filedup+0x28>
    panic("filedup");
80100fa2:	c7 04 24 94 89 10 80 	movl   $0x80108994,(%esp)
80100fa9:	e8 8c f5 ff ff       	call   8010053a <panic>
  f->ref++;
80100fae:	8b 45 08             	mov    0x8(%ebp),%eax
80100fb1:	8b 40 04             	mov    0x4(%eax),%eax
80100fb4:	8d 50 01             	lea    0x1(%eax),%edx
80100fb7:	8b 45 08             	mov    0x8(%ebp),%eax
80100fba:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fbd:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100fc4:	e8 0c 40 00 00       	call   80104fd5 <release>
  return f;
80100fc9:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fcc:	c9                   	leave  
80100fcd:	c3                   	ret    

80100fce <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fce:	55                   	push   %ebp
80100fcf:	89 e5                	mov    %esp,%ebp
80100fd1:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fd4:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80100fdb:	e8 93 3f 00 00       	call   80104f73 <acquire>
  if(f->ref < 1)
80100fe0:	8b 45 08             	mov    0x8(%ebp),%eax
80100fe3:	8b 40 04             	mov    0x4(%eax),%eax
80100fe6:	85 c0                	test   %eax,%eax
80100fe8:	7f 0c                	jg     80100ff6 <fileclose+0x28>
    panic("fileclose");
80100fea:	c7 04 24 9c 89 10 80 	movl   $0x8010899c,(%esp)
80100ff1:	e8 44 f5 ff ff       	call   8010053a <panic>
  if(--f->ref > 0){
80100ff6:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff9:	8b 40 04             	mov    0x4(%eax),%eax
80100ffc:	8d 50 ff             	lea    -0x1(%eax),%edx
80100fff:	8b 45 08             	mov    0x8(%ebp),%eax
80101002:	89 50 04             	mov    %edx,0x4(%eax)
80101005:	8b 45 08             	mov    0x8(%ebp),%eax
80101008:	8b 40 04             	mov    0x4(%eax),%eax
8010100b:	85 c0                	test   %eax,%eax
8010100d:	7e 11                	jle    80101020 <fileclose+0x52>
    release(&ftable.lock);
8010100f:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80101016:	e8 ba 3f 00 00       	call   80104fd5 <release>
8010101b:	e9 82 00 00 00       	jmp    801010a2 <fileclose+0xd4>
    return;
  }
  ff = *f;
80101020:	8b 45 08             	mov    0x8(%ebp),%eax
80101023:	8b 10                	mov    (%eax),%edx
80101025:	89 55 e0             	mov    %edx,-0x20(%ebp)
80101028:	8b 50 04             	mov    0x4(%eax),%edx
8010102b:	89 55 e4             	mov    %edx,-0x1c(%ebp)
8010102e:	8b 50 08             	mov    0x8(%eax),%edx
80101031:	89 55 e8             	mov    %edx,-0x18(%ebp)
80101034:	8b 50 0c             	mov    0xc(%eax),%edx
80101037:	89 55 ec             	mov    %edx,-0x14(%ebp)
8010103a:	8b 50 10             	mov    0x10(%eax),%edx
8010103d:	89 55 f0             	mov    %edx,-0x10(%ebp)
80101040:	8b 40 14             	mov    0x14(%eax),%eax
80101043:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
80101046:	8b 45 08             	mov    0x8(%ebp),%eax
80101049:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101050:	8b 45 08             	mov    0x8(%ebp),%eax
80101053:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
80101059:	c7 04 24 60 08 11 80 	movl   $0x80110860,(%esp)
80101060:	e8 70 3f 00 00       	call   80104fd5 <release>
  
  if(ff.type == FD_PIPE)
80101065:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101068:	83 f8 01             	cmp    $0x1,%eax
8010106b:	75 18                	jne    80101085 <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
8010106d:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
80101071:	0f be d0             	movsbl %al,%edx
80101074:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101077:	89 54 24 04          	mov    %edx,0x4(%esp)
8010107b:	89 04 24             	mov    %eax,(%esp)
8010107e:	e8 c9 2f 00 00       	call   8010404c <pipeclose>
80101083:	eb 1d                	jmp    801010a2 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
80101085:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101088:	83 f8 02             	cmp    $0x2,%eax
8010108b:	75 15                	jne    801010a2 <fileclose+0xd4>
    begin_op();
8010108d:	e8 7e 23 00 00       	call   80103410 <begin_op>
    iput(ff.ip);
80101092:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101095:	89 04 24             	mov    %eax,(%esp)
80101098:	e8 71 09 00 00       	call   80101a0e <iput>
    end_op();
8010109d:	e8 f2 23 00 00       	call   80103494 <end_op>
  }
}
801010a2:	c9                   	leave  
801010a3:	c3                   	ret    

801010a4 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
801010a4:	55                   	push   %ebp
801010a5:	89 e5                	mov    %esp,%ebp
801010a7:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801010aa:	8b 45 08             	mov    0x8(%ebp),%eax
801010ad:	8b 00                	mov    (%eax),%eax
801010af:	83 f8 02             	cmp    $0x2,%eax
801010b2:	75 38                	jne    801010ec <filestat+0x48>
    ilock(f->ip);
801010b4:	8b 45 08             	mov    0x8(%ebp),%eax
801010b7:	8b 40 10             	mov    0x10(%eax),%eax
801010ba:	89 04 24             	mov    %eax,(%esp)
801010bd:	e8 99 07 00 00       	call   8010185b <ilock>
    stati(f->ip, st);
801010c2:	8b 45 08             	mov    0x8(%ebp),%eax
801010c5:	8b 40 10             	mov    0x10(%eax),%eax
801010c8:	8b 55 0c             	mov    0xc(%ebp),%edx
801010cb:	89 54 24 04          	mov    %edx,0x4(%esp)
801010cf:	89 04 24             	mov    %eax,(%esp)
801010d2:	e8 4c 0c 00 00       	call   80101d23 <stati>
    iunlock(f->ip);
801010d7:	8b 45 08             	mov    0x8(%ebp),%eax
801010da:	8b 40 10             	mov    0x10(%eax),%eax
801010dd:	89 04 24             	mov    %eax,(%esp)
801010e0:	e8 c4 08 00 00       	call   801019a9 <iunlock>
    return 0;
801010e5:	b8 00 00 00 00       	mov    $0x0,%eax
801010ea:	eb 05                	jmp    801010f1 <filestat+0x4d>
  }
  return -1;
801010ec:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010f1:	c9                   	leave  
801010f2:	c3                   	ret    

801010f3 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010f3:	55                   	push   %ebp
801010f4:	89 e5                	mov    %esp,%ebp
801010f6:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010f9:	8b 45 08             	mov    0x8(%ebp),%eax
801010fc:	0f b6 40 08          	movzbl 0x8(%eax),%eax
80101100:	84 c0                	test   %al,%al
80101102:	75 0a                	jne    8010110e <fileread+0x1b>
    return -1;
80101104:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101109:	e9 9f 00 00 00       	jmp    801011ad <fileread+0xba>
  if(f->type == FD_PIPE)
8010110e:	8b 45 08             	mov    0x8(%ebp),%eax
80101111:	8b 00                	mov    (%eax),%eax
80101113:	83 f8 01             	cmp    $0x1,%eax
80101116:	75 1e                	jne    80101136 <fileread+0x43>
    return piperead(f->pipe, addr, n);
80101118:	8b 45 08             	mov    0x8(%ebp),%eax
8010111b:	8b 40 0c             	mov    0xc(%eax),%eax
8010111e:	8b 55 10             	mov    0x10(%ebp),%edx
80101121:	89 54 24 08          	mov    %edx,0x8(%esp)
80101125:	8b 55 0c             	mov    0xc(%ebp),%edx
80101128:	89 54 24 04          	mov    %edx,0x4(%esp)
8010112c:	89 04 24             	mov    %eax,(%esp)
8010112f:	e8 99 30 00 00       	call   801041cd <piperead>
80101134:	eb 77                	jmp    801011ad <fileread+0xba>
  if(f->type == FD_INODE){
80101136:	8b 45 08             	mov    0x8(%ebp),%eax
80101139:	8b 00                	mov    (%eax),%eax
8010113b:	83 f8 02             	cmp    $0x2,%eax
8010113e:	75 61                	jne    801011a1 <fileread+0xae>
    ilock(f->ip);
80101140:	8b 45 08             	mov    0x8(%ebp),%eax
80101143:	8b 40 10             	mov    0x10(%eax),%eax
80101146:	89 04 24             	mov    %eax,(%esp)
80101149:	e8 0d 07 00 00       	call   8010185b <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
8010114e:	8b 4d 10             	mov    0x10(%ebp),%ecx
80101151:	8b 45 08             	mov    0x8(%ebp),%eax
80101154:	8b 50 14             	mov    0x14(%eax),%edx
80101157:	8b 45 08             	mov    0x8(%ebp),%eax
8010115a:	8b 40 10             	mov    0x10(%eax),%eax
8010115d:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101161:	89 54 24 08          	mov    %edx,0x8(%esp)
80101165:	8b 55 0c             	mov    0xc(%ebp),%edx
80101168:	89 54 24 04          	mov    %edx,0x4(%esp)
8010116c:	89 04 24             	mov    %eax,(%esp)
8010116f:	e8 f4 0b 00 00       	call   80101d68 <readi>
80101174:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101177:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010117b:	7e 11                	jle    8010118e <fileread+0x9b>
      f->off += r;
8010117d:	8b 45 08             	mov    0x8(%ebp),%eax
80101180:	8b 50 14             	mov    0x14(%eax),%edx
80101183:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101186:	01 c2                	add    %eax,%edx
80101188:	8b 45 08             	mov    0x8(%ebp),%eax
8010118b:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
8010118e:	8b 45 08             	mov    0x8(%ebp),%eax
80101191:	8b 40 10             	mov    0x10(%eax),%eax
80101194:	89 04 24             	mov    %eax,(%esp)
80101197:	e8 0d 08 00 00       	call   801019a9 <iunlock>
    return r;
8010119c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010119f:	eb 0c                	jmp    801011ad <fileread+0xba>
  }
  panic("fileread");
801011a1:	c7 04 24 a6 89 10 80 	movl   $0x801089a6,(%esp)
801011a8:	e8 8d f3 ff ff       	call   8010053a <panic>
}
801011ad:	c9                   	leave  
801011ae:	c3                   	ret    

801011af <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801011af:	55                   	push   %ebp
801011b0:	89 e5                	mov    %esp,%ebp
801011b2:	53                   	push   %ebx
801011b3:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011b6:	8b 45 08             	mov    0x8(%ebp),%eax
801011b9:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011bd:	84 c0                	test   %al,%al
801011bf:	75 0a                	jne    801011cb <filewrite+0x1c>
    return -1;
801011c1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011c6:	e9 20 01 00 00       	jmp    801012eb <filewrite+0x13c>
  if(f->type == FD_PIPE)
801011cb:	8b 45 08             	mov    0x8(%ebp),%eax
801011ce:	8b 00                	mov    (%eax),%eax
801011d0:	83 f8 01             	cmp    $0x1,%eax
801011d3:	75 21                	jne    801011f6 <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011d5:	8b 45 08             	mov    0x8(%ebp),%eax
801011d8:	8b 40 0c             	mov    0xc(%eax),%eax
801011db:	8b 55 10             	mov    0x10(%ebp),%edx
801011de:	89 54 24 08          	mov    %edx,0x8(%esp)
801011e2:	8b 55 0c             	mov    0xc(%ebp),%edx
801011e5:	89 54 24 04          	mov    %edx,0x4(%esp)
801011e9:	89 04 24             	mov    %eax,(%esp)
801011ec:	e8 ed 2e 00 00       	call   801040de <pipewrite>
801011f1:	e9 f5 00 00 00       	jmp    801012eb <filewrite+0x13c>
  if(f->type == FD_INODE){
801011f6:	8b 45 08             	mov    0x8(%ebp),%eax
801011f9:	8b 00                	mov    (%eax),%eax
801011fb:	83 f8 02             	cmp    $0x2,%eax
801011fe:	0f 85 db 00 00 00    	jne    801012df <filewrite+0x130>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
80101204:	c7 45 ec 00 1a 00 00 	movl   $0x1a00,-0x14(%ebp)
    int i = 0;
8010120b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101212:	e9 a8 00 00 00       	jmp    801012bf <filewrite+0x110>
      int n1 = n - i;
80101217:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010121a:	8b 55 10             	mov    0x10(%ebp),%edx
8010121d:	29 c2                	sub    %eax,%edx
8010121f:	89 d0                	mov    %edx,%eax
80101221:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
80101224:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101227:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010122a:	7e 06                	jle    80101232 <filewrite+0x83>
        n1 = max;
8010122c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010122f:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_op();
80101232:	e8 d9 21 00 00       	call   80103410 <begin_op>
      ilock(f->ip);
80101237:	8b 45 08             	mov    0x8(%ebp),%eax
8010123a:	8b 40 10             	mov    0x10(%eax),%eax
8010123d:	89 04 24             	mov    %eax,(%esp)
80101240:	e8 16 06 00 00       	call   8010185b <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
80101245:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80101248:	8b 45 08             	mov    0x8(%ebp),%eax
8010124b:	8b 50 14             	mov    0x14(%eax),%edx
8010124e:	8b 5d f4             	mov    -0xc(%ebp),%ebx
80101251:	8b 45 0c             	mov    0xc(%ebp),%eax
80101254:	01 c3                	add    %eax,%ebx
80101256:	8b 45 08             	mov    0x8(%ebp),%eax
80101259:	8b 40 10             	mov    0x10(%eax),%eax
8010125c:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101260:	89 54 24 08          	mov    %edx,0x8(%esp)
80101264:	89 5c 24 04          	mov    %ebx,0x4(%esp)
80101268:	89 04 24             	mov    %eax,(%esp)
8010126b:	e8 5c 0c 00 00       	call   80101ecc <writei>
80101270:	89 45 e8             	mov    %eax,-0x18(%ebp)
80101273:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101277:	7e 11                	jle    8010128a <filewrite+0xdb>
        f->off += r;
80101279:	8b 45 08             	mov    0x8(%ebp),%eax
8010127c:	8b 50 14             	mov    0x14(%eax),%edx
8010127f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101282:	01 c2                	add    %eax,%edx
80101284:	8b 45 08             	mov    0x8(%ebp),%eax
80101287:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
8010128a:	8b 45 08             	mov    0x8(%ebp),%eax
8010128d:	8b 40 10             	mov    0x10(%eax),%eax
80101290:	89 04 24             	mov    %eax,(%esp)
80101293:	e8 11 07 00 00       	call   801019a9 <iunlock>
      end_op();
80101298:	e8 f7 21 00 00       	call   80103494 <end_op>

      if(r < 0)
8010129d:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801012a1:	79 02                	jns    801012a5 <filewrite+0xf6>
        break;
801012a3:	eb 26                	jmp    801012cb <filewrite+0x11c>
      if(r != n1)
801012a5:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012a8:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801012ab:	74 0c                	je     801012b9 <filewrite+0x10a>
        panic("short filewrite");
801012ad:	c7 04 24 af 89 10 80 	movl   $0x801089af,(%esp)
801012b4:	e8 81 f2 ff ff       	call   8010053a <panic>
      i += r;
801012b9:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012bc:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c2:	3b 45 10             	cmp    0x10(%ebp),%eax
801012c5:	0f 8c 4c ff ff ff    	jl     80101217 <filewrite+0x68>
        break;
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012ce:	3b 45 10             	cmp    0x10(%ebp),%eax
801012d1:	75 05                	jne    801012d8 <filewrite+0x129>
801012d3:	8b 45 10             	mov    0x10(%ebp),%eax
801012d6:	eb 05                	jmp    801012dd <filewrite+0x12e>
801012d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012dd:	eb 0c                	jmp    801012eb <filewrite+0x13c>
  }
  panic("filewrite");
801012df:	c7 04 24 bf 89 10 80 	movl   $0x801089bf,(%esp)
801012e6:	e8 4f f2 ff ff       	call   8010053a <panic>
}
801012eb:	83 c4 24             	add    $0x24,%esp
801012ee:	5b                   	pop    %ebx
801012ef:	5d                   	pop    %ebp
801012f0:	c3                   	ret    

801012f1 <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012f1:	55                   	push   %ebp
801012f2:	89 e5                	mov    %esp,%ebp
801012f4:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012f7:	8b 45 08             	mov    0x8(%ebp),%eax
801012fa:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80101301:	00 
80101302:	89 04 24             	mov    %eax,(%esp)
80101305:	e8 9c ee ff ff       	call   801001a6 <bread>
8010130a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
8010130d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101310:	83 c0 18             	add    $0x18,%eax
80101313:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
8010131a:	00 
8010131b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010131f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101322:	89 04 24             	mov    %eax,(%esp)
80101325:	e8 6c 3f 00 00       	call   80105296 <memmove>
  brelse(bp);
8010132a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010132d:	89 04 24             	mov    %eax,(%esp)
80101330:	e8 e2 ee ff ff       	call   80100217 <brelse>
}
80101335:	c9                   	leave  
80101336:	c3                   	ret    

80101337 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101337:	55                   	push   %ebp
80101338:	89 e5                	mov    %esp,%ebp
8010133a:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
8010133d:	8b 55 0c             	mov    0xc(%ebp),%edx
80101340:	8b 45 08             	mov    0x8(%ebp),%eax
80101343:	89 54 24 04          	mov    %edx,0x4(%esp)
80101347:	89 04 24             	mov    %eax,(%esp)
8010134a:	e8 57 ee ff ff       	call   801001a6 <bread>
8010134f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
80101352:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101355:	83 c0 18             	add    $0x18,%eax
80101358:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010135f:	00 
80101360:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101367:	00 
80101368:	89 04 24             	mov    %eax,(%esp)
8010136b:	e8 57 3e 00 00       	call   801051c7 <memset>
  log_write(bp);
80101370:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101373:	89 04 24             	mov    %eax,(%esp)
80101376:	e8 a0 22 00 00       	call   8010361b <log_write>
  brelse(bp);
8010137b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010137e:	89 04 24             	mov    %eax,(%esp)
80101381:	e8 91 ee ff ff       	call   80100217 <brelse>
}
80101386:	c9                   	leave  
80101387:	c3                   	ret    

80101388 <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101388:	55                   	push   %ebp
80101389:	89 e5                	mov    %esp,%ebp
8010138b:	83 ec 38             	sub    $0x38,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
8010138e:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101395:	8b 45 08             	mov    0x8(%ebp),%eax
80101398:	8d 55 d8             	lea    -0x28(%ebp),%edx
8010139b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010139f:	89 04 24             	mov    %eax,(%esp)
801013a2:	e8 4a ff ff ff       	call   801012f1 <readsb>
  for(b = 0; b < sb.size; b += BPB){
801013a7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013ae:	e9 07 01 00 00       	jmp    801014ba <balloc+0x132>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013b6:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013bc:	85 c0                	test   %eax,%eax
801013be:	0f 48 c2             	cmovs  %edx,%eax
801013c1:	c1 f8 0c             	sar    $0xc,%eax
801013c4:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013c7:	c1 ea 03             	shr    $0x3,%edx
801013ca:	01 d0                	add    %edx,%eax
801013cc:	83 c0 03             	add    $0x3,%eax
801013cf:	89 44 24 04          	mov    %eax,0x4(%esp)
801013d3:	8b 45 08             	mov    0x8(%ebp),%eax
801013d6:	89 04 24             	mov    %eax,(%esp)
801013d9:	e8 c8 ed ff ff       	call   801001a6 <bread>
801013de:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013e1:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013e8:	e9 9d 00 00 00       	jmp    8010148a <balloc+0x102>
      m = 1 << (bi % 8);
801013ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013f0:	99                   	cltd   
801013f1:	c1 ea 1d             	shr    $0x1d,%edx
801013f4:	01 d0                	add    %edx,%eax
801013f6:	83 e0 07             	and    $0x7,%eax
801013f9:	29 d0                	sub    %edx,%eax
801013fb:	ba 01 00 00 00       	mov    $0x1,%edx
80101400:	89 c1                	mov    %eax,%ecx
80101402:	d3 e2                	shl    %cl,%edx
80101404:	89 d0                	mov    %edx,%eax
80101406:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101409:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010140c:	8d 50 07             	lea    0x7(%eax),%edx
8010140f:	85 c0                	test   %eax,%eax
80101411:	0f 48 c2             	cmovs  %edx,%eax
80101414:	c1 f8 03             	sar    $0x3,%eax
80101417:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010141a:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
8010141f:	0f b6 c0             	movzbl %al,%eax
80101422:	23 45 e8             	and    -0x18(%ebp),%eax
80101425:	85 c0                	test   %eax,%eax
80101427:	75 5d                	jne    80101486 <balloc+0xfe>
        bp->data[bi/8] |= m;  // Mark block in use.
80101429:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010142c:	8d 50 07             	lea    0x7(%eax),%edx
8010142f:	85 c0                	test   %eax,%eax
80101431:	0f 48 c2             	cmovs  %edx,%eax
80101434:	c1 f8 03             	sar    $0x3,%eax
80101437:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010143a:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010143f:	89 d1                	mov    %edx,%ecx
80101441:	8b 55 e8             	mov    -0x18(%ebp),%edx
80101444:	09 ca                	or     %ecx,%edx
80101446:	89 d1                	mov    %edx,%ecx
80101448:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010144b:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
8010144f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101452:	89 04 24             	mov    %eax,(%esp)
80101455:	e8 c1 21 00 00       	call   8010361b <log_write>
        brelse(bp);
8010145a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010145d:	89 04 24             	mov    %eax,(%esp)
80101460:	e8 b2 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
80101465:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101468:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010146b:	01 c2                	add    %eax,%edx
8010146d:	8b 45 08             	mov    0x8(%ebp),%eax
80101470:	89 54 24 04          	mov    %edx,0x4(%esp)
80101474:	89 04 24             	mov    %eax,(%esp)
80101477:	e8 bb fe ff ff       	call   80101337 <bzero>
        return b + bi;
8010147c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010147f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101482:	01 d0                	add    %edx,%eax
80101484:	eb 4e                	jmp    801014d4 <balloc+0x14c>

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
80101486:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
8010148a:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
80101491:	7f 15                	jg     801014a8 <balloc+0x120>
80101493:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101496:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101499:	01 d0                	add    %edx,%eax
8010149b:	89 c2                	mov    %eax,%edx
8010149d:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014a0:	39 c2                	cmp    %eax,%edx
801014a2:	0f 82 45 ff ff ff    	jb     801013ed <balloc+0x65>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014a8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014ab:	89 04 24             	mov    %eax,(%esp)
801014ae:	e8 64 ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b3:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014ba:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014bd:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014c0:	39 c2                	cmp    %eax,%edx
801014c2:	0f 82 eb fe ff ff    	jb     801013b3 <balloc+0x2b>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014c8:	c7 04 24 c9 89 10 80 	movl   $0x801089c9,(%esp)
801014cf:	e8 66 f0 ff ff       	call   8010053a <panic>
}
801014d4:	c9                   	leave  
801014d5:	c3                   	ret    

801014d6 <bfree>:

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014d6:	55                   	push   %ebp
801014d7:	89 e5                	mov    %esp,%ebp
801014d9:	83 ec 38             	sub    $0x38,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014dc:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014df:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e3:	8b 45 08             	mov    0x8(%ebp),%eax
801014e6:	89 04 24             	mov    %eax,(%esp)
801014e9:	e8 03 fe ff ff       	call   801012f1 <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014ee:	8b 45 0c             	mov    0xc(%ebp),%eax
801014f1:	c1 e8 0c             	shr    $0xc,%eax
801014f4:	89 c2                	mov    %eax,%edx
801014f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014f9:	c1 e8 03             	shr    $0x3,%eax
801014fc:	01 d0                	add    %edx,%eax
801014fe:	8d 50 03             	lea    0x3(%eax),%edx
80101501:	8b 45 08             	mov    0x8(%ebp),%eax
80101504:	89 54 24 04          	mov    %edx,0x4(%esp)
80101508:	89 04 24             	mov    %eax,(%esp)
8010150b:	e8 96 ec ff ff       	call   801001a6 <bread>
80101510:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101513:	8b 45 0c             	mov    0xc(%ebp),%eax
80101516:	25 ff 0f 00 00       	and    $0xfff,%eax
8010151b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
8010151e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101521:	99                   	cltd   
80101522:	c1 ea 1d             	shr    $0x1d,%edx
80101525:	01 d0                	add    %edx,%eax
80101527:	83 e0 07             	and    $0x7,%eax
8010152a:	29 d0                	sub    %edx,%eax
8010152c:	ba 01 00 00 00       	mov    $0x1,%edx
80101531:	89 c1                	mov    %eax,%ecx
80101533:	d3 e2                	shl    %cl,%edx
80101535:	89 d0                	mov    %edx,%eax
80101537:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
8010153a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010153d:	8d 50 07             	lea    0x7(%eax),%edx
80101540:	85 c0                	test   %eax,%eax
80101542:	0f 48 c2             	cmovs  %edx,%eax
80101545:	c1 f8 03             	sar    $0x3,%eax
80101548:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010154b:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101550:	0f b6 c0             	movzbl %al,%eax
80101553:	23 45 ec             	and    -0x14(%ebp),%eax
80101556:	85 c0                	test   %eax,%eax
80101558:	75 0c                	jne    80101566 <bfree+0x90>
    panic("freeing free block");
8010155a:	c7 04 24 df 89 10 80 	movl   $0x801089df,(%esp)
80101561:	e8 d4 ef ff ff       	call   8010053a <panic>
  bp->data[bi/8] &= ~m;
80101566:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101569:	8d 50 07             	lea    0x7(%eax),%edx
8010156c:	85 c0                	test   %eax,%eax
8010156e:	0f 48 c2             	cmovs  %edx,%eax
80101571:	c1 f8 03             	sar    $0x3,%eax
80101574:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101577:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010157c:	8b 4d ec             	mov    -0x14(%ebp),%ecx
8010157f:	f7 d1                	not    %ecx
80101581:	21 ca                	and    %ecx,%edx
80101583:	89 d1                	mov    %edx,%ecx
80101585:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101588:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
8010158c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010158f:	89 04 24             	mov    %eax,(%esp)
80101592:	e8 84 20 00 00       	call   8010361b <log_write>
  brelse(bp);
80101597:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159a:	89 04 24             	mov    %eax,(%esp)
8010159d:	e8 75 ec ff ff       	call   80100217 <brelse>
}
801015a2:	c9                   	leave  
801015a3:	c3                   	ret    

801015a4 <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015a4:	55                   	push   %ebp
801015a5:	89 e5                	mov    %esp,%ebp
801015a7:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015aa:	c7 44 24 04 f2 89 10 	movl   $0x801089f2,0x4(%esp)
801015b1:	80 
801015b2:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
801015b9:	e8 94 39 00 00       	call   80104f52 <initlock>
}
801015be:	c9                   	leave  
801015bf:	c3                   	ret    

801015c0 <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015c0:	55                   	push   %ebp
801015c1:	89 e5                	mov    %esp,%ebp
801015c3:	83 ec 38             	sub    $0x38,%esp
801015c6:	8b 45 0c             	mov    0xc(%ebp),%eax
801015c9:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015cd:	8b 45 08             	mov    0x8(%ebp),%eax
801015d0:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015d3:	89 54 24 04          	mov    %edx,0x4(%esp)
801015d7:	89 04 24             	mov    %eax,(%esp)
801015da:	e8 12 fd ff ff       	call   801012f1 <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015df:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015e6:	e9 98 00 00 00       	jmp    80101683 <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015ee:	c1 e8 03             	shr    $0x3,%eax
801015f1:	83 c0 02             	add    $0x2,%eax
801015f4:	89 44 24 04          	mov    %eax,0x4(%esp)
801015f8:	8b 45 08             	mov    0x8(%ebp),%eax
801015fb:	89 04 24             	mov    %eax,(%esp)
801015fe:	e8 a3 eb ff ff       	call   801001a6 <bread>
80101603:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
80101606:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101609:	8d 50 18             	lea    0x18(%eax),%edx
8010160c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010160f:	83 e0 07             	and    $0x7,%eax
80101612:	c1 e0 06             	shl    $0x6,%eax
80101615:	01 d0                	add    %edx,%eax
80101617:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
8010161a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010161d:	0f b7 00             	movzwl (%eax),%eax
80101620:	66 85 c0             	test   %ax,%ax
80101623:	75 4f                	jne    80101674 <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
80101625:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
8010162c:	00 
8010162d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101634:	00 
80101635:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101638:	89 04 24             	mov    %eax,(%esp)
8010163b:	e8 87 3b 00 00       	call   801051c7 <memset>
      dip->type = type;
80101640:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101643:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
80101647:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
8010164a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010164d:	89 04 24             	mov    %eax,(%esp)
80101650:	e8 c6 1f 00 00       	call   8010361b <log_write>
      brelse(bp);
80101655:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101658:	89 04 24             	mov    %eax,(%esp)
8010165b:	e8 b7 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
80101660:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101663:	89 44 24 04          	mov    %eax,0x4(%esp)
80101667:	8b 45 08             	mov    0x8(%ebp),%eax
8010166a:	89 04 24             	mov    %eax,(%esp)
8010166d:	e8 e5 00 00 00       	call   80101757 <iget>
80101672:	eb 29                	jmp    8010169d <ialloc+0xdd>
    }
    brelse(bp);
80101674:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101677:	89 04 24             	mov    %eax,(%esp)
8010167a:	e8 98 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
8010167f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101683:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101686:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101689:	39 c2                	cmp    %eax,%edx
8010168b:	0f 82 5a ff ff ff    	jb     801015eb <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
80101691:	c7 04 24 f9 89 10 80 	movl   $0x801089f9,(%esp)
80101698:	e8 9d ee ff ff       	call   8010053a <panic>
}
8010169d:	c9                   	leave  
8010169e:	c3                   	ret    

8010169f <iupdate>:

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
8010169f:	55                   	push   %ebp
801016a0:	89 e5                	mov    %esp,%ebp
801016a2:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016a5:	8b 45 08             	mov    0x8(%ebp),%eax
801016a8:	8b 40 04             	mov    0x4(%eax),%eax
801016ab:	c1 e8 03             	shr    $0x3,%eax
801016ae:	8d 50 02             	lea    0x2(%eax),%edx
801016b1:	8b 45 08             	mov    0x8(%ebp),%eax
801016b4:	8b 00                	mov    (%eax),%eax
801016b6:	89 54 24 04          	mov    %edx,0x4(%esp)
801016ba:	89 04 24             	mov    %eax,(%esp)
801016bd:	e8 e4 ea ff ff       	call   801001a6 <bread>
801016c2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016c8:	8d 50 18             	lea    0x18(%eax),%edx
801016cb:	8b 45 08             	mov    0x8(%ebp),%eax
801016ce:	8b 40 04             	mov    0x4(%eax),%eax
801016d1:	83 e0 07             	and    $0x7,%eax
801016d4:	c1 e0 06             	shl    $0x6,%eax
801016d7:	01 d0                	add    %edx,%eax
801016d9:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016dc:	8b 45 08             	mov    0x8(%ebp),%eax
801016df:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016e3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016e6:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016e9:	8b 45 08             	mov    0x8(%ebp),%eax
801016ec:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f3:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
801016f7:	8b 45 08             	mov    0x8(%ebp),%eax
801016fa:	0f b7 50 14          	movzwl 0x14(%eax),%edx
801016fe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101701:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
80101705:	8b 45 08             	mov    0x8(%ebp),%eax
80101708:	0f b7 50 16          	movzwl 0x16(%eax),%edx
8010170c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010170f:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101713:	8b 45 08             	mov    0x8(%ebp),%eax
80101716:	8b 50 18             	mov    0x18(%eax),%edx
80101719:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010171c:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010171f:	8b 45 08             	mov    0x8(%ebp),%eax
80101722:	8d 50 1c             	lea    0x1c(%eax),%edx
80101725:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101728:	83 c0 0c             	add    $0xc,%eax
8010172b:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101732:	00 
80101733:	89 54 24 04          	mov    %edx,0x4(%esp)
80101737:	89 04 24             	mov    %eax,(%esp)
8010173a:	e8 57 3b 00 00       	call   80105296 <memmove>
  log_write(bp);
8010173f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101742:	89 04 24             	mov    %eax,(%esp)
80101745:	e8 d1 1e 00 00       	call   8010361b <log_write>
  brelse(bp);
8010174a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010174d:	89 04 24             	mov    %eax,(%esp)
80101750:	e8 c2 ea ff ff       	call   80100217 <brelse>
}
80101755:	c9                   	leave  
80101756:	c3                   	ret    

80101757 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101757:	55                   	push   %ebp
80101758:	89 e5                	mov    %esp,%ebp
8010175a:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
8010175d:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101764:	e8 0a 38 00 00       	call   80104f73 <acquire>

  // Is the inode already cached?
  empty = 0;
80101769:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101770:	c7 45 f4 94 12 11 80 	movl   $0x80111294,-0xc(%ebp)
80101777:	eb 59                	jmp    801017d2 <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101779:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010177c:	8b 40 08             	mov    0x8(%eax),%eax
8010177f:	85 c0                	test   %eax,%eax
80101781:	7e 35                	jle    801017b8 <iget+0x61>
80101783:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101786:	8b 00                	mov    (%eax),%eax
80101788:	3b 45 08             	cmp    0x8(%ebp),%eax
8010178b:	75 2b                	jne    801017b8 <iget+0x61>
8010178d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101790:	8b 40 04             	mov    0x4(%eax),%eax
80101793:	3b 45 0c             	cmp    0xc(%ebp),%eax
80101796:	75 20                	jne    801017b8 <iget+0x61>
      ip->ref++;
80101798:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010179b:	8b 40 08             	mov    0x8(%eax),%eax
8010179e:	8d 50 01             	lea    0x1(%eax),%edx
801017a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a4:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017a7:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
801017ae:	e8 22 38 00 00       	call   80104fd5 <release>
      return ip;
801017b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b6:	eb 6f                	jmp    80101827 <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017b8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017bc:	75 10                	jne    801017ce <iget+0x77>
801017be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c1:	8b 40 08             	mov    0x8(%eax),%eax
801017c4:	85 c0                	test   %eax,%eax
801017c6:	75 06                	jne    801017ce <iget+0x77>
      empty = ip;
801017c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017cb:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017ce:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017d2:	81 7d f4 34 22 11 80 	cmpl   $0x80112234,-0xc(%ebp)
801017d9:	72 9e                	jb     80101779 <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017db:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017df:	75 0c                	jne    801017ed <iget+0x96>
    panic("iget: no inodes");
801017e1:	c7 04 24 0b 8a 10 80 	movl   $0x80108a0b,(%esp)
801017e8:	e8 4d ed ff ff       	call   8010053a <panic>

  ip = empty;
801017ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017f0:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
801017f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017f6:	8b 55 08             	mov    0x8(%ebp),%edx
801017f9:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
801017fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017fe:	8b 55 0c             	mov    0xc(%ebp),%edx
80101801:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101804:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101807:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
8010180e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101811:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
80101818:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
8010181f:	e8 b1 37 00 00       	call   80104fd5 <release>

  return ip;
80101824:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80101827:	c9                   	leave  
80101828:	c3                   	ret    

80101829 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101829:	55                   	push   %ebp
8010182a:	89 e5                	mov    %esp,%ebp
8010182c:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
8010182f:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101836:	e8 38 37 00 00       	call   80104f73 <acquire>
  ip->ref++;
8010183b:	8b 45 08             	mov    0x8(%ebp),%eax
8010183e:	8b 40 08             	mov    0x8(%eax),%eax
80101841:	8d 50 01             	lea    0x1(%eax),%edx
80101844:	8b 45 08             	mov    0x8(%ebp),%eax
80101847:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
8010184a:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101851:	e8 7f 37 00 00       	call   80104fd5 <release>
  return ip;
80101856:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101859:	c9                   	leave  
8010185a:	c3                   	ret    

8010185b <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
8010185b:	55                   	push   %ebp
8010185c:	89 e5                	mov    %esp,%ebp
8010185e:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
80101861:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101865:	74 0a                	je     80101871 <ilock+0x16>
80101867:	8b 45 08             	mov    0x8(%ebp),%eax
8010186a:	8b 40 08             	mov    0x8(%eax),%eax
8010186d:	85 c0                	test   %eax,%eax
8010186f:	7f 0c                	jg     8010187d <ilock+0x22>
    panic("ilock");
80101871:	c7 04 24 1b 8a 10 80 	movl   $0x80108a1b,(%esp)
80101878:	e8 bd ec ff ff       	call   8010053a <panic>

  acquire(&icache.lock);
8010187d:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101884:	e8 ea 36 00 00       	call   80104f73 <acquire>
  while(ip->flags & I_BUSY)
80101889:	eb 13                	jmp    8010189e <ilock+0x43>
    sleep(ip, &icache.lock);
8010188b:	c7 44 24 04 60 12 11 	movl   $0x80111260,0x4(%esp)
80101892:	80 
80101893:	8b 45 08             	mov    0x8(%ebp),%eax
80101896:	89 04 24             	mov    %eax,(%esp)
80101899:	e8 ba 32 00 00       	call   80104b58 <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
8010189e:	8b 45 08             	mov    0x8(%ebp),%eax
801018a1:	8b 40 0c             	mov    0xc(%eax),%eax
801018a4:	83 e0 01             	and    $0x1,%eax
801018a7:	85 c0                	test   %eax,%eax
801018a9:	75 e0                	jne    8010188b <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018ab:	8b 45 08             	mov    0x8(%ebp),%eax
801018ae:	8b 40 0c             	mov    0xc(%eax),%eax
801018b1:	83 c8 01             	or     $0x1,%eax
801018b4:	89 c2                	mov    %eax,%edx
801018b6:	8b 45 08             	mov    0x8(%ebp),%eax
801018b9:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018bc:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
801018c3:	e8 0d 37 00 00       	call   80104fd5 <release>

  if(!(ip->flags & I_VALID)){
801018c8:	8b 45 08             	mov    0x8(%ebp),%eax
801018cb:	8b 40 0c             	mov    0xc(%eax),%eax
801018ce:	83 e0 02             	and    $0x2,%eax
801018d1:	85 c0                	test   %eax,%eax
801018d3:	0f 85 ce 00 00 00    	jne    801019a7 <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018d9:	8b 45 08             	mov    0x8(%ebp),%eax
801018dc:	8b 40 04             	mov    0x4(%eax),%eax
801018df:	c1 e8 03             	shr    $0x3,%eax
801018e2:	8d 50 02             	lea    0x2(%eax),%edx
801018e5:	8b 45 08             	mov    0x8(%ebp),%eax
801018e8:	8b 00                	mov    (%eax),%eax
801018ea:	89 54 24 04          	mov    %edx,0x4(%esp)
801018ee:	89 04 24             	mov    %eax,(%esp)
801018f1:	e8 b0 e8 ff ff       	call   801001a6 <bread>
801018f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
801018f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801018fc:	8d 50 18             	lea    0x18(%eax),%edx
801018ff:	8b 45 08             	mov    0x8(%ebp),%eax
80101902:	8b 40 04             	mov    0x4(%eax),%eax
80101905:	83 e0 07             	and    $0x7,%eax
80101908:	c1 e0 06             	shl    $0x6,%eax
8010190b:	01 d0                	add    %edx,%eax
8010190d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
80101910:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101913:	0f b7 10             	movzwl (%eax),%edx
80101916:	8b 45 08             	mov    0x8(%ebp),%eax
80101919:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
8010191d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101920:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101924:	8b 45 08             	mov    0x8(%ebp),%eax
80101927:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
8010192b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010192e:	0f b7 50 04          	movzwl 0x4(%eax),%edx
80101932:	8b 45 08             	mov    0x8(%ebp),%eax
80101935:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
80101939:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010193c:	0f b7 50 06          	movzwl 0x6(%eax),%edx
80101940:	8b 45 08             	mov    0x8(%ebp),%eax
80101943:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
80101947:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010194a:	8b 50 08             	mov    0x8(%eax),%edx
8010194d:	8b 45 08             	mov    0x8(%ebp),%eax
80101950:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101953:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101956:	8d 50 0c             	lea    0xc(%eax),%edx
80101959:	8b 45 08             	mov    0x8(%ebp),%eax
8010195c:	83 c0 1c             	add    $0x1c,%eax
8010195f:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101966:	00 
80101967:	89 54 24 04          	mov    %edx,0x4(%esp)
8010196b:	89 04 24             	mov    %eax,(%esp)
8010196e:	e8 23 39 00 00       	call   80105296 <memmove>
    brelse(bp);
80101973:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101976:	89 04 24             	mov    %eax,(%esp)
80101979:	e8 99 e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
8010197e:	8b 45 08             	mov    0x8(%ebp),%eax
80101981:	8b 40 0c             	mov    0xc(%eax),%eax
80101984:	83 c8 02             	or     $0x2,%eax
80101987:	89 c2                	mov    %eax,%edx
80101989:	8b 45 08             	mov    0x8(%ebp),%eax
8010198c:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
8010198f:	8b 45 08             	mov    0x8(%ebp),%eax
80101992:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101996:	66 85 c0             	test   %ax,%ax
80101999:	75 0c                	jne    801019a7 <ilock+0x14c>
      panic("ilock: no type");
8010199b:	c7 04 24 21 8a 10 80 	movl   $0x80108a21,(%esp)
801019a2:	e8 93 eb ff ff       	call   8010053a <panic>
  }
}
801019a7:	c9                   	leave  
801019a8:	c3                   	ret    

801019a9 <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019a9:	55                   	push   %ebp
801019aa:	89 e5                	mov    %esp,%ebp
801019ac:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019af:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019b3:	74 17                	je     801019cc <iunlock+0x23>
801019b5:	8b 45 08             	mov    0x8(%ebp),%eax
801019b8:	8b 40 0c             	mov    0xc(%eax),%eax
801019bb:	83 e0 01             	and    $0x1,%eax
801019be:	85 c0                	test   %eax,%eax
801019c0:	74 0a                	je     801019cc <iunlock+0x23>
801019c2:	8b 45 08             	mov    0x8(%ebp),%eax
801019c5:	8b 40 08             	mov    0x8(%eax),%eax
801019c8:	85 c0                	test   %eax,%eax
801019ca:	7f 0c                	jg     801019d8 <iunlock+0x2f>
    panic("iunlock");
801019cc:	c7 04 24 30 8a 10 80 	movl   $0x80108a30,(%esp)
801019d3:	e8 62 eb ff ff       	call   8010053a <panic>

  acquire(&icache.lock);
801019d8:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
801019df:	e8 8f 35 00 00       	call   80104f73 <acquire>
  ip->flags &= ~I_BUSY;
801019e4:	8b 45 08             	mov    0x8(%ebp),%eax
801019e7:	8b 40 0c             	mov    0xc(%eax),%eax
801019ea:	83 e0 fe             	and    $0xfffffffe,%eax
801019ed:	89 c2                	mov    %eax,%edx
801019ef:	8b 45 08             	mov    0x8(%ebp),%eax
801019f2:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
801019f5:	8b 45 08             	mov    0x8(%ebp),%eax
801019f8:	89 04 24             	mov    %eax,(%esp)
801019fb:	e8 34 32 00 00       	call   80104c34 <wakeup>
  release(&icache.lock);
80101a00:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101a07:	e8 c9 35 00 00       	call   80104fd5 <release>
}
80101a0c:	c9                   	leave  
80101a0d:	c3                   	ret    

80101a0e <iput>:
// to it, free the inode (and its content) on disk.
// All calls to iput() must be inside a transaction in
// case it has to free the inode.
void
iput(struct inode *ip)
{
80101a0e:	55                   	push   %ebp
80101a0f:	89 e5                	mov    %esp,%ebp
80101a11:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a14:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101a1b:	e8 53 35 00 00       	call   80104f73 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a20:	8b 45 08             	mov    0x8(%ebp),%eax
80101a23:	8b 40 08             	mov    0x8(%eax),%eax
80101a26:	83 f8 01             	cmp    $0x1,%eax
80101a29:	0f 85 93 00 00 00    	jne    80101ac2 <iput+0xb4>
80101a2f:	8b 45 08             	mov    0x8(%ebp),%eax
80101a32:	8b 40 0c             	mov    0xc(%eax),%eax
80101a35:	83 e0 02             	and    $0x2,%eax
80101a38:	85 c0                	test   %eax,%eax
80101a3a:	0f 84 82 00 00 00    	je     80101ac2 <iput+0xb4>
80101a40:	8b 45 08             	mov    0x8(%ebp),%eax
80101a43:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a47:	66 85 c0             	test   %ax,%ax
80101a4a:	75 76                	jne    80101ac2 <iput+0xb4>
    // inode has no links and no other references: truncate and free.
    if(ip->flags & I_BUSY)
80101a4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a4f:	8b 40 0c             	mov    0xc(%eax),%eax
80101a52:	83 e0 01             	and    $0x1,%eax
80101a55:	85 c0                	test   %eax,%eax
80101a57:	74 0c                	je     80101a65 <iput+0x57>
      panic("iput busy");
80101a59:	c7 04 24 38 8a 10 80 	movl   $0x80108a38,(%esp)
80101a60:	e8 d5 ea ff ff       	call   8010053a <panic>
    ip->flags |= I_BUSY;
80101a65:	8b 45 08             	mov    0x8(%ebp),%eax
80101a68:	8b 40 0c             	mov    0xc(%eax),%eax
80101a6b:	83 c8 01             	or     $0x1,%eax
80101a6e:	89 c2                	mov    %eax,%edx
80101a70:	8b 45 08             	mov    0x8(%ebp),%eax
80101a73:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a76:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101a7d:	e8 53 35 00 00       	call   80104fd5 <release>
    itrunc(ip);
80101a82:	8b 45 08             	mov    0x8(%ebp),%eax
80101a85:	89 04 24             	mov    %eax,(%esp)
80101a88:	e8 7d 01 00 00       	call   80101c0a <itrunc>
    ip->type = 0;
80101a8d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a90:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101a96:	8b 45 08             	mov    0x8(%ebp),%eax
80101a99:	89 04 24             	mov    %eax,(%esp)
80101a9c:	e8 fe fb ff ff       	call   8010169f <iupdate>
    acquire(&icache.lock);
80101aa1:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101aa8:	e8 c6 34 00 00       	call   80104f73 <acquire>
    ip->flags = 0;
80101aad:	8b 45 08             	mov    0x8(%ebp),%eax
80101ab0:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101ab7:	8b 45 08             	mov    0x8(%ebp),%eax
80101aba:	89 04 24             	mov    %eax,(%esp)
80101abd:	e8 72 31 00 00       	call   80104c34 <wakeup>
  }
  ip->ref--;
80101ac2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ac5:	8b 40 08             	mov    0x8(%eax),%eax
80101ac8:	8d 50 ff             	lea    -0x1(%eax),%edx
80101acb:	8b 45 08             	mov    0x8(%ebp),%eax
80101ace:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ad1:	c7 04 24 60 12 11 80 	movl   $0x80111260,(%esp)
80101ad8:	e8 f8 34 00 00       	call   80104fd5 <release>
}
80101add:	c9                   	leave  
80101ade:	c3                   	ret    

80101adf <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101adf:	55                   	push   %ebp
80101ae0:	89 e5                	mov    %esp,%ebp
80101ae2:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101ae5:	8b 45 08             	mov    0x8(%ebp),%eax
80101ae8:	89 04 24             	mov    %eax,(%esp)
80101aeb:	e8 b9 fe ff ff       	call   801019a9 <iunlock>
  iput(ip);
80101af0:	8b 45 08             	mov    0x8(%ebp),%eax
80101af3:	89 04 24             	mov    %eax,(%esp)
80101af6:	e8 13 ff ff ff       	call   80101a0e <iput>
}
80101afb:	c9                   	leave  
80101afc:	c3                   	ret    

80101afd <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101afd:	55                   	push   %ebp
80101afe:	89 e5                	mov    %esp,%ebp
80101b00:	53                   	push   %ebx
80101b01:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b04:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b08:	77 3e                	ja     80101b48 <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b0a:	8b 45 08             	mov    0x8(%ebp),%eax
80101b0d:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b10:	83 c2 04             	add    $0x4,%edx
80101b13:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b17:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b1a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b1e:	75 20                	jne    80101b40 <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b20:	8b 45 08             	mov    0x8(%ebp),%eax
80101b23:	8b 00                	mov    (%eax),%eax
80101b25:	89 04 24             	mov    %eax,(%esp)
80101b28:	e8 5b f8 ff ff       	call   80101388 <balloc>
80101b2d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b30:	8b 45 08             	mov    0x8(%ebp),%eax
80101b33:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b36:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b39:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b3c:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b40:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b43:	e9 bc 00 00 00       	jmp    80101c04 <bmap+0x107>
  }
  bn -= NDIRECT;
80101b48:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b4c:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b50:	0f 87 a2 00 00 00    	ja     80101bf8 <bmap+0xfb>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b56:	8b 45 08             	mov    0x8(%ebp),%eax
80101b59:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b5c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b5f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b63:	75 19                	jne    80101b7e <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b65:	8b 45 08             	mov    0x8(%ebp),%eax
80101b68:	8b 00                	mov    (%eax),%eax
80101b6a:	89 04 24             	mov    %eax,(%esp)
80101b6d:	e8 16 f8 ff ff       	call   80101388 <balloc>
80101b72:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b75:	8b 45 08             	mov    0x8(%ebp),%eax
80101b78:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b7b:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b7e:	8b 45 08             	mov    0x8(%ebp),%eax
80101b81:	8b 00                	mov    (%eax),%eax
80101b83:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b86:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b8a:	89 04 24             	mov    %eax,(%esp)
80101b8d:	e8 14 e6 ff ff       	call   801001a6 <bread>
80101b92:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101b95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101b98:	83 c0 18             	add    $0x18,%eax
80101b9b:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101b9e:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ba1:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101ba8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101bab:	01 d0                	add    %edx,%eax
80101bad:	8b 00                	mov    (%eax),%eax
80101baf:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bb6:	75 30                	jne    80101be8 <bmap+0xeb>
      a[bn] = addr = balloc(ip->dev);
80101bb8:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bbb:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101bc2:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101bc5:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80101bc8:	8b 45 08             	mov    0x8(%ebp),%eax
80101bcb:	8b 00                	mov    (%eax),%eax
80101bcd:	89 04 24             	mov    %eax,(%esp)
80101bd0:	e8 b3 f7 ff ff       	call   80101388 <balloc>
80101bd5:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bd8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bdb:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bdd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be0:	89 04 24             	mov    %eax,(%esp)
80101be3:	e8 33 1a 00 00       	call   8010361b <log_write>
    }
    brelse(bp);
80101be8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101beb:	89 04 24             	mov    %eax,(%esp)
80101bee:	e8 24 e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bf3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf6:	eb 0c                	jmp    80101c04 <bmap+0x107>
  }

  panic("bmap: out of range");
80101bf8:	c7 04 24 42 8a 10 80 	movl   $0x80108a42,(%esp)
80101bff:	e8 36 e9 ff ff       	call   8010053a <panic>
}
80101c04:	83 c4 24             	add    $0x24,%esp
80101c07:	5b                   	pop    %ebx
80101c08:	5d                   	pop    %ebp
80101c09:	c3                   	ret    

80101c0a <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c0a:	55                   	push   %ebp
80101c0b:	89 e5                	mov    %esp,%ebp
80101c0d:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c10:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c17:	eb 44                	jmp    80101c5d <itrunc+0x53>
    if(ip->addrs[i]){
80101c19:	8b 45 08             	mov    0x8(%ebp),%eax
80101c1c:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c1f:	83 c2 04             	add    $0x4,%edx
80101c22:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c26:	85 c0                	test   %eax,%eax
80101c28:	74 2f                	je     80101c59 <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c2a:	8b 45 08             	mov    0x8(%ebp),%eax
80101c2d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c30:	83 c2 04             	add    $0x4,%edx
80101c33:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c37:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3a:	8b 00                	mov    (%eax),%eax
80101c3c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c40:	89 04 24             	mov    %eax,(%esp)
80101c43:	e8 8e f8 ff ff       	call   801014d6 <bfree>
      ip->addrs[i] = 0;
80101c48:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c4e:	83 c2 04             	add    $0x4,%edx
80101c51:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c58:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c59:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c5d:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c61:	7e b6                	jle    80101c19 <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c63:	8b 45 08             	mov    0x8(%ebp),%eax
80101c66:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c69:	85 c0                	test   %eax,%eax
80101c6b:	0f 84 9b 00 00 00    	je     80101d0c <itrunc+0x102>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c71:	8b 45 08             	mov    0x8(%ebp),%eax
80101c74:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c77:	8b 45 08             	mov    0x8(%ebp),%eax
80101c7a:	8b 00                	mov    (%eax),%eax
80101c7c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c80:	89 04 24             	mov    %eax,(%esp)
80101c83:	e8 1e e5 ff ff       	call   801001a6 <bread>
80101c88:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c8b:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c8e:	83 c0 18             	add    $0x18,%eax
80101c91:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c94:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c9b:	eb 3b                	jmp    80101cd8 <itrunc+0xce>
      if(a[j])
80101c9d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca0:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101ca7:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101caa:	01 d0                	add    %edx,%eax
80101cac:	8b 00                	mov    (%eax),%eax
80101cae:	85 c0                	test   %eax,%eax
80101cb0:	74 22                	je     80101cd4 <itrunc+0xca>
        bfree(ip->dev, a[j]);
80101cb2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cb5:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101cbc:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101cbf:	01 d0                	add    %edx,%eax
80101cc1:	8b 10                	mov    (%eax),%edx
80101cc3:	8b 45 08             	mov    0x8(%ebp),%eax
80101cc6:	8b 00                	mov    (%eax),%eax
80101cc8:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ccc:	89 04 24             	mov    %eax,(%esp)
80101ccf:	e8 02 f8 ff ff       	call   801014d6 <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cd4:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cd8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cdb:	83 f8 7f             	cmp    $0x7f,%eax
80101cde:	76 bd                	jbe    80101c9d <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101ce0:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ce3:	89 04 24             	mov    %eax,(%esp)
80101ce6:	e8 2c e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ceb:	8b 45 08             	mov    0x8(%ebp),%eax
80101cee:	8b 50 4c             	mov    0x4c(%eax),%edx
80101cf1:	8b 45 08             	mov    0x8(%ebp),%eax
80101cf4:	8b 00                	mov    (%eax),%eax
80101cf6:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cfa:	89 04 24             	mov    %eax,(%esp)
80101cfd:	e8 d4 f7 ff ff       	call   801014d6 <bfree>
    ip->addrs[NDIRECT] = 0;
80101d02:	8b 45 08             	mov    0x8(%ebp),%eax
80101d05:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101d0c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d0f:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d16:	8b 45 08             	mov    0x8(%ebp),%eax
80101d19:	89 04 24             	mov    %eax,(%esp)
80101d1c:	e8 7e f9 ff ff       	call   8010169f <iupdate>
}
80101d21:	c9                   	leave  
80101d22:	c3                   	ret    

80101d23 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d23:	55                   	push   %ebp
80101d24:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d26:	8b 45 08             	mov    0x8(%ebp),%eax
80101d29:	8b 00                	mov    (%eax),%eax
80101d2b:	89 c2                	mov    %eax,%edx
80101d2d:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d30:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d33:	8b 45 08             	mov    0x8(%ebp),%eax
80101d36:	8b 50 04             	mov    0x4(%eax),%edx
80101d39:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d3c:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d3f:	8b 45 08             	mov    0x8(%ebp),%eax
80101d42:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d46:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d49:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d4f:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d53:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d56:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d5a:	8b 45 08             	mov    0x8(%ebp),%eax
80101d5d:	8b 50 18             	mov    0x18(%eax),%edx
80101d60:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d63:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d66:	5d                   	pop    %ebp
80101d67:	c3                   	ret    

80101d68 <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d68:	55                   	push   %ebp
80101d69:	89 e5                	mov    %esp,%ebp
80101d6b:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d6e:	8b 45 08             	mov    0x8(%ebp),%eax
80101d71:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d75:	66 83 f8 03          	cmp    $0x3,%ax
80101d79:	75 60                	jne    80101ddb <readi+0x73>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d7b:	8b 45 08             	mov    0x8(%ebp),%eax
80101d7e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d82:	66 85 c0             	test   %ax,%ax
80101d85:	78 20                	js     80101da7 <readi+0x3f>
80101d87:	8b 45 08             	mov    0x8(%ebp),%eax
80101d8a:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d8e:	66 83 f8 09          	cmp    $0x9,%ax
80101d92:	7f 13                	jg     80101da7 <readi+0x3f>
80101d94:	8b 45 08             	mov    0x8(%ebp),%eax
80101d97:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d9b:	98                   	cwtl   
80101d9c:	8b 04 c5 00 12 11 80 	mov    -0x7feeee00(,%eax,8),%eax
80101da3:	85 c0                	test   %eax,%eax
80101da5:	75 0a                	jne    80101db1 <readi+0x49>
      return -1;
80101da7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101dac:	e9 19 01 00 00       	jmp    80101eca <readi+0x162>
    return devsw[ip->major].read(ip, dst, n);
80101db1:	8b 45 08             	mov    0x8(%ebp),%eax
80101db4:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101db8:	98                   	cwtl   
80101db9:	8b 04 c5 00 12 11 80 	mov    -0x7feeee00(,%eax,8),%eax
80101dc0:	8b 55 14             	mov    0x14(%ebp),%edx
80101dc3:	89 54 24 08          	mov    %edx,0x8(%esp)
80101dc7:	8b 55 0c             	mov    0xc(%ebp),%edx
80101dca:	89 54 24 04          	mov    %edx,0x4(%esp)
80101dce:	8b 55 08             	mov    0x8(%ebp),%edx
80101dd1:	89 14 24             	mov    %edx,(%esp)
80101dd4:	ff d0                	call   *%eax
80101dd6:	e9 ef 00 00 00       	jmp    80101eca <readi+0x162>
  }

  if(off > ip->size || off + n < off)
80101ddb:	8b 45 08             	mov    0x8(%ebp),%eax
80101dde:	8b 40 18             	mov    0x18(%eax),%eax
80101de1:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de4:	72 0d                	jb     80101df3 <readi+0x8b>
80101de6:	8b 45 14             	mov    0x14(%ebp),%eax
80101de9:	8b 55 10             	mov    0x10(%ebp),%edx
80101dec:	01 d0                	add    %edx,%eax
80101dee:	3b 45 10             	cmp    0x10(%ebp),%eax
80101df1:	73 0a                	jae    80101dfd <readi+0x95>
    return -1;
80101df3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101df8:	e9 cd 00 00 00       	jmp    80101eca <readi+0x162>
  if(off + n > ip->size)
80101dfd:	8b 45 14             	mov    0x14(%ebp),%eax
80101e00:	8b 55 10             	mov    0x10(%ebp),%edx
80101e03:	01 c2                	add    %eax,%edx
80101e05:	8b 45 08             	mov    0x8(%ebp),%eax
80101e08:	8b 40 18             	mov    0x18(%eax),%eax
80101e0b:	39 c2                	cmp    %eax,%edx
80101e0d:	76 0c                	jbe    80101e1b <readi+0xb3>
    n = ip->size - off;
80101e0f:	8b 45 08             	mov    0x8(%ebp),%eax
80101e12:	8b 40 18             	mov    0x18(%eax),%eax
80101e15:	2b 45 10             	sub    0x10(%ebp),%eax
80101e18:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e1b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e22:	e9 94 00 00 00       	jmp    80101ebb <readi+0x153>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e27:	8b 45 10             	mov    0x10(%ebp),%eax
80101e2a:	c1 e8 09             	shr    $0x9,%eax
80101e2d:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e31:	8b 45 08             	mov    0x8(%ebp),%eax
80101e34:	89 04 24             	mov    %eax,(%esp)
80101e37:	e8 c1 fc ff ff       	call   80101afd <bmap>
80101e3c:	8b 55 08             	mov    0x8(%ebp),%edx
80101e3f:	8b 12                	mov    (%edx),%edx
80101e41:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e45:	89 14 24             	mov    %edx,(%esp)
80101e48:	e8 59 e3 ff ff       	call   801001a6 <bread>
80101e4d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e50:	8b 45 10             	mov    0x10(%ebp),%eax
80101e53:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e58:	89 c2                	mov    %eax,%edx
80101e5a:	b8 00 02 00 00       	mov    $0x200,%eax
80101e5f:	29 d0                	sub    %edx,%eax
80101e61:	89 c2                	mov    %eax,%edx
80101e63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e66:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e69:	29 c1                	sub    %eax,%ecx
80101e6b:	89 c8                	mov    %ecx,%eax
80101e6d:	39 c2                	cmp    %eax,%edx
80101e6f:	0f 46 c2             	cmovbe %edx,%eax
80101e72:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e75:	8b 45 10             	mov    0x10(%ebp),%eax
80101e78:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e7d:	8d 50 10             	lea    0x10(%eax),%edx
80101e80:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e83:	01 d0                	add    %edx,%eax
80101e85:	8d 50 08             	lea    0x8(%eax),%edx
80101e88:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e8b:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e8f:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e93:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e96:	89 04 24             	mov    %eax,(%esp)
80101e99:	e8 f8 33 00 00       	call   80105296 <memmove>
    brelse(bp);
80101e9e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ea1:	89 04 24             	mov    %eax,(%esp)
80101ea4:	e8 6e e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101ea9:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eac:	01 45 f4             	add    %eax,-0xc(%ebp)
80101eaf:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb2:	01 45 10             	add    %eax,0x10(%ebp)
80101eb5:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb8:	01 45 0c             	add    %eax,0xc(%ebp)
80101ebb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101ebe:	3b 45 14             	cmp    0x14(%ebp),%eax
80101ec1:	0f 82 60 ff ff ff    	jb     80101e27 <readi+0xbf>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101ec7:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101eca:	c9                   	leave  
80101ecb:	c3                   	ret    

80101ecc <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ecc:	55                   	push   %ebp
80101ecd:	89 e5                	mov    %esp,%ebp
80101ecf:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ed2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed5:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ed9:	66 83 f8 03          	cmp    $0x3,%ax
80101edd:	75 60                	jne    80101f3f <writei+0x73>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101edf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee2:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee6:	66 85 c0             	test   %ax,%ax
80101ee9:	78 20                	js     80101f0b <writei+0x3f>
80101eeb:	8b 45 08             	mov    0x8(%ebp),%eax
80101eee:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef2:	66 83 f8 09          	cmp    $0x9,%ax
80101ef6:	7f 13                	jg     80101f0b <writei+0x3f>
80101ef8:	8b 45 08             	mov    0x8(%ebp),%eax
80101efb:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101eff:	98                   	cwtl   
80101f00:	8b 04 c5 04 12 11 80 	mov    -0x7feeedfc(,%eax,8),%eax
80101f07:	85 c0                	test   %eax,%eax
80101f09:	75 0a                	jne    80101f15 <writei+0x49>
      return -1;
80101f0b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f10:	e9 44 01 00 00       	jmp    80102059 <writei+0x18d>
    return devsw[ip->major].write(ip, src, n);
80101f15:	8b 45 08             	mov    0x8(%ebp),%eax
80101f18:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f1c:	98                   	cwtl   
80101f1d:	8b 04 c5 04 12 11 80 	mov    -0x7feeedfc(,%eax,8),%eax
80101f24:	8b 55 14             	mov    0x14(%ebp),%edx
80101f27:	89 54 24 08          	mov    %edx,0x8(%esp)
80101f2b:	8b 55 0c             	mov    0xc(%ebp),%edx
80101f2e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101f32:	8b 55 08             	mov    0x8(%ebp),%edx
80101f35:	89 14 24             	mov    %edx,(%esp)
80101f38:	ff d0                	call   *%eax
80101f3a:	e9 1a 01 00 00       	jmp    80102059 <writei+0x18d>
  }

  if(off > ip->size || off + n < off)
80101f3f:	8b 45 08             	mov    0x8(%ebp),%eax
80101f42:	8b 40 18             	mov    0x18(%eax),%eax
80101f45:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f48:	72 0d                	jb     80101f57 <writei+0x8b>
80101f4a:	8b 45 14             	mov    0x14(%ebp),%eax
80101f4d:	8b 55 10             	mov    0x10(%ebp),%edx
80101f50:	01 d0                	add    %edx,%eax
80101f52:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f55:	73 0a                	jae    80101f61 <writei+0x95>
    return -1;
80101f57:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f5c:	e9 f8 00 00 00       	jmp    80102059 <writei+0x18d>
  if(off + n > MAXFILE*BSIZE)
80101f61:	8b 45 14             	mov    0x14(%ebp),%eax
80101f64:	8b 55 10             	mov    0x10(%ebp),%edx
80101f67:	01 d0                	add    %edx,%eax
80101f69:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f6e:	76 0a                	jbe    80101f7a <writei+0xae>
    return -1;
80101f70:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f75:	e9 df 00 00 00       	jmp    80102059 <writei+0x18d>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f7a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f81:	e9 9f 00 00 00       	jmp    80102025 <writei+0x159>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f86:	8b 45 10             	mov    0x10(%ebp),%eax
80101f89:	c1 e8 09             	shr    $0x9,%eax
80101f8c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f90:	8b 45 08             	mov    0x8(%ebp),%eax
80101f93:	89 04 24             	mov    %eax,(%esp)
80101f96:	e8 62 fb ff ff       	call   80101afd <bmap>
80101f9b:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9e:	8b 12                	mov    (%edx),%edx
80101fa0:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa4:	89 14 24             	mov    %edx,(%esp)
80101fa7:	e8 fa e1 ff ff       	call   801001a6 <bread>
80101fac:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101faf:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb2:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fb7:	89 c2                	mov    %eax,%edx
80101fb9:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbe:	29 d0                	sub    %edx,%eax
80101fc0:	89 c2                	mov    %eax,%edx
80101fc2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc5:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc8:	29 c1                	sub    %eax,%ecx
80101fca:	89 c8                	mov    %ecx,%eax
80101fcc:	39 c2                	cmp    %eax,%edx
80101fce:	0f 46 c2             	cmovbe %edx,%eax
80101fd1:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fd4:	8b 45 10             	mov    0x10(%ebp),%eax
80101fd7:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fdc:	8d 50 10             	lea    0x10(%eax),%edx
80101fdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fe2:	01 d0                	add    %edx,%eax
80101fe4:	8d 50 08             	lea    0x8(%eax),%edx
80101fe7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fea:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fee:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101ff5:	89 14 24             	mov    %edx,(%esp)
80101ff8:	e8 99 32 00 00       	call   80105296 <memmove>
    log_write(bp);
80101ffd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102000:	89 04 24             	mov    %eax,(%esp)
80102003:	e8 13 16 00 00       	call   8010361b <log_write>
    brelse(bp);
80102008:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010200b:	89 04 24             	mov    %eax,(%esp)
8010200e:	e8 04 e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102013:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102016:	01 45 f4             	add    %eax,-0xc(%ebp)
80102019:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201c:	01 45 10             	add    %eax,0x10(%ebp)
8010201f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102022:	01 45 0c             	add    %eax,0xc(%ebp)
80102025:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102028:	3b 45 14             	cmp    0x14(%ebp),%eax
8010202b:	0f 82 55 ff ff ff    	jb     80101f86 <writei+0xba>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102031:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102035:	74 1f                	je     80102056 <writei+0x18a>
80102037:	8b 45 08             	mov    0x8(%ebp),%eax
8010203a:	8b 40 18             	mov    0x18(%eax),%eax
8010203d:	3b 45 10             	cmp    0x10(%ebp),%eax
80102040:	73 14                	jae    80102056 <writei+0x18a>
    ip->size = off;
80102042:	8b 45 08             	mov    0x8(%ebp),%eax
80102045:	8b 55 10             	mov    0x10(%ebp),%edx
80102048:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	89 04 24             	mov    %eax,(%esp)
80102051:	e8 49 f6 ff ff       	call   8010169f <iupdate>
  }
  return n;
80102056:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102059:	c9                   	leave  
8010205a:	c3                   	ret    

8010205b <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
8010205b:	55                   	push   %ebp
8010205c:	89 e5                	mov    %esp,%ebp
8010205e:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
80102061:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
80102068:	00 
80102069:	8b 45 0c             	mov    0xc(%ebp),%eax
8010206c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102070:	8b 45 08             	mov    0x8(%ebp),%eax
80102073:	89 04 24             	mov    %eax,(%esp)
80102076:	e8 be 32 00 00       	call   80105339 <strncmp>
}
8010207b:	c9                   	leave  
8010207c:	c3                   	ret    

8010207d <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
8010207d:	55                   	push   %ebp
8010207e:	89 e5                	mov    %esp,%ebp
80102080:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80102083:	8b 45 08             	mov    0x8(%ebp),%eax
80102086:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010208a:	66 83 f8 01          	cmp    $0x1,%ax
8010208e:	74 0c                	je     8010209c <dirlookup+0x1f>
    panic("dirlookup not DIR");
80102090:	c7 04 24 55 8a 10 80 	movl   $0x80108a55,(%esp)
80102097:	e8 9e e4 ff ff       	call   8010053a <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
8010209c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020a3:	e9 88 00 00 00       	jmp    80102130 <dirlookup+0xb3>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020a8:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020af:	00 
801020b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020b3:	89 44 24 08          	mov    %eax,0x8(%esp)
801020b7:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020ba:	89 44 24 04          	mov    %eax,0x4(%esp)
801020be:	8b 45 08             	mov    0x8(%ebp),%eax
801020c1:	89 04 24             	mov    %eax,(%esp)
801020c4:	e8 9f fc ff ff       	call   80101d68 <readi>
801020c9:	83 f8 10             	cmp    $0x10,%eax
801020cc:	74 0c                	je     801020da <dirlookup+0x5d>
      panic("dirlink read");
801020ce:	c7 04 24 67 8a 10 80 	movl   $0x80108a67,(%esp)
801020d5:	e8 60 e4 ff ff       	call   8010053a <panic>
    if(de.inum == 0)
801020da:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020de:	66 85 c0             	test   %ax,%ax
801020e1:	75 02                	jne    801020e5 <dirlookup+0x68>
      continue;
801020e3:	eb 47                	jmp    8010212c <dirlookup+0xaf>
    if(namecmp(name, de.name) == 0){
801020e5:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020e8:	83 c0 02             	add    $0x2,%eax
801020eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801020ef:	8b 45 0c             	mov    0xc(%ebp),%eax
801020f2:	89 04 24             	mov    %eax,(%esp)
801020f5:	e8 61 ff ff ff       	call   8010205b <namecmp>
801020fa:	85 c0                	test   %eax,%eax
801020fc:	75 2e                	jne    8010212c <dirlookup+0xaf>
      // entry matches path element
      if(poff)
801020fe:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102102:	74 08                	je     8010210c <dirlookup+0x8f>
        *poff = off;
80102104:	8b 45 10             	mov    0x10(%ebp),%eax
80102107:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010210a:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010210c:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
80102110:	0f b7 c0             	movzwl %ax,%eax
80102113:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102116:	8b 45 08             	mov    0x8(%ebp),%eax
80102119:	8b 00                	mov    (%eax),%eax
8010211b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010211e:	89 54 24 04          	mov    %edx,0x4(%esp)
80102122:	89 04 24             	mov    %eax,(%esp)
80102125:	e8 2d f6 ff ff       	call   80101757 <iget>
8010212a:	eb 18                	jmp    80102144 <dirlookup+0xc7>
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010212c:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80102130:	8b 45 08             	mov    0x8(%ebp),%eax
80102133:	8b 40 18             	mov    0x18(%eax),%eax
80102136:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102139:	0f 87 69 ff ff ff    	ja     801020a8 <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
8010213f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102144:	c9                   	leave  
80102145:	c3                   	ret    

80102146 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102146:	55                   	push   %ebp
80102147:	89 e5                	mov    %esp,%ebp
80102149:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
8010214c:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102153:	00 
80102154:	8b 45 0c             	mov    0xc(%ebp),%eax
80102157:	89 44 24 04          	mov    %eax,0x4(%esp)
8010215b:	8b 45 08             	mov    0x8(%ebp),%eax
8010215e:	89 04 24             	mov    %eax,(%esp)
80102161:	e8 17 ff ff ff       	call   8010207d <dirlookup>
80102166:	89 45 f0             	mov    %eax,-0x10(%ebp)
80102169:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010216d:	74 15                	je     80102184 <dirlink+0x3e>
    iput(ip);
8010216f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102172:	89 04 24             	mov    %eax,(%esp)
80102175:	e8 94 f8 ff ff       	call   80101a0e <iput>
    return -1;
8010217a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010217f:	e9 b7 00 00 00       	jmp    8010223b <dirlink+0xf5>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102184:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010218b:	eb 46                	jmp    801021d3 <dirlink+0x8d>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010218d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102190:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102197:	00 
80102198:	89 44 24 08          	mov    %eax,0x8(%esp)
8010219c:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010219f:	89 44 24 04          	mov    %eax,0x4(%esp)
801021a3:	8b 45 08             	mov    0x8(%ebp),%eax
801021a6:	89 04 24             	mov    %eax,(%esp)
801021a9:	e8 ba fb ff ff       	call   80101d68 <readi>
801021ae:	83 f8 10             	cmp    $0x10,%eax
801021b1:	74 0c                	je     801021bf <dirlink+0x79>
      panic("dirlink read");
801021b3:	c7 04 24 67 8a 10 80 	movl   $0x80108a67,(%esp)
801021ba:	e8 7b e3 ff ff       	call   8010053a <panic>
    if(de.inum == 0)
801021bf:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021c3:	66 85 c0             	test   %ax,%ax
801021c6:	75 02                	jne    801021ca <dirlink+0x84>
      break;
801021c8:	eb 16                	jmp    801021e0 <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021cd:	83 c0 10             	add    $0x10,%eax
801021d0:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021d3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021d6:	8b 45 08             	mov    0x8(%ebp),%eax
801021d9:	8b 40 18             	mov    0x18(%eax),%eax
801021dc:	39 c2                	cmp    %eax,%edx
801021de:	72 ad                	jb     8010218d <dirlink+0x47>
      panic("dirlink read");
    if(de.inum == 0)
      break;
  }

  strncpy(de.name, name, DIRSIZ);
801021e0:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021e7:	00 
801021e8:	8b 45 0c             	mov    0xc(%ebp),%eax
801021eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801021ef:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021f2:	83 c0 02             	add    $0x2,%eax
801021f5:	89 04 24             	mov    %eax,(%esp)
801021f8:	e8 92 31 00 00       	call   8010538f <strncpy>
  de.inum = inum;
801021fd:	8b 45 10             	mov    0x10(%ebp),%eax
80102200:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102204:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102207:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010220e:	00 
8010220f:	89 44 24 08          	mov    %eax,0x8(%esp)
80102213:	8d 45 e0             	lea    -0x20(%ebp),%eax
80102216:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221a:	8b 45 08             	mov    0x8(%ebp),%eax
8010221d:	89 04 24             	mov    %eax,(%esp)
80102220:	e8 a7 fc ff ff       	call   80101ecc <writei>
80102225:	83 f8 10             	cmp    $0x10,%eax
80102228:	74 0c                	je     80102236 <dirlink+0xf0>
    panic("dirlink");
8010222a:	c7 04 24 74 8a 10 80 	movl   $0x80108a74,(%esp)
80102231:	e8 04 e3 ff ff       	call   8010053a <panic>
  
  return 0;
80102236:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010223b:	c9                   	leave  
8010223c:	c3                   	ret    

8010223d <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
8010223d:	55                   	push   %ebp
8010223e:	89 e5                	mov    %esp,%ebp
80102240:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
80102243:	eb 04                	jmp    80102249 <skipelem+0xc>
    path++;
80102245:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
80102249:	8b 45 08             	mov    0x8(%ebp),%eax
8010224c:	0f b6 00             	movzbl (%eax),%eax
8010224f:	3c 2f                	cmp    $0x2f,%al
80102251:	74 f2                	je     80102245 <skipelem+0x8>
    path++;
  if(*path == 0)
80102253:	8b 45 08             	mov    0x8(%ebp),%eax
80102256:	0f b6 00             	movzbl (%eax),%eax
80102259:	84 c0                	test   %al,%al
8010225b:	75 0a                	jne    80102267 <skipelem+0x2a>
    return 0;
8010225d:	b8 00 00 00 00       	mov    $0x0,%eax
80102262:	e9 86 00 00 00       	jmp    801022ed <skipelem+0xb0>
  s = path;
80102267:	8b 45 08             	mov    0x8(%ebp),%eax
8010226a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
8010226d:	eb 04                	jmp    80102273 <skipelem+0x36>
    path++;
8010226f:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
80102273:	8b 45 08             	mov    0x8(%ebp),%eax
80102276:	0f b6 00             	movzbl (%eax),%eax
80102279:	3c 2f                	cmp    $0x2f,%al
8010227b:	74 0a                	je     80102287 <skipelem+0x4a>
8010227d:	8b 45 08             	mov    0x8(%ebp),%eax
80102280:	0f b6 00             	movzbl (%eax),%eax
80102283:	84 c0                	test   %al,%al
80102285:	75 e8                	jne    8010226f <skipelem+0x32>
    path++;
  len = path - s;
80102287:	8b 55 08             	mov    0x8(%ebp),%edx
8010228a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010228d:	29 c2                	sub    %eax,%edx
8010228f:	89 d0                	mov    %edx,%eax
80102291:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
80102294:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
80102298:	7e 1c                	jle    801022b6 <skipelem+0x79>
    memmove(name, s, DIRSIZ);
8010229a:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801022a1:	00 
801022a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022a5:	89 44 24 04          	mov    %eax,0x4(%esp)
801022a9:	8b 45 0c             	mov    0xc(%ebp),%eax
801022ac:	89 04 24             	mov    %eax,(%esp)
801022af:	e8 e2 2f 00 00       	call   80105296 <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022b4:	eb 2a                	jmp    801022e0 <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022b6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022b9:	89 44 24 08          	mov    %eax,0x8(%esp)
801022bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022c0:	89 44 24 04          	mov    %eax,0x4(%esp)
801022c4:	8b 45 0c             	mov    0xc(%ebp),%eax
801022c7:	89 04 24             	mov    %eax,(%esp)
801022ca:	e8 c7 2f 00 00       	call   80105296 <memmove>
    name[len] = 0;
801022cf:	8b 55 f0             	mov    -0x10(%ebp),%edx
801022d2:	8b 45 0c             	mov    0xc(%ebp),%eax
801022d5:	01 d0                	add    %edx,%eax
801022d7:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022da:	eb 04                	jmp    801022e0 <skipelem+0xa3>
    path++;
801022dc:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022e0:	8b 45 08             	mov    0x8(%ebp),%eax
801022e3:	0f b6 00             	movzbl (%eax),%eax
801022e6:	3c 2f                	cmp    $0x2f,%al
801022e8:	74 f2                	je     801022dc <skipelem+0x9f>
    path++;
  return path;
801022ea:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022ed:	c9                   	leave  
801022ee:	c3                   	ret    

801022ef <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022ef:	55                   	push   %ebp
801022f0:	89 e5                	mov    %esp,%ebp
801022f2:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;

  if(*path == '/')
801022f5:	8b 45 08             	mov    0x8(%ebp),%eax
801022f8:	0f b6 00             	movzbl (%eax),%eax
801022fb:	3c 2f                	cmp    $0x2f,%al
801022fd:	75 1c                	jne    8010231b <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
801022ff:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102306:	00 
80102307:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010230e:	e8 44 f4 ff ff       	call   80101757 <iget>
80102313:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
80102316:	e9 af 00 00 00       	jmp    801023ca <namex+0xdb>
  struct inode *ip, *next;

  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
8010231b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102321:	8b 40 68             	mov    0x68(%eax),%eax
80102324:	89 04 24             	mov    %eax,(%esp)
80102327:	e8 fd f4 ff ff       	call   80101829 <idup>
8010232c:	89 45 f4             	mov    %eax,-0xc(%ebp)

  while((path = skipelem(path, name)) != 0){
8010232f:	e9 96 00 00 00       	jmp    801023ca <namex+0xdb>
    ilock(ip);
80102334:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102337:	89 04 24             	mov    %eax,(%esp)
8010233a:	e8 1c f5 ff ff       	call   8010185b <ilock>
    if(ip->type != T_DIR){
8010233f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102342:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80102346:	66 83 f8 01          	cmp    $0x1,%ax
8010234a:	74 15                	je     80102361 <namex+0x72>
      iunlockput(ip);
8010234c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010234f:	89 04 24             	mov    %eax,(%esp)
80102352:	e8 88 f7 ff ff       	call   80101adf <iunlockput>
      return 0;
80102357:	b8 00 00 00 00       	mov    $0x0,%eax
8010235c:	e9 a3 00 00 00       	jmp    80102404 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
80102361:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102365:	74 1d                	je     80102384 <namex+0x95>
80102367:	8b 45 08             	mov    0x8(%ebp),%eax
8010236a:	0f b6 00             	movzbl (%eax),%eax
8010236d:	84 c0                	test   %al,%al
8010236f:	75 13                	jne    80102384 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
80102371:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102374:	89 04 24             	mov    %eax,(%esp)
80102377:	e8 2d f6 ff ff       	call   801019a9 <iunlock>
      return ip;
8010237c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010237f:	e9 80 00 00 00       	jmp    80102404 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102384:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010238b:	00 
8010238c:	8b 45 10             	mov    0x10(%ebp),%eax
8010238f:	89 44 24 04          	mov    %eax,0x4(%esp)
80102393:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102396:	89 04 24             	mov    %eax,(%esp)
80102399:	e8 df fc ff ff       	call   8010207d <dirlookup>
8010239e:	89 45 f0             	mov    %eax,-0x10(%ebp)
801023a1:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a5:	75 12                	jne    801023b9 <namex+0xca>
      iunlockput(ip);
801023a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023aa:	89 04 24             	mov    %eax,(%esp)
801023ad:	e8 2d f7 ff ff       	call   80101adf <iunlockput>
      return 0;
801023b2:	b8 00 00 00 00       	mov    $0x0,%eax
801023b7:	eb 4b                	jmp    80102404 <namex+0x115>
    }
    iunlockput(ip);
801023b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023bc:	89 04 24             	mov    %eax,(%esp)
801023bf:	e8 1b f7 ff ff       	call   80101adf <iunlockput>
    ip = next;
801023c4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
801023ca:	8b 45 10             	mov    0x10(%ebp),%eax
801023cd:	89 44 24 04          	mov    %eax,0x4(%esp)
801023d1:	8b 45 08             	mov    0x8(%ebp),%eax
801023d4:	89 04 24             	mov    %eax,(%esp)
801023d7:	e8 61 fe ff ff       	call   8010223d <skipelem>
801023dc:	89 45 08             	mov    %eax,0x8(%ebp)
801023df:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023e3:	0f 85 4b ff ff ff    	jne    80102334 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023e9:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023ed:	74 12                	je     80102401 <namex+0x112>
    iput(ip);
801023ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023f2:	89 04 24             	mov    %eax,(%esp)
801023f5:	e8 14 f6 ff ff       	call   80101a0e <iput>
    return 0;
801023fa:	b8 00 00 00 00       	mov    $0x0,%eax
801023ff:	eb 03                	jmp    80102404 <namex+0x115>
  }
  return ip;
80102401:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102404:	c9                   	leave  
80102405:	c3                   	ret    

80102406 <namei>:

struct inode*
namei(char *path)
{
80102406:	55                   	push   %ebp
80102407:	89 e5                	mov    %esp,%ebp
80102409:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
8010240c:	8d 45 ea             	lea    -0x16(%ebp),%eax
8010240f:	89 44 24 08          	mov    %eax,0x8(%esp)
80102413:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010241a:	00 
8010241b:	8b 45 08             	mov    0x8(%ebp),%eax
8010241e:	89 04 24             	mov    %eax,(%esp)
80102421:	e8 c9 fe ff ff       	call   801022ef <namex>
}
80102426:	c9                   	leave  
80102427:	c3                   	ret    

80102428 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102428:	55                   	push   %ebp
80102429:	89 e5                	mov    %esp,%ebp
8010242b:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
8010242e:	8b 45 0c             	mov    0xc(%ebp),%eax
80102431:	89 44 24 08          	mov    %eax,0x8(%esp)
80102435:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010243c:	00 
8010243d:	8b 45 08             	mov    0x8(%ebp),%eax
80102440:	89 04 24             	mov    %eax,(%esp)
80102443:	e8 a7 fe ff ff       	call   801022ef <namex>
}
80102448:	c9                   	leave  
80102449:	c3                   	ret    

8010244a <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
8010244a:	55                   	push   %ebp
8010244b:	89 e5                	mov    %esp,%ebp
8010244d:	83 ec 14             	sub    $0x14,%esp
80102450:	8b 45 08             	mov    0x8(%ebp),%eax
80102453:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102457:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
8010245b:	89 c2                	mov    %eax,%edx
8010245d:	ec                   	in     (%dx),%al
8010245e:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102461:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102465:	c9                   	leave  
80102466:	c3                   	ret    

80102467 <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
80102467:	55                   	push   %ebp
80102468:	89 e5                	mov    %esp,%ebp
8010246a:	57                   	push   %edi
8010246b:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
8010246c:	8b 55 08             	mov    0x8(%ebp),%edx
8010246f:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102472:	8b 45 10             	mov    0x10(%ebp),%eax
80102475:	89 cb                	mov    %ecx,%ebx
80102477:	89 df                	mov    %ebx,%edi
80102479:	89 c1                	mov    %eax,%ecx
8010247b:	fc                   	cld    
8010247c:	f3 6d                	rep insl (%dx),%es:(%edi)
8010247e:	89 c8                	mov    %ecx,%eax
80102480:	89 fb                	mov    %edi,%ebx
80102482:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102485:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
80102488:	5b                   	pop    %ebx
80102489:	5f                   	pop    %edi
8010248a:	5d                   	pop    %ebp
8010248b:	c3                   	ret    

8010248c <outb>:

static inline void
outb(ushort port, uchar data)
{
8010248c:	55                   	push   %ebp
8010248d:	89 e5                	mov    %esp,%ebp
8010248f:	83 ec 08             	sub    $0x8,%esp
80102492:	8b 55 08             	mov    0x8(%ebp),%edx
80102495:	8b 45 0c             	mov    0xc(%ebp),%eax
80102498:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010249c:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010249f:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024a3:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024a7:	ee                   	out    %al,(%dx)
}
801024a8:	c9                   	leave  
801024a9:	c3                   	ret    

801024aa <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024aa:	55                   	push   %ebp
801024ab:	89 e5                	mov    %esp,%ebp
801024ad:	56                   	push   %esi
801024ae:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024af:	8b 55 08             	mov    0x8(%ebp),%edx
801024b2:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024b5:	8b 45 10             	mov    0x10(%ebp),%eax
801024b8:	89 cb                	mov    %ecx,%ebx
801024ba:	89 de                	mov    %ebx,%esi
801024bc:	89 c1                	mov    %eax,%ecx
801024be:	fc                   	cld    
801024bf:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024c1:	89 c8                	mov    %ecx,%eax
801024c3:	89 f3                	mov    %esi,%ebx
801024c5:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024c8:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024cb:	5b                   	pop    %ebx
801024cc:	5e                   	pop    %esi
801024cd:	5d                   	pop    %ebp
801024ce:	c3                   	ret    

801024cf <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024cf:	55                   	push   %ebp
801024d0:	89 e5                	mov    %esp,%ebp
801024d2:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024d5:	90                   	nop
801024d6:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024dd:	e8 68 ff ff ff       	call   8010244a <inb>
801024e2:	0f b6 c0             	movzbl %al,%eax
801024e5:	89 45 fc             	mov    %eax,-0x4(%ebp)
801024e8:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024eb:	25 c0 00 00 00       	and    $0xc0,%eax
801024f0:	83 f8 40             	cmp    $0x40,%eax
801024f3:	75 e1                	jne    801024d6 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
801024f5:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801024f9:	74 11                	je     8010250c <idewait+0x3d>
801024fb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024fe:	83 e0 21             	and    $0x21,%eax
80102501:	85 c0                	test   %eax,%eax
80102503:	74 07                	je     8010250c <idewait+0x3d>
    return -1;
80102505:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010250a:	eb 05                	jmp    80102511 <idewait+0x42>
  return 0;
8010250c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102511:	c9                   	leave  
80102512:	c3                   	ret    

80102513 <ideinit>:

void
ideinit(void)
{
80102513:	55                   	push   %ebp
80102514:	89 e5                	mov    %esp,%ebp
80102516:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
80102519:	c7 44 24 04 7c 8a 10 	movl   $0x80108a7c,0x4(%esp)
80102520:	80 
80102521:	c7 04 24 20 b6 10 80 	movl   $0x8010b620,(%esp)
80102528:	e8 25 2a 00 00       	call   80104f52 <initlock>
  picenable(IRQ_IDE);
8010252d:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102534:	e8 63 18 00 00       	call   80103d9c <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102539:	a1 60 29 11 80       	mov    0x80112960,%eax
8010253e:	83 e8 01             	sub    $0x1,%eax
80102541:	89 44 24 04          	mov    %eax,0x4(%esp)
80102545:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010254c:	e8 0c 04 00 00       	call   8010295d <ioapicenable>
  idewait(0);
80102551:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102558:	e8 72 ff ff ff       	call   801024cf <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
8010255d:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
80102564:	00 
80102565:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010256c:	e8 1b ff ff ff       	call   8010248c <outb>
  for(i=0; i<1000; i++){
80102571:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102578:	eb 20                	jmp    8010259a <ideinit+0x87>
    if(inb(0x1f7) != 0){
8010257a:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
80102581:	e8 c4 fe ff ff       	call   8010244a <inb>
80102586:	84 c0                	test   %al,%al
80102588:	74 0c                	je     80102596 <ideinit+0x83>
      havedisk1 = 1;
8010258a:	c7 05 58 b6 10 80 01 	movl   $0x1,0x8010b658
80102591:	00 00 00 
      break;
80102594:	eb 0d                	jmp    801025a3 <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
80102596:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010259a:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025a1:	7e d7                	jle    8010257a <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025a3:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025aa:	00 
801025ab:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025b2:	e8 d5 fe ff ff       	call   8010248c <outb>
}
801025b7:	c9                   	leave  
801025b8:	c3                   	ret    

801025b9 <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025b9:	55                   	push   %ebp
801025ba:	89 e5                	mov    %esp,%ebp
801025bc:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025bf:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025c3:	75 0c                	jne    801025d1 <idestart+0x18>
    panic("idestart");
801025c5:	c7 04 24 80 8a 10 80 	movl   $0x80108a80,(%esp)
801025cc:	e8 69 df ff ff       	call   8010053a <panic>

  idewait(0);
801025d1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025d8:	e8 f2 fe ff ff       	call   801024cf <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025dd:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025e4:	00 
801025e5:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
801025ec:	e8 9b fe ff ff       	call   8010248c <outb>
  outb(0x1f2, 1);  // number of sectors
801025f1:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801025f8:	00 
801025f9:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
80102600:	e8 87 fe ff ff       	call   8010248c <outb>
  outb(0x1f3, b->sector & 0xff);
80102605:	8b 45 08             	mov    0x8(%ebp),%eax
80102608:	8b 40 08             	mov    0x8(%eax),%eax
8010260b:	0f b6 c0             	movzbl %al,%eax
8010260e:	89 44 24 04          	mov    %eax,0x4(%esp)
80102612:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
80102619:	e8 6e fe ff ff       	call   8010248c <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
8010261e:	8b 45 08             	mov    0x8(%ebp),%eax
80102621:	8b 40 08             	mov    0x8(%eax),%eax
80102624:	c1 e8 08             	shr    $0x8,%eax
80102627:	0f b6 c0             	movzbl %al,%eax
8010262a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010262e:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
80102635:	e8 52 fe ff ff       	call   8010248c <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
8010263a:	8b 45 08             	mov    0x8(%ebp),%eax
8010263d:	8b 40 08             	mov    0x8(%eax),%eax
80102640:	c1 e8 10             	shr    $0x10,%eax
80102643:	0f b6 c0             	movzbl %al,%eax
80102646:	89 44 24 04          	mov    %eax,0x4(%esp)
8010264a:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
80102651:	e8 36 fe ff ff       	call   8010248c <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
80102656:	8b 45 08             	mov    0x8(%ebp),%eax
80102659:	8b 40 04             	mov    0x4(%eax),%eax
8010265c:	83 e0 01             	and    $0x1,%eax
8010265f:	c1 e0 04             	shl    $0x4,%eax
80102662:	89 c2                	mov    %eax,%edx
80102664:	8b 45 08             	mov    0x8(%ebp),%eax
80102667:	8b 40 08             	mov    0x8(%eax),%eax
8010266a:	c1 e8 18             	shr    $0x18,%eax
8010266d:	83 e0 0f             	and    $0xf,%eax
80102670:	09 d0                	or     %edx,%eax
80102672:	83 c8 e0             	or     $0xffffffe0,%eax
80102675:	0f b6 c0             	movzbl %al,%eax
80102678:	89 44 24 04          	mov    %eax,0x4(%esp)
8010267c:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102683:	e8 04 fe ff ff       	call   8010248c <outb>
  if(b->flags & B_DIRTY){
80102688:	8b 45 08             	mov    0x8(%ebp),%eax
8010268b:	8b 00                	mov    (%eax),%eax
8010268d:	83 e0 04             	and    $0x4,%eax
80102690:	85 c0                	test   %eax,%eax
80102692:	74 34                	je     801026c8 <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
80102694:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
8010269b:	00 
8010269c:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026a3:	e8 e4 fd ff ff       	call   8010248c <outb>
    outsl(0x1f0, b->data, 512/4);
801026a8:	8b 45 08             	mov    0x8(%ebp),%eax
801026ab:	83 c0 18             	add    $0x18,%eax
801026ae:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026b5:	00 
801026b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801026ba:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026c1:	e8 e4 fd ff ff       	call   801024aa <outsl>
801026c6:	eb 14                	jmp    801026dc <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026c8:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026cf:	00 
801026d0:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026d7:	e8 b0 fd ff ff       	call   8010248c <outb>
  }
}
801026dc:	c9                   	leave  
801026dd:	c3                   	ret    

801026de <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026de:	55                   	push   %ebp
801026df:	89 e5                	mov    %esp,%ebp
801026e1:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026e4:	c7 04 24 20 b6 10 80 	movl   $0x8010b620,(%esp)
801026eb:	e8 83 28 00 00       	call   80104f73 <acquire>
  if((b = idequeue) == 0){
801026f0:	a1 54 b6 10 80       	mov    0x8010b654,%eax
801026f5:	89 45 f4             	mov    %eax,-0xc(%ebp)
801026f8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801026fc:	75 11                	jne    8010270f <ideintr+0x31>
    release(&idelock);
801026fe:	c7 04 24 20 b6 10 80 	movl   $0x8010b620,(%esp)
80102705:	e8 cb 28 00 00       	call   80104fd5 <release>
    // cprintf("spurious IDE interrupt\n");
    return;
8010270a:	e9 90 00 00 00       	jmp    8010279f <ideintr+0xc1>
  }
  idequeue = b->qnext;
8010270f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102712:	8b 40 14             	mov    0x14(%eax),%eax
80102715:	a3 54 b6 10 80       	mov    %eax,0x8010b654

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
8010271a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010271d:	8b 00                	mov    (%eax),%eax
8010271f:	83 e0 04             	and    $0x4,%eax
80102722:	85 c0                	test   %eax,%eax
80102724:	75 2e                	jne    80102754 <ideintr+0x76>
80102726:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010272d:	e8 9d fd ff ff       	call   801024cf <idewait>
80102732:	85 c0                	test   %eax,%eax
80102734:	78 1e                	js     80102754 <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
80102736:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102739:	83 c0 18             	add    $0x18,%eax
8010273c:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80102743:	00 
80102744:	89 44 24 04          	mov    %eax,0x4(%esp)
80102748:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
8010274f:	e8 13 fd ff ff       	call   80102467 <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
80102754:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102757:	8b 00                	mov    (%eax),%eax
80102759:	83 c8 02             	or     $0x2,%eax
8010275c:	89 c2                	mov    %eax,%edx
8010275e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102761:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
80102763:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102766:	8b 00                	mov    (%eax),%eax
80102768:	83 e0 fb             	and    $0xfffffffb,%eax
8010276b:	89 c2                	mov    %eax,%edx
8010276d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102770:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80102772:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102775:	89 04 24             	mov    %eax,(%esp)
80102778:	e8 b7 24 00 00       	call   80104c34 <wakeup>
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
8010277d:	a1 54 b6 10 80       	mov    0x8010b654,%eax
80102782:	85 c0                	test   %eax,%eax
80102784:	74 0d                	je     80102793 <ideintr+0xb5>
    idestart(idequeue);
80102786:	a1 54 b6 10 80       	mov    0x8010b654,%eax
8010278b:	89 04 24             	mov    %eax,(%esp)
8010278e:	e8 26 fe ff ff       	call   801025b9 <idestart>

  release(&idelock);
80102793:	c7 04 24 20 b6 10 80 	movl   $0x8010b620,(%esp)
8010279a:	e8 36 28 00 00       	call   80104fd5 <release>
}
8010279f:	c9                   	leave  
801027a0:	c3                   	ret    

801027a1 <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027a1:	55                   	push   %ebp
801027a2:	89 e5                	mov    %esp,%ebp
801027a4:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027a7:	8b 45 08             	mov    0x8(%ebp),%eax
801027aa:	8b 00                	mov    (%eax),%eax
801027ac:	83 e0 01             	and    $0x1,%eax
801027af:	85 c0                	test   %eax,%eax
801027b1:	75 0c                	jne    801027bf <iderw+0x1e>
    panic("iderw: buf not busy");
801027b3:	c7 04 24 89 8a 10 80 	movl   $0x80108a89,(%esp)
801027ba:	e8 7b dd ff ff       	call   8010053a <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027bf:	8b 45 08             	mov    0x8(%ebp),%eax
801027c2:	8b 00                	mov    (%eax),%eax
801027c4:	83 e0 06             	and    $0x6,%eax
801027c7:	83 f8 02             	cmp    $0x2,%eax
801027ca:	75 0c                	jne    801027d8 <iderw+0x37>
    panic("iderw: nothing to do");
801027cc:	c7 04 24 9d 8a 10 80 	movl   $0x80108a9d,(%esp)
801027d3:	e8 62 dd ff ff       	call   8010053a <panic>
  if(b->dev != 0 && !havedisk1)
801027d8:	8b 45 08             	mov    0x8(%ebp),%eax
801027db:	8b 40 04             	mov    0x4(%eax),%eax
801027de:	85 c0                	test   %eax,%eax
801027e0:	74 15                	je     801027f7 <iderw+0x56>
801027e2:	a1 58 b6 10 80       	mov    0x8010b658,%eax
801027e7:	85 c0                	test   %eax,%eax
801027e9:	75 0c                	jne    801027f7 <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027eb:	c7 04 24 b2 8a 10 80 	movl   $0x80108ab2,(%esp)
801027f2:	e8 43 dd ff ff       	call   8010053a <panic>

  acquire(&idelock);  //DOC:acquire-lock
801027f7:	c7 04 24 20 b6 10 80 	movl   $0x8010b620,(%esp)
801027fe:	e8 70 27 00 00       	call   80104f73 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
80102803:	8b 45 08             	mov    0x8(%ebp),%eax
80102806:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
8010280d:	c7 45 f4 54 b6 10 80 	movl   $0x8010b654,-0xc(%ebp)
80102814:	eb 0b                	jmp    80102821 <iderw+0x80>
80102816:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102819:	8b 00                	mov    (%eax),%eax
8010281b:	83 c0 14             	add    $0x14,%eax
8010281e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102821:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102824:	8b 00                	mov    (%eax),%eax
80102826:	85 c0                	test   %eax,%eax
80102828:	75 ec                	jne    80102816 <iderw+0x75>
    ;
  *pp = b;
8010282a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010282d:	8b 55 08             	mov    0x8(%ebp),%edx
80102830:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
80102832:	a1 54 b6 10 80       	mov    0x8010b654,%eax
80102837:	3b 45 08             	cmp    0x8(%ebp),%eax
8010283a:	75 0d                	jne    80102849 <iderw+0xa8>
    idestart(b);
8010283c:	8b 45 08             	mov    0x8(%ebp),%eax
8010283f:	89 04 24             	mov    %eax,(%esp)
80102842:	e8 72 fd ff ff       	call   801025b9 <idestart>
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102847:	eb 15                	jmp    8010285e <iderw+0xbd>
80102849:	eb 13                	jmp    8010285e <iderw+0xbd>
    sleep(b, &idelock);
8010284b:	c7 44 24 04 20 b6 10 	movl   $0x8010b620,0x4(%esp)
80102852:	80 
80102853:	8b 45 08             	mov    0x8(%ebp),%eax
80102856:	89 04 24             	mov    %eax,(%esp)
80102859:	e8 fa 22 00 00       	call   80104b58 <sleep>
  // Start disk if necessary.
  if(idequeue == b)
    idestart(b);
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010285e:	8b 45 08             	mov    0x8(%ebp),%eax
80102861:	8b 00                	mov    (%eax),%eax
80102863:	83 e0 06             	and    $0x6,%eax
80102866:	83 f8 02             	cmp    $0x2,%eax
80102869:	75 e0                	jne    8010284b <iderw+0xaa>
    sleep(b, &idelock);
  }

  release(&idelock);
8010286b:	c7 04 24 20 b6 10 80 	movl   $0x8010b620,(%esp)
80102872:	e8 5e 27 00 00       	call   80104fd5 <release>
}
80102877:	c9                   	leave  
80102878:	c3                   	ret    

80102879 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102879:	55                   	push   %ebp
8010287a:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
8010287c:	a1 34 22 11 80       	mov    0x80112234,%eax
80102881:	8b 55 08             	mov    0x8(%ebp),%edx
80102884:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
80102886:	a1 34 22 11 80       	mov    0x80112234,%eax
8010288b:	8b 40 10             	mov    0x10(%eax),%eax
}
8010288e:	5d                   	pop    %ebp
8010288f:	c3                   	ret    

80102890 <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
80102890:	55                   	push   %ebp
80102891:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102893:	a1 34 22 11 80       	mov    0x80112234,%eax
80102898:	8b 55 08             	mov    0x8(%ebp),%edx
8010289b:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
8010289d:	a1 34 22 11 80       	mov    0x80112234,%eax
801028a2:	8b 55 0c             	mov    0xc(%ebp),%edx
801028a5:	89 50 10             	mov    %edx,0x10(%eax)
}
801028a8:	5d                   	pop    %ebp
801028a9:	c3                   	ret    

801028aa <ioapicinit>:

void
ioapicinit(void)
{
801028aa:	55                   	push   %ebp
801028ab:	89 e5                	mov    %esp,%ebp
801028ad:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028b0:	a1 64 23 11 80       	mov    0x80112364,%eax
801028b5:	85 c0                	test   %eax,%eax
801028b7:	75 05                	jne    801028be <ioapicinit+0x14>
    return;
801028b9:	e9 9d 00 00 00       	jmp    8010295b <ioapicinit+0xb1>

  ioapic = (volatile struct ioapic*)IOAPIC;
801028be:	c7 05 34 22 11 80 00 	movl   $0xfec00000,0x80112234
801028c5:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028c8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028cf:	e8 a5 ff ff ff       	call   80102879 <ioapicread>
801028d4:	c1 e8 10             	shr    $0x10,%eax
801028d7:	25 ff 00 00 00       	and    $0xff,%eax
801028dc:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028df:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801028e6:	e8 8e ff ff ff       	call   80102879 <ioapicread>
801028eb:	c1 e8 18             	shr    $0x18,%eax
801028ee:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
801028f1:	0f b6 05 60 23 11 80 	movzbl 0x80112360,%eax
801028f8:	0f b6 c0             	movzbl %al,%eax
801028fb:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801028fe:	74 0c                	je     8010290c <ioapicinit+0x62>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
80102900:	c7 04 24 d0 8a 10 80 	movl   $0x80108ad0,(%esp)
80102907:	e8 94 da ff ff       	call   801003a0 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010290c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102913:	eb 3e                	jmp    80102953 <ioapicinit+0xa9>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102915:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102918:	83 c0 20             	add    $0x20,%eax
8010291b:	0d 00 00 01 00       	or     $0x10000,%eax
80102920:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102923:	83 c2 08             	add    $0x8,%edx
80102926:	01 d2                	add    %edx,%edx
80102928:	89 44 24 04          	mov    %eax,0x4(%esp)
8010292c:	89 14 24             	mov    %edx,(%esp)
8010292f:	e8 5c ff ff ff       	call   80102890 <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
80102934:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102937:	83 c0 08             	add    $0x8,%eax
8010293a:	01 c0                	add    %eax,%eax
8010293c:	83 c0 01             	add    $0x1,%eax
8010293f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102946:	00 
80102947:	89 04 24             	mov    %eax,(%esp)
8010294a:	e8 41 ff ff ff       	call   80102890 <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010294f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102953:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102956:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102959:	7e ba                	jle    80102915 <ioapicinit+0x6b>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010295b:	c9                   	leave  
8010295c:	c3                   	ret    

8010295d <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
8010295d:	55                   	push   %ebp
8010295e:	89 e5                	mov    %esp,%ebp
80102960:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
80102963:	a1 64 23 11 80       	mov    0x80112364,%eax
80102968:	85 c0                	test   %eax,%eax
8010296a:	75 02                	jne    8010296e <ioapicenable+0x11>
    return;
8010296c:	eb 37                	jmp    801029a5 <ioapicenable+0x48>

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
8010296e:	8b 45 08             	mov    0x8(%ebp),%eax
80102971:	83 c0 20             	add    $0x20,%eax
80102974:	8b 55 08             	mov    0x8(%ebp),%edx
80102977:	83 c2 08             	add    $0x8,%edx
8010297a:	01 d2                	add    %edx,%edx
8010297c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102980:	89 14 24             	mov    %edx,(%esp)
80102983:	e8 08 ff ff ff       	call   80102890 <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102988:	8b 45 0c             	mov    0xc(%ebp),%eax
8010298b:	c1 e0 18             	shl    $0x18,%eax
8010298e:	8b 55 08             	mov    0x8(%ebp),%edx
80102991:	83 c2 08             	add    $0x8,%edx
80102994:	01 d2                	add    %edx,%edx
80102996:	83 c2 01             	add    $0x1,%edx
80102999:	89 44 24 04          	mov    %eax,0x4(%esp)
8010299d:	89 14 24             	mov    %edx,(%esp)
801029a0:	e8 eb fe ff ff       	call   80102890 <ioapicwrite>
}
801029a5:	c9                   	leave  
801029a6:	c3                   	ret    

801029a7 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029a7:	55                   	push   %ebp
801029a8:	89 e5                	mov    %esp,%ebp
801029aa:	8b 45 08             	mov    0x8(%ebp),%eax
801029ad:	05 00 00 00 80       	add    $0x80000000,%eax
801029b2:	5d                   	pop    %ebp
801029b3:	c3                   	ret    

801029b4 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029b4:	55                   	push   %ebp
801029b5:	89 e5                	mov    %esp,%ebp
801029b7:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029ba:	c7 44 24 04 02 8b 10 	movl   $0x80108b02,0x4(%esp)
801029c1:	80 
801029c2:	c7 04 24 40 22 11 80 	movl   $0x80112240,(%esp)
801029c9:	e8 84 25 00 00       	call   80104f52 <initlock>
  kmem.use_lock = 0;
801029ce:	c7 05 74 22 11 80 00 	movl   $0x0,0x80112274
801029d5:	00 00 00 
  freerange(vstart, vend);
801029d8:	8b 45 0c             	mov    0xc(%ebp),%eax
801029db:	89 44 24 04          	mov    %eax,0x4(%esp)
801029df:	8b 45 08             	mov    0x8(%ebp),%eax
801029e2:	89 04 24             	mov    %eax,(%esp)
801029e5:	e8 26 00 00 00       	call   80102a10 <freerange>
}
801029ea:	c9                   	leave  
801029eb:	c3                   	ret    

801029ec <kinit2>:

void
kinit2(void *vstart, void *vend)
{
801029ec:	55                   	push   %ebp
801029ed:	89 e5                	mov    %esp,%ebp
801029ef:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
801029f2:	8b 45 0c             	mov    0xc(%ebp),%eax
801029f5:	89 44 24 04          	mov    %eax,0x4(%esp)
801029f9:	8b 45 08             	mov    0x8(%ebp),%eax
801029fc:	89 04 24             	mov    %eax,(%esp)
801029ff:	e8 0c 00 00 00       	call   80102a10 <freerange>
  kmem.use_lock = 1;
80102a04:	c7 05 74 22 11 80 01 	movl   $0x1,0x80112274
80102a0b:	00 00 00 
}
80102a0e:	c9                   	leave  
80102a0f:	c3                   	ret    

80102a10 <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a10:	55                   	push   %ebp
80102a11:	89 e5                	mov    %esp,%ebp
80102a13:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a16:	8b 45 08             	mov    0x8(%ebp),%eax
80102a19:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a1e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a23:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a26:	eb 12                	jmp    80102a3a <freerange+0x2a>
    kfree(p);
80102a28:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a2b:	89 04 24             	mov    %eax,(%esp)
80102a2e:	e8 16 00 00 00       	call   80102a49 <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a33:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a3d:	05 00 10 00 00       	add    $0x1000,%eax
80102a42:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a45:	76 e1                	jbe    80102a28 <freerange+0x18>
    kfree(p);
}
80102a47:	c9                   	leave  
80102a48:	c3                   	ret    

80102a49 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a49:	55                   	push   %ebp
80102a4a:	89 e5                	mov    %esp,%ebp
80102a4c:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a4f:	8b 45 08             	mov    0x8(%ebp),%eax
80102a52:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a57:	85 c0                	test   %eax,%eax
80102a59:	75 1b                	jne    80102a76 <kfree+0x2d>
80102a5b:	81 7d 08 7c 56 11 80 	cmpl   $0x8011567c,0x8(%ebp)
80102a62:	72 12                	jb     80102a76 <kfree+0x2d>
80102a64:	8b 45 08             	mov    0x8(%ebp),%eax
80102a67:	89 04 24             	mov    %eax,(%esp)
80102a6a:	e8 38 ff ff ff       	call   801029a7 <v2p>
80102a6f:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102a74:	76 0c                	jbe    80102a82 <kfree+0x39>
    panic("kfree");
80102a76:	c7 04 24 07 8b 10 80 	movl   $0x80108b07,(%esp)
80102a7d:	e8 b8 da ff ff       	call   8010053a <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102a82:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102a89:	00 
80102a8a:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102a91:	00 
80102a92:	8b 45 08             	mov    0x8(%ebp),%eax
80102a95:	89 04 24             	mov    %eax,(%esp)
80102a98:	e8 2a 27 00 00       	call   801051c7 <memset>

  if(kmem.use_lock)
80102a9d:	a1 74 22 11 80       	mov    0x80112274,%eax
80102aa2:	85 c0                	test   %eax,%eax
80102aa4:	74 0c                	je     80102ab2 <kfree+0x69>
    acquire(&kmem.lock);
80102aa6:	c7 04 24 40 22 11 80 	movl   $0x80112240,(%esp)
80102aad:	e8 c1 24 00 00       	call   80104f73 <acquire>
  r = (struct run*)v;
80102ab2:	8b 45 08             	mov    0x8(%ebp),%eax
80102ab5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102ab8:	8b 15 78 22 11 80    	mov    0x80112278,%edx
80102abe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac1:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102ac3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac6:	a3 78 22 11 80       	mov    %eax,0x80112278
  if(kmem.use_lock)
80102acb:	a1 74 22 11 80       	mov    0x80112274,%eax
80102ad0:	85 c0                	test   %eax,%eax
80102ad2:	74 0c                	je     80102ae0 <kfree+0x97>
    release(&kmem.lock);
80102ad4:	c7 04 24 40 22 11 80 	movl   $0x80112240,(%esp)
80102adb:	e8 f5 24 00 00       	call   80104fd5 <release>
}
80102ae0:	c9                   	leave  
80102ae1:	c3                   	ret    

80102ae2 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102ae2:	55                   	push   %ebp
80102ae3:	89 e5                	mov    %esp,%ebp
80102ae5:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102ae8:	a1 74 22 11 80       	mov    0x80112274,%eax
80102aed:	85 c0                	test   %eax,%eax
80102aef:	74 0c                	je     80102afd <kalloc+0x1b>
    acquire(&kmem.lock);
80102af1:	c7 04 24 40 22 11 80 	movl   $0x80112240,(%esp)
80102af8:	e8 76 24 00 00       	call   80104f73 <acquire>
  r = kmem.freelist;
80102afd:	a1 78 22 11 80       	mov    0x80112278,%eax
80102b02:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b05:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b09:	74 0a                	je     80102b15 <kalloc+0x33>
    kmem.freelist = r->next;
80102b0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b0e:	8b 00                	mov    (%eax),%eax
80102b10:	a3 78 22 11 80       	mov    %eax,0x80112278
  if(kmem.use_lock)
80102b15:	a1 74 22 11 80       	mov    0x80112274,%eax
80102b1a:	85 c0                	test   %eax,%eax
80102b1c:	74 0c                	je     80102b2a <kalloc+0x48>
    release(&kmem.lock);
80102b1e:	c7 04 24 40 22 11 80 	movl   $0x80112240,(%esp)
80102b25:	e8 ab 24 00 00       	call   80104fd5 <release>
  return (char*)r;
80102b2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b2d:	c9                   	leave  
80102b2e:	c3                   	ret    

80102b2f <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102b2f:	55                   	push   %ebp
80102b30:	89 e5                	mov    %esp,%ebp
80102b32:	83 ec 14             	sub    $0x14,%esp
80102b35:	8b 45 08             	mov    0x8(%ebp),%eax
80102b38:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b3c:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80102b40:	89 c2                	mov    %eax,%edx
80102b42:	ec                   	in     (%dx),%al
80102b43:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102b46:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102b4a:	c9                   	leave  
80102b4b:	c3                   	ret    

80102b4c <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b4c:	55                   	push   %ebp
80102b4d:	89 e5                	mov    %esp,%ebp
80102b4f:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102b52:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102b59:	e8 d1 ff ff ff       	call   80102b2f <inb>
80102b5e:	0f b6 c0             	movzbl %al,%eax
80102b61:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102b64:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b67:	83 e0 01             	and    $0x1,%eax
80102b6a:	85 c0                	test   %eax,%eax
80102b6c:	75 0a                	jne    80102b78 <kbdgetc+0x2c>
    return -1;
80102b6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b73:	e9 25 01 00 00       	jmp    80102c9d <kbdgetc+0x151>
  data = inb(KBDATAP);
80102b78:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102b7f:	e8 ab ff ff ff       	call   80102b2f <inb>
80102b84:	0f b6 c0             	movzbl %al,%eax
80102b87:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102b8a:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102b91:	75 17                	jne    80102baa <kbdgetc+0x5e>
    shift |= E0ESC;
80102b93:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102b98:	83 c8 40             	or     $0x40,%eax
80102b9b:	a3 5c b6 10 80       	mov    %eax,0x8010b65c
    return 0;
80102ba0:	b8 00 00 00 00       	mov    $0x0,%eax
80102ba5:	e9 f3 00 00 00       	jmp    80102c9d <kbdgetc+0x151>
  } else if(data & 0x80){
80102baa:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bad:	25 80 00 00 00       	and    $0x80,%eax
80102bb2:	85 c0                	test   %eax,%eax
80102bb4:	74 45                	je     80102bfb <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102bb6:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102bbb:	83 e0 40             	and    $0x40,%eax
80102bbe:	85 c0                	test   %eax,%eax
80102bc0:	75 08                	jne    80102bca <kbdgetc+0x7e>
80102bc2:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bc5:	83 e0 7f             	and    $0x7f,%eax
80102bc8:	eb 03                	jmp    80102bcd <kbdgetc+0x81>
80102bca:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bcd:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102bd0:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bd3:	05 20 90 10 80       	add    $0x80109020,%eax
80102bd8:	0f b6 00             	movzbl (%eax),%eax
80102bdb:	83 c8 40             	or     $0x40,%eax
80102bde:	0f b6 c0             	movzbl %al,%eax
80102be1:	f7 d0                	not    %eax
80102be3:	89 c2                	mov    %eax,%edx
80102be5:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102bea:	21 d0                	and    %edx,%eax
80102bec:	a3 5c b6 10 80       	mov    %eax,0x8010b65c
    return 0;
80102bf1:	b8 00 00 00 00       	mov    $0x0,%eax
80102bf6:	e9 a2 00 00 00       	jmp    80102c9d <kbdgetc+0x151>
  } else if(shift & E0ESC){
80102bfb:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102c00:	83 e0 40             	and    $0x40,%eax
80102c03:	85 c0                	test   %eax,%eax
80102c05:	74 14                	je     80102c1b <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102c07:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102c0e:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102c13:	83 e0 bf             	and    $0xffffffbf,%eax
80102c16:	a3 5c b6 10 80       	mov    %eax,0x8010b65c
  }

  shift |= shiftcode[data];
80102c1b:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c1e:	05 20 90 10 80       	add    $0x80109020,%eax
80102c23:	0f b6 00             	movzbl (%eax),%eax
80102c26:	0f b6 d0             	movzbl %al,%edx
80102c29:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102c2e:	09 d0                	or     %edx,%eax
80102c30:	a3 5c b6 10 80       	mov    %eax,0x8010b65c
  shift ^= togglecode[data];
80102c35:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c38:	05 20 91 10 80       	add    $0x80109120,%eax
80102c3d:	0f b6 00             	movzbl (%eax),%eax
80102c40:	0f b6 d0             	movzbl %al,%edx
80102c43:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102c48:	31 d0                	xor    %edx,%eax
80102c4a:	a3 5c b6 10 80       	mov    %eax,0x8010b65c
  c = charcode[shift & (CTL | SHIFT)][data];
80102c4f:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102c54:	83 e0 03             	and    $0x3,%eax
80102c57:	8b 14 85 20 95 10 80 	mov    -0x7fef6ae0(,%eax,4),%edx
80102c5e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c61:	01 d0                	add    %edx,%eax
80102c63:	0f b6 00             	movzbl (%eax),%eax
80102c66:	0f b6 c0             	movzbl %al,%eax
80102c69:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102c6c:	a1 5c b6 10 80       	mov    0x8010b65c,%eax
80102c71:	83 e0 08             	and    $0x8,%eax
80102c74:	85 c0                	test   %eax,%eax
80102c76:	74 22                	je     80102c9a <kbdgetc+0x14e>
    if('a' <= c && c <= 'z')
80102c78:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102c7c:	76 0c                	jbe    80102c8a <kbdgetc+0x13e>
80102c7e:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102c82:	77 06                	ja     80102c8a <kbdgetc+0x13e>
      c += 'A' - 'a';
80102c84:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102c88:	eb 10                	jmp    80102c9a <kbdgetc+0x14e>
    else if('A' <= c && c <= 'Z')
80102c8a:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102c8e:	76 0a                	jbe    80102c9a <kbdgetc+0x14e>
80102c90:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102c94:	77 04                	ja     80102c9a <kbdgetc+0x14e>
      c += 'a' - 'A';
80102c96:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102c9a:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102c9d:	c9                   	leave  
80102c9e:	c3                   	ret    

80102c9f <kbdintr>:

void
kbdintr(void)
{
80102c9f:	55                   	push   %ebp
80102ca0:	89 e5                	mov    %esp,%ebp
80102ca2:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102ca5:	c7 04 24 4c 2b 10 80 	movl   $0x80102b4c,(%esp)
80102cac:	e8 fc da ff ff       	call   801007ad <consoleintr>
}
80102cb1:	c9                   	leave  
80102cb2:	c3                   	ret    

80102cb3 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102cb3:	55                   	push   %ebp
80102cb4:	89 e5                	mov    %esp,%ebp
80102cb6:	83 ec 14             	sub    $0x14,%esp
80102cb9:	8b 45 08             	mov    0x8(%ebp),%eax
80102cbc:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102cc0:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80102cc4:	89 c2                	mov    %eax,%edx
80102cc6:	ec                   	in     (%dx),%al
80102cc7:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102cca:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102cce:	c9                   	leave  
80102ccf:	c3                   	ret    

80102cd0 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102cd0:	55                   	push   %ebp
80102cd1:	89 e5                	mov    %esp,%ebp
80102cd3:	83 ec 08             	sub    $0x8,%esp
80102cd6:	8b 55 08             	mov    0x8(%ebp),%edx
80102cd9:	8b 45 0c             	mov    0xc(%ebp),%eax
80102cdc:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80102ce0:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ce3:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80102ce7:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80102ceb:	ee                   	out    %al,(%dx)
}
80102cec:	c9                   	leave  
80102ced:	c3                   	ret    

80102cee <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80102cee:	55                   	push   %ebp
80102cef:	89 e5                	mov    %esp,%ebp
80102cf1:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80102cf4:	9c                   	pushf  
80102cf5:	58                   	pop    %eax
80102cf6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80102cf9:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80102cfc:	c9                   	leave  
80102cfd:	c3                   	ret    

80102cfe <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
80102cfe:	55                   	push   %ebp
80102cff:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102d01:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102d06:	8b 55 08             	mov    0x8(%ebp),%edx
80102d09:	c1 e2 02             	shl    $0x2,%edx
80102d0c:	01 c2                	add    %eax,%edx
80102d0e:	8b 45 0c             	mov    0xc(%ebp),%eax
80102d11:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102d13:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102d18:	83 c0 20             	add    $0x20,%eax
80102d1b:	8b 00                	mov    (%eax),%eax
}
80102d1d:	5d                   	pop    %ebp
80102d1e:	c3                   	ret    

80102d1f <lapicinit>:
//PAGEBREAK!

void
lapicinit(void)
{
80102d1f:	55                   	push   %ebp
80102d20:	89 e5                	mov    %esp,%ebp
80102d22:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80102d25:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102d2a:	85 c0                	test   %eax,%eax
80102d2c:	75 05                	jne    80102d33 <lapicinit+0x14>
    return;
80102d2e:	e9 43 01 00 00       	jmp    80102e76 <lapicinit+0x157>

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102d33:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102d3a:	00 
80102d3b:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102d42:	e8 b7 ff ff ff       	call   80102cfe <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102d47:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102d4e:	00 
80102d4f:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102d56:	e8 a3 ff ff ff       	call   80102cfe <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102d5b:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102d62:	00 
80102d63:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102d6a:	e8 8f ff ff ff       	call   80102cfe <lapicw>
  lapicw(TICR, 10000000); 
80102d6f:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102d76:	00 
80102d77:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102d7e:	e8 7b ff ff ff       	call   80102cfe <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102d83:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d8a:	00 
80102d8b:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102d92:	e8 67 ff ff ff       	call   80102cfe <lapicw>
  lapicw(LINT1, MASKED);
80102d97:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d9e:	00 
80102d9f:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102da6:	e8 53 ff ff ff       	call   80102cfe <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102dab:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102db0:	83 c0 30             	add    $0x30,%eax
80102db3:	8b 00                	mov    (%eax),%eax
80102db5:	c1 e8 10             	shr    $0x10,%eax
80102db8:	0f b6 c0             	movzbl %al,%eax
80102dbb:	83 f8 03             	cmp    $0x3,%eax
80102dbe:	76 14                	jbe    80102dd4 <lapicinit+0xb5>
    lapicw(PCINT, MASKED);
80102dc0:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102dc7:	00 
80102dc8:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102dcf:	e8 2a ff ff ff       	call   80102cfe <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102dd4:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102ddb:	00 
80102ddc:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102de3:	e8 16 ff ff ff       	call   80102cfe <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102de8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102def:	00 
80102df0:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102df7:	e8 02 ff ff ff       	call   80102cfe <lapicw>
  lapicw(ESR, 0);
80102dfc:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e03:	00 
80102e04:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e0b:	e8 ee fe ff ff       	call   80102cfe <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102e10:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e17:	00 
80102e18:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102e1f:	e8 da fe ff ff       	call   80102cfe <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102e24:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e2b:	00 
80102e2c:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102e33:	e8 c6 fe ff ff       	call   80102cfe <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102e38:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102e3f:	00 
80102e40:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102e47:	e8 b2 fe ff ff       	call   80102cfe <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102e4c:	90                   	nop
80102e4d:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102e52:	05 00 03 00 00       	add    $0x300,%eax
80102e57:	8b 00                	mov    (%eax),%eax
80102e59:	25 00 10 00 00       	and    $0x1000,%eax
80102e5e:	85 c0                	test   %eax,%eax
80102e60:	75 eb                	jne    80102e4d <lapicinit+0x12e>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102e62:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e69:	00 
80102e6a:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102e71:	e8 88 fe ff ff       	call   80102cfe <lapicw>
}
80102e76:	c9                   	leave  
80102e77:	c3                   	ret    

80102e78 <cpunum>:

int
cpunum(void)
{
80102e78:	55                   	push   %ebp
80102e79:	89 e5                	mov    %esp,%ebp
80102e7b:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
80102e7e:	e8 6b fe ff ff       	call   80102cee <readeflags>
80102e83:	25 00 02 00 00       	and    $0x200,%eax
80102e88:	85 c0                	test   %eax,%eax
80102e8a:	74 25                	je     80102eb1 <cpunum+0x39>
    static int n;
    if(n++ == 0)
80102e8c:	a1 60 b6 10 80       	mov    0x8010b660,%eax
80102e91:	8d 50 01             	lea    0x1(%eax),%edx
80102e94:	89 15 60 b6 10 80    	mov    %edx,0x8010b660
80102e9a:	85 c0                	test   %eax,%eax
80102e9c:	75 13                	jne    80102eb1 <cpunum+0x39>
      cprintf("cpu called from %x with interrupts enabled\n",
80102e9e:	8b 45 04             	mov    0x4(%ebp),%eax
80102ea1:	89 44 24 04          	mov    %eax,0x4(%esp)
80102ea5:	c7 04 24 10 8b 10 80 	movl   $0x80108b10,(%esp)
80102eac:	e8 ef d4 ff ff       	call   801003a0 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
80102eb1:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102eb6:	85 c0                	test   %eax,%eax
80102eb8:	74 0f                	je     80102ec9 <cpunum+0x51>
    return lapic[ID]>>24;
80102eba:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102ebf:	83 c0 20             	add    $0x20,%eax
80102ec2:	8b 00                	mov    (%eax),%eax
80102ec4:	c1 e8 18             	shr    $0x18,%eax
80102ec7:	eb 05                	jmp    80102ece <cpunum+0x56>
  return 0;
80102ec9:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102ece:	c9                   	leave  
80102ecf:	c3                   	ret    

80102ed0 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102ed0:	55                   	push   %ebp
80102ed1:	89 e5                	mov    %esp,%ebp
80102ed3:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80102ed6:	a1 7c 22 11 80       	mov    0x8011227c,%eax
80102edb:	85 c0                	test   %eax,%eax
80102edd:	74 14                	je     80102ef3 <lapiceoi+0x23>
    lapicw(EOI, 0);
80102edf:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102ee6:	00 
80102ee7:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102eee:	e8 0b fe ff ff       	call   80102cfe <lapicw>
}
80102ef3:	c9                   	leave  
80102ef4:	c3                   	ret    

80102ef5 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102ef5:	55                   	push   %ebp
80102ef6:	89 e5                	mov    %esp,%ebp
}
80102ef8:	5d                   	pop    %ebp
80102ef9:	c3                   	ret    

80102efa <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102efa:	55                   	push   %ebp
80102efb:	89 e5                	mov    %esp,%ebp
80102efd:	83 ec 1c             	sub    $0x1c,%esp
80102f00:	8b 45 08             	mov    0x8(%ebp),%eax
80102f03:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(CMOS_PORT, 0xF);  // offset 0xF is shutdown code
80102f06:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80102f0d:	00 
80102f0e:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80102f15:	e8 b6 fd ff ff       	call   80102cd0 <outb>
  outb(CMOS_PORT+1, 0x0A);
80102f1a:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80102f21:	00 
80102f22:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80102f29:	e8 a2 fd ff ff       	call   80102cd0 <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80102f2e:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80102f35:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f38:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80102f3d:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f40:	8d 50 02             	lea    0x2(%eax),%edx
80102f43:	8b 45 0c             	mov    0xc(%ebp),%eax
80102f46:	c1 e8 04             	shr    $0x4,%eax
80102f49:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102f4c:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f50:	c1 e0 18             	shl    $0x18,%eax
80102f53:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f57:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f5e:	e8 9b fd ff ff       	call   80102cfe <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80102f63:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
80102f6a:	00 
80102f6b:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f72:	e8 87 fd ff ff       	call   80102cfe <lapicw>
  microdelay(200);
80102f77:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102f7e:	e8 72 ff ff ff       	call   80102ef5 <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80102f83:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
80102f8a:	00 
80102f8b:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f92:	e8 67 fd ff ff       	call   80102cfe <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
80102f97:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102f9e:	e8 52 ff ff ff       	call   80102ef5 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fa3:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80102faa:	eb 40                	jmp    80102fec <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
80102fac:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102fb0:	c1 e0 18             	shl    $0x18,%eax
80102fb3:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fb7:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102fbe:	e8 3b fd ff ff       	call   80102cfe <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
80102fc3:	8b 45 0c             	mov    0xc(%ebp),%eax
80102fc6:	c1 e8 0c             	shr    $0xc,%eax
80102fc9:	80 cc 06             	or     $0x6,%ah
80102fcc:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fd0:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fd7:	e8 22 fd ff ff       	call   80102cfe <lapicw>
    microdelay(200);
80102fdc:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102fe3:	e8 0d ff ff ff       	call   80102ef5 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fe8:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80102fec:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80102ff0:	7e ba                	jle    80102fac <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80102ff2:	c9                   	leave  
80102ff3:	c3                   	ret    

80102ff4 <cmos_read>:
#define DAY     0x07
#define MONTH   0x08
#define YEAR    0x09

static uint cmos_read(uint reg)
{
80102ff4:	55                   	push   %ebp
80102ff5:	89 e5                	mov    %esp,%ebp
80102ff7:	83 ec 08             	sub    $0x8,%esp
  outb(CMOS_PORT,  reg);
80102ffa:	8b 45 08             	mov    0x8(%ebp),%eax
80102ffd:	0f b6 c0             	movzbl %al,%eax
80103000:	89 44 24 04          	mov    %eax,0x4(%esp)
80103004:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
8010300b:	e8 c0 fc ff ff       	call   80102cd0 <outb>
  microdelay(200);
80103010:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80103017:	e8 d9 fe ff ff       	call   80102ef5 <microdelay>

  return inb(CMOS_RETURN);
8010301c:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80103023:	e8 8b fc ff ff       	call   80102cb3 <inb>
80103028:	0f b6 c0             	movzbl %al,%eax
}
8010302b:	c9                   	leave  
8010302c:	c3                   	ret    

8010302d <fill_rtcdate>:

static void fill_rtcdate(struct rtcdate *r)
{
8010302d:	55                   	push   %ebp
8010302e:	89 e5                	mov    %esp,%ebp
80103030:	83 ec 04             	sub    $0x4,%esp
  r->second = cmos_read(SECS);
80103033:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010303a:	e8 b5 ff ff ff       	call   80102ff4 <cmos_read>
8010303f:	8b 55 08             	mov    0x8(%ebp),%edx
80103042:	89 02                	mov    %eax,(%edx)
  r->minute = cmos_read(MINS);
80103044:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010304b:	e8 a4 ff ff ff       	call   80102ff4 <cmos_read>
80103050:	8b 55 08             	mov    0x8(%ebp),%edx
80103053:	89 42 04             	mov    %eax,0x4(%edx)
  r->hour   = cmos_read(HOURS);
80103056:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
8010305d:	e8 92 ff ff ff       	call   80102ff4 <cmos_read>
80103062:	8b 55 08             	mov    0x8(%ebp),%edx
80103065:	89 42 08             	mov    %eax,0x8(%edx)
  r->day    = cmos_read(DAY);
80103068:	c7 04 24 07 00 00 00 	movl   $0x7,(%esp)
8010306f:	e8 80 ff ff ff       	call   80102ff4 <cmos_read>
80103074:	8b 55 08             	mov    0x8(%ebp),%edx
80103077:	89 42 0c             	mov    %eax,0xc(%edx)
  r->month  = cmos_read(MONTH);
8010307a:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80103081:	e8 6e ff ff ff       	call   80102ff4 <cmos_read>
80103086:	8b 55 08             	mov    0x8(%ebp),%edx
80103089:	89 42 10             	mov    %eax,0x10(%edx)
  r->year   = cmos_read(YEAR);
8010308c:	c7 04 24 09 00 00 00 	movl   $0x9,(%esp)
80103093:	e8 5c ff ff ff       	call   80102ff4 <cmos_read>
80103098:	8b 55 08             	mov    0x8(%ebp),%edx
8010309b:	89 42 14             	mov    %eax,0x14(%edx)
}
8010309e:	c9                   	leave  
8010309f:	c3                   	ret    

801030a0 <cmostime>:

// qemu seems to use 24-hour GWT and the values are BCD encoded
void cmostime(struct rtcdate *r)
{
801030a0:	55                   	push   %ebp
801030a1:	89 e5                	mov    %esp,%ebp
801030a3:	83 ec 58             	sub    $0x58,%esp
  struct rtcdate t1, t2;
  int sb, bcd;

  sb = cmos_read(CMOS_STATB);
801030a6:	c7 04 24 0b 00 00 00 	movl   $0xb,(%esp)
801030ad:	e8 42 ff ff ff       	call   80102ff4 <cmos_read>
801030b2:	89 45 f4             	mov    %eax,-0xc(%ebp)

  bcd = (sb & (1 << 2)) == 0;
801030b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801030b8:	83 e0 04             	and    $0x4,%eax
801030bb:	85 c0                	test   %eax,%eax
801030bd:	0f 94 c0             	sete   %al
801030c0:	0f b6 c0             	movzbl %al,%eax
801030c3:	89 45 f0             	mov    %eax,-0x10(%ebp)

  // make sure CMOS doesn't modify time while we read it
  for (;;) {
    fill_rtcdate(&t1);
801030c6:	8d 45 d8             	lea    -0x28(%ebp),%eax
801030c9:	89 04 24             	mov    %eax,(%esp)
801030cc:	e8 5c ff ff ff       	call   8010302d <fill_rtcdate>
    if (cmos_read(CMOS_STATA) & CMOS_UIP)
801030d1:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
801030d8:	e8 17 ff ff ff       	call   80102ff4 <cmos_read>
801030dd:	25 80 00 00 00       	and    $0x80,%eax
801030e2:	85 c0                	test   %eax,%eax
801030e4:	74 02                	je     801030e8 <cmostime+0x48>
        continue;
801030e6:	eb 36                	jmp    8010311e <cmostime+0x7e>
    fill_rtcdate(&t2);
801030e8:	8d 45 c0             	lea    -0x40(%ebp),%eax
801030eb:	89 04 24             	mov    %eax,(%esp)
801030ee:	e8 3a ff ff ff       	call   8010302d <fill_rtcdate>
    if (memcmp(&t1, &t2, sizeof(t1)) == 0)
801030f3:	c7 44 24 08 18 00 00 	movl   $0x18,0x8(%esp)
801030fa:	00 
801030fb:	8d 45 c0             	lea    -0x40(%ebp),%eax
801030fe:	89 44 24 04          	mov    %eax,0x4(%esp)
80103102:	8d 45 d8             	lea    -0x28(%ebp),%eax
80103105:	89 04 24             	mov    %eax,(%esp)
80103108:	e8 31 21 00 00       	call   8010523e <memcmp>
8010310d:	85 c0                	test   %eax,%eax
8010310f:	75 0d                	jne    8010311e <cmostime+0x7e>
      break;
80103111:	90                   	nop
  }

  // convert
  if (bcd) {
80103112:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103116:	0f 84 ac 00 00 00    	je     801031c8 <cmostime+0x128>
8010311c:	eb 02                	jmp    80103120 <cmostime+0x80>
    if (cmos_read(CMOS_STATA) & CMOS_UIP)
        continue;
    fill_rtcdate(&t2);
    if (memcmp(&t1, &t2, sizeof(t1)) == 0)
      break;
  }
8010311e:	eb a6                	jmp    801030c6 <cmostime+0x26>

  // convert
  if (bcd) {
#define    CONV(x)     (t1.x = ((t1.x >> 4) * 10) + (t1.x & 0xf))
    CONV(second);
80103120:	8b 45 d8             	mov    -0x28(%ebp),%eax
80103123:	c1 e8 04             	shr    $0x4,%eax
80103126:	89 c2                	mov    %eax,%edx
80103128:	89 d0                	mov    %edx,%eax
8010312a:	c1 e0 02             	shl    $0x2,%eax
8010312d:	01 d0                	add    %edx,%eax
8010312f:	01 c0                	add    %eax,%eax
80103131:	8b 55 d8             	mov    -0x28(%ebp),%edx
80103134:	83 e2 0f             	and    $0xf,%edx
80103137:	01 d0                	add    %edx,%eax
80103139:	89 45 d8             	mov    %eax,-0x28(%ebp)
    CONV(minute);
8010313c:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010313f:	c1 e8 04             	shr    $0x4,%eax
80103142:	89 c2                	mov    %eax,%edx
80103144:	89 d0                	mov    %edx,%eax
80103146:	c1 e0 02             	shl    $0x2,%eax
80103149:	01 d0                	add    %edx,%eax
8010314b:	01 c0                	add    %eax,%eax
8010314d:	8b 55 dc             	mov    -0x24(%ebp),%edx
80103150:	83 e2 0f             	and    $0xf,%edx
80103153:	01 d0                	add    %edx,%eax
80103155:	89 45 dc             	mov    %eax,-0x24(%ebp)
    CONV(hour  );
80103158:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010315b:	c1 e8 04             	shr    $0x4,%eax
8010315e:	89 c2                	mov    %eax,%edx
80103160:	89 d0                	mov    %edx,%eax
80103162:	c1 e0 02             	shl    $0x2,%eax
80103165:	01 d0                	add    %edx,%eax
80103167:	01 c0                	add    %eax,%eax
80103169:	8b 55 e0             	mov    -0x20(%ebp),%edx
8010316c:	83 e2 0f             	and    $0xf,%edx
8010316f:	01 d0                	add    %edx,%eax
80103171:	89 45 e0             	mov    %eax,-0x20(%ebp)
    CONV(day   );
80103174:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103177:	c1 e8 04             	shr    $0x4,%eax
8010317a:	89 c2                	mov    %eax,%edx
8010317c:	89 d0                	mov    %edx,%eax
8010317e:	c1 e0 02             	shl    $0x2,%eax
80103181:	01 d0                	add    %edx,%eax
80103183:	01 c0                	add    %eax,%eax
80103185:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80103188:	83 e2 0f             	and    $0xf,%edx
8010318b:	01 d0                	add    %edx,%eax
8010318d:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    CONV(month );
80103190:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103193:	c1 e8 04             	shr    $0x4,%eax
80103196:	89 c2                	mov    %eax,%edx
80103198:	89 d0                	mov    %edx,%eax
8010319a:	c1 e0 02             	shl    $0x2,%eax
8010319d:	01 d0                	add    %edx,%eax
8010319f:	01 c0                	add    %eax,%eax
801031a1:	8b 55 e8             	mov    -0x18(%ebp),%edx
801031a4:	83 e2 0f             	and    $0xf,%edx
801031a7:	01 d0                	add    %edx,%eax
801031a9:	89 45 e8             	mov    %eax,-0x18(%ebp)
    CONV(year  );
801031ac:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031af:	c1 e8 04             	shr    $0x4,%eax
801031b2:	89 c2                	mov    %eax,%edx
801031b4:	89 d0                	mov    %edx,%eax
801031b6:	c1 e0 02             	shl    $0x2,%eax
801031b9:	01 d0                	add    %edx,%eax
801031bb:	01 c0                	add    %eax,%eax
801031bd:	8b 55 ec             	mov    -0x14(%ebp),%edx
801031c0:	83 e2 0f             	and    $0xf,%edx
801031c3:	01 d0                	add    %edx,%eax
801031c5:	89 45 ec             	mov    %eax,-0x14(%ebp)
#undef     CONV
  }

  *r = t1;
801031c8:	8b 45 08             	mov    0x8(%ebp),%eax
801031cb:	8b 55 d8             	mov    -0x28(%ebp),%edx
801031ce:	89 10                	mov    %edx,(%eax)
801031d0:	8b 55 dc             	mov    -0x24(%ebp),%edx
801031d3:	89 50 04             	mov    %edx,0x4(%eax)
801031d6:	8b 55 e0             	mov    -0x20(%ebp),%edx
801031d9:	89 50 08             	mov    %edx,0x8(%eax)
801031dc:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801031df:	89 50 0c             	mov    %edx,0xc(%eax)
801031e2:	8b 55 e8             	mov    -0x18(%ebp),%edx
801031e5:	89 50 10             	mov    %edx,0x10(%eax)
801031e8:	8b 55 ec             	mov    -0x14(%ebp),%edx
801031eb:	89 50 14             	mov    %edx,0x14(%eax)
  r->year += 2000;
801031ee:	8b 45 08             	mov    0x8(%ebp),%eax
801031f1:	8b 40 14             	mov    0x14(%eax),%eax
801031f4:	8d 90 d0 07 00 00    	lea    0x7d0(%eax),%edx
801031fa:	8b 45 08             	mov    0x8(%ebp),%eax
801031fd:	89 50 14             	mov    %edx,0x14(%eax)
}
80103200:	c9                   	leave  
80103201:	c3                   	ret    

80103202 <initlog>:
static void recover_from_log(void);
static void commit();

void
initlog(void)
{
80103202:	55                   	push   %ebp
80103203:	89 e5                	mov    %esp,%ebp
80103205:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
80103208:	c7 44 24 04 3c 8b 10 	movl   $0x80108b3c,0x4(%esp)
8010320f:	80 
80103210:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
80103217:	e8 36 1d 00 00       	call   80104f52 <initlock>
  readsb(ROOTDEV, &sb);
8010321c:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010321f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103223:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010322a:	e8 c2 e0 ff ff       	call   801012f1 <readsb>
  log.start = sb.size - sb.nlog;
8010322f:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103232:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103235:	29 c2                	sub    %eax,%edx
80103237:	89 d0                	mov    %edx,%eax
80103239:	a3 b4 22 11 80       	mov    %eax,0x801122b4
  log.size = sb.nlog;
8010323e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103241:	a3 b8 22 11 80       	mov    %eax,0x801122b8
  log.dev = ROOTDEV;
80103246:	c7 05 c4 22 11 80 01 	movl   $0x1,0x801122c4
8010324d:	00 00 00 
  recover_from_log();
80103250:	e8 9a 01 00 00       	call   801033ef <recover_from_log>
}
80103255:	c9                   	leave  
80103256:	c3                   	ret    

80103257 <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
80103257:	55                   	push   %ebp
80103258:	89 e5                	mov    %esp,%ebp
8010325a:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
8010325d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103264:	e9 8c 00 00 00       	jmp    801032f5 <install_trans+0x9e>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80103269:	8b 15 b4 22 11 80    	mov    0x801122b4,%edx
8010326f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103272:	01 d0                	add    %edx,%eax
80103274:	83 c0 01             	add    $0x1,%eax
80103277:	89 c2                	mov    %eax,%edx
80103279:	a1 c4 22 11 80       	mov    0x801122c4,%eax
8010327e:	89 54 24 04          	mov    %edx,0x4(%esp)
80103282:	89 04 24             	mov    %eax,(%esp)
80103285:	e8 1c cf ff ff       	call   801001a6 <bread>
8010328a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
8010328d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103290:	83 c0 10             	add    $0x10,%eax
80103293:	8b 04 85 8c 22 11 80 	mov    -0x7feedd74(,%eax,4),%eax
8010329a:	89 c2                	mov    %eax,%edx
8010329c:	a1 c4 22 11 80       	mov    0x801122c4,%eax
801032a1:	89 54 24 04          	mov    %edx,0x4(%esp)
801032a5:	89 04 24             	mov    %eax,(%esp)
801032a8:	e8 f9 ce ff ff       	call   801001a6 <bread>
801032ad:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801032b0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801032b3:	8d 50 18             	lea    0x18(%eax),%edx
801032b6:	8b 45 ec             	mov    -0x14(%ebp),%eax
801032b9:	83 c0 18             	add    $0x18,%eax
801032bc:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801032c3:	00 
801032c4:	89 54 24 04          	mov    %edx,0x4(%esp)
801032c8:	89 04 24             	mov    %eax,(%esp)
801032cb:	e8 c6 1f 00 00       	call   80105296 <memmove>
    bwrite(dbuf);  // write dst to disk
801032d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801032d3:	89 04 24             	mov    %eax,(%esp)
801032d6:	e8 02 cf ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801032db:	8b 45 f0             	mov    -0x10(%ebp),%eax
801032de:	89 04 24             	mov    %eax,(%esp)
801032e1:	e8 31 cf ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801032e6:	8b 45 ec             	mov    -0x14(%ebp),%eax
801032e9:	89 04 24             	mov    %eax,(%esp)
801032ec:	e8 26 cf ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801032f1:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801032f5:	a1 c8 22 11 80       	mov    0x801122c8,%eax
801032fa:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801032fd:	0f 8f 66 ff ff ff    	jg     80103269 <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
80103303:	c9                   	leave  
80103304:	c3                   	ret    

80103305 <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
80103305:	55                   	push   %ebp
80103306:	89 e5                	mov    %esp,%ebp
80103308:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
8010330b:	a1 b4 22 11 80       	mov    0x801122b4,%eax
80103310:	89 c2                	mov    %eax,%edx
80103312:	a1 c4 22 11 80       	mov    0x801122c4,%eax
80103317:	89 54 24 04          	mov    %edx,0x4(%esp)
8010331b:	89 04 24             	mov    %eax,(%esp)
8010331e:	e8 83 ce ff ff       	call   801001a6 <bread>
80103323:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
80103326:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103329:	83 c0 18             	add    $0x18,%eax
8010332c:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
8010332f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103332:	8b 00                	mov    (%eax),%eax
80103334:	a3 c8 22 11 80       	mov    %eax,0x801122c8
  for (i = 0; i < log.lh.n; i++) {
80103339:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103340:	eb 1b                	jmp    8010335d <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
80103342:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103345:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103348:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
8010334c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010334f:	83 c2 10             	add    $0x10,%edx
80103352:	89 04 95 8c 22 11 80 	mov    %eax,-0x7feedd74(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
80103359:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010335d:	a1 c8 22 11 80       	mov    0x801122c8,%eax
80103362:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103365:	7f db                	jg     80103342 <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
80103367:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010336a:	89 04 24             	mov    %eax,(%esp)
8010336d:	e8 a5 ce ff ff       	call   80100217 <brelse>
}
80103372:	c9                   	leave  
80103373:	c3                   	ret    

80103374 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103374:	55                   	push   %ebp
80103375:	89 e5                	mov    %esp,%ebp
80103377:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
8010337a:	a1 b4 22 11 80       	mov    0x801122b4,%eax
8010337f:	89 c2                	mov    %eax,%edx
80103381:	a1 c4 22 11 80       	mov    0x801122c4,%eax
80103386:	89 54 24 04          	mov    %edx,0x4(%esp)
8010338a:	89 04 24             	mov    %eax,(%esp)
8010338d:	e8 14 ce ff ff       	call   801001a6 <bread>
80103392:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
80103395:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103398:	83 c0 18             	add    $0x18,%eax
8010339b:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
8010339e:	8b 15 c8 22 11 80    	mov    0x801122c8,%edx
801033a4:	8b 45 ec             	mov    -0x14(%ebp),%eax
801033a7:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
801033a9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801033b0:	eb 1b                	jmp    801033cd <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
801033b2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801033b5:	83 c0 10             	add    $0x10,%eax
801033b8:	8b 0c 85 8c 22 11 80 	mov    -0x7feedd74(,%eax,4),%ecx
801033bf:	8b 45 ec             	mov    -0x14(%ebp),%eax
801033c2:	8b 55 f4             	mov    -0xc(%ebp),%edx
801033c5:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
801033c9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801033cd:	a1 c8 22 11 80       	mov    0x801122c8,%eax
801033d2:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801033d5:	7f db                	jg     801033b2 <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801033d7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801033da:	89 04 24             	mov    %eax,(%esp)
801033dd:	e8 fb cd ff ff       	call   801001dd <bwrite>
  brelse(buf);
801033e2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801033e5:	89 04 24             	mov    %eax,(%esp)
801033e8:	e8 2a ce ff ff       	call   80100217 <brelse>
}
801033ed:	c9                   	leave  
801033ee:	c3                   	ret    

801033ef <recover_from_log>:

static void
recover_from_log(void)
{
801033ef:	55                   	push   %ebp
801033f0:	89 e5                	mov    %esp,%ebp
801033f2:	83 ec 08             	sub    $0x8,%esp
  read_head();      
801033f5:	e8 0b ff ff ff       	call   80103305 <read_head>
  install_trans(); // if committed, copy from log to disk
801033fa:	e8 58 fe ff ff       	call   80103257 <install_trans>
  log.lh.n = 0;
801033ff:	c7 05 c8 22 11 80 00 	movl   $0x0,0x801122c8
80103406:	00 00 00 
  write_head(); // clear the log
80103409:	e8 66 ff ff ff       	call   80103374 <write_head>
}
8010340e:	c9                   	leave  
8010340f:	c3                   	ret    

80103410 <begin_op>:

// called at the start of each FS system call.
void
begin_op(void)
{
80103410:	55                   	push   %ebp
80103411:	89 e5                	mov    %esp,%ebp
80103413:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
80103416:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
8010341d:	e8 51 1b 00 00       	call   80104f73 <acquire>
  while(1){
    if(log.committing){
80103422:	a1 c0 22 11 80       	mov    0x801122c0,%eax
80103427:	85 c0                	test   %eax,%eax
80103429:	74 16                	je     80103441 <begin_op+0x31>
      sleep(&log, &log.lock);
8010342b:	c7 44 24 04 80 22 11 	movl   $0x80112280,0x4(%esp)
80103432:	80 
80103433:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
8010343a:	e8 19 17 00 00       	call   80104b58 <sleep>
8010343f:	eb 4f                	jmp    80103490 <begin_op+0x80>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
80103441:	8b 0d c8 22 11 80    	mov    0x801122c8,%ecx
80103447:	a1 bc 22 11 80       	mov    0x801122bc,%eax
8010344c:	8d 50 01             	lea    0x1(%eax),%edx
8010344f:	89 d0                	mov    %edx,%eax
80103451:	c1 e0 02             	shl    $0x2,%eax
80103454:	01 d0                	add    %edx,%eax
80103456:	01 c0                	add    %eax,%eax
80103458:	01 c8                	add    %ecx,%eax
8010345a:	83 f8 1e             	cmp    $0x1e,%eax
8010345d:	7e 16                	jle    80103475 <begin_op+0x65>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
8010345f:	c7 44 24 04 80 22 11 	movl   $0x80112280,0x4(%esp)
80103466:	80 
80103467:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
8010346e:	e8 e5 16 00 00       	call   80104b58 <sleep>
80103473:	eb 1b                	jmp    80103490 <begin_op+0x80>
    } else {
      log.outstanding += 1;
80103475:	a1 bc 22 11 80       	mov    0x801122bc,%eax
8010347a:	83 c0 01             	add    $0x1,%eax
8010347d:	a3 bc 22 11 80       	mov    %eax,0x801122bc
      release(&log.lock);
80103482:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
80103489:	e8 47 1b 00 00       	call   80104fd5 <release>
      break;
8010348e:	eb 02                	jmp    80103492 <begin_op+0x82>
    }
  }
80103490:	eb 90                	jmp    80103422 <begin_op+0x12>
}
80103492:	c9                   	leave  
80103493:	c3                   	ret    

80103494 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
80103494:	55                   	push   %ebp
80103495:	89 e5                	mov    %esp,%ebp
80103497:	83 ec 28             	sub    $0x28,%esp
  int do_commit = 0;
8010349a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

  acquire(&log.lock);
801034a1:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
801034a8:	e8 c6 1a 00 00       	call   80104f73 <acquire>
  log.outstanding -= 1;
801034ad:	a1 bc 22 11 80       	mov    0x801122bc,%eax
801034b2:	83 e8 01             	sub    $0x1,%eax
801034b5:	a3 bc 22 11 80       	mov    %eax,0x801122bc
  if(log.committing)
801034ba:	a1 c0 22 11 80       	mov    0x801122c0,%eax
801034bf:	85 c0                	test   %eax,%eax
801034c1:	74 0c                	je     801034cf <end_op+0x3b>
    panic("log.committing");
801034c3:	c7 04 24 40 8b 10 80 	movl   $0x80108b40,(%esp)
801034ca:	e8 6b d0 ff ff       	call   8010053a <panic>
  if(log.outstanding == 0){
801034cf:	a1 bc 22 11 80       	mov    0x801122bc,%eax
801034d4:	85 c0                	test   %eax,%eax
801034d6:	75 13                	jne    801034eb <end_op+0x57>
    do_commit = 1;
801034d8:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
    log.committing = 1;
801034df:	c7 05 c0 22 11 80 01 	movl   $0x1,0x801122c0
801034e6:	00 00 00 
801034e9:	eb 0c                	jmp    801034f7 <end_op+0x63>
  } else {
    // begin_op() may be waiting for log space.
    wakeup(&log);
801034eb:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
801034f2:	e8 3d 17 00 00       	call   80104c34 <wakeup>
  }
  release(&log.lock);
801034f7:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
801034fe:	e8 d2 1a 00 00       	call   80104fd5 <release>

  if(do_commit){
80103503:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103507:	74 33                	je     8010353c <end_op+0xa8>
    // call commit w/o holding locks, since not allowed
    // to sleep with locks.
    commit();
80103509:	e8 de 00 00 00       	call   801035ec <commit>
    acquire(&log.lock);
8010350e:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
80103515:	e8 59 1a 00 00       	call   80104f73 <acquire>
    log.committing = 0;
8010351a:	c7 05 c0 22 11 80 00 	movl   $0x0,0x801122c0
80103521:	00 00 00 
    wakeup(&log);
80103524:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
8010352b:	e8 04 17 00 00       	call   80104c34 <wakeup>
    release(&log.lock);
80103530:	c7 04 24 80 22 11 80 	movl   $0x80112280,(%esp)
80103537:	e8 99 1a 00 00       	call   80104fd5 <release>
  }
}
8010353c:	c9                   	leave  
8010353d:	c3                   	ret    

8010353e <write_log>:

// Copy modified blocks from cache to log.
static void 
write_log(void)
{
8010353e:	55                   	push   %ebp
8010353f:	89 e5                	mov    %esp,%ebp
80103541:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103544:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010354b:	e9 8c 00 00 00       	jmp    801035dc <write_log+0x9e>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
80103550:	8b 15 b4 22 11 80    	mov    0x801122b4,%edx
80103556:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103559:	01 d0                	add    %edx,%eax
8010355b:	83 c0 01             	add    $0x1,%eax
8010355e:	89 c2                	mov    %eax,%edx
80103560:	a1 c4 22 11 80       	mov    0x801122c4,%eax
80103565:	89 54 24 04          	mov    %edx,0x4(%esp)
80103569:	89 04 24             	mov    %eax,(%esp)
8010356c:	e8 35 cc ff ff       	call   801001a6 <bread>
80103571:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *from = bread(log.dev, log.lh.sector[tail]); // cache block
80103574:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103577:	83 c0 10             	add    $0x10,%eax
8010357a:	8b 04 85 8c 22 11 80 	mov    -0x7feedd74(,%eax,4),%eax
80103581:	89 c2                	mov    %eax,%edx
80103583:	a1 c4 22 11 80       	mov    0x801122c4,%eax
80103588:	89 54 24 04          	mov    %edx,0x4(%esp)
8010358c:	89 04 24             	mov    %eax,(%esp)
8010358f:	e8 12 cc ff ff       	call   801001a6 <bread>
80103594:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(to->data, from->data, BSIZE);
80103597:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010359a:	8d 50 18             	lea    0x18(%eax),%edx
8010359d:	8b 45 f0             	mov    -0x10(%ebp),%eax
801035a0:	83 c0 18             	add    $0x18,%eax
801035a3:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801035aa:	00 
801035ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801035af:	89 04 24             	mov    %eax,(%esp)
801035b2:	e8 df 1c 00 00       	call   80105296 <memmove>
    bwrite(to);  // write the log
801035b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801035ba:	89 04 24             	mov    %eax,(%esp)
801035bd:	e8 1b cc ff ff       	call   801001dd <bwrite>
    brelse(from); 
801035c2:	8b 45 ec             	mov    -0x14(%ebp),%eax
801035c5:	89 04 24             	mov    %eax,(%esp)
801035c8:	e8 4a cc ff ff       	call   80100217 <brelse>
    brelse(to);
801035cd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801035d0:	89 04 24             	mov    %eax,(%esp)
801035d3:	e8 3f cc ff ff       	call   80100217 <brelse>
static void 
write_log(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801035d8:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801035dc:	a1 c8 22 11 80       	mov    0x801122c8,%eax
801035e1:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801035e4:	0f 8f 66 ff ff ff    	jg     80103550 <write_log+0x12>
    memmove(to->data, from->data, BSIZE);
    bwrite(to);  // write the log
    brelse(from); 
    brelse(to);
  }
}
801035ea:	c9                   	leave  
801035eb:	c3                   	ret    

801035ec <commit>:

static void
commit()
{
801035ec:	55                   	push   %ebp
801035ed:	89 e5                	mov    %esp,%ebp
801035ef:	83 ec 08             	sub    $0x8,%esp
  if (log.lh.n > 0) {
801035f2:	a1 c8 22 11 80       	mov    0x801122c8,%eax
801035f7:	85 c0                	test   %eax,%eax
801035f9:	7e 1e                	jle    80103619 <commit+0x2d>
    write_log();     // Write modified blocks from cache to log
801035fb:	e8 3e ff ff ff       	call   8010353e <write_log>
    write_head();    // Write header to disk -- the real commit
80103600:	e8 6f fd ff ff       	call   80103374 <write_head>
    install_trans(); // Now install writes to home locations
80103605:	e8 4d fc ff ff       	call   80103257 <install_trans>
    log.lh.n = 0; 
8010360a:	c7 05 c8 22 11 80 00 	movl   $0x0,0x801122c8
80103611:	00 00 00 
    write_head();    // Erase the transaction from the log
80103614:	e8 5b fd ff ff       	call   80103374 <write_head>
  }
}
80103619:	c9                   	leave  
8010361a:	c3                   	ret    

8010361b <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
8010361b:	55                   	push   %ebp
8010361c:	89 e5                	mov    %esp,%ebp
8010361e:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103621:	a1 c8 22 11 80       	mov    0x801122c8,%eax
80103626:	83 f8 1d             	cmp    $0x1d,%eax
80103629:	7f 12                	jg     8010363d <log_write+0x22>
8010362b:	a1 c8 22 11 80       	mov    0x801122c8,%eax
80103630:	8b 15 b8 22 11 80    	mov    0x801122b8,%edx
80103636:	83 ea 01             	sub    $0x1,%edx
80103639:	39 d0                	cmp    %edx,%eax
8010363b:	7c 0c                	jl     80103649 <log_write+0x2e>
    panic("too big a transaction");
8010363d:	c7 04 24 4f 8b 10 80 	movl   $0x80108b4f,(%esp)
80103644:	e8 f1 ce ff ff       	call   8010053a <panic>
  if (log.outstanding < 1)
80103649:	a1 bc 22 11 80       	mov    0x801122bc,%eax
8010364e:	85 c0                	test   %eax,%eax
80103650:	7f 0c                	jg     8010365e <log_write+0x43>
    panic("log_write outside of trans");
80103652:	c7 04 24 65 8b 10 80 	movl   $0x80108b65,(%esp)
80103659:	e8 dc ce ff ff       	call   8010053a <panic>

  for (i = 0; i < log.lh.n; i++) {
8010365e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103665:	eb 1f                	jmp    80103686 <log_write+0x6b>
    if (log.lh.sector[i] == b->sector)   // log absorbtion
80103667:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010366a:	83 c0 10             	add    $0x10,%eax
8010366d:	8b 04 85 8c 22 11 80 	mov    -0x7feedd74(,%eax,4),%eax
80103674:	89 c2                	mov    %eax,%edx
80103676:	8b 45 08             	mov    0x8(%ebp),%eax
80103679:	8b 40 08             	mov    0x8(%eax),%eax
8010367c:	39 c2                	cmp    %eax,%edx
8010367e:	75 02                	jne    80103682 <log_write+0x67>
      break;
80103680:	eb 0e                	jmp    80103690 <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (log.outstanding < 1)
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
80103682:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103686:	a1 c8 22 11 80       	mov    0x801122c8,%eax
8010368b:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010368e:	7f d7                	jg     80103667 <log_write+0x4c>
    if (log.lh.sector[i] == b->sector)   // log absorbtion
      break;
  }
  log.lh.sector[i] = b->sector;
80103690:	8b 45 08             	mov    0x8(%ebp),%eax
80103693:	8b 40 08             	mov    0x8(%eax),%eax
80103696:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103699:	83 c2 10             	add    $0x10,%edx
8010369c:	89 04 95 8c 22 11 80 	mov    %eax,-0x7feedd74(,%edx,4)
  if (i == log.lh.n)
801036a3:	a1 c8 22 11 80       	mov    0x801122c8,%eax
801036a8:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801036ab:	75 0d                	jne    801036ba <log_write+0x9f>
    log.lh.n++;
801036ad:	a1 c8 22 11 80       	mov    0x801122c8,%eax
801036b2:	83 c0 01             	add    $0x1,%eax
801036b5:	a3 c8 22 11 80       	mov    %eax,0x801122c8
  b->flags |= B_DIRTY; // prevent eviction
801036ba:	8b 45 08             	mov    0x8(%ebp),%eax
801036bd:	8b 00                	mov    (%eax),%eax
801036bf:	83 c8 04             	or     $0x4,%eax
801036c2:	89 c2                	mov    %eax,%edx
801036c4:	8b 45 08             	mov    0x8(%ebp),%eax
801036c7:	89 10                	mov    %edx,(%eax)
}
801036c9:	c9                   	leave  
801036ca:	c3                   	ret    

801036cb <v2p>:
801036cb:	55                   	push   %ebp
801036cc:	89 e5                	mov    %esp,%ebp
801036ce:	8b 45 08             	mov    0x8(%ebp),%eax
801036d1:	05 00 00 00 80       	add    $0x80000000,%eax
801036d6:	5d                   	pop    %ebp
801036d7:	c3                   	ret    

801036d8 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801036d8:	55                   	push   %ebp
801036d9:	89 e5                	mov    %esp,%ebp
801036db:	8b 45 08             	mov    0x8(%ebp),%eax
801036de:	05 00 00 00 80       	add    $0x80000000,%eax
801036e3:	5d                   	pop    %ebp
801036e4:	c3                   	ret    

801036e5 <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801036e5:	55                   	push   %ebp
801036e6:	89 e5                	mov    %esp,%ebp
801036e8:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801036eb:	8b 55 08             	mov    0x8(%ebp),%edx
801036ee:	8b 45 0c             	mov    0xc(%ebp),%eax
801036f1:	8b 4d 08             	mov    0x8(%ebp),%ecx
801036f4:	f0 87 02             	lock xchg %eax,(%edx)
801036f7:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
801036fa:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801036fd:	c9                   	leave  
801036fe:	c3                   	ret    

801036ff <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
801036ff:	55                   	push   %ebp
80103700:	89 e5                	mov    %esp,%ebp
80103702:	83 e4 f0             	and    $0xfffffff0,%esp
80103705:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
80103708:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
8010370f:	80 
80103710:	c7 04 24 7c 56 11 80 	movl   $0x8011567c,(%esp)
80103717:	e8 98 f2 ff ff       	call   801029b4 <kinit1>
  kvmalloc();      // kernel page table
8010371c:	e8 63 4a 00 00       	call   80108184 <kvmalloc>
  mpinit();        // collect info about this machine
80103721:	e8 46 04 00 00       	call   80103b6c <mpinit>
  lapicinit();
80103726:	e8 f4 f5 ff ff       	call   80102d1f <lapicinit>
  seginit();       // set up segments
8010372b:	e8 e7 43 00 00       	call   80107b17 <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
80103730:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103736:	0f b6 00             	movzbl (%eax),%eax
80103739:	0f b6 c0             	movzbl %al,%eax
8010373c:	89 44 24 04          	mov    %eax,0x4(%esp)
80103740:	c7 04 24 80 8b 10 80 	movl   $0x80108b80,(%esp)
80103747:	e8 54 cc ff ff       	call   801003a0 <cprintf>
  picinit();       // interrupt controller
8010374c:	e8 79 06 00 00       	call   80103dca <picinit>
  ioapicinit();    // another interrupt controller
80103751:	e8 54 f1 ff ff       	call   801028aa <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
80103756:	e8 26 d3 ff ff       	call   80100a81 <consoleinit>
  uartinit();      // serial port
8010375b:	e8 06 37 00 00       	call   80106e66 <uartinit>
  pinit();         // process table
80103760:	e8 6f 0b 00 00       	call   801042d4 <pinit>
  tvinit();        // trap vectors
80103765:	e8 18 31 00 00       	call   80106882 <tvinit>
  binit();         // buffer cache
8010376a:	e8 c5 c8 ff ff       	call   80100034 <binit>
  fileinit();      // file table
8010376f:	e8 96 d7 ff ff       	call   80100f0a <fileinit>
  iinit();         // inode cache
80103774:	e8 2b de ff ff       	call   801015a4 <iinit>
  ideinit();       // disk
80103779:	e8 95 ed ff ff       	call   80102513 <ideinit>
  if(!ismp)
8010377e:	a1 64 23 11 80       	mov    0x80112364,%eax
80103783:	85 c0                	test   %eax,%eax
80103785:	75 05                	jne    8010378c <main+0x8d>
    timerinit();   // uniprocessor timer
80103787:	e8 41 30 00 00       	call   801067cd <timerinit>
  startothers();   // start other processors
8010378c:	e8 7f 00 00 00       	call   80103810 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103791:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
80103798:	8e 
80103799:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
801037a0:	e8 47 f2 ff ff       	call   801029ec <kinit2>
  userinit();      // first user process
801037a5:	e8 8c 0c 00 00       	call   80104436 <userinit>
  // Finish setting up this processor in mpmain.
  mpmain();
801037aa:	e8 1a 00 00 00       	call   801037c9 <mpmain>

801037af <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
801037af:	55                   	push   %ebp
801037b0:	89 e5                	mov    %esp,%ebp
801037b2:	83 ec 08             	sub    $0x8,%esp
  switchkvm(); 
801037b5:	e8 e1 49 00 00       	call   8010819b <switchkvm>
  seginit();
801037ba:	e8 58 43 00 00       	call   80107b17 <seginit>
  lapicinit();
801037bf:	e8 5b f5 ff ff       	call   80102d1f <lapicinit>
  mpmain();
801037c4:	e8 00 00 00 00       	call   801037c9 <mpmain>

801037c9 <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801037c9:	55                   	push   %ebp
801037ca:	89 e5                	mov    %esp,%ebp
801037cc:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
801037cf:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801037d5:	0f b6 00             	movzbl (%eax),%eax
801037d8:	0f b6 c0             	movzbl %al,%eax
801037db:	89 44 24 04          	mov    %eax,0x4(%esp)
801037df:	c7 04 24 97 8b 10 80 	movl   $0x80108b97,(%esp)
801037e6:	e8 b5 cb ff ff       	call   801003a0 <cprintf>
  idtinit();       // load idt register
801037eb:	e8 06 32 00 00       	call   801069f6 <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
801037f0:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801037f6:	05 a8 00 00 00       	add    $0xa8,%eax
801037fb:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80103802:	00 
80103803:	89 04 24             	mov    %eax,(%esp)
80103806:	e8 da fe ff ff       	call   801036e5 <xchg>
  scheduler();     // start running processes
8010380b:	e8 9d 11 00 00       	call   801049ad <scheduler>

80103810 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
80103810:	55                   	push   %ebp
80103811:	89 e5                	mov    %esp,%ebp
80103813:	53                   	push   %ebx
80103814:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
80103817:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
8010381e:	e8 b5 fe ff ff       	call   801036d8 <p2v>
80103823:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
80103826:	b8 8a 00 00 00       	mov    $0x8a,%eax
8010382b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010382f:	c7 44 24 04 2c b5 10 	movl   $0x8010b52c,0x4(%esp)
80103836:	80 
80103837:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010383a:	89 04 24             	mov    %eax,(%esp)
8010383d:	e8 54 1a 00 00       	call   80105296 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103842:	c7 45 f4 80 23 11 80 	movl   $0x80112380,-0xc(%ebp)
80103849:	e9 85 00 00 00       	jmp    801038d3 <startothers+0xc3>
    if(c == cpus+cpunum())  // We've started already.
8010384e:	e8 25 f6 ff ff       	call   80102e78 <cpunum>
80103853:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103859:	05 80 23 11 80       	add    $0x80112380,%eax
8010385e:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103861:	75 02                	jne    80103865 <startothers+0x55>
      continue;
80103863:	eb 67                	jmp    801038cc <startothers+0xbc>

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
80103865:	e8 78 f2 ff ff       	call   80102ae2 <kalloc>
8010386a:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
8010386d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103870:	83 e8 04             	sub    $0x4,%eax
80103873:	8b 55 ec             	mov    -0x14(%ebp),%edx
80103876:	81 c2 00 10 00 00    	add    $0x1000,%edx
8010387c:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
8010387e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103881:	83 e8 08             	sub    $0x8,%eax
80103884:	c7 00 af 37 10 80    	movl   $0x801037af,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
8010388a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010388d:	8d 58 f4             	lea    -0xc(%eax),%ebx
80103890:	c7 04 24 00 a0 10 80 	movl   $0x8010a000,(%esp)
80103897:	e8 2f fe ff ff       	call   801036cb <v2p>
8010389c:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
8010389e:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038a1:	89 04 24             	mov    %eax,(%esp)
801038a4:	e8 22 fe ff ff       	call   801036cb <v2p>
801038a9:	8b 55 f4             	mov    -0xc(%ebp),%edx
801038ac:	0f b6 12             	movzbl (%edx),%edx
801038af:	0f b6 d2             	movzbl %dl,%edx
801038b2:	89 44 24 04          	mov    %eax,0x4(%esp)
801038b6:	89 14 24             	mov    %edx,(%esp)
801038b9:	e8 3c f6 ff ff       	call   80102efa <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
801038be:	90                   	nop
801038bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038c2:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
801038c8:	85 c0                	test   %eax,%eax
801038ca:	74 f3                	je     801038bf <startothers+0xaf>
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
801038cc:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
801038d3:	a1 60 29 11 80       	mov    0x80112960,%eax
801038d8:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801038de:	05 80 23 11 80       	add    $0x80112380,%eax
801038e3:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801038e6:	0f 87 62 ff ff ff    	ja     8010384e <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801038ec:	83 c4 24             	add    $0x24,%esp
801038ef:	5b                   	pop    %ebx
801038f0:	5d                   	pop    %ebp
801038f1:	c3                   	ret    

801038f2 <p2v>:
801038f2:	55                   	push   %ebp
801038f3:	89 e5                	mov    %esp,%ebp
801038f5:	8b 45 08             	mov    0x8(%ebp),%eax
801038f8:	05 00 00 00 80       	add    $0x80000000,%eax
801038fd:	5d                   	pop    %ebp
801038fe:	c3                   	ret    

801038ff <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801038ff:	55                   	push   %ebp
80103900:	89 e5                	mov    %esp,%ebp
80103902:	83 ec 14             	sub    $0x14,%esp
80103905:	8b 45 08             	mov    0x8(%ebp),%eax
80103908:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010390c:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80103910:	89 c2                	mov    %eax,%edx
80103912:	ec                   	in     (%dx),%al
80103913:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80103916:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
8010391a:	c9                   	leave  
8010391b:	c3                   	ret    

8010391c <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
8010391c:	55                   	push   %ebp
8010391d:	89 e5                	mov    %esp,%ebp
8010391f:	83 ec 08             	sub    $0x8,%esp
80103922:	8b 55 08             	mov    0x8(%ebp),%edx
80103925:	8b 45 0c             	mov    0xc(%ebp),%eax
80103928:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010392c:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010392f:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103933:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103937:	ee                   	out    %al,(%dx)
}
80103938:	c9                   	leave  
80103939:	c3                   	ret    

8010393a <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
8010393a:	55                   	push   %ebp
8010393b:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
8010393d:	a1 64 b6 10 80       	mov    0x8010b664,%eax
80103942:	89 c2                	mov    %eax,%edx
80103944:	b8 80 23 11 80       	mov    $0x80112380,%eax
80103949:	29 c2                	sub    %eax,%edx
8010394b:	89 d0                	mov    %edx,%eax
8010394d:	c1 f8 02             	sar    $0x2,%eax
80103950:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
80103956:	5d                   	pop    %ebp
80103957:	c3                   	ret    

80103958 <sum>:

static uchar
sum(uchar *addr, int len)
{
80103958:	55                   	push   %ebp
80103959:	89 e5                	mov    %esp,%ebp
8010395b:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
8010395e:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
80103965:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
8010396c:	eb 15                	jmp    80103983 <sum+0x2b>
    sum += addr[i];
8010396e:	8b 55 fc             	mov    -0x4(%ebp),%edx
80103971:	8b 45 08             	mov    0x8(%ebp),%eax
80103974:	01 d0                	add    %edx,%eax
80103976:	0f b6 00             	movzbl (%eax),%eax
80103979:	0f b6 c0             	movzbl %al,%eax
8010397c:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
8010397f:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103983:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103986:	3b 45 0c             	cmp    0xc(%ebp),%eax
80103989:	7c e3                	jl     8010396e <sum+0x16>
    sum += addr[i];
  return sum;
8010398b:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
8010398e:	c9                   	leave  
8010398f:	c3                   	ret    

80103990 <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103990:	55                   	push   %ebp
80103991:	89 e5                	mov    %esp,%ebp
80103993:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
80103996:	8b 45 08             	mov    0x8(%ebp),%eax
80103999:	89 04 24             	mov    %eax,(%esp)
8010399c:	e8 51 ff ff ff       	call   801038f2 <p2v>
801039a1:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
801039a4:	8b 55 0c             	mov    0xc(%ebp),%edx
801039a7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801039aa:	01 d0                	add    %edx,%eax
801039ac:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
801039af:	8b 45 f0             	mov    -0x10(%ebp),%eax
801039b2:	89 45 f4             	mov    %eax,-0xc(%ebp)
801039b5:	eb 3f                	jmp    801039f6 <mpsearch1+0x66>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
801039b7:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801039be:	00 
801039bf:	c7 44 24 04 a8 8b 10 	movl   $0x80108ba8,0x4(%esp)
801039c6:	80 
801039c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039ca:	89 04 24             	mov    %eax,(%esp)
801039cd:	e8 6c 18 00 00       	call   8010523e <memcmp>
801039d2:	85 c0                	test   %eax,%eax
801039d4:	75 1c                	jne    801039f2 <mpsearch1+0x62>
801039d6:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801039dd:	00 
801039de:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039e1:	89 04 24             	mov    %eax,(%esp)
801039e4:	e8 6f ff ff ff       	call   80103958 <sum>
801039e9:	84 c0                	test   %al,%al
801039eb:	75 05                	jne    801039f2 <mpsearch1+0x62>
      return (struct mp*)p;
801039ed:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039f0:	eb 11                	jmp    80103a03 <mpsearch1+0x73>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
801039f2:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
801039f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039f9:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801039fc:	72 b9                	jb     801039b7 <mpsearch1+0x27>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
801039fe:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103a03:	c9                   	leave  
80103a04:	c3                   	ret    

80103a05 <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
80103a05:	55                   	push   %ebp
80103a06:	89 e5                	mov    %esp,%ebp
80103a08:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
80103a0b:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
80103a12:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a15:	83 c0 0f             	add    $0xf,%eax
80103a18:	0f b6 00             	movzbl (%eax),%eax
80103a1b:	0f b6 c0             	movzbl %al,%eax
80103a1e:	c1 e0 08             	shl    $0x8,%eax
80103a21:	89 c2                	mov    %eax,%edx
80103a23:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a26:	83 c0 0e             	add    $0xe,%eax
80103a29:	0f b6 00             	movzbl (%eax),%eax
80103a2c:	0f b6 c0             	movzbl %al,%eax
80103a2f:	09 d0                	or     %edx,%eax
80103a31:	c1 e0 04             	shl    $0x4,%eax
80103a34:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103a37:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103a3b:	74 21                	je     80103a5e <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
80103a3d:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103a44:	00 
80103a45:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103a48:	89 04 24             	mov    %eax,(%esp)
80103a4b:	e8 40 ff ff ff       	call   80103990 <mpsearch1>
80103a50:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103a53:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103a57:	74 50                	je     80103aa9 <mpsearch+0xa4>
      return mp;
80103a59:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103a5c:	eb 5f                	jmp    80103abd <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103a5e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a61:	83 c0 14             	add    $0x14,%eax
80103a64:	0f b6 00             	movzbl (%eax),%eax
80103a67:	0f b6 c0             	movzbl %al,%eax
80103a6a:	c1 e0 08             	shl    $0x8,%eax
80103a6d:	89 c2                	mov    %eax,%edx
80103a6f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a72:	83 c0 13             	add    $0x13,%eax
80103a75:	0f b6 00             	movzbl (%eax),%eax
80103a78:	0f b6 c0             	movzbl %al,%eax
80103a7b:	09 d0                	or     %edx,%eax
80103a7d:	c1 e0 0a             	shl    $0xa,%eax
80103a80:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103a83:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103a86:	2d 00 04 00 00       	sub    $0x400,%eax
80103a8b:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103a92:	00 
80103a93:	89 04 24             	mov    %eax,(%esp)
80103a96:	e8 f5 fe ff ff       	call   80103990 <mpsearch1>
80103a9b:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103a9e:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103aa2:	74 05                	je     80103aa9 <mpsearch+0xa4>
      return mp;
80103aa4:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103aa7:	eb 14                	jmp    80103abd <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
80103aa9:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80103ab0:	00 
80103ab1:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
80103ab8:	e8 d3 fe ff ff       	call   80103990 <mpsearch1>
}
80103abd:	c9                   	leave  
80103abe:	c3                   	ret    

80103abf <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
80103abf:	55                   	push   %ebp
80103ac0:	89 e5                	mov    %esp,%ebp
80103ac2:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
80103ac5:	e8 3b ff ff ff       	call   80103a05 <mpsearch>
80103aca:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103acd:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103ad1:	74 0a                	je     80103add <mpconfig+0x1e>
80103ad3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ad6:	8b 40 04             	mov    0x4(%eax),%eax
80103ad9:	85 c0                	test   %eax,%eax
80103adb:	75 0a                	jne    80103ae7 <mpconfig+0x28>
    return 0;
80103add:	b8 00 00 00 00       	mov    $0x0,%eax
80103ae2:	e9 83 00 00 00       	jmp    80103b6a <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
80103ae7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103aea:	8b 40 04             	mov    0x4(%eax),%eax
80103aed:	89 04 24             	mov    %eax,(%esp)
80103af0:	e8 fd fd ff ff       	call   801038f2 <p2v>
80103af5:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
80103af8:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103aff:	00 
80103b00:	c7 44 24 04 ad 8b 10 	movl   $0x80108bad,0x4(%esp)
80103b07:	80 
80103b08:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b0b:	89 04 24             	mov    %eax,(%esp)
80103b0e:	e8 2b 17 00 00       	call   8010523e <memcmp>
80103b13:	85 c0                	test   %eax,%eax
80103b15:	74 07                	je     80103b1e <mpconfig+0x5f>
    return 0;
80103b17:	b8 00 00 00 00       	mov    $0x0,%eax
80103b1c:	eb 4c                	jmp    80103b6a <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
80103b1e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b21:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103b25:	3c 01                	cmp    $0x1,%al
80103b27:	74 12                	je     80103b3b <mpconfig+0x7c>
80103b29:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b2c:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103b30:	3c 04                	cmp    $0x4,%al
80103b32:	74 07                	je     80103b3b <mpconfig+0x7c>
    return 0;
80103b34:	b8 00 00 00 00       	mov    $0x0,%eax
80103b39:	eb 2f                	jmp    80103b6a <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
80103b3b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b3e:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103b42:	0f b7 c0             	movzwl %ax,%eax
80103b45:	89 44 24 04          	mov    %eax,0x4(%esp)
80103b49:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b4c:	89 04 24             	mov    %eax,(%esp)
80103b4f:	e8 04 fe ff ff       	call   80103958 <sum>
80103b54:	84 c0                	test   %al,%al
80103b56:	74 07                	je     80103b5f <mpconfig+0xa0>
    return 0;
80103b58:	b8 00 00 00 00       	mov    $0x0,%eax
80103b5d:	eb 0b                	jmp    80103b6a <mpconfig+0xab>
  *pmp = mp;
80103b5f:	8b 45 08             	mov    0x8(%ebp),%eax
80103b62:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103b65:	89 10                	mov    %edx,(%eax)
  return conf;
80103b67:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80103b6a:	c9                   	leave  
80103b6b:	c3                   	ret    

80103b6c <mpinit>:

void
mpinit(void)
{
80103b6c:	55                   	push   %ebp
80103b6d:	89 e5                	mov    %esp,%ebp
80103b6f:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
80103b72:	c7 05 64 b6 10 80 80 	movl   $0x80112380,0x8010b664
80103b79:	23 11 80 
  if((conf = mpconfig(&mp)) == 0)
80103b7c:	8d 45 e0             	lea    -0x20(%ebp),%eax
80103b7f:	89 04 24             	mov    %eax,(%esp)
80103b82:	e8 38 ff ff ff       	call   80103abf <mpconfig>
80103b87:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103b8a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103b8e:	75 05                	jne    80103b95 <mpinit+0x29>
    return;
80103b90:	e9 9c 01 00 00       	jmp    80103d31 <mpinit+0x1c5>
  ismp = 1;
80103b95:	c7 05 64 23 11 80 01 	movl   $0x1,0x80112364
80103b9c:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
80103b9f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103ba2:	8b 40 24             	mov    0x24(%eax),%eax
80103ba5:	a3 7c 22 11 80       	mov    %eax,0x8011227c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103baa:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bad:	83 c0 2c             	add    $0x2c,%eax
80103bb0:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103bb3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bb6:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103bba:	0f b7 d0             	movzwl %ax,%edx
80103bbd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bc0:	01 d0                	add    %edx,%eax
80103bc2:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103bc5:	e9 f4 00 00 00       	jmp    80103cbe <mpinit+0x152>
    switch(*p){
80103bca:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103bcd:	0f b6 00             	movzbl (%eax),%eax
80103bd0:	0f b6 c0             	movzbl %al,%eax
80103bd3:	83 f8 04             	cmp    $0x4,%eax
80103bd6:	0f 87 bf 00 00 00    	ja     80103c9b <mpinit+0x12f>
80103bdc:	8b 04 85 f0 8b 10 80 	mov    -0x7fef7410(,%eax,4),%eax
80103be3:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
80103be5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103be8:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
80103beb:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103bee:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103bf2:	0f b6 d0             	movzbl %al,%edx
80103bf5:	a1 60 29 11 80       	mov    0x80112960,%eax
80103bfa:	39 c2                	cmp    %eax,%edx
80103bfc:	74 2d                	je     80103c2b <mpinit+0xbf>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
80103bfe:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103c01:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103c05:	0f b6 d0             	movzbl %al,%edx
80103c08:	a1 60 29 11 80       	mov    0x80112960,%eax
80103c0d:	89 54 24 08          	mov    %edx,0x8(%esp)
80103c11:	89 44 24 04          	mov    %eax,0x4(%esp)
80103c15:	c7 04 24 b2 8b 10 80 	movl   $0x80108bb2,(%esp)
80103c1c:	e8 7f c7 ff ff       	call   801003a0 <cprintf>
        ismp = 0;
80103c21:	c7 05 64 23 11 80 00 	movl   $0x0,0x80112364
80103c28:	00 00 00 
      }
      if(proc->flags & MPBOOT)
80103c2b:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103c2e:	0f b6 40 03          	movzbl 0x3(%eax),%eax
80103c32:	0f b6 c0             	movzbl %al,%eax
80103c35:	83 e0 02             	and    $0x2,%eax
80103c38:	85 c0                	test   %eax,%eax
80103c3a:	74 15                	je     80103c51 <mpinit+0xe5>
        bcpu = &cpus[ncpu];
80103c3c:	a1 60 29 11 80       	mov    0x80112960,%eax
80103c41:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103c47:	05 80 23 11 80       	add    $0x80112380,%eax
80103c4c:	a3 64 b6 10 80       	mov    %eax,0x8010b664
      cpus[ncpu].id = ncpu;
80103c51:	8b 15 60 29 11 80    	mov    0x80112960,%edx
80103c57:	a1 60 29 11 80       	mov    0x80112960,%eax
80103c5c:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
80103c62:	81 c2 80 23 11 80    	add    $0x80112380,%edx
80103c68:	88 02                	mov    %al,(%edx)
      ncpu++;
80103c6a:	a1 60 29 11 80       	mov    0x80112960,%eax
80103c6f:	83 c0 01             	add    $0x1,%eax
80103c72:	a3 60 29 11 80       	mov    %eax,0x80112960
      p += sizeof(struct mpproc);
80103c77:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80103c7b:	eb 41                	jmp    80103cbe <mpinit+0x152>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
80103c7d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c80:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
80103c83:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103c86:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103c8a:	a2 60 23 11 80       	mov    %al,0x80112360
      p += sizeof(struct mpioapic);
80103c8f:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103c93:	eb 29                	jmp    80103cbe <mpinit+0x152>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
80103c95:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103c99:	eb 23                	jmp    80103cbe <mpinit+0x152>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
80103c9b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c9e:	0f b6 00             	movzbl (%eax),%eax
80103ca1:	0f b6 c0             	movzbl %al,%eax
80103ca4:	89 44 24 04          	mov    %eax,0x4(%esp)
80103ca8:	c7 04 24 d0 8b 10 80 	movl   $0x80108bd0,(%esp)
80103caf:	e8 ec c6 ff ff       	call   801003a0 <cprintf>
      ismp = 0;
80103cb4:	c7 05 64 23 11 80 00 	movl   $0x0,0x80112364
80103cbb:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103cbe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cc1:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80103cc4:	0f 82 00 ff ff ff    	jb     80103bca <mpinit+0x5e>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
80103cca:	a1 64 23 11 80       	mov    0x80112364,%eax
80103ccf:	85 c0                	test   %eax,%eax
80103cd1:	75 1d                	jne    80103cf0 <mpinit+0x184>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
80103cd3:	c7 05 60 29 11 80 01 	movl   $0x1,0x80112960
80103cda:	00 00 00 
    lapic = 0;
80103cdd:	c7 05 7c 22 11 80 00 	movl   $0x0,0x8011227c
80103ce4:	00 00 00 
    ioapicid = 0;
80103ce7:	c6 05 60 23 11 80 00 	movb   $0x0,0x80112360
    return;
80103cee:	eb 41                	jmp    80103d31 <mpinit+0x1c5>
  }

  if(mp->imcrp){
80103cf0:	8b 45 e0             	mov    -0x20(%ebp),%eax
80103cf3:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
80103cf7:	84 c0                	test   %al,%al
80103cf9:	74 36                	je     80103d31 <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
80103cfb:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80103d02:	00 
80103d03:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80103d0a:	e8 0d fc ff ff       	call   8010391c <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103d0f:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103d16:	e8 e4 fb ff ff       	call   801038ff <inb>
80103d1b:	83 c8 01             	or     $0x1,%eax
80103d1e:	0f b6 c0             	movzbl %al,%eax
80103d21:	89 44 24 04          	mov    %eax,0x4(%esp)
80103d25:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103d2c:	e8 eb fb ff ff       	call   8010391c <outb>
  }
}
80103d31:	c9                   	leave  
80103d32:	c3                   	ret    

80103d33 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103d33:	55                   	push   %ebp
80103d34:	89 e5                	mov    %esp,%ebp
80103d36:	83 ec 08             	sub    $0x8,%esp
80103d39:	8b 55 08             	mov    0x8(%ebp),%edx
80103d3c:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d3f:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103d43:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103d46:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103d4a:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103d4e:	ee                   	out    %al,(%dx)
}
80103d4f:	c9                   	leave  
80103d50:	c3                   	ret    

80103d51 <picsetmask>:
// Initial IRQ mask has interrupt 2 enabled (for slave 8259A).
static ushort irqmask = 0xFFFF & ~(1<<IRQ_SLAVE);

static void
picsetmask(ushort mask)
{
80103d51:	55                   	push   %ebp
80103d52:	89 e5                	mov    %esp,%ebp
80103d54:	83 ec 0c             	sub    $0xc,%esp
80103d57:	8b 45 08             	mov    0x8(%ebp),%eax
80103d5a:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  irqmask = mask;
80103d5e:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103d62:	66 a3 00 b0 10 80    	mov    %ax,0x8010b000
  outb(IO_PIC1+1, mask);
80103d68:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103d6c:	0f b6 c0             	movzbl %al,%eax
80103d6f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103d73:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103d7a:	e8 b4 ff ff ff       	call   80103d33 <outb>
  outb(IO_PIC2+1, mask >> 8);
80103d7f:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103d83:	66 c1 e8 08          	shr    $0x8,%ax
80103d87:	0f b6 c0             	movzbl %al,%eax
80103d8a:	89 44 24 04          	mov    %eax,0x4(%esp)
80103d8e:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103d95:	e8 99 ff ff ff       	call   80103d33 <outb>
}
80103d9a:	c9                   	leave  
80103d9b:	c3                   	ret    

80103d9c <picenable>:

void
picenable(int irq)
{
80103d9c:	55                   	push   %ebp
80103d9d:	89 e5                	mov    %esp,%ebp
80103d9f:	83 ec 04             	sub    $0x4,%esp
  picsetmask(irqmask & ~(1<<irq));
80103da2:	8b 45 08             	mov    0x8(%ebp),%eax
80103da5:	ba 01 00 00 00       	mov    $0x1,%edx
80103daa:	89 c1                	mov    %eax,%ecx
80103dac:	d3 e2                	shl    %cl,%edx
80103dae:	89 d0                	mov    %edx,%eax
80103db0:	f7 d0                	not    %eax
80103db2:	89 c2                	mov    %eax,%edx
80103db4:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103dbb:	21 d0                	and    %edx,%eax
80103dbd:	0f b7 c0             	movzwl %ax,%eax
80103dc0:	89 04 24             	mov    %eax,(%esp)
80103dc3:	e8 89 ff ff ff       	call   80103d51 <picsetmask>
}
80103dc8:	c9                   	leave  
80103dc9:	c3                   	ret    

80103dca <picinit>:

// Initialize the 8259A interrupt controllers.
void
picinit(void)
{
80103dca:	55                   	push   %ebp
80103dcb:	89 e5                	mov    %esp,%ebp
80103dcd:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
80103dd0:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103dd7:	00 
80103dd8:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103ddf:	e8 4f ff ff ff       	call   80103d33 <outb>
  outb(IO_PIC2+1, 0xFF);
80103de4:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103deb:	00 
80103dec:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103df3:	e8 3b ff ff ff       	call   80103d33 <outb>

  // ICW1:  0001g0hi
  //    g:  0 = edge triggering, 1 = level triggering
  //    h:  0 = cascaded PICs, 1 = master only
  //    i:  0 = no ICW4, 1 = ICW4 required
  outb(IO_PIC1, 0x11);
80103df8:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103dff:	00 
80103e00:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103e07:	e8 27 ff ff ff       	call   80103d33 <outb>

  // ICW2:  Vector offset
  outb(IO_PIC1+1, T_IRQ0);
80103e0c:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
80103e13:	00 
80103e14:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103e1b:	e8 13 ff ff ff       	call   80103d33 <outb>

  // ICW3:  (master PIC) bit mask of IR lines connected to slaves
  //        (slave PIC) 3-bit # of slave's connection to master
  outb(IO_PIC1+1, 1<<IRQ_SLAVE);
80103e20:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
80103e27:	00 
80103e28:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103e2f:	e8 ff fe ff ff       	call   80103d33 <outb>
  //    m:  0 = slave PIC, 1 = master PIC
  //      (ignored when b is 0, as the master/slave role
  //      can be hardwired).
  //    a:  1 = Automatic EOI mode
  //    p:  0 = MCS-80/85 mode, 1 = intel x86 mode
  outb(IO_PIC1+1, 0x3);
80103e34:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103e3b:	00 
80103e3c:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103e43:	e8 eb fe ff ff       	call   80103d33 <outb>

  // Set up slave (8259A-2)
  outb(IO_PIC2, 0x11);                  // ICW1
80103e48:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103e4f:	00 
80103e50:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103e57:	e8 d7 fe ff ff       	call   80103d33 <outb>
  outb(IO_PIC2+1, T_IRQ0 + 8);      // ICW2
80103e5c:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
80103e63:	00 
80103e64:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103e6b:	e8 c3 fe ff ff       	call   80103d33 <outb>
  outb(IO_PIC2+1, IRQ_SLAVE);           // ICW3
80103e70:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80103e77:	00 
80103e78:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103e7f:	e8 af fe ff ff       	call   80103d33 <outb>
  // NB Automatic EOI mode doesn't tend to work on the slave.
  // Linux source code says it's "to be investigated".
  outb(IO_PIC2+1, 0x3);                 // ICW4
80103e84:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103e8b:	00 
80103e8c:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103e93:	e8 9b fe ff ff       	call   80103d33 <outb>

  // OCW3:  0ef01prs
  //   ef:  0x = NOP, 10 = clear specific mask, 11 = set specific mask
  //    p:  0 = no polling, 1 = polling mode
  //   rs:  0x = NOP, 10 = read IRR, 11 = read ISR
  outb(IO_PIC1, 0x68);             // clear specific mask
80103e98:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103e9f:	00 
80103ea0:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103ea7:	e8 87 fe ff ff       	call   80103d33 <outb>
  outb(IO_PIC1, 0x0a);             // read IRR by default
80103eac:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103eb3:	00 
80103eb4:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103ebb:	e8 73 fe ff ff       	call   80103d33 <outb>

  outb(IO_PIC2, 0x68);             // OCW3
80103ec0:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103ec7:	00 
80103ec8:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103ecf:	e8 5f fe ff ff       	call   80103d33 <outb>
  outb(IO_PIC2, 0x0a);             // OCW3
80103ed4:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103edb:	00 
80103edc:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103ee3:	e8 4b fe ff ff       	call   80103d33 <outb>

  if(irqmask != 0xFFFF)
80103ee8:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103eef:	66 83 f8 ff          	cmp    $0xffff,%ax
80103ef3:	74 12                	je     80103f07 <picinit+0x13d>
    picsetmask(irqmask);
80103ef5:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103efc:	0f b7 c0             	movzwl %ax,%eax
80103eff:	89 04 24             	mov    %eax,(%esp)
80103f02:	e8 4a fe ff ff       	call   80103d51 <picsetmask>
}
80103f07:	c9                   	leave  
80103f08:	c3                   	ret    

80103f09 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103f09:	55                   	push   %ebp
80103f0a:	89 e5                	mov    %esp,%ebp
80103f0c:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103f0f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103f16:	8b 45 0c             	mov    0xc(%ebp),%eax
80103f19:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103f1f:	8b 45 0c             	mov    0xc(%ebp),%eax
80103f22:	8b 10                	mov    (%eax),%edx
80103f24:	8b 45 08             	mov    0x8(%ebp),%eax
80103f27:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103f29:	e8 f8 cf ff ff       	call   80100f26 <filealloc>
80103f2e:	8b 55 08             	mov    0x8(%ebp),%edx
80103f31:	89 02                	mov    %eax,(%edx)
80103f33:	8b 45 08             	mov    0x8(%ebp),%eax
80103f36:	8b 00                	mov    (%eax),%eax
80103f38:	85 c0                	test   %eax,%eax
80103f3a:	0f 84 c8 00 00 00    	je     80104008 <pipealloc+0xff>
80103f40:	e8 e1 cf ff ff       	call   80100f26 <filealloc>
80103f45:	8b 55 0c             	mov    0xc(%ebp),%edx
80103f48:	89 02                	mov    %eax,(%edx)
80103f4a:	8b 45 0c             	mov    0xc(%ebp),%eax
80103f4d:	8b 00                	mov    (%eax),%eax
80103f4f:	85 c0                	test   %eax,%eax
80103f51:	0f 84 b1 00 00 00    	je     80104008 <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103f57:	e8 86 eb ff ff       	call   80102ae2 <kalloc>
80103f5c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103f5f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103f63:	75 05                	jne    80103f6a <pipealloc+0x61>
    goto bad;
80103f65:	e9 9e 00 00 00       	jmp    80104008 <pipealloc+0xff>
  p->readopen = 1;
80103f6a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f6d:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103f74:	00 00 00 
  p->writeopen = 1;
80103f77:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f7a:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103f81:	00 00 00 
  p->nwrite = 0;
80103f84:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f87:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103f8e:	00 00 00 
  p->nread = 0;
80103f91:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f94:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103f9b:	00 00 00 
  initlock(&p->lock, "pipe");
80103f9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103fa1:	c7 44 24 04 04 8c 10 	movl   $0x80108c04,0x4(%esp)
80103fa8:	80 
80103fa9:	89 04 24             	mov    %eax,(%esp)
80103fac:	e8 a1 0f 00 00       	call   80104f52 <initlock>
  (*f0)->type = FD_PIPE;
80103fb1:	8b 45 08             	mov    0x8(%ebp),%eax
80103fb4:	8b 00                	mov    (%eax),%eax
80103fb6:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103fbc:	8b 45 08             	mov    0x8(%ebp),%eax
80103fbf:	8b 00                	mov    (%eax),%eax
80103fc1:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103fc5:	8b 45 08             	mov    0x8(%ebp),%eax
80103fc8:	8b 00                	mov    (%eax),%eax
80103fca:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103fce:	8b 45 08             	mov    0x8(%ebp),%eax
80103fd1:	8b 00                	mov    (%eax),%eax
80103fd3:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103fd6:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103fd9:	8b 45 0c             	mov    0xc(%ebp),%eax
80103fdc:	8b 00                	mov    (%eax),%eax
80103fde:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103fe4:	8b 45 0c             	mov    0xc(%ebp),%eax
80103fe7:	8b 00                	mov    (%eax),%eax
80103fe9:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103fed:	8b 45 0c             	mov    0xc(%ebp),%eax
80103ff0:	8b 00                	mov    (%eax),%eax
80103ff2:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103ff6:	8b 45 0c             	mov    0xc(%ebp),%eax
80103ff9:	8b 00                	mov    (%eax),%eax
80103ffb:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103ffe:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80104001:	b8 00 00 00 00       	mov    $0x0,%eax
80104006:	eb 42                	jmp    8010404a <pipealloc+0x141>

//PAGEBREAK: 20
 bad:
  if(p)
80104008:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010400c:	74 0b                	je     80104019 <pipealloc+0x110>
    kfree((char*)p);
8010400e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104011:	89 04 24             	mov    %eax,(%esp)
80104014:	e8 30 ea ff ff       	call   80102a49 <kfree>
  if(*f0)
80104019:	8b 45 08             	mov    0x8(%ebp),%eax
8010401c:	8b 00                	mov    (%eax),%eax
8010401e:	85 c0                	test   %eax,%eax
80104020:	74 0d                	je     8010402f <pipealloc+0x126>
    fileclose(*f0);
80104022:	8b 45 08             	mov    0x8(%ebp),%eax
80104025:	8b 00                	mov    (%eax),%eax
80104027:	89 04 24             	mov    %eax,(%esp)
8010402a:	e8 9f cf ff ff       	call   80100fce <fileclose>
  if(*f1)
8010402f:	8b 45 0c             	mov    0xc(%ebp),%eax
80104032:	8b 00                	mov    (%eax),%eax
80104034:	85 c0                	test   %eax,%eax
80104036:	74 0d                	je     80104045 <pipealloc+0x13c>
    fileclose(*f1);
80104038:	8b 45 0c             	mov    0xc(%ebp),%eax
8010403b:	8b 00                	mov    (%eax),%eax
8010403d:	89 04 24             	mov    %eax,(%esp)
80104040:	e8 89 cf ff ff       	call   80100fce <fileclose>
  return -1;
80104045:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010404a:	c9                   	leave  
8010404b:	c3                   	ret    

8010404c <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
8010404c:	55                   	push   %ebp
8010404d:	89 e5                	mov    %esp,%ebp
8010404f:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80104052:	8b 45 08             	mov    0x8(%ebp),%eax
80104055:	89 04 24             	mov    %eax,(%esp)
80104058:	e8 16 0f 00 00       	call   80104f73 <acquire>
  if(writable){
8010405d:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104061:	74 1f                	je     80104082 <pipeclose+0x36>
    p->writeopen = 0;
80104063:	8b 45 08             	mov    0x8(%ebp),%eax
80104066:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
8010406d:	00 00 00 
    wakeup(&p->nread);
80104070:	8b 45 08             	mov    0x8(%ebp),%eax
80104073:	05 34 02 00 00       	add    $0x234,%eax
80104078:	89 04 24             	mov    %eax,(%esp)
8010407b:	e8 b4 0b 00 00       	call   80104c34 <wakeup>
80104080:	eb 1d                	jmp    8010409f <pipeclose+0x53>
  } else {
    p->readopen = 0;
80104082:	8b 45 08             	mov    0x8(%ebp),%eax
80104085:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
8010408c:	00 00 00 
    wakeup(&p->nwrite);
8010408f:	8b 45 08             	mov    0x8(%ebp),%eax
80104092:	05 38 02 00 00       	add    $0x238,%eax
80104097:	89 04 24             	mov    %eax,(%esp)
8010409a:	e8 95 0b 00 00       	call   80104c34 <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
8010409f:	8b 45 08             	mov    0x8(%ebp),%eax
801040a2:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
801040a8:	85 c0                	test   %eax,%eax
801040aa:	75 25                	jne    801040d1 <pipeclose+0x85>
801040ac:	8b 45 08             	mov    0x8(%ebp),%eax
801040af:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
801040b5:	85 c0                	test   %eax,%eax
801040b7:	75 18                	jne    801040d1 <pipeclose+0x85>
    release(&p->lock);
801040b9:	8b 45 08             	mov    0x8(%ebp),%eax
801040bc:	89 04 24             	mov    %eax,(%esp)
801040bf:	e8 11 0f 00 00       	call   80104fd5 <release>
    kfree((char*)p);
801040c4:	8b 45 08             	mov    0x8(%ebp),%eax
801040c7:	89 04 24             	mov    %eax,(%esp)
801040ca:	e8 7a e9 ff ff       	call   80102a49 <kfree>
801040cf:	eb 0b                	jmp    801040dc <pipeclose+0x90>
  } else
    release(&p->lock);
801040d1:	8b 45 08             	mov    0x8(%ebp),%eax
801040d4:	89 04 24             	mov    %eax,(%esp)
801040d7:	e8 f9 0e 00 00       	call   80104fd5 <release>
}
801040dc:	c9                   	leave  
801040dd:	c3                   	ret    

801040de <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
801040de:	55                   	push   %ebp
801040df:	89 e5                	mov    %esp,%ebp
801040e1:	83 ec 28             	sub    $0x28,%esp
  int i;

  acquire(&p->lock);
801040e4:	8b 45 08             	mov    0x8(%ebp),%eax
801040e7:	89 04 24             	mov    %eax,(%esp)
801040ea:	e8 84 0e 00 00       	call   80104f73 <acquire>
  for(i = 0; i < n; i++){
801040ef:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801040f6:	e9 a6 00 00 00       	jmp    801041a1 <pipewrite+0xc3>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
801040fb:	eb 57                	jmp    80104154 <pipewrite+0x76>
      if(p->readopen == 0 || proc->killed){
801040fd:	8b 45 08             	mov    0x8(%ebp),%eax
80104100:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80104106:	85 c0                	test   %eax,%eax
80104108:	74 0d                	je     80104117 <pipewrite+0x39>
8010410a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104110:	8b 40 24             	mov    0x24(%eax),%eax
80104113:	85 c0                	test   %eax,%eax
80104115:	74 15                	je     8010412c <pipewrite+0x4e>
        release(&p->lock);
80104117:	8b 45 08             	mov    0x8(%ebp),%eax
8010411a:	89 04 24             	mov    %eax,(%esp)
8010411d:	e8 b3 0e 00 00       	call   80104fd5 <release>
        return -1;
80104122:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104127:	e9 9f 00 00 00       	jmp    801041cb <pipewrite+0xed>
      }
      wakeup(&p->nread);
8010412c:	8b 45 08             	mov    0x8(%ebp),%eax
8010412f:	05 34 02 00 00       	add    $0x234,%eax
80104134:	89 04 24             	mov    %eax,(%esp)
80104137:	e8 f8 0a 00 00       	call   80104c34 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
8010413c:	8b 45 08             	mov    0x8(%ebp),%eax
8010413f:	8b 55 08             	mov    0x8(%ebp),%edx
80104142:	81 c2 38 02 00 00    	add    $0x238,%edx
80104148:	89 44 24 04          	mov    %eax,0x4(%esp)
8010414c:	89 14 24             	mov    %edx,(%esp)
8010414f:	e8 04 0a 00 00       	call   80104b58 <sleep>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80104154:	8b 45 08             	mov    0x8(%ebp),%eax
80104157:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
8010415d:	8b 45 08             	mov    0x8(%ebp),%eax
80104160:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80104166:	05 00 02 00 00       	add    $0x200,%eax
8010416b:	39 c2                	cmp    %eax,%edx
8010416d:	74 8e                	je     801040fd <pipewrite+0x1f>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
8010416f:	8b 45 08             	mov    0x8(%ebp),%eax
80104172:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80104178:	8d 48 01             	lea    0x1(%eax),%ecx
8010417b:	8b 55 08             	mov    0x8(%ebp),%edx
8010417e:	89 8a 38 02 00 00    	mov    %ecx,0x238(%edx)
80104184:	25 ff 01 00 00       	and    $0x1ff,%eax
80104189:	89 c1                	mov    %eax,%ecx
8010418b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010418e:	8b 45 0c             	mov    0xc(%ebp),%eax
80104191:	01 d0                	add    %edx,%eax
80104193:	0f b6 10             	movzbl (%eax),%edx
80104196:	8b 45 08             	mov    0x8(%ebp),%eax
80104199:	88 54 08 34          	mov    %dl,0x34(%eax,%ecx,1)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
8010419d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801041a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041a4:	3b 45 10             	cmp    0x10(%ebp),%eax
801041a7:	0f 8c 4e ff ff ff    	jl     801040fb <pipewrite+0x1d>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
801041ad:	8b 45 08             	mov    0x8(%ebp),%eax
801041b0:	05 34 02 00 00       	add    $0x234,%eax
801041b5:	89 04 24             	mov    %eax,(%esp)
801041b8:	e8 77 0a 00 00       	call   80104c34 <wakeup>
  release(&p->lock);
801041bd:	8b 45 08             	mov    0x8(%ebp),%eax
801041c0:	89 04 24             	mov    %eax,(%esp)
801041c3:	e8 0d 0e 00 00       	call   80104fd5 <release>
  return n;
801041c8:	8b 45 10             	mov    0x10(%ebp),%eax
}
801041cb:	c9                   	leave  
801041cc:	c3                   	ret    

801041cd <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
801041cd:	55                   	push   %ebp
801041ce:	89 e5                	mov    %esp,%ebp
801041d0:	53                   	push   %ebx
801041d1:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
801041d4:	8b 45 08             	mov    0x8(%ebp),%eax
801041d7:	89 04 24             	mov    %eax,(%esp)
801041da:	e8 94 0d 00 00       	call   80104f73 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
801041df:	eb 3a                	jmp    8010421b <piperead+0x4e>
    if(proc->killed){
801041e1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801041e7:	8b 40 24             	mov    0x24(%eax),%eax
801041ea:	85 c0                	test   %eax,%eax
801041ec:	74 15                	je     80104203 <piperead+0x36>
      release(&p->lock);
801041ee:	8b 45 08             	mov    0x8(%ebp),%eax
801041f1:	89 04 24             	mov    %eax,(%esp)
801041f4:	e8 dc 0d 00 00       	call   80104fd5 <release>
      return -1;
801041f9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801041fe:	e9 b5 00 00 00       	jmp    801042b8 <piperead+0xeb>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80104203:	8b 45 08             	mov    0x8(%ebp),%eax
80104206:	8b 55 08             	mov    0x8(%ebp),%edx
80104209:	81 c2 34 02 00 00    	add    $0x234,%edx
8010420f:	89 44 24 04          	mov    %eax,0x4(%esp)
80104213:	89 14 24             	mov    %edx,(%esp)
80104216:	e8 3d 09 00 00       	call   80104b58 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
8010421b:	8b 45 08             	mov    0x8(%ebp),%eax
8010421e:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80104224:	8b 45 08             	mov    0x8(%ebp),%eax
80104227:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
8010422d:	39 c2                	cmp    %eax,%edx
8010422f:	75 0d                	jne    8010423e <piperead+0x71>
80104231:	8b 45 08             	mov    0x8(%ebp),%eax
80104234:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
8010423a:	85 c0                	test   %eax,%eax
8010423c:	75 a3                	jne    801041e1 <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
8010423e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104245:	eb 4b                	jmp    80104292 <piperead+0xc5>
    if(p->nread == p->nwrite)
80104247:	8b 45 08             	mov    0x8(%ebp),%eax
8010424a:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80104250:	8b 45 08             	mov    0x8(%ebp),%eax
80104253:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80104259:	39 c2                	cmp    %eax,%edx
8010425b:	75 02                	jne    8010425f <piperead+0x92>
      break;
8010425d:	eb 3b                	jmp    8010429a <piperead+0xcd>
    addr[i] = p->data[p->nread++ % PIPESIZE];
8010425f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104262:	8b 45 0c             	mov    0xc(%ebp),%eax
80104265:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80104268:	8b 45 08             	mov    0x8(%ebp),%eax
8010426b:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80104271:	8d 48 01             	lea    0x1(%eax),%ecx
80104274:	8b 55 08             	mov    0x8(%ebp),%edx
80104277:	89 8a 34 02 00 00    	mov    %ecx,0x234(%edx)
8010427d:	25 ff 01 00 00       	and    $0x1ff,%eax
80104282:	89 c2                	mov    %eax,%edx
80104284:	8b 45 08             	mov    0x8(%ebp),%eax
80104287:	0f b6 44 10 34       	movzbl 0x34(%eax,%edx,1),%eax
8010428c:	88 03                	mov    %al,(%ebx)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
8010428e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104292:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104295:	3b 45 10             	cmp    0x10(%ebp),%eax
80104298:	7c ad                	jl     80104247 <piperead+0x7a>
    if(p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
8010429a:	8b 45 08             	mov    0x8(%ebp),%eax
8010429d:	05 38 02 00 00       	add    $0x238,%eax
801042a2:	89 04 24             	mov    %eax,(%esp)
801042a5:	e8 8a 09 00 00       	call   80104c34 <wakeup>
  release(&p->lock);
801042aa:	8b 45 08             	mov    0x8(%ebp),%eax
801042ad:	89 04 24             	mov    %eax,(%esp)
801042b0:	e8 20 0d 00 00       	call   80104fd5 <release>
  return i;
801042b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801042b8:	83 c4 24             	add    $0x24,%esp
801042bb:	5b                   	pop    %ebx
801042bc:	5d                   	pop    %ebp
801042bd:	c3                   	ret    

801042be <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
801042be:	55                   	push   %ebp
801042bf:	89 e5                	mov    %esp,%ebp
801042c1:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
801042c4:	9c                   	pushf  
801042c5:	58                   	pop    %eax
801042c6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
801042c9:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801042cc:	c9                   	leave  
801042cd:	c3                   	ret    

801042ce <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
801042ce:	55                   	push   %ebp
801042cf:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
801042d1:	fb                   	sti    
}
801042d2:	5d                   	pop    %ebp
801042d3:	c3                   	ret    

801042d4 <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
801042d4:	55                   	push   %ebp
801042d5:	89 e5                	mov    %esp,%ebp
801042d7:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
801042da:	c7 44 24 04 09 8c 10 	movl   $0x80108c09,0x4(%esp)
801042e1:	80 
801042e2:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
801042e9:	e8 64 0c 00 00       	call   80104f52 <initlock>
}
801042ee:	c9                   	leave  
801042ef:	c3                   	ret    

801042f0 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
801042f0:	55                   	push   %ebp
801042f1:	89 e5                	mov    %esp,%ebp
801042f3:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
801042f6:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
801042fd:	e8 71 0c 00 00       	call   80104f73 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104302:	c7 45 f4 b4 29 11 80 	movl   $0x801129b4,-0xc(%ebp)
80104309:	eb 53                	jmp    8010435e <allocproc+0x6e>
    if(p->state == UNUSED)
8010430b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010430e:	8b 40 0c             	mov    0xc(%eax),%eax
80104311:	85 c0                	test   %eax,%eax
80104313:	75 42                	jne    80104357 <allocproc+0x67>
      goto found;
80104315:	90                   	nop
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
80104316:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104319:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
80104320:	a1 04 b0 10 80       	mov    0x8010b004,%eax
80104325:	8d 50 01             	lea    0x1(%eax),%edx
80104328:	89 15 04 b0 10 80    	mov    %edx,0x8010b004
8010432e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104331:	89 42 10             	mov    %eax,0x10(%edx)
  release(&ptable.lock);
80104334:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
8010433b:	e8 95 0c 00 00       	call   80104fd5 <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
80104340:	e8 9d e7 ff ff       	call   80102ae2 <kalloc>
80104345:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104348:	89 42 08             	mov    %eax,0x8(%edx)
8010434b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010434e:	8b 40 08             	mov    0x8(%eax),%eax
80104351:	85 c0                	test   %eax,%eax
80104353:	75 3c                	jne    80104391 <allocproc+0xa1>
80104355:	eb 26                	jmp    8010437d <allocproc+0x8d>
{
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104357:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
8010435e:	81 7d f4 b4 4d 11 80 	cmpl   $0x80114db4,-0xc(%ebp)
80104365:	72 a4                	jb     8010430b <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;
  release(&ptable.lock);
80104367:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
8010436e:	e8 62 0c 00 00       	call   80104fd5 <release>
  return 0;
80104373:	b8 00 00 00 00       	mov    $0x0,%eax
80104378:	e9 b7 00 00 00       	jmp    80104434 <allocproc+0x144>
  p->pid = nextpid++;
  release(&ptable.lock);

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
    p->state = UNUSED;
8010437d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104380:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
80104387:	b8 00 00 00 00       	mov    $0x0,%eax
8010438c:	e9 a3 00 00 00       	jmp    80104434 <allocproc+0x144>
  }
  sp = p->kstack + KSTACKSIZE;
80104391:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104394:	8b 40 08             	mov    0x8(%eax),%eax
80104397:	05 00 10 00 00       	add    $0x1000,%eax
8010439c:	89 45 f0             	mov    %eax,-0x10(%ebp)

  //
  p->sighandle[0]=(sighandler_t)-1;
8010439f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043a2:	c7 40 7c ff ff ff ff 	movl   $0xffffffff,0x7c(%eax)
  p->sighandle[1]=(sighandler_t)-1;  
801043a9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043ac:	c7 80 80 00 00 00 ff 	movl   $0xffffffff,0x80(%eax)
801043b3:	ff ff ff 
  p->sighandle[2]=(sighandler_t)-1;  
801043b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043b9:	c7 80 84 00 00 00 ff 	movl   $0xffffffff,0x84(%eax)
801043c0:	ff ff ff 
  p->ticks=-1;
801043c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043c6:	c7 80 88 00 00 00 ff 	movl   $0xffffffff,0x88(%eax)
801043cd:	ff ff ff 
  p->flag=0;
801043d0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043d3:	c7 80 8c 00 00 00 00 	movl   $0x0,0x8c(%eax)
801043da:	00 00 00 
  // Leave room for trap frame.
  sp -= sizeof *p->tf;
801043dd:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
801043e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043e4:	8b 55 f0             	mov    -0x10(%ebp),%edx
801043e7:	89 50 18             	mov    %edx,0x18(%eax)
  
  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
801043ea:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
801043ee:	ba 3d 68 10 80       	mov    $0x8010683d,%edx
801043f3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801043f6:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
801043f8:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
801043fc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043ff:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104402:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
80104405:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104408:	8b 40 1c             	mov    0x1c(%eax),%eax
8010440b:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80104412:	00 
80104413:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010441a:	00 
8010441b:	89 04 24             	mov    %eax,(%esp)
8010441e:	e8 a4 0d 00 00       	call   801051c7 <memset>
  p->context->eip = (uint)forkret;
80104423:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104426:	8b 40 1c             	mov    0x1c(%eax),%eax
80104429:	ba 2c 4b 10 80       	mov    $0x80104b2c,%edx
8010442e:	89 50 10             	mov    %edx,0x10(%eax)

  return p;
80104431:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104434:	c9                   	leave  
80104435:	c3                   	ret    

80104436 <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
80104436:	55                   	push   %ebp
80104437:	89 e5                	mov    %esp,%ebp
80104439:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];
  
  p = allocproc();
8010443c:	e8 af fe ff ff       	call   801042f0 <allocproc>
80104441:	89 45 f4             	mov    %eax,-0xc(%ebp)
  initproc = p;
80104444:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104447:	a3 68 b6 10 80       	mov    %eax,0x8010b668
  if((p->pgdir = setupkvm()) == 0)
8010444c:	e8 76 3c 00 00       	call   801080c7 <setupkvm>
80104451:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104454:	89 42 04             	mov    %eax,0x4(%edx)
80104457:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010445a:	8b 40 04             	mov    0x4(%eax),%eax
8010445d:	85 c0                	test   %eax,%eax
8010445f:	75 0c                	jne    8010446d <userinit+0x37>
    panic("userinit: out of memory?");
80104461:	c7 04 24 10 8c 10 80 	movl   $0x80108c10,(%esp)
80104468:	e8 cd c0 ff ff       	call   8010053a <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
8010446d:	ba 2c 00 00 00       	mov    $0x2c,%edx
80104472:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104475:	8b 40 04             	mov    0x4(%eax),%eax
80104478:	89 54 24 08          	mov    %edx,0x8(%esp)
8010447c:	c7 44 24 04 00 b5 10 	movl   $0x8010b500,0x4(%esp)
80104483:	80 
80104484:	89 04 24             	mov    %eax,(%esp)
80104487:	e8 93 3e 00 00       	call   8010831f <inituvm>
  p->sz = PGSIZE;
8010448c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010448f:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
80104495:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104498:	8b 40 18             	mov    0x18(%eax),%eax
8010449b:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
801044a2:	00 
801044a3:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801044aa:	00 
801044ab:	89 04 24             	mov    %eax,(%esp)
801044ae:	e8 14 0d 00 00       	call   801051c7 <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
801044b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044b6:	8b 40 18             	mov    0x18(%eax),%eax
801044b9:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
801044bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044c2:	8b 40 18             	mov    0x18(%eax),%eax
801044c5:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
  p->tf->es = p->tf->ds;
801044cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044ce:	8b 40 18             	mov    0x18(%eax),%eax
801044d1:	8b 55 f4             	mov    -0xc(%ebp),%edx
801044d4:	8b 52 18             	mov    0x18(%edx),%edx
801044d7:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801044db:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
801044df:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044e2:	8b 40 18             	mov    0x18(%eax),%eax
801044e5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801044e8:	8b 52 18             	mov    0x18(%edx),%edx
801044eb:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801044ef:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
801044f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044f6:	8b 40 18             	mov    0x18(%eax),%eax
801044f9:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
80104500:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104503:	8b 40 18             	mov    0x18(%eax),%eax
80104506:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
8010450d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104510:	8b 40 18             	mov    0x18(%eax),%eax
80104513:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
8010451a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010451d:	83 c0 6c             	add    $0x6c,%eax
80104520:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104527:	00 
80104528:	c7 44 24 04 29 8c 10 	movl   $0x80108c29,0x4(%esp)
8010452f:	80 
80104530:	89 04 24             	mov    %eax,(%esp)
80104533:	e8 af 0e 00 00       	call   801053e7 <safestrcpy>
  p->cwd = namei("/");
80104538:	c7 04 24 32 8c 10 80 	movl   $0x80108c32,(%esp)
8010453f:	e8 c2 de ff ff       	call   80102406 <namei>
80104544:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104547:	89 42 68             	mov    %eax,0x68(%edx)

  p->state = RUNNABLE;
8010454a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010454d:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
}
80104554:	c9                   	leave  
80104555:	c3                   	ret    

80104556 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
80104556:	55                   	push   %ebp
80104557:	89 e5                	mov    %esp,%ebp
80104559:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  
  sz = proc->sz;
8010455c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104562:	8b 00                	mov    (%eax),%eax
80104564:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
80104567:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010456b:	7e 34                	jle    801045a1 <growproc+0x4b>
    if((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
8010456d:	8b 55 08             	mov    0x8(%ebp),%edx
80104570:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104573:	01 c2                	add    %eax,%edx
80104575:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010457b:	8b 40 04             	mov    0x4(%eax),%eax
8010457e:	89 54 24 08          	mov    %edx,0x8(%esp)
80104582:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104585:	89 54 24 04          	mov    %edx,0x4(%esp)
80104589:	89 04 24             	mov    %eax,(%esp)
8010458c:	e8 04 3f 00 00       	call   80108495 <allocuvm>
80104591:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104594:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104598:	75 41                	jne    801045db <growproc+0x85>
      return -1;
8010459a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010459f:	eb 58                	jmp    801045f9 <growproc+0xa3>
  } else if(n < 0){
801045a1:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801045a5:	79 34                	jns    801045db <growproc+0x85>
    if((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
801045a7:	8b 55 08             	mov    0x8(%ebp),%edx
801045aa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045ad:	01 c2                	add    %eax,%edx
801045af:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045b5:	8b 40 04             	mov    0x4(%eax),%eax
801045b8:	89 54 24 08          	mov    %edx,0x8(%esp)
801045bc:	8b 55 f4             	mov    -0xc(%ebp),%edx
801045bf:	89 54 24 04          	mov    %edx,0x4(%esp)
801045c3:	89 04 24             	mov    %eax,(%esp)
801045c6:	e8 a4 3f 00 00       	call   8010856f <deallocuvm>
801045cb:	89 45 f4             	mov    %eax,-0xc(%ebp)
801045ce:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801045d2:	75 07                	jne    801045db <growproc+0x85>
      return -1;
801045d4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801045d9:	eb 1e                	jmp    801045f9 <growproc+0xa3>
  }
  proc->sz = sz;
801045db:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045e1:	8b 55 f4             	mov    -0xc(%ebp),%edx
801045e4:	89 10                	mov    %edx,(%eax)
  switchuvm(proc);
801045e6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045ec:	89 04 24             	mov    %eax,(%esp)
801045ef:	e8 c4 3b 00 00       	call   801081b8 <switchuvm>
  return 0;
801045f4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801045f9:	c9                   	leave  
801045fa:	c3                   	ret    

801045fb <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(void)
{
801045fb:	55                   	push   %ebp
801045fc:	89 e5                	mov    %esp,%ebp
801045fe:	57                   	push   %edi
801045ff:	56                   	push   %esi
80104600:	53                   	push   %ebx
80104601:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
80104604:	e8 e7 fc ff ff       	call   801042f0 <allocproc>
80104609:	89 45 e0             	mov    %eax,-0x20(%ebp)
8010460c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80104610:	75 0a                	jne    8010461c <fork+0x21>
    return -1;
80104612:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104617:	e9 52 01 00 00       	jmp    8010476e <fork+0x173>

  // Copy process state from p.
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
8010461c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104622:	8b 10                	mov    (%eax),%edx
80104624:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010462a:	8b 40 04             	mov    0x4(%eax),%eax
8010462d:	89 54 24 04          	mov    %edx,0x4(%esp)
80104631:	89 04 24             	mov    %eax,(%esp)
80104634:	e8 d2 40 00 00       	call   8010870b <copyuvm>
80104639:	8b 55 e0             	mov    -0x20(%ebp),%edx
8010463c:	89 42 04             	mov    %eax,0x4(%edx)
8010463f:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104642:	8b 40 04             	mov    0x4(%eax),%eax
80104645:	85 c0                	test   %eax,%eax
80104647:	75 2c                	jne    80104675 <fork+0x7a>
    kfree(np->kstack);
80104649:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010464c:	8b 40 08             	mov    0x8(%eax),%eax
8010464f:	89 04 24             	mov    %eax,(%esp)
80104652:	e8 f2 e3 ff ff       	call   80102a49 <kfree>
    np->kstack = 0;
80104657:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010465a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
    np->state = UNUSED;
80104661:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104664:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return -1;
8010466b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104670:	e9 f9 00 00 00       	jmp    8010476e <fork+0x173>
  }
  np->sz = proc->sz;
80104675:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010467b:	8b 10                	mov    (%eax),%edx
8010467d:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104680:	89 10                	mov    %edx,(%eax)
  np->parent = proc;
80104682:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104689:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010468c:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *proc->tf;
8010468f:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104692:	8b 50 18             	mov    0x18(%eax),%edx
80104695:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010469b:	8b 40 18             	mov    0x18(%eax),%eax
8010469e:	89 c3                	mov    %eax,%ebx
801046a0:	b8 13 00 00 00       	mov    $0x13,%eax
801046a5:	89 d7                	mov    %edx,%edi
801046a7:	89 de                	mov    %ebx,%esi
801046a9:	89 c1                	mov    %eax,%ecx
801046ab:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
801046ad:	8b 45 e0             	mov    -0x20(%ebp),%eax
801046b0:	8b 40 18             	mov    0x18(%eax),%eax
801046b3:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
801046ba:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801046c1:	eb 3d                	jmp    80104700 <fork+0x105>
    if(proc->ofile[i])
801046c3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046c9:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801046cc:	83 c2 08             	add    $0x8,%edx
801046cf:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801046d3:	85 c0                	test   %eax,%eax
801046d5:	74 25                	je     801046fc <fork+0x101>
      np->ofile[i] = filedup(proc->ofile[i]);
801046d7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046dd:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801046e0:	83 c2 08             	add    $0x8,%edx
801046e3:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801046e7:	89 04 24             	mov    %eax,(%esp)
801046ea:	e8 97 c8 ff ff       	call   80100f86 <filedup>
801046ef:	8b 55 e0             	mov    -0x20(%ebp),%edx
801046f2:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
801046f5:	83 c1 08             	add    $0x8,%ecx
801046f8:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
  *np->tf = *proc->tf;

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
801046fc:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80104700:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
80104704:	7e bd                	jle    801046c3 <fork+0xc8>
    if(proc->ofile[i])
      np->ofile[i] = filedup(proc->ofile[i]);
  np->cwd = idup(proc->cwd);
80104706:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010470c:	8b 40 68             	mov    0x68(%eax),%eax
8010470f:	89 04 24             	mov    %eax,(%esp)
80104712:	e8 12 d1 ff ff       	call   80101829 <idup>
80104717:	8b 55 e0             	mov    -0x20(%ebp),%edx
8010471a:	89 42 68             	mov    %eax,0x68(%edx)

  safestrcpy(np->name, proc->name, sizeof(proc->name));
8010471d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104723:	8d 50 6c             	lea    0x6c(%eax),%edx
80104726:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104729:	83 c0 6c             	add    $0x6c,%eax
8010472c:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104733:	00 
80104734:	89 54 24 04          	mov    %edx,0x4(%esp)
80104738:	89 04 24             	mov    %eax,(%esp)
8010473b:	e8 a7 0c 00 00       	call   801053e7 <safestrcpy>
 
  pid = np->pid;
80104740:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104743:	8b 40 10             	mov    0x10(%eax),%eax
80104746:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // lock to force the compiler to emit the np->state write last.
  acquire(&ptable.lock);
80104749:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104750:	e8 1e 08 00 00       	call   80104f73 <acquire>
  np->state = RUNNABLE;
80104755:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104758:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  release(&ptable.lock);
8010475f:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104766:	e8 6a 08 00 00       	call   80104fd5 <release>
  
  return pid;
8010476b:	8b 45 dc             	mov    -0x24(%ebp),%eax
}
8010476e:	83 c4 2c             	add    $0x2c,%esp
80104771:	5b                   	pop    %ebx
80104772:	5e                   	pop    %esi
80104773:	5f                   	pop    %edi
80104774:	5d                   	pop    %ebp
80104775:	c3                   	ret    

80104776 <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
80104776:	55                   	push   %ebp
80104777:	89 e5                	mov    %esp,%ebp
80104779:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int fd;

  if(proc == initproc)
8010477c:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104783:	a1 68 b6 10 80       	mov    0x8010b668,%eax
80104788:	39 c2                	cmp    %eax,%edx
8010478a:	75 0c                	jne    80104798 <exit+0x22>
    panic("init exiting");
8010478c:	c7 04 24 34 8c 10 80 	movl   $0x80108c34,(%esp)
80104793:	e8 a2 bd ff ff       	call   8010053a <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
80104798:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
8010479f:	eb 44                	jmp    801047e5 <exit+0x6f>
    if(proc->ofile[fd]){
801047a1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047a7:	8b 55 f0             	mov    -0x10(%ebp),%edx
801047aa:	83 c2 08             	add    $0x8,%edx
801047ad:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801047b1:	85 c0                	test   %eax,%eax
801047b3:	74 2c                	je     801047e1 <exit+0x6b>
      fileclose(proc->ofile[fd]);
801047b5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047bb:	8b 55 f0             	mov    -0x10(%ebp),%edx
801047be:	83 c2 08             	add    $0x8,%edx
801047c1:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801047c5:	89 04 24             	mov    %eax,(%esp)
801047c8:	e8 01 c8 ff ff       	call   80100fce <fileclose>
      proc->ofile[fd] = 0;
801047cd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047d3:	8b 55 f0             	mov    -0x10(%ebp),%edx
801047d6:	83 c2 08             	add    $0x8,%edx
801047d9:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801047e0:	00 

  if(proc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
801047e1:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
801047e5:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
801047e9:	7e b6                	jle    801047a1 <exit+0x2b>
      fileclose(proc->ofile[fd]);
      proc->ofile[fd] = 0;
    }
  }

  begin_op();
801047eb:	e8 20 ec ff ff       	call   80103410 <begin_op>
  iput(proc->cwd);
801047f0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047f6:	8b 40 68             	mov    0x68(%eax),%eax
801047f9:	89 04 24             	mov    %eax,(%esp)
801047fc:	e8 0d d2 ff ff       	call   80101a0e <iput>
  end_op();
80104801:	e8 8e ec ff ff       	call   80103494 <end_op>
  proc->cwd = 0;
80104806:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010480c:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

  acquire(&ptable.lock);
80104813:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
8010481a:	e8 54 07 00 00       	call   80104f73 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);
8010481f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104825:	8b 40 14             	mov    0x14(%eax),%eax
80104828:	89 04 24             	mov    %eax,(%esp)
8010482b:	e8 c3 03 00 00       	call   80104bf3 <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104830:	c7 45 f4 b4 29 11 80 	movl   $0x801129b4,-0xc(%ebp)
80104837:	eb 3b                	jmp    80104874 <exit+0xfe>
    if(p->parent == proc){
80104839:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010483c:	8b 50 14             	mov    0x14(%eax),%edx
8010483f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104845:	39 c2                	cmp    %eax,%edx
80104847:	75 24                	jne    8010486d <exit+0xf7>
      p->parent = initproc;
80104849:	8b 15 68 b6 10 80    	mov    0x8010b668,%edx
8010484f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104852:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
80104855:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104858:	8b 40 0c             	mov    0xc(%eax),%eax
8010485b:	83 f8 05             	cmp    $0x5,%eax
8010485e:	75 0d                	jne    8010486d <exit+0xf7>
        wakeup1(initproc);
80104860:	a1 68 b6 10 80       	mov    0x8010b668,%eax
80104865:	89 04 24             	mov    %eax,(%esp)
80104868:	e8 86 03 00 00       	call   80104bf3 <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010486d:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80104874:	81 7d f4 b4 4d 11 80 	cmpl   $0x80114db4,-0xc(%ebp)
8010487b:	72 bc                	jb     80104839 <exit+0xc3>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  proc->state = ZOMBIE;
8010487d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104883:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
8010488a:	e8 b9 01 00 00       	call   80104a48 <sched>
  panic("zombie exit");
8010488f:	c7 04 24 41 8c 10 80 	movl   $0x80108c41,(%esp)
80104896:	e8 9f bc ff ff       	call   8010053a <panic>

8010489b <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(void)
{
8010489b:	55                   	push   %ebp
8010489c:	89 e5                	mov    %esp,%ebp
8010489e:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;

  acquire(&ptable.lock);
801048a1:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
801048a8:	e8 c6 06 00 00       	call   80104f73 <acquire>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
801048ad:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801048b4:	c7 45 f4 b4 29 11 80 	movl   $0x801129b4,-0xc(%ebp)
801048bb:	e9 9d 00 00 00       	jmp    8010495d <wait+0xc2>
      if(p->parent != proc)
801048c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048c3:	8b 50 14             	mov    0x14(%eax),%edx
801048c6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048cc:	39 c2                	cmp    %eax,%edx
801048ce:	74 05                	je     801048d5 <wait+0x3a>
        continue;
801048d0:	e9 81 00 00 00       	jmp    80104956 <wait+0xbb>
      havekids = 1;
801048d5:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
      if(p->state == ZOMBIE){
801048dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048df:	8b 40 0c             	mov    0xc(%eax),%eax
801048e2:	83 f8 05             	cmp    $0x5,%eax
801048e5:	75 6f                	jne    80104956 <wait+0xbb>
        // Found one.
        pid = p->pid;
801048e7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048ea:	8b 40 10             	mov    0x10(%eax),%eax
801048ed:	89 45 ec             	mov    %eax,-0x14(%ebp)
        kfree(p->kstack);
801048f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048f3:	8b 40 08             	mov    0x8(%eax),%eax
801048f6:	89 04 24             	mov    %eax,(%esp)
801048f9:	e8 4b e1 ff ff       	call   80102a49 <kfree>
        p->kstack = 0;
801048fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104901:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
        freevm(p->pgdir);
80104908:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010490b:	8b 40 04             	mov    0x4(%eax),%eax
8010490e:	89 04 24             	mov    %eax,(%esp)
80104911:	e8 15 3d 00 00       	call   8010862b <freevm>
        p->state = UNUSED;
80104916:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104919:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        p->pid = 0;
80104920:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104923:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
8010492a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010492d:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
80104934:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104937:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
8010493b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010493e:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        release(&ptable.lock);
80104945:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
8010494c:	e8 84 06 00 00       	call   80104fd5 <release>
        return pid;
80104951:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104954:	eb 55                	jmp    801049ab <wait+0x110>

  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104956:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
8010495d:	81 7d f4 b4 4d 11 80 	cmpl   $0x80114db4,-0xc(%ebp)
80104964:	0f 82 56 ff ff ff    	jb     801048c0 <wait+0x25>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || proc->killed){
8010496a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010496e:	74 0d                	je     8010497d <wait+0xe2>
80104970:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104976:	8b 40 24             	mov    0x24(%eax),%eax
80104979:	85 c0                	test   %eax,%eax
8010497b:	74 13                	je     80104990 <wait+0xf5>
      release(&ptable.lock);
8010497d:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104984:	e8 4c 06 00 00       	call   80104fd5 <release>
      return -1;
80104989:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010498e:	eb 1b                	jmp    801049ab <wait+0x110>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(proc, &ptable.lock);  //DOC: wait-sleep
80104990:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104996:	c7 44 24 04 80 29 11 	movl   $0x80112980,0x4(%esp)
8010499d:	80 
8010499e:	89 04 24             	mov    %eax,(%esp)
801049a1:	e8 b2 01 00 00       	call   80104b58 <sleep>
  }
801049a6:	e9 02 ff ff ff       	jmp    801048ad <wait+0x12>
}
801049ab:	c9                   	leave  
801049ac:	c3                   	ret    

801049ad <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
801049ad:	55                   	push   %ebp
801049ae:	89 e5                	mov    %esp,%ebp
801049b0:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  for(;;){
    // Enable interrupts on this processor.
    sti();
801049b3:	e8 16 f9 ff ff       	call   801042ce <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
801049b8:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
801049bf:	e8 af 05 00 00       	call   80104f73 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801049c4:	c7 45 f4 b4 29 11 80 	movl   $0x801129b4,-0xc(%ebp)
801049cb:	eb 61                	jmp    80104a2e <scheduler+0x81>
      if(p->state != RUNNABLE)
801049cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049d0:	8b 40 0c             	mov    0xc(%eax),%eax
801049d3:	83 f8 03             	cmp    $0x3,%eax
801049d6:	74 02                	je     801049da <scheduler+0x2d>
        continue;
801049d8:	eb 4d                	jmp    80104a27 <scheduler+0x7a>

      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      proc = p;
801049da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049dd:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
      switchuvm(p);
801049e3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049e6:	89 04 24             	mov    %eax,(%esp)
801049e9:	e8 ca 37 00 00       	call   801081b8 <switchuvm>
      p->state = RUNNING;
801049ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049f1:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
      swtch(&cpu->scheduler, proc->context);
801049f8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049fe:	8b 40 1c             	mov    0x1c(%eax),%eax
80104a01:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104a08:	83 c2 04             	add    $0x4,%edx
80104a0b:	89 44 24 04          	mov    %eax,0x4(%esp)
80104a0f:	89 14 24             	mov    %edx,(%esp)
80104a12:	e8 41 0a 00 00       	call   80105458 <swtch>
      switchkvm();
80104a17:	e8 7f 37 00 00       	call   8010819b <switchkvm>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
80104a1c:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80104a23:	00 00 00 00 
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104a27:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80104a2e:	81 7d f4 b4 4d 11 80 	cmpl   $0x80114db4,-0xc(%ebp)
80104a35:	72 96                	jb     801049cd <scheduler+0x20>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
    }
    release(&ptable.lock);
80104a37:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104a3e:	e8 92 05 00 00       	call   80104fd5 <release>

  }
80104a43:	e9 6b ff ff ff       	jmp    801049b3 <scheduler+0x6>

80104a48 <sched>:

// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void
sched(void)
{
80104a48:	55                   	push   %ebp
80104a49:	89 e5                	mov    %esp,%ebp
80104a4b:	83 ec 28             	sub    $0x28,%esp
  int intena;

  if(!holding(&ptable.lock))
80104a4e:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104a55:	e8 43 06 00 00       	call   8010509d <holding>
80104a5a:	85 c0                	test   %eax,%eax
80104a5c:	75 0c                	jne    80104a6a <sched+0x22>
    panic("sched ptable.lock");
80104a5e:	c7 04 24 4d 8c 10 80 	movl   $0x80108c4d,(%esp)
80104a65:	e8 d0 ba ff ff       	call   8010053a <panic>
  if(cpu->ncli != 1)
80104a6a:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a70:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80104a76:	83 f8 01             	cmp    $0x1,%eax
80104a79:	74 0c                	je     80104a87 <sched+0x3f>
    panic("sched locks");
80104a7b:	c7 04 24 5f 8c 10 80 	movl   $0x80108c5f,(%esp)
80104a82:	e8 b3 ba ff ff       	call   8010053a <panic>
  if(proc->state == RUNNING)
80104a87:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104a8d:	8b 40 0c             	mov    0xc(%eax),%eax
80104a90:	83 f8 04             	cmp    $0x4,%eax
80104a93:	75 0c                	jne    80104aa1 <sched+0x59>
    panic("sched running");
80104a95:	c7 04 24 6b 8c 10 80 	movl   $0x80108c6b,(%esp)
80104a9c:	e8 99 ba ff ff       	call   8010053a <panic>
  if(readeflags()&FL_IF)
80104aa1:	e8 18 f8 ff ff       	call   801042be <readeflags>
80104aa6:	25 00 02 00 00       	and    $0x200,%eax
80104aab:	85 c0                	test   %eax,%eax
80104aad:	74 0c                	je     80104abb <sched+0x73>
    panic("sched interruptible");
80104aaf:	c7 04 24 79 8c 10 80 	movl   $0x80108c79,(%esp)
80104ab6:	e8 7f ba ff ff       	call   8010053a <panic>
  intena = cpu->intena;
80104abb:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104ac1:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104ac7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  swtch(&proc->context, cpu->scheduler);
80104aca:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104ad0:	8b 40 04             	mov    0x4(%eax),%eax
80104ad3:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104ada:	83 c2 1c             	add    $0x1c,%edx
80104add:	89 44 24 04          	mov    %eax,0x4(%esp)
80104ae1:	89 14 24             	mov    %edx,(%esp)
80104ae4:	e8 6f 09 00 00       	call   80105458 <swtch>
  cpu->intena = intena;
80104ae9:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104aef:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104af2:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80104af8:	c9                   	leave  
80104af9:	c3                   	ret    

80104afa <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
80104afa:	55                   	push   %ebp
80104afb:	89 e5                	mov    %esp,%ebp
80104afd:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
80104b00:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104b07:	e8 67 04 00 00       	call   80104f73 <acquire>
  proc->state = RUNNABLE;
80104b0c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b12:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
80104b19:	e8 2a ff ff ff       	call   80104a48 <sched>
  release(&ptable.lock);
80104b1e:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104b25:	e8 ab 04 00 00       	call   80104fd5 <release>
}
80104b2a:	c9                   	leave  
80104b2b:	c3                   	ret    

80104b2c <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80104b2c:	55                   	push   %ebp
80104b2d:	89 e5                	mov    %esp,%ebp
80104b2f:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80104b32:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104b39:	e8 97 04 00 00       	call   80104fd5 <release>

  if (first) {
80104b3e:	a1 08 b0 10 80       	mov    0x8010b008,%eax
80104b43:	85 c0                	test   %eax,%eax
80104b45:	74 0f                	je     80104b56 <forkret+0x2a>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot 
    // be run from main().
    first = 0;
80104b47:	c7 05 08 b0 10 80 00 	movl   $0x0,0x8010b008
80104b4e:	00 00 00 
    initlog();
80104b51:	e8 ac e6 ff ff       	call   80103202 <initlog>
  }
  
  // Return to "caller", actually trapret (see allocproc).
}
80104b56:	c9                   	leave  
80104b57:	c3                   	ret    

80104b58 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
80104b58:	55                   	push   %ebp
80104b59:	89 e5                	mov    %esp,%ebp
80104b5b:	83 ec 18             	sub    $0x18,%esp
  if(proc == 0)
80104b5e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b64:	85 c0                	test   %eax,%eax
80104b66:	75 0c                	jne    80104b74 <sleep+0x1c>
    panic("sleep");
80104b68:	c7 04 24 8d 8c 10 80 	movl   $0x80108c8d,(%esp)
80104b6f:	e8 c6 b9 ff ff       	call   8010053a <panic>

  if(lk == 0)
80104b74:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104b78:	75 0c                	jne    80104b86 <sleep+0x2e>
    panic("sleep without lk");
80104b7a:	c7 04 24 93 8c 10 80 	movl   $0x80108c93,(%esp)
80104b81:	e8 b4 b9 ff ff       	call   8010053a <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
80104b86:	81 7d 0c 80 29 11 80 	cmpl   $0x80112980,0xc(%ebp)
80104b8d:	74 17                	je     80104ba6 <sleep+0x4e>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104b8f:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104b96:	e8 d8 03 00 00       	call   80104f73 <acquire>
    release(lk);
80104b9b:	8b 45 0c             	mov    0xc(%ebp),%eax
80104b9e:	89 04 24             	mov    %eax,(%esp)
80104ba1:	e8 2f 04 00 00       	call   80104fd5 <release>
  }

  // Go to sleep.
  proc->chan = chan;
80104ba6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bac:	8b 55 08             	mov    0x8(%ebp),%edx
80104baf:	89 50 20             	mov    %edx,0x20(%eax)
  proc->state = SLEEPING;
80104bb2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bb8:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
  sched();
80104bbf:	e8 84 fe ff ff       	call   80104a48 <sched>

  // Tidy up.
  proc->chan = 0;
80104bc4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bca:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
80104bd1:	81 7d 0c 80 29 11 80 	cmpl   $0x80112980,0xc(%ebp)
80104bd8:	74 17                	je     80104bf1 <sleep+0x99>
    release(&ptable.lock);
80104bda:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104be1:	e8 ef 03 00 00       	call   80104fd5 <release>
    acquire(lk);
80104be6:	8b 45 0c             	mov    0xc(%ebp),%eax
80104be9:	89 04 24             	mov    %eax,(%esp)
80104bec:	e8 82 03 00 00       	call   80104f73 <acquire>
  }
}
80104bf1:	c9                   	leave  
80104bf2:	c3                   	ret    

80104bf3 <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
80104bf3:	55                   	push   %ebp
80104bf4:	89 e5                	mov    %esp,%ebp
80104bf6:	83 ec 10             	sub    $0x10,%esp
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104bf9:	c7 45 fc b4 29 11 80 	movl   $0x801129b4,-0x4(%ebp)
80104c00:	eb 27                	jmp    80104c29 <wakeup1+0x36>
    if(p->state == SLEEPING && p->chan == chan)
80104c02:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c05:	8b 40 0c             	mov    0xc(%eax),%eax
80104c08:	83 f8 02             	cmp    $0x2,%eax
80104c0b:	75 15                	jne    80104c22 <wakeup1+0x2f>
80104c0d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c10:	8b 40 20             	mov    0x20(%eax),%eax
80104c13:	3b 45 08             	cmp    0x8(%ebp),%eax
80104c16:	75 0a                	jne    80104c22 <wakeup1+0x2f>
      p->state = RUNNABLE;
80104c18:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c1b:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104c22:	81 45 fc 90 00 00 00 	addl   $0x90,-0x4(%ebp)
80104c29:	81 7d fc b4 4d 11 80 	cmpl   $0x80114db4,-0x4(%ebp)
80104c30:	72 d0                	jb     80104c02 <wakeup1+0xf>
    if(p->state == SLEEPING && p->chan == chan)
      p->state = RUNNABLE;
}
80104c32:	c9                   	leave  
80104c33:	c3                   	ret    

80104c34 <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80104c34:	55                   	push   %ebp
80104c35:	89 e5                	mov    %esp,%ebp
80104c37:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
80104c3a:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104c41:	e8 2d 03 00 00       	call   80104f73 <acquire>
  wakeup1(chan);
80104c46:	8b 45 08             	mov    0x8(%ebp),%eax
80104c49:	89 04 24             	mov    %eax,(%esp)
80104c4c:	e8 a2 ff ff ff       	call   80104bf3 <wakeup1>
  release(&ptable.lock);
80104c51:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104c58:	e8 78 03 00 00       	call   80104fd5 <release>
}
80104c5d:	c9                   	leave  
80104c5e:	c3                   	ret    

80104c5f <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
80104c5f:	55                   	push   %ebp
80104c60:	89 e5                	mov    %esp,%ebp
80104c62:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
80104c65:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104c6c:	e8 02 03 00 00       	call   80104f73 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104c71:	c7 45 f4 b4 29 11 80 	movl   $0x801129b4,-0xc(%ebp)
80104c78:	eb 44                	jmp    80104cbe <kill+0x5f>
    if(p->pid == pid){
80104c7a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c7d:	8b 40 10             	mov    0x10(%eax),%eax
80104c80:	3b 45 08             	cmp    0x8(%ebp),%eax
80104c83:	75 32                	jne    80104cb7 <kill+0x58>
      p->killed = 1;
80104c85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c88:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104c8f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c92:	8b 40 0c             	mov    0xc(%eax),%eax
80104c95:	83 f8 02             	cmp    $0x2,%eax
80104c98:	75 0a                	jne    80104ca4 <kill+0x45>
        p->state = RUNNABLE;
80104c9a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c9d:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
80104ca4:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104cab:	e8 25 03 00 00       	call   80104fd5 <release>
      return 0;
80104cb0:	b8 00 00 00 00       	mov    $0x0,%eax
80104cb5:	eb 21                	jmp    80104cd8 <kill+0x79>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104cb7:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
80104cbe:	81 7d f4 b4 4d 11 80 	cmpl   $0x80114db4,-0xc(%ebp)
80104cc5:	72 b3                	jb     80104c7a <kill+0x1b>
        p->state = RUNNABLE;
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
80104cc7:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80104cce:	e8 02 03 00 00       	call   80104fd5 <release>
  return -1;
80104cd3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104cd8:	c9                   	leave  
80104cd9:	c3                   	ret    

80104cda <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104cda:	55                   	push   %ebp
80104cdb:	89 e5                	mov    %esp,%ebp
80104cdd:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104ce0:	c7 45 f0 b4 29 11 80 	movl   $0x801129b4,-0x10(%ebp)
80104ce7:	e9 d9 00 00 00       	jmp    80104dc5 <procdump+0xeb>
    if(p->state == UNUSED)
80104cec:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104cef:	8b 40 0c             	mov    0xc(%eax),%eax
80104cf2:	85 c0                	test   %eax,%eax
80104cf4:	75 05                	jne    80104cfb <procdump+0x21>
      continue;
80104cf6:	e9 c3 00 00 00       	jmp    80104dbe <procdump+0xe4>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
80104cfb:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104cfe:	8b 40 0c             	mov    0xc(%eax),%eax
80104d01:	83 f8 05             	cmp    $0x5,%eax
80104d04:	77 23                	ja     80104d29 <procdump+0x4f>
80104d06:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d09:	8b 40 0c             	mov    0xc(%eax),%eax
80104d0c:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104d13:	85 c0                	test   %eax,%eax
80104d15:	74 12                	je     80104d29 <procdump+0x4f>
      state = states[p->state];
80104d17:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d1a:	8b 40 0c             	mov    0xc(%eax),%eax
80104d1d:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104d24:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104d27:	eb 07                	jmp    80104d30 <procdump+0x56>
    else
      state = "???";
80104d29:	c7 45 ec a4 8c 10 80 	movl   $0x80108ca4,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
80104d30:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d33:	8d 50 6c             	lea    0x6c(%eax),%edx
80104d36:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d39:	8b 40 10             	mov    0x10(%eax),%eax
80104d3c:	89 54 24 0c          	mov    %edx,0xc(%esp)
80104d40:	8b 55 ec             	mov    -0x14(%ebp),%edx
80104d43:	89 54 24 08          	mov    %edx,0x8(%esp)
80104d47:	89 44 24 04          	mov    %eax,0x4(%esp)
80104d4b:	c7 04 24 a8 8c 10 80 	movl   $0x80108ca8,(%esp)
80104d52:	e8 49 b6 ff ff       	call   801003a0 <cprintf>
    if(p->state == SLEEPING){
80104d57:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d5a:	8b 40 0c             	mov    0xc(%eax),%eax
80104d5d:	83 f8 02             	cmp    $0x2,%eax
80104d60:	75 50                	jne    80104db2 <procdump+0xd8>
      getcallerpcs((uint*)p->context->ebp+2, pc);
80104d62:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104d65:	8b 40 1c             	mov    0x1c(%eax),%eax
80104d68:	8b 40 0c             	mov    0xc(%eax),%eax
80104d6b:	83 c0 08             	add    $0x8,%eax
80104d6e:	8d 55 c4             	lea    -0x3c(%ebp),%edx
80104d71:	89 54 24 04          	mov    %edx,0x4(%esp)
80104d75:	89 04 24             	mov    %eax,(%esp)
80104d78:	e8 a7 02 00 00       	call   80105024 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
80104d7d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104d84:	eb 1b                	jmp    80104da1 <procdump+0xc7>
        cprintf(" %p", pc[i]);
80104d86:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d89:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104d8d:	89 44 24 04          	mov    %eax,0x4(%esp)
80104d91:	c7 04 24 b1 8c 10 80 	movl   $0x80108cb1,(%esp)
80104d98:	e8 03 b6 ff ff       	call   801003a0 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
80104d9d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104da1:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80104da5:	7f 0b                	jg     80104db2 <procdump+0xd8>
80104da7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104daa:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104dae:	85 c0                	test   %eax,%eax
80104db0:	75 d4                	jne    80104d86 <procdump+0xac>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104db2:	c7 04 24 b5 8c 10 80 	movl   $0x80108cb5,(%esp)
80104db9:	e8 e2 b5 ff ff       	call   801003a0 <cprintf>
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104dbe:	81 45 f0 90 00 00 00 	addl   $0x90,-0x10(%ebp)
80104dc5:	81 7d f0 b4 4d 11 80 	cmpl   $0x80114db4,-0x10(%ebp)
80104dcc:	0f 82 1a ff ff ff    	jb     80104cec <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
}
80104dd2:	c9                   	leave  
80104dd3:	c3                   	ret    

80104dd4 <getproc>:


int getproc(int sadd)
{
80104dd4:	55                   	push   %ebp
80104dd5:	89 e5                	mov    %esp,%ebp
80104dd7:	83 ec 28             	sub    $0x28,%esp
        {
                int id;
                char state[10];
                char name[16];
        } *p;
        p= (struct procentry*)sadd;
80104dda:	8b 45 08             	mov    0x8(%ebp),%eax
80104ddd:	89 45 f4             	mov    %eax,-0xc(%ebp)
        struct proc *pe;
        int no=0;
80104de0:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
        for(pe=ptable.proc;pe<&ptable.proc[NPROC];pe++)
80104de7:	c7 45 f0 b4 29 11 80 	movl   $0x801129b4,-0x10(%ebp)
80104dee:	e9 da 00 00 00       	jmp    80104ecd <getproc+0xf9>
        {
        if ( pe->state == SLEEPING || pe->state==RUNNABLE || pe->state==RUNNING)
80104df3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104df6:	8b 40 0c             	mov    0xc(%eax),%eax
80104df9:	83 f8 02             	cmp    $0x2,%eax
80104dfc:	74 1a                	je     80104e18 <getproc+0x44>
80104dfe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e01:	8b 40 0c             	mov    0xc(%eax),%eax
80104e04:	83 f8 03             	cmp    $0x3,%eax
80104e07:	74 0f                	je     80104e18 <getproc+0x44>
80104e09:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e0c:	8b 40 0c             	mov    0xc(%eax),%eax
80104e0f:	83 f8 04             	cmp    $0x4,%eax
80104e12:	0f 85 ae 00 00 00    	jne    80104ec6 <getproc+0xf2>
        {
                p->id = pe->pid;
80104e18:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e1b:	8b 50 10             	mov    0x10(%eax),%edx
80104e1e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e21:	89 10                	mov    %edx,(%eax)
                safestrcpy(p->name,pe->name,sizeof(pe->name));
80104e23:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e26:	8d 50 6c             	lea    0x6c(%eax),%edx
80104e29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e2c:	83 c0 0e             	add    $0xe,%eax
80104e2f:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104e36:	00 
80104e37:	89 54 24 04          	mov    %edx,0x4(%esp)
80104e3b:	89 04 24             	mov    %eax,(%esp)
80104e3e:	e8 a4 05 00 00       	call   801053e7 <safestrcpy>
                if(pe->state == SLEEPING)
80104e43:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e46:	8b 40 0c             	mov    0xc(%eax),%eax
80104e49:	83 f8 02             	cmp    $0x2,%eax
80104e4c:	75 1e                	jne    80104e6c <getproc+0x98>
                        safestrcpy(p->state,"SLEEPING",sizeof("SLEEPING"));
80104e4e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e51:	83 c0 04             	add    $0x4,%eax
80104e54:	c7 44 24 08 09 00 00 	movl   $0x9,0x8(%esp)
80104e5b:	00 
80104e5c:	c7 44 24 04 b7 8c 10 	movl   $0x80108cb7,0x4(%esp)
80104e63:	80 
80104e64:	89 04 24             	mov    %eax,(%esp)
80104e67:	e8 7b 05 00 00       	call   801053e7 <safestrcpy>
                if(pe->state == RUNNABLE)
80104e6c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e6f:	8b 40 0c             	mov    0xc(%eax),%eax
80104e72:	83 f8 03             	cmp    $0x3,%eax
80104e75:	75 1e                	jne    80104e95 <getproc+0xc1>
                        safestrcpy(p->state,"RUNNABLE",sizeof("RUNNABLE"));
80104e77:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e7a:	83 c0 04             	add    $0x4,%eax
80104e7d:	c7 44 24 08 09 00 00 	movl   $0x9,0x8(%esp)
80104e84:	00 
80104e85:	c7 44 24 04 c0 8c 10 	movl   $0x80108cc0,0x4(%esp)
80104e8c:	80 
80104e8d:	89 04 24             	mov    %eax,(%esp)
80104e90:	e8 52 05 00 00       	call   801053e7 <safestrcpy>
                if(pe->state == RUNNING)
80104e95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104e98:	8b 40 0c             	mov    0xc(%eax),%eax
80104e9b:	83 f8 04             	cmp    $0x4,%eax
80104e9e:	75 1e                	jne    80104ebe <getproc+0xea>
                        safestrcpy(p->state,"RUNNING",sizeof("RUNNING"));
80104ea0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ea3:	83 c0 04             	add    $0x4,%eax
80104ea6:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
80104ead:	00 
80104eae:	c7 44 24 04 c9 8c 10 	movl   $0x80108cc9,0x4(%esp)
80104eb5:	80 
80104eb6:	89 04 24             	mov    %eax,(%esp)
80104eb9:	e8 29 05 00 00       	call   801053e7 <safestrcpy>
                p++;
80104ebe:	83 45 f4 20          	addl   $0x20,-0xc(%ebp)
                no++;
80104ec2:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
                char name[16];
        } *p;
        p= (struct procentry*)sadd;
        struct proc *pe;
        int no=0;
        for(pe=ptable.proc;pe<&ptable.proc[NPROC];pe++)
80104ec6:	81 45 f0 90 00 00 00 	addl   $0x90,-0x10(%ebp)
80104ecd:	81 7d f0 b4 4d 11 80 	cmpl   $0x80114db4,-0x10(%ebp)
80104ed4:	0f 82 19 ff ff ff    	jb     80104df3 <getproc+0x1f>
                        safestrcpy(p->state,"RUNNING",sizeof("RUNNING"));
                p++;
                no++;
        }
        }
        return no++;
80104eda:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104edd:	8d 50 01             	lea    0x1(%eax),%edx
80104ee0:	89 55 ec             	mov    %edx,-0x14(%ebp)
}
80104ee3:	c9                   	leave  
80104ee4:	c3                   	ret    

80104ee5 <signal>:

int signal(int num, sighandler_t handler)
{
80104ee5:	55                   	push   %ebp
80104ee6:	89 e5                	mov    %esp,%ebp
//	cprintf("Inside signal with num: %d and handler addr:0x%x\n ",num,handler);
	proc->sighandle[num+1]=handler;
80104ee8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104eee:	8b 55 08             	mov    0x8(%ebp),%edx
80104ef1:	83 c2 01             	add    $0x1,%edx
80104ef4:	8d 4a 1c             	lea    0x1c(%edx),%ecx
80104ef7:	8b 55 0c             	mov    0xc(%ebp),%edx
80104efa:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
	return 0;
80104efe:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104f03:	5d                   	pop    %ebp
80104f04:	c3                   	ret    

80104f05 <alarm>:

int alarm(int val)
{
80104f05:	55                   	push   %ebp
80104f06:	89 e5                	mov    %esp,%ebp
	proc->ticks=val;
80104f08:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104f0e:	8b 55 08             	mov    0x8(%ebp),%edx
80104f11:	89 90 88 00 00 00    	mov    %edx,0x88(%eax)
	return val;
80104f17:	8b 45 08             	mov    0x8(%ebp),%eax
}
80104f1a:	5d                   	pop    %ebp
80104f1b:	c3                   	ret    

80104f1c <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104f1c:	55                   	push   %ebp
80104f1d:	89 e5                	mov    %esp,%ebp
80104f1f:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104f22:	9c                   	pushf  
80104f23:	58                   	pop    %eax
80104f24:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80104f27:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104f2a:	c9                   	leave  
80104f2b:	c3                   	ret    

80104f2c <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80104f2c:	55                   	push   %ebp
80104f2d:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80104f2f:	fa                   	cli    
}
80104f30:	5d                   	pop    %ebp
80104f31:	c3                   	ret    

80104f32 <sti>:

static inline void
sti(void)
{
80104f32:	55                   	push   %ebp
80104f33:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80104f35:	fb                   	sti    
}
80104f36:	5d                   	pop    %ebp
80104f37:	c3                   	ret    

80104f38 <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
80104f38:	55                   	push   %ebp
80104f39:	89 e5                	mov    %esp,%ebp
80104f3b:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80104f3e:	8b 55 08             	mov    0x8(%ebp),%edx
80104f41:	8b 45 0c             	mov    0xc(%ebp),%eax
80104f44:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104f47:	f0 87 02             	lock xchg %eax,(%edx)
80104f4a:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80104f4d:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104f50:	c9                   	leave  
80104f51:	c3                   	ret    

80104f52 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80104f52:	55                   	push   %ebp
80104f53:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80104f55:	8b 45 08             	mov    0x8(%ebp),%eax
80104f58:	8b 55 0c             	mov    0xc(%ebp),%edx
80104f5b:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
80104f5e:	8b 45 08             	mov    0x8(%ebp),%eax
80104f61:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80104f67:	8b 45 08             	mov    0x8(%ebp),%eax
80104f6a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80104f71:	5d                   	pop    %ebp
80104f72:	c3                   	ret    

80104f73 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80104f73:	55                   	push   %ebp
80104f74:	89 e5                	mov    %esp,%ebp
80104f76:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80104f79:	e8 49 01 00 00       	call   801050c7 <pushcli>
  if(holding(lk))
80104f7e:	8b 45 08             	mov    0x8(%ebp),%eax
80104f81:	89 04 24             	mov    %eax,(%esp)
80104f84:	e8 14 01 00 00       	call   8010509d <holding>
80104f89:	85 c0                	test   %eax,%eax
80104f8b:	74 0c                	je     80104f99 <acquire+0x26>
    panic("acquire");
80104f8d:	c7 04 24 fb 8c 10 80 	movl   $0x80108cfb,(%esp)
80104f94:	e8 a1 b5 ff ff       	call   8010053a <panic>

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
80104f99:	90                   	nop
80104f9a:	8b 45 08             	mov    0x8(%ebp),%eax
80104f9d:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80104fa4:	00 
80104fa5:	89 04 24             	mov    %eax,(%esp)
80104fa8:	e8 8b ff ff ff       	call   80104f38 <xchg>
80104fad:	85 c0                	test   %eax,%eax
80104faf:	75 e9                	jne    80104f9a <acquire+0x27>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
80104fb1:	8b 45 08             	mov    0x8(%ebp),%eax
80104fb4:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104fbb:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
80104fbe:	8b 45 08             	mov    0x8(%ebp),%eax
80104fc1:	83 c0 0c             	add    $0xc,%eax
80104fc4:	89 44 24 04          	mov    %eax,0x4(%esp)
80104fc8:	8d 45 08             	lea    0x8(%ebp),%eax
80104fcb:	89 04 24             	mov    %eax,(%esp)
80104fce:	e8 51 00 00 00       	call   80105024 <getcallerpcs>
}
80104fd3:	c9                   	leave  
80104fd4:	c3                   	ret    

80104fd5 <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
80104fd5:	55                   	push   %ebp
80104fd6:	89 e5                	mov    %esp,%ebp
80104fd8:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
80104fdb:	8b 45 08             	mov    0x8(%ebp),%eax
80104fde:	89 04 24             	mov    %eax,(%esp)
80104fe1:	e8 b7 00 00 00       	call   8010509d <holding>
80104fe6:	85 c0                	test   %eax,%eax
80104fe8:	75 0c                	jne    80104ff6 <release+0x21>
    panic("release");
80104fea:	c7 04 24 03 8d 10 80 	movl   $0x80108d03,(%esp)
80104ff1:	e8 44 b5 ff ff       	call   8010053a <panic>

  lk->pcs[0] = 0;
80104ff6:	8b 45 08             	mov    0x8(%ebp),%eax
80104ff9:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80105000:	8b 45 08             	mov    0x8(%ebp),%eax
80105003:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
8010500a:	8b 45 08             	mov    0x8(%ebp),%eax
8010500d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105014:	00 
80105015:	89 04 24             	mov    %eax,(%esp)
80105018:	e8 1b ff ff ff       	call   80104f38 <xchg>

  popcli();
8010501d:	e8 e9 00 00 00       	call   8010510b <popcli>
}
80105022:	c9                   	leave  
80105023:	c3                   	ret    

80105024 <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80105024:	55                   	push   %ebp
80105025:	89 e5                	mov    %esp,%ebp
80105027:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
8010502a:	8b 45 08             	mov    0x8(%ebp),%eax
8010502d:	83 e8 08             	sub    $0x8,%eax
80105030:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80105033:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
8010503a:	eb 38                	jmp    80105074 <getcallerpcs+0x50>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
8010503c:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80105040:	74 38                	je     8010507a <getcallerpcs+0x56>
80105042:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80105049:	76 2f                	jbe    8010507a <getcallerpcs+0x56>
8010504b:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
8010504f:	74 29                	je     8010507a <getcallerpcs+0x56>
      break;
    pcs[i] = ebp[1];     // saved %eip
80105051:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105054:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
8010505b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010505e:	01 c2                	add    %eax,%edx
80105060:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105063:	8b 40 04             	mov    0x4(%eax),%eax
80105066:	89 02                	mov    %eax,(%edx)
    ebp = (uint*)ebp[0]; // saved %ebp
80105068:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010506b:	8b 00                	mov    (%eax),%eax
8010506d:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80105070:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105074:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80105078:	7e c2                	jle    8010503c <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
8010507a:	eb 19                	jmp    80105095 <getcallerpcs+0x71>
    pcs[i] = 0;
8010507c:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010507f:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80105086:	8b 45 0c             	mov    0xc(%ebp),%eax
80105089:	01 d0                	add    %edx,%eax
8010508b:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80105091:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105095:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80105099:	7e e1                	jle    8010507c <getcallerpcs+0x58>
    pcs[i] = 0;
}
8010509b:	c9                   	leave  
8010509c:	c3                   	ret    

8010509d <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
8010509d:	55                   	push   %ebp
8010509e:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
801050a0:	8b 45 08             	mov    0x8(%ebp),%eax
801050a3:	8b 00                	mov    (%eax),%eax
801050a5:	85 c0                	test   %eax,%eax
801050a7:	74 17                	je     801050c0 <holding+0x23>
801050a9:	8b 45 08             	mov    0x8(%ebp),%eax
801050ac:	8b 50 08             	mov    0x8(%eax),%edx
801050af:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801050b5:	39 c2                	cmp    %eax,%edx
801050b7:	75 07                	jne    801050c0 <holding+0x23>
801050b9:	b8 01 00 00 00       	mov    $0x1,%eax
801050be:	eb 05                	jmp    801050c5 <holding+0x28>
801050c0:	b8 00 00 00 00       	mov    $0x0,%eax
}
801050c5:	5d                   	pop    %ebp
801050c6:	c3                   	ret    

801050c7 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
801050c7:	55                   	push   %ebp
801050c8:	89 e5                	mov    %esp,%ebp
801050ca:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
801050cd:	e8 4a fe ff ff       	call   80104f1c <readeflags>
801050d2:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
801050d5:	e8 52 fe ff ff       	call   80104f2c <cli>
  if(cpu->ncli++ == 0)
801050da:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801050e1:	8b 82 ac 00 00 00    	mov    0xac(%edx),%eax
801050e7:	8d 48 01             	lea    0x1(%eax),%ecx
801050ea:	89 8a ac 00 00 00    	mov    %ecx,0xac(%edx)
801050f0:	85 c0                	test   %eax,%eax
801050f2:	75 15                	jne    80105109 <pushcli+0x42>
    cpu->intena = eflags & FL_IF;
801050f4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801050fa:	8b 55 fc             	mov    -0x4(%ebp),%edx
801050fd:	81 e2 00 02 00 00    	and    $0x200,%edx
80105103:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80105109:	c9                   	leave  
8010510a:	c3                   	ret    

8010510b <popcli>:

void
popcli(void)
{
8010510b:	55                   	push   %ebp
8010510c:	89 e5                	mov    %esp,%ebp
8010510e:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
80105111:	e8 06 fe ff ff       	call   80104f1c <readeflags>
80105116:	25 00 02 00 00       	and    $0x200,%eax
8010511b:	85 c0                	test   %eax,%eax
8010511d:	74 0c                	je     8010512b <popcli+0x20>
    panic("popcli - interruptible");
8010511f:	c7 04 24 0b 8d 10 80 	movl   $0x80108d0b,(%esp)
80105126:	e8 0f b4 ff ff       	call   8010053a <panic>
  if(--cpu->ncli < 0)
8010512b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105131:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80105137:	83 ea 01             	sub    $0x1,%edx
8010513a:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80105140:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105146:	85 c0                	test   %eax,%eax
80105148:	79 0c                	jns    80105156 <popcli+0x4b>
    panic("popcli");
8010514a:	c7 04 24 22 8d 10 80 	movl   $0x80108d22,(%esp)
80105151:	e8 e4 b3 ff ff       	call   8010053a <panic>
  if(cpu->ncli == 0 && cpu->intena)
80105156:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010515c:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80105162:	85 c0                	test   %eax,%eax
80105164:	75 15                	jne    8010517b <popcli+0x70>
80105166:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010516c:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80105172:	85 c0                	test   %eax,%eax
80105174:	74 05                	je     8010517b <popcli+0x70>
    sti();
80105176:	e8 b7 fd ff ff       	call   80104f32 <sti>
}
8010517b:	c9                   	leave  
8010517c:	c3                   	ret    

8010517d <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
8010517d:	55                   	push   %ebp
8010517e:	89 e5                	mov    %esp,%ebp
80105180:	57                   	push   %edi
80105181:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
80105182:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105185:	8b 55 10             	mov    0x10(%ebp),%edx
80105188:	8b 45 0c             	mov    0xc(%ebp),%eax
8010518b:	89 cb                	mov    %ecx,%ebx
8010518d:	89 df                	mov    %ebx,%edi
8010518f:	89 d1                	mov    %edx,%ecx
80105191:	fc                   	cld    
80105192:	f3 aa                	rep stos %al,%es:(%edi)
80105194:	89 ca                	mov    %ecx,%edx
80105196:	89 fb                	mov    %edi,%ebx
80105198:	89 5d 08             	mov    %ebx,0x8(%ebp)
8010519b:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
8010519e:	5b                   	pop    %ebx
8010519f:	5f                   	pop    %edi
801051a0:	5d                   	pop    %ebp
801051a1:	c3                   	ret    

801051a2 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
801051a2:	55                   	push   %ebp
801051a3:	89 e5                	mov    %esp,%ebp
801051a5:	57                   	push   %edi
801051a6:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
801051a7:	8b 4d 08             	mov    0x8(%ebp),%ecx
801051aa:	8b 55 10             	mov    0x10(%ebp),%edx
801051ad:	8b 45 0c             	mov    0xc(%ebp),%eax
801051b0:	89 cb                	mov    %ecx,%ebx
801051b2:	89 df                	mov    %ebx,%edi
801051b4:	89 d1                	mov    %edx,%ecx
801051b6:	fc                   	cld    
801051b7:	f3 ab                	rep stos %eax,%es:(%edi)
801051b9:	89 ca                	mov    %ecx,%edx
801051bb:	89 fb                	mov    %edi,%ebx
801051bd:	89 5d 08             	mov    %ebx,0x8(%ebp)
801051c0:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
801051c3:	5b                   	pop    %ebx
801051c4:	5f                   	pop    %edi
801051c5:	5d                   	pop    %ebp
801051c6:	c3                   	ret    

801051c7 <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
801051c7:	55                   	push   %ebp
801051c8:	89 e5                	mov    %esp,%ebp
801051ca:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
801051cd:	8b 45 08             	mov    0x8(%ebp),%eax
801051d0:	83 e0 03             	and    $0x3,%eax
801051d3:	85 c0                	test   %eax,%eax
801051d5:	75 49                	jne    80105220 <memset+0x59>
801051d7:	8b 45 10             	mov    0x10(%ebp),%eax
801051da:	83 e0 03             	and    $0x3,%eax
801051dd:	85 c0                	test   %eax,%eax
801051df:	75 3f                	jne    80105220 <memset+0x59>
    c &= 0xFF;
801051e1:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
801051e8:	8b 45 10             	mov    0x10(%ebp),%eax
801051eb:	c1 e8 02             	shr    $0x2,%eax
801051ee:	89 c2                	mov    %eax,%edx
801051f0:	8b 45 0c             	mov    0xc(%ebp),%eax
801051f3:	c1 e0 18             	shl    $0x18,%eax
801051f6:	89 c1                	mov    %eax,%ecx
801051f8:	8b 45 0c             	mov    0xc(%ebp),%eax
801051fb:	c1 e0 10             	shl    $0x10,%eax
801051fe:	09 c1                	or     %eax,%ecx
80105200:	8b 45 0c             	mov    0xc(%ebp),%eax
80105203:	c1 e0 08             	shl    $0x8,%eax
80105206:	09 c8                	or     %ecx,%eax
80105208:	0b 45 0c             	or     0xc(%ebp),%eax
8010520b:	89 54 24 08          	mov    %edx,0x8(%esp)
8010520f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105213:	8b 45 08             	mov    0x8(%ebp),%eax
80105216:	89 04 24             	mov    %eax,(%esp)
80105219:	e8 84 ff ff ff       	call   801051a2 <stosl>
8010521e:	eb 19                	jmp    80105239 <memset+0x72>
  } else
    stosb(dst, c, n);
80105220:	8b 45 10             	mov    0x10(%ebp),%eax
80105223:	89 44 24 08          	mov    %eax,0x8(%esp)
80105227:	8b 45 0c             	mov    0xc(%ebp),%eax
8010522a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010522e:	8b 45 08             	mov    0x8(%ebp),%eax
80105231:	89 04 24             	mov    %eax,(%esp)
80105234:	e8 44 ff ff ff       	call   8010517d <stosb>
  return dst;
80105239:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010523c:	c9                   	leave  
8010523d:	c3                   	ret    

8010523e <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
8010523e:	55                   	push   %ebp
8010523f:	89 e5                	mov    %esp,%ebp
80105241:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;
  
  s1 = v1;
80105244:	8b 45 08             	mov    0x8(%ebp),%eax
80105247:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
8010524a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010524d:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
80105250:	eb 30                	jmp    80105282 <memcmp+0x44>
    if(*s1 != *s2)
80105252:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105255:	0f b6 10             	movzbl (%eax),%edx
80105258:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010525b:	0f b6 00             	movzbl (%eax),%eax
8010525e:	38 c2                	cmp    %al,%dl
80105260:	74 18                	je     8010527a <memcmp+0x3c>
      return *s1 - *s2;
80105262:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105265:	0f b6 00             	movzbl (%eax),%eax
80105268:	0f b6 d0             	movzbl %al,%edx
8010526b:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010526e:	0f b6 00             	movzbl (%eax),%eax
80105271:	0f b6 c0             	movzbl %al,%eax
80105274:	29 c2                	sub    %eax,%edx
80105276:	89 d0                	mov    %edx,%eax
80105278:	eb 1a                	jmp    80105294 <memcmp+0x56>
    s1++, s2++;
8010527a:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010527e:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
{
  const uchar *s1, *s2;
  
  s1 = v1;
  s2 = v2;
  while(n-- > 0){
80105282:	8b 45 10             	mov    0x10(%ebp),%eax
80105285:	8d 50 ff             	lea    -0x1(%eax),%edx
80105288:	89 55 10             	mov    %edx,0x10(%ebp)
8010528b:	85 c0                	test   %eax,%eax
8010528d:	75 c3                	jne    80105252 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
8010528f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105294:	c9                   	leave  
80105295:	c3                   	ret    

80105296 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
80105296:	55                   	push   %ebp
80105297:	89 e5                	mov    %esp,%ebp
80105299:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
8010529c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010529f:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
801052a2:	8b 45 08             	mov    0x8(%ebp),%eax
801052a5:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
801052a8:	8b 45 fc             	mov    -0x4(%ebp),%eax
801052ab:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801052ae:	73 3d                	jae    801052ed <memmove+0x57>
801052b0:	8b 45 10             	mov    0x10(%ebp),%eax
801052b3:	8b 55 fc             	mov    -0x4(%ebp),%edx
801052b6:	01 d0                	add    %edx,%eax
801052b8:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801052bb:	76 30                	jbe    801052ed <memmove+0x57>
    s += n;
801052bd:	8b 45 10             	mov    0x10(%ebp),%eax
801052c0:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
801052c3:	8b 45 10             	mov    0x10(%ebp),%eax
801052c6:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
801052c9:	eb 13                	jmp    801052de <memmove+0x48>
      *--d = *--s;
801052cb:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
801052cf:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
801052d3:	8b 45 fc             	mov    -0x4(%ebp),%eax
801052d6:	0f b6 10             	movzbl (%eax),%edx
801052d9:	8b 45 f8             	mov    -0x8(%ebp),%eax
801052dc:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
801052de:	8b 45 10             	mov    0x10(%ebp),%eax
801052e1:	8d 50 ff             	lea    -0x1(%eax),%edx
801052e4:	89 55 10             	mov    %edx,0x10(%ebp)
801052e7:	85 c0                	test   %eax,%eax
801052e9:	75 e0                	jne    801052cb <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
801052eb:	eb 26                	jmp    80105313 <memmove+0x7d>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
801052ed:	eb 17                	jmp    80105306 <memmove+0x70>
      *d++ = *s++;
801052ef:	8b 45 f8             	mov    -0x8(%ebp),%eax
801052f2:	8d 50 01             	lea    0x1(%eax),%edx
801052f5:	89 55 f8             	mov    %edx,-0x8(%ebp)
801052f8:	8b 55 fc             	mov    -0x4(%ebp),%edx
801052fb:	8d 4a 01             	lea    0x1(%edx),%ecx
801052fe:	89 4d fc             	mov    %ecx,-0x4(%ebp)
80105301:	0f b6 12             	movzbl (%edx),%edx
80105304:	88 10                	mov    %dl,(%eax)
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
80105306:	8b 45 10             	mov    0x10(%ebp),%eax
80105309:	8d 50 ff             	lea    -0x1(%eax),%edx
8010530c:	89 55 10             	mov    %edx,0x10(%ebp)
8010530f:	85 c0                	test   %eax,%eax
80105311:	75 dc                	jne    801052ef <memmove+0x59>
      *d++ = *s++;

  return dst;
80105313:	8b 45 08             	mov    0x8(%ebp),%eax
}
80105316:	c9                   	leave  
80105317:	c3                   	ret    

80105318 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
80105318:	55                   	push   %ebp
80105319:	89 e5                	mov    %esp,%ebp
8010531b:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
8010531e:	8b 45 10             	mov    0x10(%ebp),%eax
80105321:	89 44 24 08          	mov    %eax,0x8(%esp)
80105325:	8b 45 0c             	mov    0xc(%ebp),%eax
80105328:	89 44 24 04          	mov    %eax,0x4(%esp)
8010532c:	8b 45 08             	mov    0x8(%ebp),%eax
8010532f:	89 04 24             	mov    %eax,(%esp)
80105332:	e8 5f ff ff ff       	call   80105296 <memmove>
}
80105337:	c9                   	leave  
80105338:	c3                   	ret    

80105339 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
80105339:	55                   	push   %ebp
8010533a:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
8010533c:	eb 0c                	jmp    8010534a <strncmp+0x11>
    n--, p++, q++;
8010533e:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105342:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80105346:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
8010534a:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010534e:	74 1a                	je     8010536a <strncmp+0x31>
80105350:	8b 45 08             	mov    0x8(%ebp),%eax
80105353:	0f b6 00             	movzbl (%eax),%eax
80105356:	84 c0                	test   %al,%al
80105358:	74 10                	je     8010536a <strncmp+0x31>
8010535a:	8b 45 08             	mov    0x8(%ebp),%eax
8010535d:	0f b6 10             	movzbl (%eax),%edx
80105360:	8b 45 0c             	mov    0xc(%ebp),%eax
80105363:	0f b6 00             	movzbl (%eax),%eax
80105366:	38 c2                	cmp    %al,%dl
80105368:	74 d4                	je     8010533e <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
8010536a:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010536e:	75 07                	jne    80105377 <strncmp+0x3e>
    return 0;
80105370:	b8 00 00 00 00       	mov    $0x0,%eax
80105375:	eb 16                	jmp    8010538d <strncmp+0x54>
  return (uchar)*p - (uchar)*q;
80105377:	8b 45 08             	mov    0x8(%ebp),%eax
8010537a:	0f b6 00             	movzbl (%eax),%eax
8010537d:	0f b6 d0             	movzbl %al,%edx
80105380:	8b 45 0c             	mov    0xc(%ebp),%eax
80105383:	0f b6 00             	movzbl (%eax),%eax
80105386:	0f b6 c0             	movzbl %al,%eax
80105389:	29 c2                	sub    %eax,%edx
8010538b:	89 d0                	mov    %edx,%eax
}
8010538d:	5d                   	pop    %ebp
8010538e:	c3                   	ret    

8010538f <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
8010538f:	55                   	push   %ebp
80105390:	89 e5                	mov    %esp,%ebp
80105392:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80105395:	8b 45 08             	mov    0x8(%ebp),%eax
80105398:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
8010539b:	90                   	nop
8010539c:	8b 45 10             	mov    0x10(%ebp),%eax
8010539f:	8d 50 ff             	lea    -0x1(%eax),%edx
801053a2:	89 55 10             	mov    %edx,0x10(%ebp)
801053a5:	85 c0                	test   %eax,%eax
801053a7:	7e 1e                	jle    801053c7 <strncpy+0x38>
801053a9:	8b 45 08             	mov    0x8(%ebp),%eax
801053ac:	8d 50 01             	lea    0x1(%eax),%edx
801053af:	89 55 08             	mov    %edx,0x8(%ebp)
801053b2:	8b 55 0c             	mov    0xc(%ebp),%edx
801053b5:	8d 4a 01             	lea    0x1(%edx),%ecx
801053b8:	89 4d 0c             	mov    %ecx,0xc(%ebp)
801053bb:	0f b6 12             	movzbl (%edx),%edx
801053be:	88 10                	mov    %dl,(%eax)
801053c0:	0f b6 00             	movzbl (%eax),%eax
801053c3:	84 c0                	test   %al,%al
801053c5:	75 d5                	jne    8010539c <strncpy+0xd>
    ;
  while(n-- > 0)
801053c7:	eb 0c                	jmp    801053d5 <strncpy+0x46>
    *s++ = 0;
801053c9:	8b 45 08             	mov    0x8(%ebp),%eax
801053cc:	8d 50 01             	lea    0x1(%eax),%edx
801053cf:	89 55 08             	mov    %edx,0x8(%ebp)
801053d2:	c6 00 00             	movb   $0x0,(%eax)
  char *os;
  
  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
801053d5:	8b 45 10             	mov    0x10(%ebp),%eax
801053d8:	8d 50 ff             	lea    -0x1(%eax),%edx
801053db:	89 55 10             	mov    %edx,0x10(%ebp)
801053de:	85 c0                	test   %eax,%eax
801053e0:	7f e7                	jg     801053c9 <strncpy+0x3a>
    *s++ = 0;
  return os;
801053e2:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801053e5:	c9                   	leave  
801053e6:	c3                   	ret    

801053e7 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
801053e7:	55                   	push   %ebp
801053e8:	89 e5                	mov    %esp,%ebp
801053ea:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
801053ed:	8b 45 08             	mov    0x8(%ebp),%eax
801053f0:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
801053f3:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801053f7:	7f 05                	jg     801053fe <safestrcpy+0x17>
    return os;
801053f9:	8b 45 fc             	mov    -0x4(%ebp),%eax
801053fc:	eb 31                	jmp    8010542f <safestrcpy+0x48>
  while(--n > 0 && (*s++ = *t++) != 0)
801053fe:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105402:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105406:	7e 1e                	jle    80105426 <safestrcpy+0x3f>
80105408:	8b 45 08             	mov    0x8(%ebp),%eax
8010540b:	8d 50 01             	lea    0x1(%eax),%edx
8010540e:	89 55 08             	mov    %edx,0x8(%ebp)
80105411:	8b 55 0c             	mov    0xc(%ebp),%edx
80105414:	8d 4a 01             	lea    0x1(%edx),%ecx
80105417:	89 4d 0c             	mov    %ecx,0xc(%ebp)
8010541a:	0f b6 12             	movzbl (%edx),%edx
8010541d:	88 10                	mov    %dl,(%eax)
8010541f:	0f b6 00             	movzbl (%eax),%eax
80105422:	84 c0                	test   %al,%al
80105424:	75 d8                	jne    801053fe <safestrcpy+0x17>
    ;
  *s = 0;
80105426:	8b 45 08             	mov    0x8(%ebp),%eax
80105429:	c6 00 00             	movb   $0x0,(%eax)
  return os;
8010542c:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010542f:	c9                   	leave  
80105430:	c3                   	ret    

80105431 <strlen>:

int
strlen(const char *s)
{
80105431:	55                   	push   %ebp
80105432:	89 e5                	mov    %esp,%ebp
80105434:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
80105437:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
8010543e:	eb 04                	jmp    80105444 <strlen+0x13>
80105440:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105444:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105447:	8b 45 08             	mov    0x8(%ebp),%eax
8010544a:	01 d0                	add    %edx,%eax
8010544c:	0f b6 00             	movzbl (%eax),%eax
8010544f:	84 c0                	test   %al,%al
80105451:	75 ed                	jne    80105440 <strlen+0xf>
    ;
  return n;
80105453:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80105456:	c9                   	leave  
80105457:	c3                   	ret    

80105458 <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
80105458:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
8010545c:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
80105460:	55                   	push   %ebp
  pushl %ebx
80105461:	53                   	push   %ebx
  pushl %esi
80105462:	56                   	push   %esi
  pushl %edi
80105463:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
80105464:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
80105466:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
80105468:	5f                   	pop    %edi
  popl %esi
80105469:	5e                   	pop    %esi
  popl %ebx
8010546a:	5b                   	pop    %ebx
  popl %ebp
8010546b:	5d                   	pop    %ebp
  ret
8010546c:	c3                   	ret    

8010546d <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
8010546d:	55                   	push   %ebp
8010546e:	89 e5                	mov    %esp,%ebp
  if(addr >= proc->sz || addr+4 > proc->sz)
80105470:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105476:	8b 00                	mov    (%eax),%eax
80105478:	3b 45 08             	cmp    0x8(%ebp),%eax
8010547b:	76 12                	jbe    8010548f <fetchint+0x22>
8010547d:	8b 45 08             	mov    0x8(%ebp),%eax
80105480:	8d 50 04             	lea    0x4(%eax),%edx
80105483:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105489:	8b 00                	mov    (%eax),%eax
8010548b:	39 c2                	cmp    %eax,%edx
8010548d:	76 07                	jbe    80105496 <fetchint+0x29>
    return -1;
8010548f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105494:	eb 0f                	jmp    801054a5 <fetchint+0x38>
  *ip = *(int*)(addr);
80105496:	8b 45 08             	mov    0x8(%ebp),%eax
80105499:	8b 10                	mov    (%eax),%edx
8010549b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010549e:	89 10                	mov    %edx,(%eax)
  return 0;
801054a0:	b8 00 00 00 00       	mov    $0x0,%eax
}
801054a5:	5d                   	pop    %ebp
801054a6:	c3                   	ret    

801054a7 <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
801054a7:	55                   	push   %ebp
801054a8:	89 e5                	mov    %esp,%ebp
801054aa:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= proc->sz)
801054ad:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054b3:	8b 00                	mov    (%eax),%eax
801054b5:	3b 45 08             	cmp    0x8(%ebp),%eax
801054b8:	77 07                	ja     801054c1 <fetchstr+0x1a>
    return -1;
801054ba:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801054bf:	eb 46                	jmp    80105507 <fetchstr+0x60>
  *pp = (char*)addr;
801054c1:	8b 55 08             	mov    0x8(%ebp),%edx
801054c4:	8b 45 0c             	mov    0xc(%ebp),%eax
801054c7:	89 10                	mov    %edx,(%eax)
  ep = (char*)proc->sz;
801054c9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054cf:	8b 00                	mov    (%eax),%eax
801054d1:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
801054d4:	8b 45 0c             	mov    0xc(%ebp),%eax
801054d7:	8b 00                	mov    (%eax),%eax
801054d9:	89 45 fc             	mov    %eax,-0x4(%ebp)
801054dc:	eb 1c                	jmp    801054fa <fetchstr+0x53>
    if(*s == 0)
801054de:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054e1:	0f b6 00             	movzbl (%eax),%eax
801054e4:	84 c0                	test   %al,%al
801054e6:	75 0e                	jne    801054f6 <fetchstr+0x4f>
      return s - *pp;
801054e8:	8b 55 fc             	mov    -0x4(%ebp),%edx
801054eb:	8b 45 0c             	mov    0xc(%ebp),%eax
801054ee:	8b 00                	mov    (%eax),%eax
801054f0:	29 c2                	sub    %eax,%edx
801054f2:	89 d0                	mov    %edx,%eax
801054f4:	eb 11                	jmp    80105507 <fetchstr+0x60>

  if(addr >= proc->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)proc->sz;
  for(s = *pp; s < ep; s++)
801054f6:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801054fa:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054fd:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105500:	72 dc                	jb     801054de <fetchstr+0x37>
    if(*s == 0)
      return s - *pp;
  return -1;
80105502:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105507:	c9                   	leave  
80105508:	c3                   	ret    

80105509 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
80105509:	55                   	push   %ebp
8010550a:	89 e5                	mov    %esp,%ebp
8010550c:	83 ec 08             	sub    $0x8,%esp
  return fetchint(proc->tf->esp + 4 + 4*n, ip);
8010550f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105515:	8b 40 18             	mov    0x18(%eax),%eax
80105518:	8b 50 44             	mov    0x44(%eax),%edx
8010551b:	8b 45 08             	mov    0x8(%ebp),%eax
8010551e:	c1 e0 02             	shl    $0x2,%eax
80105521:	01 d0                	add    %edx,%eax
80105523:	8d 50 04             	lea    0x4(%eax),%edx
80105526:	8b 45 0c             	mov    0xc(%ebp),%eax
80105529:	89 44 24 04          	mov    %eax,0x4(%esp)
8010552d:	89 14 24             	mov    %edx,(%esp)
80105530:	e8 38 ff ff ff       	call   8010546d <fetchint>
}
80105535:	c9                   	leave  
80105536:	c3                   	ret    

80105537 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
80105537:	55                   	push   %ebp
80105538:	89 e5                	mov    %esp,%ebp
8010553a:	83 ec 18             	sub    $0x18,%esp
  int i;
  
  if(argint(n, &i) < 0)
8010553d:	8d 45 fc             	lea    -0x4(%ebp),%eax
80105540:	89 44 24 04          	mov    %eax,0x4(%esp)
80105544:	8b 45 08             	mov    0x8(%ebp),%eax
80105547:	89 04 24             	mov    %eax,(%esp)
8010554a:	e8 ba ff ff ff       	call   80105509 <argint>
8010554f:	85 c0                	test   %eax,%eax
80105551:	79 07                	jns    8010555a <argptr+0x23>
    return -1;
80105553:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105558:	eb 3d                	jmp    80105597 <argptr+0x60>
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
8010555a:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010555d:	89 c2                	mov    %eax,%edx
8010555f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105565:	8b 00                	mov    (%eax),%eax
80105567:	39 c2                	cmp    %eax,%edx
80105569:	73 16                	jae    80105581 <argptr+0x4a>
8010556b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010556e:	89 c2                	mov    %eax,%edx
80105570:	8b 45 10             	mov    0x10(%ebp),%eax
80105573:	01 c2                	add    %eax,%edx
80105575:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010557b:	8b 00                	mov    (%eax),%eax
8010557d:	39 c2                	cmp    %eax,%edx
8010557f:	76 07                	jbe    80105588 <argptr+0x51>
    return -1;
80105581:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105586:	eb 0f                	jmp    80105597 <argptr+0x60>
  *pp = (char*)i;
80105588:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010558b:	89 c2                	mov    %eax,%edx
8010558d:	8b 45 0c             	mov    0xc(%ebp),%eax
80105590:	89 10                	mov    %edx,(%eax)
  return 0;
80105592:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105597:	c9                   	leave  
80105598:	c3                   	ret    

80105599 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
80105599:	55                   	push   %ebp
8010559a:	89 e5                	mov    %esp,%ebp
8010559c:	83 ec 18             	sub    $0x18,%esp
  int addr;
  if(argint(n, &addr) < 0)
8010559f:	8d 45 fc             	lea    -0x4(%ebp),%eax
801055a2:	89 44 24 04          	mov    %eax,0x4(%esp)
801055a6:	8b 45 08             	mov    0x8(%ebp),%eax
801055a9:	89 04 24             	mov    %eax,(%esp)
801055ac:	e8 58 ff ff ff       	call   80105509 <argint>
801055b1:	85 c0                	test   %eax,%eax
801055b3:	79 07                	jns    801055bc <argstr+0x23>
    return -1;
801055b5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801055ba:	eb 12                	jmp    801055ce <argstr+0x35>
  return fetchstr(addr, pp);
801055bc:	8b 45 fc             	mov    -0x4(%ebp),%eax
801055bf:	8b 55 0c             	mov    0xc(%ebp),%edx
801055c2:	89 54 24 04          	mov    %edx,0x4(%esp)
801055c6:	89 04 24             	mov    %eax,(%esp)
801055c9:	e8 d9 fe ff ff       	call   801054a7 <fetchstr>
}
801055ce:	c9                   	leave  
801055cf:	c3                   	ret    

801055d0 <syscall>:
[SYS_alarm] sys_alarm,
};

void
syscall(void)
{
801055d0:	55                   	push   %ebp
801055d1:	89 e5                	mov    %esp,%ebp
801055d3:	53                   	push   %ebx
801055d4:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
801055d7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801055dd:	8b 40 18             	mov    0x18(%eax),%eax
801055e0:	8b 40 1c             	mov    0x1c(%eax),%eax
801055e3:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
801055e6:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801055ea:	7e 30                	jle    8010561c <syscall+0x4c>
801055ec:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055ef:	83 f8 1a             	cmp    $0x1a,%eax
801055f2:	77 28                	ja     8010561c <syscall+0x4c>
801055f4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055f7:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
801055fe:	85 c0                	test   %eax,%eax
80105600:	74 1a                	je     8010561c <syscall+0x4c>
    proc->tf->eax = syscalls[num]();
80105602:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105608:	8b 58 18             	mov    0x18(%eax),%ebx
8010560b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010560e:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
80105615:	ff d0                	call   *%eax
80105617:	89 43 1c             	mov    %eax,0x1c(%ebx)
8010561a:	eb 3d                	jmp    80105659 <syscall+0x89>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
8010561c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105622:	8d 48 6c             	lea    0x6c(%eax),%ecx
80105625:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax

  num = proc->tf->eax;
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
8010562b:	8b 40 10             	mov    0x10(%eax),%eax
8010562e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105631:	89 54 24 0c          	mov    %edx,0xc(%esp)
80105635:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105639:	89 44 24 04          	mov    %eax,0x4(%esp)
8010563d:	c7 04 24 29 8d 10 80 	movl   $0x80108d29,(%esp)
80105644:	e8 57 ad ff ff       	call   801003a0 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
80105649:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010564f:	8b 40 18             	mov    0x18(%eax),%eax
80105652:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
80105659:	83 c4 24             	add    $0x24,%esp
8010565c:	5b                   	pop    %ebx
8010565d:	5d                   	pop    %ebp
8010565e:	c3                   	ret    

8010565f <sys_sysreplace>:

int sys_sysreplace(void)
{
8010565f:	55                   	push   %ebp
80105660:	89 e5                	mov    %esp,%ebp
80105662:	83 ec 28             	sub    $0x28,%esp

        int syscall_num,fun_add,oldfun_add;
	if(argint(0, &syscall_num) < 0)
80105665:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105668:	89 44 24 04          	mov    %eax,0x4(%esp)
8010566c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105673:	e8 91 fe ff ff       	call   80105509 <argint>
80105678:	85 c0                	test   %eax,%eax
8010567a:	79 07                	jns    80105683 <sys_sysreplace+0x24>
                return -1;
8010567c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105681:	eb 69                	jmp    801056ec <sys_sysreplace+0x8d>
	if(argint(1, &fun_add) < 0)
80105683:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105686:	89 44 24 04          	mov    %eax,0x4(%esp)
8010568a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105691:	e8 73 fe ff ff       	call   80105509 <argint>
80105696:	85 c0                	test   %eax,%eax
80105698:	79 07                	jns    801056a1 <sys_sysreplace+0x42>
                return -1;
8010569a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010569f:	eb 4b                	jmp    801056ec <sys_sysreplace+0x8d>
        if(argint(2, &oldfun_add) < 0)
801056a1:	8d 45 ec             	lea    -0x14(%ebp),%eax
801056a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801056a8:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801056af:	e8 55 fe ff ff       	call   80105509 <argint>
801056b4:	85 c0                	test   %eax,%eax
801056b6:	79 07                	jns    801056bf <sys_sysreplace+0x60>
                return -1;
801056b8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801056bd:	eb 2d                	jmp    801056ec <sys_sysreplace+0x8d>
	cprintf("In sysreplace\n");
801056bf:	c7 04 24 45 8d 10 80 	movl   $0x80108d45,(%esp)
801056c6:	e8 d5 ac ff ff       	call   801003a0 <cprintf>
	*((int*)oldfun_add)=(int)syscalls[syscall_num];
801056cb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801056ce:	8b 55 f4             	mov    -0xc(%ebp),%edx
801056d1:	8b 14 95 40 b0 10 80 	mov    -0x7fef4fc0(,%edx,4),%edx
801056d8:	89 10                	mov    %edx,(%eax)
	syscalls[syscall_num]=(int (*)(void))fun_add;
801056da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801056dd:	8b 55 f0             	mov    -0x10(%ebp),%edx
801056e0:	89 14 85 40 b0 10 80 	mov    %edx,-0x7fef4fc0(,%eax,4)
	return 0;
801056e7:	b8 00 00 00 00       	mov    $0x0,%eax
	
}
801056ec:	c9                   	leave  
801056ed:	c3                   	ret    

801056ee <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
801056ee:	55                   	push   %ebp
801056ef:	89 e5                	mov    %esp,%ebp
801056f1:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
801056f4:	8d 45 f0             	lea    -0x10(%ebp),%eax
801056f7:	89 44 24 04          	mov    %eax,0x4(%esp)
801056fb:	8b 45 08             	mov    0x8(%ebp),%eax
801056fe:	89 04 24             	mov    %eax,(%esp)
80105701:	e8 03 fe ff ff       	call   80105509 <argint>
80105706:	85 c0                	test   %eax,%eax
80105708:	79 07                	jns    80105711 <argfd+0x23>
    return -1;
8010570a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010570f:	eb 50                	jmp    80105761 <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
80105711:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105714:	85 c0                	test   %eax,%eax
80105716:	78 21                	js     80105739 <argfd+0x4b>
80105718:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010571b:	83 f8 0f             	cmp    $0xf,%eax
8010571e:	7f 19                	jg     80105739 <argfd+0x4b>
80105720:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105726:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105729:	83 c2 08             	add    $0x8,%edx
8010572c:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80105730:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105733:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105737:	75 07                	jne    80105740 <argfd+0x52>
    return -1;
80105739:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010573e:	eb 21                	jmp    80105761 <argfd+0x73>
  if(pfd)
80105740:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80105744:	74 08                	je     8010574e <argfd+0x60>
    *pfd = fd;
80105746:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105749:	8b 45 0c             	mov    0xc(%ebp),%eax
8010574c:	89 10                	mov    %edx,(%eax)
  if(pf)
8010574e:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105752:	74 08                	je     8010575c <argfd+0x6e>
    *pf = f;
80105754:	8b 45 10             	mov    0x10(%ebp),%eax
80105757:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010575a:	89 10                	mov    %edx,(%eax)
  return 0;
8010575c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105761:	c9                   	leave  
80105762:	c3                   	ret    

80105763 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
80105763:	55                   	push   %ebp
80105764:	89 e5                	mov    %esp,%ebp
80105766:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
80105769:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80105770:	eb 30                	jmp    801057a2 <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
80105772:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105778:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010577b:	83 c2 08             	add    $0x8,%edx
8010577e:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80105782:	85 c0                	test   %eax,%eax
80105784:	75 18                	jne    8010579e <fdalloc+0x3b>
      proc->ofile[fd] = f;
80105786:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010578c:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010578f:	8d 4a 08             	lea    0x8(%edx),%ecx
80105792:	8b 55 08             	mov    0x8(%ebp),%edx
80105795:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
80105799:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010579c:	eb 0f                	jmp    801057ad <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
8010579e:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801057a2:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
801057a6:	7e ca                	jle    80105772 <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
801057a8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801057ad:	c9                   	leave  
801057ae:	c3                   	ret    

801057af <sys_dup>:

int
sys_dup(void)
{
801057af:	55                   	push   %ebp
801057b0:	89 e5                	mov    %esp,%ebp
801057b2:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
801057b5:	8d 45 f0             	lea    -0x10(%ebp),%eax
801057b8:	89 44 24 08          	mov    %eax,0x8(%esp)
801057bc:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801057c3:	00 
801057c4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801057cb:	e8 1e ff ff ff       	call   801056ee <argfd>
801057d0:	85 c0                	test   %eax,%eax
801057d2:	79 07                	jns    801057db <sys_dup+0x2c>
    return -1;
801057d4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057d9:	eb 29                	jmp    80105804 <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
801057db:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057de:	89 04 24             	mov    %eax,(%esp)
801057e1:	e8 7d ff ff ff       	call   80105763 <fdalloc>
801057e6:	89 45 f4             	mov    %eax,-0xc(%ebp)
801057e9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801057ed:	79 07                	jns    801057f6 <sys_dup+0x47>
    return -1;
801057ef:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057f4:	eb 0e                	jmp    80105804 <sys_dup+0x55>
  filedup(f);
801057f6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057f9:	89 04 24             	mov    %eax,(%esp)
801057fc:	e8 85 b7 ff ff       	call   80100f86 <filedup>
  return fd;
80105801:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80105804:	c9                   	leave  
80105805:	c3                   	ret    

80105806 <sys_read>:

int
sys_read(void)
{
80105806:	55                   	push   %ebp
80105807:	89 e5                	mov    %esp,%ebp
80105809:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
8010580c:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010580f:	89 44 24 08          	mov    %eax,0x8(%esp)
80105813:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010581a:	00 
8010581b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105822:	e8 c7 fe ff ff       	call   801056ee <argfd>
80105827:	85 c0                	test   %eax,%eax
80105829:	78 35                	js     80105860 <sys_read+0x5a>
8010582b:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010582e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105832:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105839:	e8 cb fc ff ff       	call   80105509 <argint>
8010583e:	85 c0                	test   %eax,%eax
80105840:	78 1e                	js     80105860 <sys_read+0x5a>
80105842:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105845:	89 44 24 08          	mov    %eax,0x8(%esp)
80105849:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010584c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105850:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105857:	e8 db fc ff ff       	call   80105537 <argptr>
8010585c:	85 c0                	test   %eax,%eax
8010585e:	79 07                	jns    80105867 <sys_read+0x61>
    return -1;
80105860:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105865:	eb 19                	jmp    80105880 <sys_read+0x7a>
  return fileread(f, p, n);
80105867:	8b 4d f0             	mov    -0x10(%ebp),%ecx
8010586a:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010586d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105870:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105874:	89 54 24 04          	mov    %edx,0x4(%esp)
80105878:	89 04 24             	mov    %eax,(%esp)
8010587b:	e8 73 b8 ff ff       	call   801010f3 <fileread>
}
80105880:	c9                   	leave  
80105881:	c3                   	ret    

80105882 <sys_write>:

int
sys_write(void)
{
80105882:	55                   	push   %ebp
80105883:	89 e5                	mov    %esp,%ebp
80105885:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105888:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010588b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010588f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105896:	00 
80105897:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010589e:	e8 4b fe ff ff       	call   801056ee <argfd>
801058a3:	85 c0                	test   %eax,%eax
801058a5:	78 35                	js     801058dc <sys_write+0x5a>
801058a7:	8d 45 f0             	lea    -0x10(%ebp),%eax
801058aa:	89 44 24 04          	mov    %eax,0x4(%esp)
801058ae:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801058b5:	e8 4f fc ff ff       	call   80105509 <argint>
801058ba:	85 c0                	test   %eax,%eax
801058bc:	78 1e                	js     801058dc <sys_write+0x5a>
801058be:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058c1:	89 44 24 08          	mov    %eax,0x8(%esp)
801058c5:	8d 45 ec             	lea    -0x14(%ebp),%eax
801058c8:	89 44 24 04          	mov    %eax,0x4(%esp)
801058cc:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801058d3:	e8 5f fc ff ff       	call   80105537 <argptr>
801058d8:	85 c0                	test   %eax,%eax
801058da:	79 07                	jns    801058e3 <sys_write+0x61>
    return -1;
801058dc:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801058e1:	eb 19                	jmp    801058fc <sys_write+0x7a>
  return filewrite(f, p, n);
801058e3:	8b 4d f0             	mov    -0x10(%ebp),%ecx
801058e6:	8b 55 ec             	mov    -0x14(%ebp),%edx
801058e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058ec:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801058f0:	89 54 24 04          	mov    %edx,0x4(%esp)
801058f4:	89 04 24             	mov    %eax,(%esp)
801058f7:	e8 b3 b8 ff ff       	call   801011af <filewrite>
}
801058fc:	c9                   	leave  
801058fd:	c3                   	ret    

801058fe <sys_close>:

int
sys_close(void)
{
801058fe:	55                   	push   %ebp
801058ff:	89 e5                	mov    %esp,%ebp
80105901:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
80105904:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105907:	89 44 24 08          	mov    %eax,0x8(%esp)
8010590b:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010590e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105912:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105919:	e8 d0 fd ff ff       	call   801056ee <argfd>
8010591e:	85 c0                	test   %eax,%eax
80105920:	79 07                	jns    80105929 <sys_close+0x2b>
    return -1;
80105922:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105927:	eb 24                	jmp    8010594d <sys_close+0x4f>
  proc->ofile[fd] = 0;
80105929:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010592f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105932:	83 c2 08             	add    $0x8,%edx
80105935:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
8010593c:	00 
  fileclose(f);
8010593d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105940:	89 04 24             	mov    %eax,(%esp)
80105943:	e8 86 b6 ff ff       	call   80100fce <fileclose>
  return 0;
80105948:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010594d:	c9                   	leave  
8010594e:	c3                   	ret    

8010594f <sys_fstat>:

int
sys_fstat(void)
{
8010594f:	55                   	push   %ebp
80105950:	89 e5                	mov    %esp,%ebp
80105952:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80105955:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105958:	89 44 24 08          	mov    %eax,0x8(%esp)
8010595c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105963:	00 
80105964:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010596b:	e8 7e fd ff ff       	call   801056ee <argfd>
80105970:	85 c0                	test   %eax,%eax
80105972:	78 1f                	js     80105993 <sys_fstat+0x44>
80105974:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
8010597b:	00 
8010597c:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010597f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105983:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010598a:	e8 a8 fb ff ff       	call   80105537 <argptr>
8010598f:	85 c0                	test   %eax,%eax
80105991:	79 07                	jns    8010599a <sys_fstat+0x4b>
    return -1;
80105993:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105998:	eb 12                	jmp    801059ac <sys_fstat+0x5d>
  return filestat(f, st);
8010599a:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010599d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059a0:	89 54 24 04          	mov    %edx,0x4(%esp)
801059a4:	89 04 24             	mov    %eax,(%esp)
801059a7:	e8 f8 b6 ff ff       	call   801010a4 <filestat>
}
801059ac:	c9                   	leave  
801059ad:	c3                   	ret    

801059ae <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
801059ae:	55                   	push   %ebp
801059af:	89 e5                	mov    %esp,%ebp
801059b1:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
801059b4:	8d 45 d8             	lea    -0x28(%ebp),%eax
801059b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801059bb:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801059c2:	e8 d2 fb ff ff       	call   80105599 <argstr>
801059c7:	85 c0                	test   %eax,%eax
801059c9:	78 17                	js     801059e2 <sys_link+0x34>
801059cb:	8d 45 dc             	lea    -0x24(%ebp),%eax
801059ce:	89 44 24 04          	mov    %eax,0x4(%esp)
801059d2:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801059d9:	e8 bb fb ff ff       	call   80105599 <argstr>
801059de:	85 c0                	test   %eax,%eax
801059e0:	79 0a                	jns    801059ec <sys_link+0x3e>
    return -1;
801059e2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801059e7:	e9 42 01 00 00       	jmp    80105b2e <sys_link+0x180>

  begin_op();
801059ec:	e8 1f da ff ff       	call   80103410 <begin_op>
  if((ip = namei(old)) == 0){
801059f1:	8b 45 d8             	mov    -0x28(%ebp),%eax
801059f4:	89 04 24             	mov    %eax,(%esp)
801059f7:	e8 0a ca ff ff       	call   80102406 <namei>
801059fc:	89 45 f4             	mov    %eax,-0xc(%ebp)
801059ff:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105a03:	75 0f                	jne    80105a14 <sys_link+0x66>
    end_op();
80105a05:	e8 8a da ff ff       	call   80103494 <end_op>
    return -1;
80105a0a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a0f:	e9 1a 01 00 00       	jmp    80105b2e <sys_link+0x180>
  }

  ilock(ip);
80105a14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a17:	89 04 24             	mov    %eax,(%esp)
80105a1a:	e8 3c be ff ff       	call   8010185b <ilock>
  if(ip->type == T_DIR){
80105a1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a22:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105a26:	66 83 f8 01          	cmp    $0x1,%ax
80105a2a:	75 1a                	jne    80105a46 <sys_link+0x98>
    iunlockput(ip);
80105a2c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a2f:	89 04 24             	mov    %eax,(%esp)
80105a32:	e8 a8 c0 ff ff       	call   80101adf <iunlockput>
    end_op();
80105a37:	e8 58 da ff ff       	call   80103494 <end_op>
    return -1;
80105a3c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a41:	e9 e8 00 00 00       	jmp    80105b2e <sys_link+0x180>
  }

  ip->nlink++;
80105a46:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a49:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105a4d:	8d 50 01             	lea    0x1(%eax),%edx
80105a50:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a53:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105a57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a5a:	89 04 24             	mov    %eax,(%esp)
80105a5d:	e8 3d bc ff ff       	call   8010169f <iupdate>
  iunlock(ip);
80105a62:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a65:	89 04 24             	mov    %eax,(%esp)
80105a68:	e8 3c bf ff ff       	call   801019a9 <iunlock>

  if((dp = nameiparent(new, name)) == 0)
80105a6d:	8b 45 dc             	mov    -0x24(%ebp),%eax
80105a70:	8d 55 e2             	lea    -0x1e(%ebp),%edx
80105a73:	89 54 24 04          	mov    %edx,0x4(%esp)
80105a77:	89 04 24             	mov    %eax,(%esp)
80105a7a:	e8 a9 c9 ff ff       	call   80102428 <nameiparent>
80105a7f:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105a82:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105a86:	75 02                	jne    80105a8a <sys_link+0xdc>
    goto bad;
80105a88:	eb 68                	jmp    80105af2 <sys_link+0x144>
  ilock(dp);
80105a8a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a8d:	89 04 24             	mov    %eax,(%esp)
80105a90:	e8 c6 bd ff ff       	call   8010185b <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80105a95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a98:	8b 10                	mov    (%eax),%edx
80105a9a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a9d:	8b 00                	mov    (%eax),%eax
80105a9f:	39 c2                	cmp    %eax,%edx
80105aa1:	75 20                	jne    80105ac3 <sys_link+0x115>
80105aa3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105aa6:	8b 40 04             	mov    0x4(%eax),%eax
80105aa9:	89 44 24 08          	mov    %eax,0x8(%esp)
80105aad:	8d 45 e2             	lea    -0x1e(%ebp),%eax
80105ab0:	89 44 24 04          	mov    %eax,0x4(%esp)
80105ab4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ab7:	89 04 24             	mov    %eax,(%esp)
80105aba:	e8 87 c6 ff ff       	call   80102146 <dirlink>
80105abf:	85 c0                	test   %eax,%eax
80105ac1:	79 0d                	jns    80105ad0 <sys_link+0x122>
    iunlockput(dp);
80105ac3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ac6:	89 04 24             	mov    %eax,(%esp)
80105ac9:	e8 11 c0 ff ff       	call   80101adf <iunlockput>
    goto bad;
80105ace:	eb 22                	jmp    80105af2 <sys_link+0x144>
  }
  iunlockput(dp);
80105ad0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ad3:	89 04 24             	mov    %eax,(%esp)
80105ad6:	e8 04 c0 ff ff       	call   80101adf <iunlockput>
  iput(ip);
80105adb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ade:	89 04 24             	mov    %eax,(%esp)
80105ae1:	e8 28 bf ff ff       	call   80101a0e <iput>

  end_op();
80105ae6:	e8 a9 d9 ff ff       	call   80103494 <end_op>

  return 0;
80105aeb:	b8 00 00 00 00       	mov    $0x0,%eax
80105af0:	eb 3c                	jmp    80105b2e <sys_link+0x180>

bad:
  ilock(ip);
80105af2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105af5:	89 04 24             	mov    %eax,(%esp)
80105af8:	e8 5e bd ff ff       	call   8010185b <ilock>
  ip->nlink--;
80105afd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b00:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105b04:	8d 50 ff             	lea    -0x1(%eax),%edx
80105b07:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b0a:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105b0e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b11:	89 04 24             	mov    %eax,(%esp)
80105b14:	e8 86 bb ff ff       	call   8010169f <iupdate>
  iunlockput(ip);
80105b19:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b1c:	89 04 24             	mov    %eax,(%esp)
80105b1f:	e8 bb bf ff ff       	call   80101adf <iunlockput>
  end_op();
80105b24:	e8 6b d9 ff ff       	call   80103494 <end_op>
  return -1;
80105b29:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105b2e:	c9                   	leave  
80105b2f:	c3                   	ret    

80105b30 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
80105b30:	55                   	push   %ebp
80105b31:	89 e5                	mov    %esp,%ebp
80105b33:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105b36:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
80105b3d:	eb 4b                	jmp    80105b8a <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105b3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b42:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105b49:	00 
80105b4a:	89 44 24 08          	mov    %eax,0x8(%esp)
80105b4e:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105b51:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b55:	8b 45 08             	mov    0x8(%ebp),%eax
80105b58:	89 04 24             	mov    %eax,(%esp)
80105b5b:	e8 08 c2 ff ff       	call   80101d68 <readi>
80105b60:	83 f8 10             	cmp    $0x10,%eax
80105b63:	74 0c                	je     80105b71 <isdirempty+0x41>
      panic("isdirempty: readi");
80105b65:	c7 04 24 54 8d 10 80 	movl   $0x80108d54,(%esp)
80105b6c:	e8 c9 a9 ff ff       	call   8010053a <panic>
    if(de.inum != 0)
80105b71:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80105b75:	66 85 c0             	test   %ax,%ax
80105b78:	74 07                	je     80105b81 <isdirempty+0x51>
      return 0;
80105b7a:	b8 00 00 00 00       	mov    $0x0,%eax
80105b7f:	eb 1b                	jmp    80105b9c <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105b81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b84:	83 c0 10             	add    $0x10,%eax
80105b87:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105b8a:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105b8d:	8b 45 08             	mov    0x8(%ebp),%eax
80105b90:	8b 40 18             	mov    0x18(%eax),%eax
80105b93:	39 c2                	cmp    %eax,%edx
80105b95:	72 a8                	jb     80105b3f <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
80105b97:	b8 01 00 00 00       	mov    $0x1,%eax
}
80105b9c:	c9                   	leave  
80105b9d:	c3                   	ret    

80105b9e <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
80105b9e:	55                   	push   %ebp
80105b9f:	89 e5                	mov    %esp,%ebp
80105ba1:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
80105ba4:	8d 45 cc             	lea    -0x34(%ebp),%eax
80105ba7:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bab:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105bb2:	e8 e2 f9 ff ff       	call   80105599 <argstr>
80105bb7:	85 c0                	test   %eax,%eax
80105bb9:	79 0a                	jns    80105bc5 <sys_unlink+0x27>
    return -1;
80105bbb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105bc0:	e9 af 01 00 00       	jmp    80105d74 <sys_unlink+0x1d6>

  begin_op();
80105bc5:	e8 46 d8 ff ff       	call   80103410 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
80105bca:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105bcd:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80105bd0:	89 54 24 04          	mov    %edx,0x4(%esp)
80105bd4:	89 04 24             	mov    %eax,(%esp)
80105bd7:	e8 4c c8 ff ff       	call   80102428 <nameiparent>
80105bdc:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105bdf:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105be3:	75 0f                	jne    80105bf4 <sys_unlink+0x56>
    end_op();
80105be5:	e8 aa d8 ff ff       	call   80103494 <end_op>
    return -1;
80105bea:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105bef:	e9 80 01 00 00       	jmp    80105d74 <sys_unlink+0x1d6>
  }

  ilock(dp);
80105bf4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bf7:	89 04 24             	mov    %eax,(%esp)
80105bfa:	e8 5c bc ff ff       	call   8010185b <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105bff:	c7 44 24 04 66 8d 10 	movl   $0x80108d66,0x4(%esp)
80105c06:	80 
80105c07:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105c0a:	89 04 24             	mov    %eax,(%esp)
80105c0d:	e8 49 c4 ff ff       	call   8010205b <namecmp>
80105c12:	85 c0                	test   %eax,%eax
80105c14:	0f 84 45 01 00 00    	je     80105d5f <sys_unlink+0x1c1>
80105c1a:	c7 44 24 04 68 8d 10 	movl   $0x80108d68,0x4(%esp)
80105c21:	80 
80105c22:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105c25:	89 04 24             	mov    %eax,(%esp)
80105c28:	e8 2e c4 ff ff       	call   8010205b <namecmp>
80105c2d:	85 c0                	test   %eax,%eax
80105c2f:	0f 84 2a 01 00 00    	je     80105d5f <sys_unlink+0x1c1>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
80105c35:	8d 45 c8             	lea    -0x38(%ebp),%eax
80105c38:	89 44 24 08          	mov    %eax,0x8(%esp)
80105c3c:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105c3f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c46:	89 04 24             	mov    %eax,(%esp)
80105c49:	e8 2f c4 ff ff       	call   8010207d <dirlookup>
80105c4e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105c51:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105c55:	75 05                	jne    80105c5c <sys_unlink+0xbe>
    goto bad;
80105c57:	e9 03 01 00 00       	jmp    80105d5f <sys_unlink+0x1c1>
  ilock(ip);
80105c5c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c5f:	89 04 24             	mov    %eax,(%esp)
80105c62:	e8 f4 bb ff ff       	call   8010185b <ilock>

  if(ip->nlink < 1)
80105c67:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c6a:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105c6e:	66 85 c0             	test   %ax,%ax
80105c71:	7f 0c                	jg     80105c7f <sys_unlink+0xe1>
    panic("unlink: nlink < 1");
80105c73:	c7 04 24 6b 8d 10 80 	movl   $0x80108d6b,(%esp)
80105c7a:	e8 bb a8 ff ff       	call   8010053a <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105c7f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c82:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105c86:	66 83 f8 01          	cmp    $0x1,%ax
80105c8a:	75 1f                	jne    80105cab <sys_unlink+0x10d>
80105c8c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c8f:	89 04 24             	mov    %eax,(%esp)
80105c92:	e8 99 fe ff ff       	call   80105b30 <isdirempty>
80105c97:	85 c0                	test   %eax,%eax
80105c99:	75 10                	jne    80105cab <sys_unlink+0x10d>
    iunlockput(ip);
80105c9b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c9e:	89 04 24             	mov    %eax,(%esp)
80105ca1:	e8 39 be ff ff       	call   80101adf <iunlockput>
    goto bad;
80105ca6:	e9 b4 00 00 00       	jmp    80105d5f <sys_unlink+0x1c1>
  }

  memset(&de, 0, sizeof(de));
80105cab:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105cb2:	00 
80105cb3:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105cba:	00 
80105cbb:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105cbe:	89 04 24             	mov    %eax,(%esp)
80105cc1:	e8 01 f5 ff ff       	call   801051c7 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105cc6:	8b 45 c8             	mov    -0x38(%ebp),%eax
80105cc9:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105cd0:	00 
80105cd1:	89 44 24 08          	mov    %eax,0x8(%esp)
80105cd5:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105cd8:	89 44 24 04          	mov    %eax,0x4(%esp)
80105cdc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cdf:	89 04 24             	mov    %eax,(%esp)
80105ce2:	e8 e5 c1 ff ff       	call   80101ecc <writei>
80105ce7:	83 f8 10             	cmp    $0x10,%eax
80105cea:	74 0c                	je     80105cf8 <sys_unlink+0x15a>
    panic("unlink: writei");
80105cec:	c7 04 24 7d 8d 10 80 	movl   $0x80108d7d,(%esp)
80105cf3:	e8 42 a8 ff ff       	call   8010053a <panic>
  if(ip->type == T_DIR){
80105cf8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105cfb:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105cff:	66 83 f8 01          	cmp    $0x1,%ax
80105d03:	75 1c                	jne    80105d21 <sys_unlink+0x183>
    dp->nlink--;
80105d05:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d08:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105d0c:	8d 50 ff             	lea    -0x1(%eax),%edx
80105d0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d12:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105d16:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d19:	89 04 24             	mov    %eax,(%esp)
80105d1c:	e8 7e b9 ff ff       	call   8010169f <iupdate>
  }
  iunlockput(dp);
80105d21:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d24:	89 04 24             	mov    %eax,(%esp)
80105d27:	e8 b3 bd ff ff       	call   80101adf <iunlockput>

  ip->nlink--;
80105d2c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d2f:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105d33:	8d 50 ff             	lea    -0x1(%eax),%edx
80105d36:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d39:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105d3d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d40:	89 04 24             	mov    %eax,(%esp)
80105d43:	e8 57 b9 ff ff       	call   8010169f <iupdate>
  iunlockput(ip);
80105d48:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d4b:	89 04 24             	mov    %eax,(%esp)
80105d4e:	e8 8c bd ff ff       	call   80101adf <iunlockput>

  end_op();
80105d53:	e8 3c d7 ff ff       	call   80103494 <end_op>

  return 0;
80105d58:	b8 00 00 00 00       	mov    $0x0,%eax
80105d5d:	eb 15                	jmp    80105d74 <sys_unlink+0x1d6>

bad:
  iunlockput(dp);
80105d5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d62:	89 04 24             	mov    %eax,(%esp)
80105d65:	e8 75 bd ff ff       	call   80101adf <iunlockput>
  end_op();
80105d6a:	e8 25 d7 ff ff       	call   80103494 <end_op>
  return -1;
80105d6f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105d74:	c9                   	leave  
80105d75:	c3                   	ret    

80105d76 <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80105d76:	55                   	push   %ebp
80105d77:	89 e5                	mov    %esp,%ebp
80105d79:	83 ec 48             	sub    $0x48,%esp
80105d7c:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105d7f:	8b 55 10             	mov    0x10(%ebp),%edx
80105d82:	8b 45 14             	mov    0x14(%ebp),%eax
80105d85:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80105d89:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105d8d:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105d91:	8d 45 de             	lea    -0x22(%ebp),%eax
80105d94:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d98:	8b 45 08             	mov    0x8(%ebp),%eax
80105d9b:	89 04 24             	mov    %eax,(%esp)
80105d9e:	e8 85 c6 ff ff       	call   80102428 <nameiparent>
80105da3:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105da6:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105daa:	75 0a                	jne    80105db6 <create+0x40>
    return 0;
80105dac:	b8 00 00 00 00       	mov    $0x0,%eax
80105db1:	e9 7e 01 00 00       	jmp    80105f34 <create+0x1be>
  ilock(dp);
80105db6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105db9:	89 04 24             	mov    %eax,(%esp)
80105dbc:	e8 9a ba ff ff       	call   8010185b <ilock>

  if((ip = dirlookup(dp, name, &off)) != 0){
80105dc1:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105dc4:	89 44 24 08          	mov    %eax,0x8(%esp)
80105dc8:	8d 45 de             	lea    -0x22(%ebp),%eax
80105dcb:	89 44 24 04          	mov    %eax,0x4(%esp)
80105dcf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105dd2:	89 04 24             	mov    %eax,(%esp)
80105dd5:	e8 a3 c2 ff ff       	call   8010207d <dirlookup>
80105dda:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105ddd:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105de1:	74 47                	je     80105e2a <create+0xb4>
    iunlockput(dp);
80105de3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105de6:	89 04 24             	mov    %eax,(%esp)
80105de9:	e8 f1 bc ff ff       	call   80101adf <iunlockput>
    ilock(ip);
80105dee:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105df1:	89 04 24             	mov    %eax,(%esp)
80105df4:	e8 62 ba ff ff       	call   8010185b <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80105df9:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105dfe:	75 15                	jne    80105e15 <create+0x9f>
80105e00:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e03:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105e07:	66 83 f8 02          	cmp    $0x2,%ax
80105e0b:	75 08                	jne    80105e15 <create+0x9f>
      return ip;
80105e0d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e10:	e9 1f 01 00 00       	jmp    80105f34 <create+0x1be>
    iunlockput(ip);
80105e15:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e18:	89 04 24             	mov    %eax,(%esp)
80105e1b:	e8 bf bc ff ff       	call   80101adf <iunlockput>
    return 0;
80105e20:	b8 00 00 00 00       	mov    $0x0,%eax
80105e25:	e9 0a 01 00 00       	jmp    80105f34 <create+0x1be>
  }

  if((ip = ialloc(dp->dev, type)) == 0)
80105e2a:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80105e2e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e31:	8b 00                	mov    (%eax),%eax
80105e33:	89 54 24 04          	mov    %edx,0x4(%esp)
80105e37:	89 04 24             	mov    %eax,(%esp)
80105e3a:	e8 81 b7 ff ff       	call   801015c0 <ialloc>
80105e3f:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105e42:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105e46:	75 0c                	jne    80105e54 <create+0xde>
    panic("create: ialloc");
80105e48:	c7 04 24 8c 8d 10 80 	movl   $0x80108d8c,(%esp)
80105e4f:	e8 e6 a6 ff ff       	call   8010053a <panic>

  ilock(ip);
80105e54:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e57:	89 04 24             	mov    %eax,(%esp)
80105e5a:	e8 fc b9 ff ff       	call   8010185b <ilock>
  ip->major = major;
80105e5f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e62:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
80105e66:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
80105e6a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e6d:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80105e71:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
80105e75:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e78:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80105e7e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e81:	89 04 24             	mov    %eax,(%esp)
80105e84:	e8 16 b8 ff ff       	call   8010169f <iupdate>

  if(type == T_DIR){  // Create . and .. entries.
80105e89:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80105e8e:	75 6a                	jne    80105efa <create+0x184>
    dp->nlink++;  // for ".."
80105e90:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e93:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105e97:	8d 50 01             	lea    0x1(%eax),%edx
80105e9a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e9d:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105ea1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ea4:	89 04 24             	mov    %eax,(%esp)
80105ea7:	e8 f3 b7 ff ff       	call   8010169f <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80105eac:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105eaf:	8b 40 04             	mov    0x4(%eax),%eax
80105eb2:	89 44 24 08          	mov    %eax,0x8(%esp)
80105eb6:	c7 44 24 04 66 8d 10 	movl   $0x80108d66,0x4(%esp)
80105ebd:	80 
80105ebe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ec1:	89 04 24             	mov    %eax,(%esp)
80105ec4:	e8 7d c2 ff ff       	call   80102146 <dirlink>
80105ec9:	85 c0                	test   %eax,%eax
80105ecb:	78 21                	js     80105eee <create+0x178>
80105ecd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ed0:	8b 40 04             	mov    0x4(%eax),%eax
80105ed3:	89 44 24 08          	mov    %eax,0x8(%esp)
80105ed7:	c7 44 24 04 68 8d 10 	movl   $0x80108d68,0x4(%esp)
80105ede:	80 
80105edf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ee2:	89 04 24             	mov    %eax,(%esp)
80105ee5:	e8 5c c2 ff ff       	call   80102146 <dirlink>
80105eea:	85 c0                	test   %eax,%eax
80105eec:	79 0c                	jns    80105efa <create+0x184>
      panic("create dots");
80105eee:	c7 04 24 9b 8d 10 80 	movl   $0x80108d9b,(%esp)
80105ef5:	e8 40 a6 ff ff       	call   8010053a <panic>
  }

  if(dirlink(dp, name, ip->inum) < 0)
80105efa:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105efd:	8b 40 04             	mov    0x4(%eax),%eax
80105f00:	89 44 24 08          	mov    %eax,0x8(%esp)
80105f04:	8d 45 de             	lea    -0x22(%ebp),%eax
80105f07:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f0e:	89 04 24             	mov    %eax,(%esp)
80105f11:	e8 30 c2 ff ff       	call   80102146 <dirlink>
80105f16:	85 c0                	test   %eax,%eax
80105f18:	79 0c                	jns    80105f26 <create+0x1b0>
    panic("create: dirlink");
80105f1a:	c7 04 24 a7 8d 10 80 	movl   $0x80108da7,(%esp)
80105f21:	e8 14 a6 ff ff       	call   8010053a <panic>

  iunlockput(dp);
80105f26:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f29:	89 04 24             	mov    %eax,(%esp)
80105f2c:	e8 ae bb ff ff       	call   80101adf <iunlockput>

  return ip;
80105f31:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105f34:	c9                   	leave  
80105f35:	c3                   	ret    

80105f36 <sys_open>:

int
sys_open(void)
{
80105f36:	55                   	push   %ebp
80105f37:	89 e5                	mov    %esp,%ebp
80105f39:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105f3c:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105f3f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f43:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f4a:	e8 4a f6 ff ff       	call   80105599 <argstr>
80105f4f:	85 c0                	test   %eax,%eax
80105f51:	78 17                	js     80105f6a <sys_open+0x34>
80105f53:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105f56:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f5a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105f61:	e8 a3 f5 ff ff       	call   80105509 <argint>
80105f66:	85 c0                	test   %eax,%eax
80105f68:	79 0a                	jns    80105f74 <sys_open+0x3e>
    return -1;
80105f6a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105f6f:	e9 5c 01 00 00       	jmp    801060d0 <sys_open+0x19a>

  begin_op();
80105f74:	e8 97 d4 ff ff       	call   80103410 <begin_op>

  if(omode & O_CREATE){
80105f79:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105f7c:	25 00 02 00 00       	and    $0x200,%eax
80105f81:	85 c0                	test   %eax,%eax
80105f83:	74 3b                	je     80105fc0 <sys_open+0x8a>
    ip = create(path, T_FILE, 0, 0);
80105f85:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105f88:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105f8f:	00 
80105f90:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105f97:	00 
80105f98:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105f9f:	00 
80105fa0:	89 04 24             	mov    %eax,(%esp)
80105fa3:	e8 ce fd ff ff       	call   80105d76 <create>
80105fa8:	89 45 f4             	mov    %eax,-0xc(%ebp)
    if(ip == 0){
80105fab:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105faf:	75 6b                	jne    8010601c <sys_open+0xe6>
      end_op();
80105fb1:	e8 de d4 ff ff       	call   80103494 <end_op>
      return -1;
80105fb6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fbb:	e9 10 01 00 00       	jmp    801060d0 <sys_open+0x19a>
    }
  } else {
    if((ip = namei(path)) == 0){
80105fc0:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105fc3:	89 04 24             	mov    %eax,(%esp)
80105fc6:	e8 3b c4 ff ff       	call   80102406 <namei>
80105fcb:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105fce:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105fd2:	75 0f                	jne    80105fe3 <sys_open+0xad>
      end_op();
80105fd4:	e8 bb d4 ff ff       	call   80103494 <end_op>
      return -1;
80105fd9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fde:	e9 ed 00 00 00       	jmp    801060d0 <sys_open+0x19a>
    }
    ilock(ip);
80105fe3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fe6:	89 04 24             	mov    %eax,(%esp)
80105fe9:	e8 6d b8 ff ff       	call   8010185b <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80105fee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ff1:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105ff5:	66 83 f8 01          	cmp    $0x1,%ax
80105ff9:	75 21                	jne    8010601c <sys_open+0xe6>
80105ffb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105ffe:	85 c0                	test   %eax,%eax
80106000:	74 1a                	je     8010601c <sys_open+0xe6>
      iunlockput(ip);
80106002:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106005:	89 04 24             	mov    %eax,(%esp)
80106008:	e8 d2 ba ff ff       	call   80101adf <iunlockput>
      end_op();
8010600d:	e8 82 d4 ff ff       	call   80103494 <end_op>
      return -1;
80106012:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106017:	e9 b4 00 00 00       	jmp    801060d0 <sys_open+0x19a>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
8010601c:	e8 05 af ff ff       	call   80100f26 <filealloc>
80106021:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106024:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106028:	74 14                	je     8010603e <sys_open+0x108>
8010602a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010602d:	89 04 24             	mov    %eax,(%esp)
80106030:	e8 2e f7 ff ff       	call   80105763 <fdalloc>
80106035:	89 45 ec             	mov    %eax,-0x14(%ebp)
80106038:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010603c:	79 28                	jns    80106066 <sys_open+0x130>
    if(f)
8010603e:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106042:	74 0b                	je     8010604f <sys_open+0x119>
      fileclose(f);
80106044:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106047:	89 04 24             	mov    %eax,(%esp)
8010604a:	e8 7f af ff ff       	call   80100fce <fileclose>
    iunlockput(ip);
8010604f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106052:	89 04 24             	mov    %eax,(%esp)
80106055:	e8 85 ba ff ff       	call   80101adf <iunlockput>
    end_op();
8010605a:	e8 35 d4 ff ff       	call   80103494 <end_op>
    return -1;
8010605f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106064:	eb 6a                	jmp    801060d0 <sys_open+0x19a>
  }
  iunlock(ip);
80106066:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106069:	89 04 24             	mov    %eax,(%esp)
8010606c:	e8 38 b9 ff ff       	call   801019a9 <iunlock>
  end_op();
80106071:	e8 1e d4 ff ff       	call   80103494 <end_op>

  f->type = FD_INODE;
80106076:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106079:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
8010607f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106082:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106085:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80106088:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010608b:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80106092:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106095:	83 e0 01             	and    $0x1,%eax
80106098:	85 c0                	test   %eax,%eax
8010609a:	0f 94 c0             	sete   %al
8010609d:	89 c2                	mov    %eax,%edx
8010609f:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060a2:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
801060a5:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060a8:	83 e0 01             	and    $0x1,%eax
801060ab:	85 c0                	test   %eax,%eax
801060ad:	75 0a                	jne    801060b9 <sys_open+0x183>
801060af:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060b2:	83 e0 02             	and    $0x2,%eax
801060b5:	85 c0                	test   %eax,%eax
801060b7:	74 07                	je     801060c0 <sys_open+0x18a>
801060b9:	b8 01 00 00 00       	mov    $0x1,%eax
801060be:	eb 05                	jmp    801060c5 <sys_open+0x18f>
801060c0:	b8 00 00 00 00       	mov    $0x0,%eax
801060c5:	89 c2                	mov    %eax,%edx
801060c7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060ca:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
801060cd:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
801060d0:	c9                   	leave  
801060d1:	c3                   	ret    

801060d2 <sys_mkdir>:

int
sys_mkdir(void)
{
801060d2:	55                   	push   %ebp
801060d3:	89 e5                	mov    %esp,%ebp
801060d5:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_op();
801060d8:	e8 33 d3 ff ff       	call   80103410 <begin_op>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
801060dd:	8d 45 f0             	lea    -0x10(%ebp),%eax
801060e0:	89 44 24 04          	mov    %eax,0x4(%esp)
801060e4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801060eb:	e8 a9 f4 ff ff       	call   80105599 <argstr>
801060f0:	85 c0                	test   %eax,%eax
801060f2:	78 2c                	js     80106120 <sys_mkdir+0x4e>
801060f4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060f7:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
801060fe:	00 
801060ff:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80106106:	00 
80106107:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010610e:	00 
8010610f:	89 04 24             	mov    %eax,(%esp)
80106112:	e8 5f fc ff ff       	call   80105d76 <create>
80106117:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010611a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010611e:	75 0c                	jne    8010612c <sys_mkdir+0x5a>
    end_op();
80106120:	e8 6f d3 ff ff       	call   80103494 <end_op>
    return -1;
80106125:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010612a:	eb 15                	jmp    80106141 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
8010612c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010612f:	89 04 24             	mov    %eax,(%esp)
80106132:	e8 a8 b9 ff ff       	call   80101adf <iunlockput>
  end_op();
80106137:	e8 58 d3 ff ff       	call   80103494 <end_op>
  return 0;
8010613c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106141:	c9                   	leave  
80106142:	c3                   	ret    

80106143 <sys_mknod>:

int
sys_mknod(void)
{
80106143:	55                   	push   %ebp
80106144:	89 e5                	mov    %esp,%ebp
80106146:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_op();
80106149:	e8 c2 d2 ff ff       	call   80103410 <begin_op>
  if((len=argstr(0, &path)) < 0 ||
8010614e:	8d 45 ec             	lea    -0x14(%ebp),%eax
80106151:	89 44 24 04          	mov    %eax,0x4(%esp)
80106155:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010615c:	e8 38 f4 ff ff       	call   80105599 <argstr>
80106161:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106164:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106168:	78 5e                	js     801061c8 <sys_mknod+0x85>
     argint(1, &major) < 0 ||
8010616a:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010616d:	89 44 24 04          	mov    %eax,0x4(%esp)
80106171:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106178:	e8 8c f3 ff ff       	call   80105509 <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_op();
  if((len=argstr(0, &path)) < 0 ||
8010617d:	85 c0                	test   %eax,%eax
8010617f:	78 47                	js     801061c8 <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80106181:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106184:	89 44 24 04          	mov    %eax,0x4(%esp)
80106188:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010618f:	e8 75 f3 ff ff       	call   80105509 <argint>
  int len;
  int major, minor;
  
  begin_op();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
80106194:	85 c0                	test   %eax,%eax
80106196:	78 30                	js     801061c8 <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
80106198:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010619b:	0f bf c8             	movswl %ax,%ecx
8010619e:	8b 45 e8             	mov    -0x18(%ebp),%eax
801061a1:	0f bf d0             	movswl %ax,%edx
801061a4:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_op();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
801061a7:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801061ab:	89 54 24 08          	mov    %edx,0x8(%esp)
801061af:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801061b6:	00 
801061b7:	89 04 24             	mov    %eax,(%esp)
801061ba:	e8 b7 fb ff ff       	call   80105d76 <create>
801061bf:	89 45 f0             	mov    %eax,-0x10(%ebp)
801061c2:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801061c6:	75 0c                	jne    801061d4 <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    end_op();
801061c8:	e8 c7 d2 ff ff       	call   80103494 <end_op>
    return -1;
801061cd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801061d2:	eb 15                	jmp    801061e9 <sys_mknod+0xa6>
  }
  iunlockput(ip);
801061d4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801061d7:	89 04 24             	mov    %eax,(%esp)
801061da:	e8 00 b9 ff ff       	call   80101adf <iunlockput>
  end_op();
801061df:	e8 b0 d2 ff ff       	call   80103494 <end_op>
  return 0;
801061e4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801061e9:	c9                   	leave  
801061ea:	c3                   	ret    

801061eb <sys_chdir>:

int
sys_chdir(void)
{
801061eb:	55                   	push   %ebp
801061ec:	89 e5                	mov    %esp,%ebp
801061ee:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_op();
801061f1:	e8 1a d2 ff ff       	call   80103410 <begin_op>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
801061f6:	8d 45 f0             	lea    -0x10(%ebp),%eax
801061f9:	89 44 24 04          	mov    %eax,0x4(%esp)
801061fd:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106204:	e8 90 f3 ff ff       	call   80105599 <argstr>
80106209:	85 c0                	test   %eax,%eax
8010620b:	78 14                	js     80106221 <sys_chdir+0x36>
8010620d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106210:	89 04 24             	mov    %eax,(%esp)
80106213:	e8 ee c1 ff ff       	call   80102406 <namei>
80106218:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010621b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010621f:	75 0c                	jne    8010622d <sys_chdir+0x42>
    end_op();
80106221:	e8 6e d2 ff ff       	call   80103494 <end_op>
    return -1;
80106226:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010622b:	eb 61                	jmp    8010628e <sys_chdir+0xa3>
  }
  ilock(ip);
8010622d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106230:	89 04 24             	mov    %eax,(%esp)
80106233:	e8 23 b6 ff ff       	call   8010185b <ilock>
  if(ip->type != T_DIR){
80106238:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010623b:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010623f:	66 83 f8 01          	cmp    $0x1,%ax
80106243:	74 17                	je     8010625c <sys_chdir+0x71>
    iunlockput(ip);
80106245:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106248:	89 04 24             	mov    %eax,(%esp)
8010624b:	e8 8f b8 ff ff       	call   80101adf <iunlockput>
    end_op();
80106250:	e8 3f d2 ff ff       	call   80103494 <end_op>
    return -1;
80106255:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010625a:	eb 32                	jmp    8010628e <sys_chdir+0xa3>
  }
  iunlock(ip);
8010625c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010625f:	89 04 24             	mov    %eax,(%esp)
80106262:	e8 42 b7 ff ff       	call   801019a9 <iunlock>
  iput(proc->cwd);
80106267:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010626d:	8b 40 68             	mov    0x68(%eax),%eax
80106270:	89 04 24             	mov    %eax,(%esp)
80106273:	e8 96 b7 ff ff       	call   80101a0e <iput>
  end_op();
80106278:	e8 17 d2 ff ff       	call   80103494 <end_op>
  proc->cwd = ip;
8010627d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106283:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106286:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
80106289:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010628e:	c9                   	leave  
8010628f:	c3                   	ret    

80106290 <sys_exec>:

int
sys_exec(void)
{
80106290:	55                   	push   %ebp
80106291:	89 e5                	mov    %esp,%ebp
80106293:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80106299:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010629c:	89 44 24 04          	mov    %eax,0x4(%esp)
801062a0:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801062a7:	e8 ed f2 ff ff       	call   80105599 <argstr>
801062ac:	85 c0                	test   %eax,%eax
801062ae:	78 1a                	js     801062ca <sys_exec+0x3a>
801062b0:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
801062b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801062ba:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801062c1:	e8 43 f2 ff ff       	call   80105509 <argint>
801062c6:	85 c0                	test   %eax,%eax
801062c8:	79 0a                	jns    801062d4 <sys_exec+0x44>
    return -1;
801062ca:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062cf:	e9 c8 00 00 00       	jmp    8010639c <sys_exec+0x10c>
  }
  memset(argv, 0, sizeof(argv));
801062d4:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801062db:	00 
801062dc:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801062e3:	00 
801062e4:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
801062ea:	89 04 24             	mov    %eax,(%esp)
801062ed:	e8 d5 ee ff ff       	call   801051c7 <memset>
  for(i=0;; i++){
801062f2:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
801062f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801062fc:	83 f8 1f             	cmp    $0x1f,%eax
801062ff:	76 0a                	jbe    8010630b <sys_exec+0x7b>
      return -1;
80106301:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106306:	e9 91 00 00 00       	jmp    8010639c <sys_exec+0x10c>
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
8010630b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010630e:	c1 e0 02             	shl    $0x2,%eax
80106311:	89 c2                	mov    %eax,%edx
80106313:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
80106319:	01 c2                	add    %eax,%edx
8010631b:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
80106321:	89 44 24 04          	mov    %eax,0x4(%esp)
80106325:	89 14 24             	mov    %edx,(%esp)
80106328:	e8 40 f1 ff ff       	call   8010546d <fetchint>
8010632d:	85 c0                	test   %eax,%eax
8010632f:	79 07                	jns    80106338 <sys_exec+0xa8>
      return -1;
80106331:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106336:	eb 64                	jmp    8010639c <sys_exec+0x10c>
    if(uarg == 0){
80106338:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
8010633e:	85 c0                	test   %eax,%eax
80106340:	75 26                	jne    80106368 <sys_exec+0xd8>
      argv[i] = 0;
80106342:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106345:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
8010634c:	00 00 00 00 
      break;
80106350:	90                   	nop
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
80106351:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106354:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
8010635a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010635e:	89 04 24             	mov    %eax,(%esp)
80106361:	e8 89 a7 ff ff       	call   80100aef <exec>
80106366:	eb 34                	jmp    8010639c <sys_exec+0x10c>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
80106368:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
8010636e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106371:	c1 e2 02             	shl    $0x2,%edx
80106374:	01 c2                	add    %eax,%edx
80106376:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
8010637c:	89 54 24 04          	mov    %edx,0x4(%esp)
80106380:	89 04 24             	mov    %eax,(%esp)
80106383:	e8 1f f1 ff ff       	call   801054a7 <fetchstr>
80106388:	85 c0                	test   %eax,%eax
8010638a:	79 07                	jns    80106393 <sys_exec+0x103>
      return -1;
8010638c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106391:	eb 09                	jmp    8010639c <sys_exec+0x10c>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
80106393:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
80106397:	e9 5d ff ff ff       	jmp    801062f9 <sys_exec+0x69>
  return exec(path, argv);
}
8010639c:	c9                   	leave  
8010639d:	c3                   	ret    

8010639e <sys_pipe>:

int
sys_pipe(void)
{
8010639e:	55                   	push   %ebp
8010639f:	89 e5                	mov    %esp,%ebp
801063a1:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
801063a4:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801063ab:	00 
801063ac:	8d 45 ec             	lea    -0x14(%ebp),%eax
801063af:	89 44 24 04          	mov    %eax,0x4(%esp)
801063b3:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801063ba:	e8 78 f1 ff ff       	call   80105537 <argptr>
801063bf:	85 c0                	test   %eax,%eax
801063c1:	79 0a                	jns    801063cd <sys_pipe+0x2f>
    return -1;
801063c3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063c8:	e9 9b 00 00 00       	jmp    80106468 <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
801063cd:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801063d0:	89 44 24 04          	mov    %eax,0x4(%esp)
801063d4:	8d 45 e8             	lea    -0x18(%ebp),%eax
801063d7:	89 04 24             	mov    %eax,(%esp)
801063da:	e8 2a db ff ff       	call   80103f09 <pipealloc>
801063df:	85 c0                	test   %eax,%eax
801063e1:	79 07                	jns    801063ea <sys_pipe+0x4c>
    return -1;
801063e3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063e8:	eb 7e                	jmp    80106468 <sys_pipe+0xca>
  fd0 = -1;
801063ea:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
801063f1:	8b 45 e8             	mov    -0x18(%ebp),%eax
801063f4:	89 04 24             	mov    %eax,(%esp)
801063f7:	e8 67 f3 ff ff       	call   80105763 <fdalloc>
801063fc:	89 45 f4             	mov    %eax,-0xc(%ebp)
801063ff:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106403:	78 14                	js     80106419 <sys_pipe+0x7b>
80106405:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106408:	89 04 24             	mov    %eax,(%esp)
8010640b:	e8 53 f3 ff ff       	call   80105763 <fdalloc>
80106410:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106413:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106417:	79 37                	jns    80106450 <sys_pipe+0xb2>
    if(fd0 >= 0)
80106419:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010641d:	78 14                	js     80106433 <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
8010641f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106425:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106428:	83 c2 08             	add    $0x8,%edx
8010642b:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80106432:	00 
    fileclose(rf);
80106433:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106436:	89 04 24             	mov    %eax,(%esp)
80106439:	e8 90 ab ff ff       	call   80100fce <fileclose>
    fileclose(wf);
8010643e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106441:	89 04 24             	mov    %eax,(%esp)
80106444:	e8 85 ab ff ff       	call   80100fce <fileclose>
    return -1;
80106449:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010644e:	eb 18                	jmp    80106468 <sys_pipe+0xca>
  }
  fd[0] = fd0;
80106450:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106453:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106456:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
80106458:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010645b:	8d 50 04             	lea    0x4(%eax),%edx
8010645e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106461:	89 02                	mov    %eax,(%edx)
  return 0;
80106463:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106468:	c9                   	leave  
80106469:	c3                   	ret    

8010646a <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
8010646a:	55                   	push   %ebp
8010646b:	89 e5                	mov    %esp,%ebp
8010646d:	83 ec 08             	sub    $0x8,%esp
  return fork();
80106470:	e8 86 e1 ff ff       	call   801045fb <fork>
}
80106475:	c9                   	leave  
80106476:	c3                   	ret    

80106477 <sys_exit>:

int
sys_exit(void)
{
80106477:	55                   	push   %ebp
80106478:	89 e5                	mov    %esp,%ebp
8010647a:	83 ec 08             	sub    $0x8,%esp
  exit();
8010647d:	e8 f4 e2 ff ff       	call   80104776 <exit>
  return 0;  // not reached
80106482:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106487:	c9                   	leave  
80106488:	c3                   	ret    

80106489 <sys_wait>:

int
sys_wait(void)
{
80106489:	55                   	push   %ebp
8010648a:	89 e5                	mov    %esp,%ebp
8010648c:	83 ec 08             	sub    $0x8,%esp
  return wait();
8010648f:	e8 07 e4 ff ff       	call   8010489b <wait>
}
80106494:	c9                   	leave  
80106495:	c3                   	ret    

80106496 <sys_kill>:

int
sys_kill(void)
{
80106496:	55                   	push   %ebp
80106497:	89 e5                	mov    %esp,%ebp
80106499:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
8010649c:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010649f:	89 44 24 04          	mov    %eax,0x4(%esp)
801064a3:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801064aa:	e8 5a f0 ff ff       	call   80105509 <argint>
801064af:	85 c0                	test   %eax,%eax
801064b1:	79 07                	jns    801064ba <sys_kill+0x24>
    return -1;
801064b3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801064b8:	eb 0b                	jmp    801064c5 <sys_kill+0x2f>
  return kill(pid);
801064ba:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064bd:	89 04 24             	mov    %eax,(%esp)
801064c0:	e8 9a e7 ff ff       	call   80104c5f <kill>
}
801064c5:	c9                   	leave  
801064c6:	c3                   	ret    

801064c7 <sys_getpid>:

int
sys_getpid(void)
{
801064c7:	55                   	push   %ebp
801064c8:	89 e5                	mov    %esp,%ebp
  return proc->pid;
801064ca:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801064d0:	8b 40 10             	mov    0x10(%eax),%eax
}
801064d3:	5d                   	pop    %ebp
801064d4:	c3                   	ret    

801064d5 <sys_sbrk>:

int
sys_sbrk(void)
{
801064d5:	55                   	push   %ebp
801064d6:	89 e5                	mov    %esp,%ebp
801064d8:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
801064db:	8d 45 f0             	lea    -0x10(%ebp),%eax
801064de:	89 44 24 04          	mov    %eax,0x4(%esp)
801064e2:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801064e9:	e8 1b f0 ff ff       	call   80105509 <argint>
801064ee:	85 c0                	test   %eax,%eax
801064f0:	79 07                	jns    801064f9 <sys_sbrk+0x24>
    return -1;
801064f2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801064f7:	eb 24                	jmp    8010651d <sys_sbrk+0x48>
  addr = proc->sz;
801064f9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801064ff:	8b 00                	mov    (%eax),%eax
80106501:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
80106504:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106507:	89 04 24             	mov    %eax,(%esp)
8010650a:	e8 47 e0 ff ff       	call   80104556 <growproc>
8010650f:	85 c0                	test   %eax,%eax
80106511:	79 07                	jns    8010651a <sys_sbrk+0x45>
    return -1;
80106513:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106518:	eb 03                	jmp    8010651d <sys_sbrk+0x48>
  return addr;
8010651a:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010651d:	c9                   	leave  
8010651e:	c3                   	ret    

8010651f <sys_sleep>:

int
sys_sleep(void)
{
8010651f:	55                   	push   %ebp
80106520:	89 e5                	mov    %esp,%ebp
80106522:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
80106525:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106528:	89 44 24 04          	mov    %eax,0x4(%esp)
8010652c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106533:	e8 d1 ef ff ff       	call   80105509 <argint>
80106538:	85 c0                	test   %eax,%eax
8010653a:	79 07                	jns    80106543 <sys_sleep+0x24>
    return -1;
8010653c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106541:	eb 6c                	jmp    801065af <sys_sleep+0x90>
  acquire(&tickslock);
80106543:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
8010654a:	e8 24 ea ff ff       	call   80104f73 <acquire>
  ticks0 = ticks;
8010654f:	a1 20 56 11 80       	mov    0x80115620,%eax
80106554:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
80106557:	eb 34                	jmp    8010658d <sys_sleep+0x6e>
    if(proc->killed){
80106559:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010655f:	8b 40 24             	mov    0x24(%eax),%eax
80106562:	85 c0                	test   %eax,%eax
80106564:	74 13                	je     80106579 <sys_sleep+0x5a>
      release(&tickslock);
80106566:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
8010656d:	e8 63 ea ff ff       	call   80104fd5 <release>
      return -1;
80106572:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106577:	eb 36                	jmp    801065af <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
80106579:	c7 44 24 04 e0 4d 11 	movl   $0x80114de0,0x4(%esp)
80106580:	80 
80106581:	c7 04 24 20 56 11 80 	movl   $0x80115620,(%esp)
80106588:	e8 cb e5 ff ff       	call   80104b58 <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
8010658d:	a1 20 56 11 80       	mov    0x80115620,%eax
80106592:	2b 45 f4             	sub    -0xc(%ebp),%eax
80106595:	89 c2                	mov    %eax,%edx
80106597:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010659a:	39 c2                	cmp    %eax,%edx
8010659c:	72 bb                	jb     80106559 <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
8010659e:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
801065a5:	e8 2b ea ff ff       	call   80104fd5 <release>
  return 0;
801065aa:	b8 00 00 00 00       	mov    $0x0,%eax
}
801065af:	c9                   	leave  
801065b0:	c3                   	ret    

801065b1 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
801065b1:	55                   	push   %ebp
801065b2:	89 e5                	mov    %esp,%ebp
801065b4:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
801065b7:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
801065be:	e8 b0 e9 ff ff       	call   80104f73 <acquire>
  xticks = ticks;
801065c3:	a1 20 56 11 80       	mov    0x80115620,%eax
801065c8:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
801065cb:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
801065d2:	e8 fe e9 ff ff       	call   80104fd5 <release>
  return xticks;
801065d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801065da:	c9                   	leave  
801065db:	c3                   	ret    

801065dc <sys_getproc>:

int sys_getproc(void)
{
801065dc:	55                   	push   %ebp
801065dd:	89 e5                	mov    %esp,%ebp
801065df:	83 ec 28             	sub    $0x28,%esp
        int sadd;

        if(argint(0, &sadd) < 0)
801065e2:	8d 45 f4             	lea    -0xc(%ebp),%eax
801065e5:	89 44 24 04          	mov    %eax,0x4(%esp)
801065e9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801065f0:	e8 14 ef ff ff       	call   80105509 <argint>
801065f5:	85 c0                	test   %eax,%eax
801065f7:	79 07                	jns    80106600 <sys_getproc+0x24>
                return -1;
801065f9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801065fe:	eb 0b                	jmp    8010660b <sys_getproc+0x2f>
        return(getproc(sadd));
80106600:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106603:	89 04 24             	mov    %eax,(%esp)
80106606:	e8 c9 e7 ff ff       	call   80104dd4 <getproc>
}
8010660b:	c9                   	leave  
8010660c:	c3                   	ret    

8010660d <strcmp>:

int
strcmp(const char *p, const char *q)
{
8010660d:	55                   	push   %ebp
8010660e:	89 e5                	mov    %esp,%ebp
        while(*p && *p == *q)
80106610:	eb 08                	jmp    8010661a <strcmp+0xd>
        p++, q++;
80106612:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80106616:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strcmp(const char *p, const char *q)
{
        while(*p && *p == *q)
8010661a:	8b 45 08             	mov    0x8(%ebp),%eax
8010661d:	0f b6 00             	movzbl (%eax),%eax
80106620:	84 c0                	test   %al,%al
80106622:	74 10                	je     80106634 <strcmp+0x27>
80106624:	8b 45 08             	mov    0x8(%ebp),%eax
80106627:	0f b6 10             	movzbl (%eax),%edx
8010662a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010662d:	0f b6 00             	movzbl (%eax),%eax
80106630:	38 c2                	cmp    %al,%dl
80106632:	74 de                	je     80106612 <strcmp+0x5>
        p++, q++;
        return (uchar)*p - (uchar)*q;
80106634:	8b 45 08             	mov    0x8(%ebp),%eax
80106637:	0f b6 00             	movzbl (%eax),%eax
8010663a:	0f b6 d0             	movzbl %al,%edx
8010663d:	8b 45 0c             	mov    0xc(%ebp),%eax
80106640:	0f b6 00             	movzbl (%eax),%eax
80106643:	0f b6 c0             	movzbl %al,%eax
80106646:	29 c2                	sub    %eax,%edx
80106648:	89 d0                	mov    %edx,%eax
}
8010664a:	5d                   	pop    %ebp
8010664b:	c3                   	ret    

8010664c <mygetproc>:

int mygetproc(int sadd)
{
8010664c:	55                   	push   %ebp
8010664d:	89 e5                	mov    %esp,%ebp
8010664f:	83 ec 28             	sub    $0x28,%esp
        {
                int id;
                char state[10];
                char name[16];
        } *p;
        p= (struct procentry*)sadd;
80106652:	8b 45 08             	mov    0x8(%ebp),%eax
80106655:	89 45 ec             	mov    %eax,-0x14(%ebp)
        no=getproc(sadd);
80106658:	8b 45 08             	mov    0x8(%ebp),%eax
8010665b:	89 04 24             	mov    %eax,(%esp)
8010665e:	e8 71 e7 ff ff       	call   80104dd4 <getproc>
80106663:	89 45 e8             	mov    %eax,-0x18(%ebp)
        num=no;
80106666:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106669:	89 45 f0             	mov    %eax,-0x10(%ebp)
        for (i=0;i<no;i++)
8010666c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106673:	eb 69                	jmp    801066de <mygetproc+0x92>
        {
                if (!strcmp(p->name,"badproc"))
80106675:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106678:	83 c0 0e             	add    $0xe,%eax
8010667b:	c7 44 24 04 b7 8d 10 	movl   $0x80108db7,0x4(%esp)
80106682:	80 
80106683:	89 04 24             	mov    %eax,(%esp)
80106686:	e8 82 ff ff ff       	call   8010660d <strcmp>
8010668b:	85 c0                	test   %eax,%eax
8010668d:	75 47                	jne    801066d6 <mygetproc+0x8a>
                {
                        *p=*(p+(no-1));
8010668f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106692:	05 ff ff ff 07       	add    $0x7ffffff,%eax
80106697:	c1 e0 05             	shl    $0x5,%eax
8010669a:	89 c2                	mov    %eax,%edx
8010669c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010669f:	01 c2                	add    %eax,%edx
801066a1:	8b 45 ec             	mov    -0x14(%ebp),%eax
801066a4:	8b 0a                	mov    (%edx),%ecx
801066a6:	89 08                	mov    %ecx,(%eax)
801066a8:	8b 4a 04             	mov    0x4(%edx),%ecx
801066ab:	89 48 04             	mov    %ecx,0x4(%eax)
801066ae:	8b 4a 08             	mov    0x8(%edx),%ecx
801066b1:	89 48 08             	mov    %ecx,0x8(%eax)
801066b4:	8b 4a 0c             	mov    0xc(%edx),%ecx
801066b7:	89 48 0c             	mov    %ecx,0xc(%eax)
801066ba:	8b 4a 10             	mov    0x10(%edx),%ecx
801066bd:	89 48 10             	mov    %ecx,0x10(%eax)
801066c0:	8b 4a 14             	mov    0x14(%edx),%ecx
801066c3:	89 48 14             	mov    %ecx,0x14(%eax)
801066c6:	8b 4a 18             	mov    0x18(%edx),%ecx
801066c9:	89 48 18             	mov    %ecx,0x18(%eax)
801066cc:	8b 52 1c             	mov    0x1c(%edx),%edx
801066cf:	89 50 1c             	mov    %edx,0x1c(%eax)
                        num--;
801066d2:	83 6d f0 01          	subl   $0x1,-0x10(%ebp)
                }
        p++;
801066d6:	83 45 ec 20          	addl   $0x20,-0x14(%ebp)
                char name[16];
        } *p;
        p= (struct procentry*)sadd;
        no=getproc(sadd);
        num=no;
        for (i=0;i<no;i++)
801066da:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801066de:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066e1:	3b 45 e8             	cmp    -0x18(%ebp),%eax
801066e4:	7c 8f                	jl     80106675 <mygetproc+0x29>
                        *p=*(p+(no-1));
                        num--;
                }
        p++;
        }
        return num;
801066e6:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
801066e9:	c9                   	leave  
801066ea:	c3                   	ret    

801066eb <sys_mygetproc>:

int sys_mygetproc(void)
{
801066eb:	55                   	push   %ebp
801066ec:	89 e5                	mov    %esp,%ebp
801066ee:	83 ec 28             	sub    $0x28,%esp
	int sadd;

        if(argint(0, &sadd) < 0)
801066f1:	8d 45 f4             	lea    -0xc(%ebp),%eax
801066f4:	89 44 24 04          	mov    %eax,0x4(%esp)
801066f8:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801066ff:	e8 05 ee ff ff       	call   80105509 <argint>
80106704:	85 c0                	test   %eax,%eax
80106706:	79 07                	jns    8010670f <sys_mygetproc+0x24>
                return -1;
80106708:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010670d:	eb 0b                	jmp    8010671a <sys_mygetproc+0x2f>
        return(mygetproc(sadd));
8010670f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106712:	89 04 24             	mov    %eax,(%esp)
80106715:	e8 32 ff ff ff       	call   8010664c <mygetproc>
}
8010671a:	c9                   	leave  
8010671b:	c3                   	ret    

8010671c <sys_findkalloc>:

int sys_findkalloc(void)
{
8010671c:	55                   	push   %ebp
8010671d:	89 e5                	mov    %esp,%ebp
//	char* kalloc_addr(void)=kalloc;
	return((int)kalloc);
8010671f:	b8 e2 2a 10 80       	mov    $0x80102ae2,%eax
}
80106724:	5d                   	pop    %ebp
80106725:	c3                   	ret    

80106726 <sys_signal>:

int sys_signal(void)
{
80106726:	55                   	push   %ebp
80106727:	89 e5                	mov    %esp,%ebp
80106729:	83 ec 28             	sub    $0x28,%esp
	int num,handler;
	if(argint(0,&num)<0)
8010672c:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010672f:	89 44 24 04          	mov    %eax,0x4(%esp)
80106733:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010673a:	e8 ca ed ff ff       	call   80105509 <argint>
8010673f:	85 c0                	test   %eax,%eax
80106741:	79 07                	jns    8010674a <sys_signal+0x24>
		return -1;
80106743:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106748:	eb 32                	jmp    8010677c <sys_signal+0x56>
	if(argint(1,&handler)<0)
8010674a:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010674d:	89 44 24 04          	mov    %eax,0x4(%esp)
80106751:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106758:	e8 ac ed ff ff       	call   80105509 <argint>
8010675d:	85 c0                	test   %eax,%eax
8010675f:	79 07                	jns    80106768 <sys_signal+0x42>
		return -1;
80106761:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106766:	eb 14                	jmp    8010677c <sys_signal+0x56>
	return(signal(num,(sighandler_t)handler));
80106768:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010676b:	89 c2                	mov    %eax,%edx
8010676d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106770:	89 54 24 04          	mov    %edx,0x4(%esp)
80106774:	89 04 24             	mov    %eax,(%esp)
80106777:	e8 69 e7 ff ff       	call   80104ee5 <signal>
}
8010677c:	c9                   	leave  
8010677d:	c3                   	ret    

8010677e <sys_alarm>:

int sys_alarm(void)
{
8010677e:	55                   	push   %ebp
8010677f:	89 e5                	mov    %esp,%ebp
80106781:	83 ec 28             	sub    $0x28,%esp
	int val;
	if(argint(0,&val)<0)
80106784:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106787:	89 44 24 04          	mov    %eax,0x4(%esp)
8010678b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106792:	e8 72 ed ff ff       	call   80105509 <argint>
80106797:	85 c0                	test   %eax,%eax
80106799:	79 07                	jns    801067a2 <sys_alarm+0x24>
		return -1;
8010679b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801067a0:	eb 0b                	jmp    801067ad <sys_alarm+0x2f>
	return(alarm(val));
801067a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067a5:	89 04 24             	mov    %eax,(%esp)
801067a8:	e8 58 e7 ff ff       	call   80104f05 <alarm>
}
801067ad:	c9                   	leave  
801067ae:	c3                   	ret    

801067af <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801067af:	55                   	push   %ebp
801067b0:	89 e5                	mov    %esp,%ebp
801067b2:	83 ec 08             	sub    $0x8,%esp
801067b5:	8b 55 08             	mov    0x8(%ebp),%edx
801067b8:	8b 45 0c             	mov    0xc(%ebp),%eax
801067bb:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801067bf:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801067c2:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801067c6:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801067ca:	ee                   	out    %al,(%dx)
}
801067cb:	c9                   	leave  
801067cc:	c3                   	ret    

801067cd <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
801067cd:	55                   	push   %ebp
801067ce:	89 e5                	mov    %esp,%ebp
801067d0:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
801067d3:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
801067da:	00 
801067db:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
801067e2:	e8 c8 ff ff ff       	call   801067af <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
801067e7:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
801067ee:	00 
801067ef:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
801067f6:	e8 b4 ff ff ff       	call   801067af <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
801067fb:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
80106802:	00 
80106803:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
8010680a:	e8 a0 ff ff ff       	call   801067af <outb>
  picenable(IRQ_TIMER);
8010680f:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106816:	e8 81 d5 ff ff       	call   80103d9c <picenable>
}
8010681b:	c9                   	leave  
8010681c:	c3                   	ret    

8010681d <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
8010681d:	1e                   	push   %ds
  pushl %es
8010681e:	06                   	push   %es
  pushl %fs
8010681f:	0f a0                	push   %fs
  pushl %gs
80106821:	0f a8                	push   %gs
  pushal
80106823:	60                   	pusha  
  
  # Set up data and per-cpu segments.
  movw $(SEG_KDATA<<3), %ax
80106824:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
80106828:	8e d8                	mov    %eax,%ds
  movw %ax, %es
8010682a:	8e c0                	mov    %eax,%es
  movw $(SEG_KCPU<<3), %ax
8010682c:	66 b8 18 00          	mov    $0x18,%ax
  movw %ax, %fs
80106830:	8e e0                	mov    %eax,%fs
  movw %ax, %gs
80106832:	8e e8                	mov    %eax,%gs

  # Call trap(tf), where tf=%esp
  pushl %esp
80106834:	54                   	push   %esp
  call trap
80106835:	e8 d8 01 00 00       	call   80106a12 <trap>
  addl $4, %esp
8010683a:	83 c4 04             	add    $0x4,%esp

8010683d <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
8010683d:	61                   	popa   
  popl %gs
8010683e:	0f a9                	pop    %gs
  popl %fs
80106840:	0f a1                	pop    %fs
  popl %es
80106842:	07                   	pop    %es
  popl %ds
80106843:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
80106844:	83 c4 08             	add    $0x8,%esp
  iret
80106847:	cf                   	iret   

80106848 <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
80106848:	55                   	push   %ebp
80106849:	89 e5                	mov    %esp,%ebp
8010684b:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
8010684e:	8b 45 0c             	mov    0xc(%ebp),%eax
80106851:	83 e8 01             	sub    $0x1,%eax
80106854:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80106858:	8b 45 08             	mov    0x8(%ebp),%eax
8010685b:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
8010685f:	8b 45 08             	mov    0x8(%ebp),%eax
80106862:	c1 e8 10             	shr    $0x10,%eax
80106865:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
80106869:	8d 45 fa             	lea    -0x6(%ebp),%eax
8010686c:	0f 01 18             	lidtl  (%eax)
}
8010686f:	c9                   	leave  
80106870:	c3                   	ret    

80106871 <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
80106871:	55                   	push   %ebp
80106872:	89 e5                	mov    %esp,%ebp
80106874:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80106877:	0f 20 d0             	mov    %cr2,%eax
8010687a:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return val;
8010687d:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80106880:	c9                   	leave  
80106881:	c3                   	ret    

80106882 <tvinit>:
uint ticks;
struct proc *p;

void
tvinit(void)
{
80106882:	55                   	push   %ebp
80106883:	89 e5                	mov    %esp,%ebp
80106885:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
80106888:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010688f:	e9 c3 00 00 00       	jmp    80106957 <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
80106894:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106897:	8b 04 85 ac b0 10 80 	mov    -0x7fef4f54(,%eax,4),%eax
8010689e:	89 c2                	mov    %eax,%edx
801068a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068a3:	66 89 14 c5 20 4e 11 	mov    %dx,-0x7feeb1e0(,%eax,8)
801068aa:	80 
801068ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068ae:	66 c7 04 c5 22 4e 11 	movw   $0x8,-0x7feeb1de(,%eax,8)
801068b5:	80 08 00 
801068b8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068bb:	0f b6 14 c5 24 4e 11 	movzbl -0x7feeb1dc(,%eax,8),%edx
801068c2:	80 
801068c3:	83 e2 e0             	and    $0xffffffe0,%edx
801068c6:	88 14 c5 24 4e 11 80 	mov    %dl,-0x7feeb1dc(,%eax,8)
801068cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068d0:	0f b6 14 c5 24 4e 11 	movzbl -0x7feeb1dc(,%eax,8),%edx
801068d7:	80 
801068d8:	83 e2 1f             	and    $0x1f,%edx
801068db:	88 14 c5 24 4e 11 80 	mov    %dl,-0x7feeb1dc(,%eax,8)
801068e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068e5:	0f b6 14 c5 25 4e 11 	movzbl -0x7feeb1db(,%eax,8),%edx
801068ec:	80 
801068ed:	83 e2 f0             	and    $0xfffffff0,%edx
801068f0:	83 ca 0e             	or     $0xe,%edx
801068f3:	88 14 c5 25 4e 11 80 	mov    %dl,-0x7feeb1db(,%eax,8)
801068fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068fd:	0f b6 14 c5 25 4e 11 	movzbl -0x7feeb1db(,%eax,8),%edx
80106904:	80 
80106905:	83 e2 ef             	and    $0xffffffef,%edx
80106908:	88 14 c5 25 4e 11 80 	mov    %dl,-0x7feeb1db(,%eax,8)
8010690f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106912:	0f b6 14 c5 25 4e 11 	movzbl -0x7feeb1db(,%eax,8),%edx
80106919:	80 
8010691a:	83 e2 9f             	and    $0xffffff9f,%edx
8010691d:	88 14 c5 25 4e 11 80 	mov    %dl,-0x7feeb1db(,%eax,8)
80106924:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106927:	0f b6 14 c5 25 4e 11 	movzbl -0x7feeb1db(,%eax,8),%edx
8010692e:	80 
8010692f:	83 ca 80             	or     $0xffffff80,%edx
80106932:	88 14 c5 25 4e 11 80 	mov    %dl,-0x7feeb1db(,%eax,8)
80106939:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010693c:	8b 04 85 ac b0 10 80 	mov    -0x7fef4f54(,%eax,4),%eax
80106943:	c1 e8 10             	shr    $0x10,%eax
80106946:	89 c2                	mov    %eax,%edx
80106948:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010694b:	66 89 14 c5 26 4e 11 	mov    %dx,-0x7feeb1da(,%eax,8)
80106952:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
80106953:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106957:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
8010695e:	0f 8e 30 ff ff ff    	jle    80106894 <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80106964:	a1 ac b1 10 80       	mov    0x8010b1ac,%eax
80106969:	66 a3 20 50 11 80    	mov    %ax,0x80115020
8010696f:	66 c7 05 22 50 11 80 	movw   $0x8,0x80115022
80106976:	08 00 
80106978:	0f b6 05 24 50 11 80 	movzbl 0x80115024,%eax
8010697f:	83 e0 e0             	and    $0xffffffe0,%eax
80106982:	a2 24 50 11 80       	mov    %al,0x80115024
80106987:	0f b6 05 24 50 11 80 	movzbl 0x80115024,%eax
8010698e:	83 e0 1f             	and    $0x1f,%eax
80106991:	a2 24 50 11 80       	mov    %al,0x80115024
80106996:	0f b6 05 25 50 11 80 	movzbl 0x80115025,%eax
8010699d:	83 c8 0f             	or     $0xf,%eax
801069a0:	a2 25 50 11 80       	mov    %al,0x80115025
801069a5:	0f b6 05 25 50 11 80 	movzbl 0x80115025,%eax
801069ac:	83 e0 ef             	and    $0xffffffef,%eax
801069af:	a2 25 50 11 80       	mov    %al,0x80115025
801069b4:	0f b6 05 25 50 11 80 	movzbl 0x80115025,%eax
801069bb:	83 c8 60             	or     $0x60,%eax
801069be:	a2 25 50 11 80       	mov    %al,0x80115025
801069c3:	0f b6 05 25 50 11 80 	movzbl 0x80115025,%eax
801069ca:	83 c8 80             	or     $0xffffff80,%eax
801069cd:	a2 25 50 11 80       	mov    %al,0x80115025
801069d2:	a1 ac b1 10 80       	mov    0x8010b1ac,%eax
801069d7:	c1 e8 10             	shr    $0x10,%eax
801069da:	66 a3 26 50 11 80    	mov    %ax,0x80115026
  
  initlock(&tickslock, "time");
801069e0:	c7 44 24 04 c0 8d 10 	movl   $0x80108dc0,0x4(%esp)
801069e7:	80 
801069e8:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
801069ef:	e8 5e e5 ff ff       	call   80104f52 <initlock>
}
801069f4:	c9                   	leave  
801069f5:	c3                   	ret    

801069f6 <idtinit>:

void
idtinit(void)
{
801069f6:	55                   	push   %ebp
801069f7:	89 e5                	mov    %esp,%ebp
801069f9:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
801069fc:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
80106a03:	00 
80106a04:	c7 04 24 20 4e 11 80 	movl   $0x80114e20,(%esp)
80106a0b:	e8 38 fe ff ff       	call   80106848 <lidt>
}
80106a10:	c9                   	leave  
80106a11:	c3                   	ret    

80106a12 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
80106a12:	55                   	push   %ebp
80106a13:	89 e5                	mov    %esp,%ebp
80106a15:	57                   	push   %edi
80106a16:	56                   	push   %esi
80106a17:	53                   	push   %ebx
80106a18:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
80106a1b:	8b 45 08             	mov    0x8(%ebp),%eax
80106a1e:	8b 40 30             	mov    0x30(%eax),%eax
80106a21:	83 f8 40             	cmp    $0x40,%eax
80106a24:	75 3f                	jne    80106a65 <trap+0x53>
    if(proc->killed)
80106a26:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a2c:	8b 40 24             	mov    0x24(%eax),%eax
80106a2f:	85 c0                	test   %eax,%eax
80106a31:	74 05                	je     80106a38 <trap+0x26>
      exit();
80106a33:	e8 3e dd ff ff       	call   80104776 <exit>
    proc->tf = tf;
80106a38:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a3e:	8b 55 08             	mov    0x8(%ebp),%edx
80106a41:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
80106a44:	e8 87 eb ff ff       	call   801055d0 <syscall>
    if(proc->killed)
80106a49:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a4f:	8b 40 24             	mov    0x24(%eax),%eax
80106a52:	85 c0                	test   %eax,%eax
80106a54:	74 0a                	je     80106a60 <trap+0x4e>
      exit();
80106a56:	e8 1b dd ff ff       	call   80104776 <exit>
    return;
80106a5b:	e9 c3 03 00 00       	jmp    80106e23 <trap+0x411>
80106a60:	e9 be 03 00 00       	jmp    80106e23 <trap+0x411>
  }

  switch(tf->trapno){
80106a65:	8b 45 08             	mov    0x8(%ebp),%eax
80106a68:	8b 40 30             	mov    0x30(%eax),%eax
80106a6b:	83 e8 20             	sub    $0x20,%eax
80106a6e:	83 f8 1f             	cmp    $0x1f,%eax
80106a71:	0f 87 e5 01 00 00    	ja     80106c5c <trap+0x24a>
80106a77:	8b 04 85 68 8e 10 80 	mov    -0x7fef7198(,%eax,4),%eax
80106a7e:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
80106a80:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106a86:	0f b6 00             	movzbl (%eax),%eax
80106a89:	84 c0                	test   %al,%al
80106a8b:	75 31                	jne    80106abe <trap+0xac>
      acquire(&tickslock);
80106a8d:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
80106a94:	e8 da e4 ff ff       	call   80104f73 <acquire>
      ticks++;
80106a99:	a1 20 56 11 80       	mov    0x80115620,%eax
80106a9e:	83 c0 01             	add    $0x1,%eax
80106aa1:	a3 20 56 11 80       	mov    %eax,0x80115620
      wakeup(&ticks);
80106aa6:	c7 04 24 20 56 11 80 	movl   $0x80115620,(%esp)
80106aad:	e8 82 e1 ff ff       	call   80104c34 <wakeup>
      release(&tickslock);
80106ab2:	c7 04 24 e0 4d 11 80 	movl   $0x80114de0,(%esp)
80106ab9:	e8 17 e5 ff ff       	call   80104fd5 <release>
    }
    acquire(&ptable.lock);
80106abe:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80106ac5:	e8 a9 e4 ff ff       	call   80104f73 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80106aca:	c7 05 c0 4d 11 80 b4 	movl   $0x801129b4,0x80114dc0
80106ad1:	29 11 80 
80106ad4:	eb 60                	jmp    80106b36 <trap+0x124>
    {
    	if(p->sighandle[2] != ((sighandler_t)-1) && p->ticks)
80106ad6:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106adb:	8b 80 84 00 00 00    	mov    0x84(%eax),%eax
80106ae1:	83 f8 ff             	cmp    $0xffffffff,%eax
80106ae4:	74 41                	je     80106b27 <trap+0x115>
80106ae6:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106aeb:	8b 80 88 00 00 00    	mov    0x88(%eax),%eax
80106af1:	85 c0                	test   %eax,%eax
80106af3:	74 32                	je     80106b27 <trap+0x115>
	{
		p->ticks--;
80106af5:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106afa:	8b 90 88 00 00 00    	mov    0x88(%eax),%edx
80106b00:	83 ea 01             	sub    $0x1,%edx
80106b03:	89 90 88 00 00 00    	mov    %edx,0x88(%eax)
		if(p->ticks==0)
80106b09:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106b0e:	8b 80 88 00 00 00    	mov    0x88(%eax),%eax
80106b14:	85 c0                	test   %eax,%eax
80106b16:	75 0f                	jne    80106b27 <trap+0x115>
			p->flag=1;
80106b18:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106b1d:	c7 80 8c 00 00 00 01 	movl   $0x1,0x8c(%eax)
80106b24:	00 00 00 
      ticks++;
      wakeup(&ticks);
      release(&tickslock);
    }
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80106b27:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106b2c:	05 90 00 00 00       	add    $0x90,%eax
80106b31:	a3 c0 4d 11 80       	mov    %eax,0x80114dc0
80106b36:	a1 c0 4d 11 80       	mov    0x80114dc0,%eax
80106b3b:	3d b4 4d 11 80       	cmp    $0x80114db4,%eax
80106b40:	72 94                	jb     80106ad6 <trap+0xc4>
		p->ticks--;
		if(p->ticks==0)
			p->flag=1;
	}
    }
    release(&ptable.lock);
80106b42:	c7 04 24 80 29 11 80 	movl   $0x80112980,(%esp)
80106b49:	e8 87 e4 ff ff       	call   80104fd5 <release>
    if( (tf->cs&3)==3 && proc->flag && proc->sighandle[2]!=(sighandler_t)-1)
80106b4e:	8b 45 08             	mov    0x8(%ebp),%eax
80106b51:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106b55:	0f b7 c0             	movzwl %ax,%eax
80106b58:	83 e0 03             	and    $0x3,%eax
80106b5b:	83 f8 03             	cmp    $0x3,%eax
80106b5e:	0f 85 83 00 00 00    	jne    80106be7 <trap+0x1d5>
80106b64:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b6a:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
80106b70:	85 c0                	test   %eax,%eax
80106b72:	74 73                	je     80106be7 <trap+0x1d5>
80106b74:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b7a:	8b 80 84 00 00 00    	mov    0x84(%eax),%eax
80106b80:	83 f8 ff             	cmp    $0xffffffff,%eax
80106b83:	74 62                	je     80106be7 <trap+0x1d5>
    {
	tf->esp-=4;
80106b85:	8b 45 08             	mov    0x8(%ebp),%eax
80106b88:	8b 40 44             	mov    0x44(%eax),%eax
80106b8b:	8d 50 fc             	lea    -0x4(%eax),%edx
80106b8e:	8b 45 08             	mov    0x8(%ebp),%eax
80106b91:	89 50 44             	mov    %edx,0x44(%eax)
        *((int*)tf->esp)=(uint)proc->sighandle[2];
80106b94:	8b 45 08             	mov    0x8(%ebp),%eax
80106b97:	8b 40 44             	mov    0x44(%eax),%eax
80106b9a:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80106ba1:	8b 92 84 00 00 00    	mov    0x84(%edx),%edx
80106ba7:	89 10                	mov    %edx,(%eax)
	tf->esp-=4;
80106ba9:	8b 45 08             	mov    0x8(%ebp),%eax
80106bac:	8b 40 44             	mov    0x44(%eax),%eax
80106baf:	8d 50 fc             	lea    -0x4(%eax),%edx
80106bb2:	8b 45 08             	mov    0x8(%ebp),%eax
80106bb5:	89 50 44             	mov    %edx,0x44(%eax)
        *((int*)tf->esp)=(uint)tf->eip;
80106bb8:	8b 45 08             	mov    0x8(%ebp),%eax
80106bbb:	8b 40 44             	mov    0x44(%eax),%eax
80106bbe:	8b 55 08             	mov    0x8(%ebp),%edx
80106bc1:	8b 52 38             	mov    0x38(%edx),%edx
80106bc4:	89 10                	mov    %edx,(%eax)
      	tf->eip=(uint)proc->sighandle[0];
80106bc6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106bcc:	8b 40 7c             	mov    0x7c(%eax),%eax
80106bcf:	89 c2                	mov    %eax,%edx
80106bd1:	8b 45 08             	mov    0x8(%ebp),%eax
80106bd4:	89 50 38             	mov    %edx,0x38(%eax)
	proc->flag=0;
80106bd7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106bdd:	c7 80 8c 00 00 00 00 	movl   $0x0,0x8c(%eax)
80106be4:	00 00 00 
        
    }

    lapiceoi();
80106be7:	e8 e4 c2 ff ff       	call   80102ed0 <lapiceoi>
    break;
80106bec:	e9 ae 01 00 00       	jmp    80106d9f <trap+0x38d>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
80106bf1:	e8 e8 ba ff ff       	call   801026de <ideintr>
    lapiceoi();
80106bf6:	e8 d5 c2 ff ff       	call   80102ed0 <lapiceoi>
    break;
80106bfb:	e9 9f 01 00 00       	jmp    80106d9f <trap+0x38d>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
80106c00:	e8 9a c0 ff ff       	call   80102c9f <kbdintr>
    lapiceoi();
80106c05:	e8 c6 c2 ff ff       	call   80102ed0 <lapiceoi>
    break;
80106c0a:	e9 90 01 00 00       	jmp    80106d9f <trap+0x38d>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
80106c0f:	e8 04 04 00 00       	call   80107018 <uartintr>
    lapiceoi();
80106c14:	e8 b7 c2 ff ff       	call   80102ed0 <lapiceoi>
    break;
80106c19:	e9 81 01 00 00       	jmp    80106d9f <trap+0x38d>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106c1e:	8b 45 08             	mov    0x8(%ebp),%eax
80106c21:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
80106c24:	8b 45 08             	mov    0x8(%ebp),%eax
80106c27:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106c2b:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
80106c2e:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106c34:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106c37:	0f b6 c0             	movzbl %al,%eax
80106c3a:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106c3e:	89 54 24 08          	mov    %edx,0x8(%esp)
80106c42:	89 44 24 04          	mov    %eax,0x4(%esp)
80106c46:	c7 04 24 c8 8d 10 80 	movl   $0x80108dc8,(%esp)
80106c4d:	e8 4e 97 ff ff       	call   801003a0 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
80106c52:	e8 79 c2 ff ff       	call   80102ed0 <lapiceoi>
    break;
80106c57:	e9 43 01 00 00       	jmp    80106d9f <trap+0x38d>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
80106c5c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c62:	85 c0                	test   %eax,%eax
80106c64:	74 11                	je     80106c77 <trap+0x265>
80106c66:	8b 45 08             	mov    0x8(%ebp),%eax
80106c69:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106c6d:	0f b7 c0             	movzwl %ax,%eax
80106c70:	83 e0 03             	and    $0x3,%eax
80106c73:	85 c0                	test   %eax,%eax
80106c75:	75 46                	jne    80106cbd <trap+0x2ab>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106c77:	e8 f5 fb ff ff       	call   80106871 <rcr2>
80106c7c:	8b 55 08             	mov    0x8(%ebp),%edx
80106c7f:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
80106c82:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80106c89:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106c8c:	0f b6 ca             	movzbl %dl,%ecx
80106c8f:	8b 55 08             	mov    0x8(%ebp),%edx
80106c92:	8b 52 30             	mov    0x30(%edx),%edx
80106c95:	89 44 24 10          	mov    %eax,0x10(%esp)
80106c99:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80106c9d:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80106ca1:	89 54 24 04          	mov    %edx,0x4(%esp)
80106ca5:	c7 04 24 ec 8d 10 80 	movl   $0x80108dec,(%esp)
80106cac:	e8 ef 96 ff ff       	call   801003a0 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
80106cb1:	c7 04 24 1e 8e 10 80 	movl   $0x80108e1e,(%esp)
80106cb8:	e8 7d 98 ff ff       	call   8010053a <panic>
    }
    
    if(tf->trapno==14 && proc->sighandle[0]!=(sighandler_t)-1)
80106cbd:	8b 45 08             	mov    0x8(%ebp),%eax
80106cc0:	8b 40 30             	mov    0x30(%eax),%eax
80106cc3:	83 f8 0e             	cmp    $0xe,%eax
80106cc6:	75 62                	jne    80106d2a <trap+0x318>
80106cc8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106cce:	8b 40 7c             	mov    0x7c(%eax),%eax
80106cd1:	83 f8 ff             	cmp    $0xffffffff,%eax
80106cd4:	74 54                	je     80106d2a <trap+0x318>
    {
//	cprintf("\nInside segmentation fault, proc needs to call sighandler :0x%x wrapper:0x%x and eip value: 0x%x\n",proc->sighandle[1],proc->sighandle[0],tf->eip);
//	cprintf("\nFaulting instruction is: 0x%x",tf->eip); 
	tf->esp=tf->esp-4;
80106cd6:	8b 45 08             	mov    0x8(%ebp),%eax
80106cd9:	8b 40 44             	mov    0x44(%eax),%eax
80106cdc:	8d 50 fc             	lea    -0x4(%eax),%edx
80106cdf:	8b 45 08             	mov    0x8(%ebp),%eax
80106ce2:	89 50 44             	mov    %edx,0x44(%eax)
	*((int*)tf->esp)=(uint)proc->sighandle[1];
80106ce5:	8b 45 08             	mov    0x8(%ebp),%eax
80106ce8:	8b 40 44             	mov    0x44(%eax),%eax
80106ceb:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80106cf2:	8b 92 80 00 00 00    	mov    0x80(%edx),%edx
80106cf8:	89 10                	mov    %edx,(%eax)
	tf->esp=tf->esp-4;
80106cfa:	8b 45 08             	mov    0x8(%ebp),%eax
80106cfd:	8b 40 44             	mov    0x44(%eax),%eax
80106d00:	8d 50 fc             	lea    -0x4(%eax),%edx
80106d03:	8b 45 08             	mov    0x8(%ebp),%eax
80106d06:	89 50 44             	mov    %edx,0x44(%eax)
	*((int*)tf->esp)=(uint)tf->eip;
80106d09:	8b 45 08             	mov    0x8(%ebp),%eax
80106d0c:	8b 40 44             	mov    0x44(%eax),%eax
80106d0f:	8b 55 08             	mov    0x8(%ebp),%edx
80106d12:	8b 52 38             	mov    0x38(%edx),%edx
80106d15:	89 10                	mov    %edx,(%eax)
	tf->eip=(uint)proc->sighandle[0];
80106d17:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d1d:	8b 40 7c             	mov    0x7c(%eax),%eax
80106d20:	89 c2                	mov    %eax,%edx
80106d22:	8b 45 08             	mov    0x8(%ebp),%eax
80106d25:	89 50 38             	mov    %edx,0x38(%eax)
80106d28:	eb 75                	jmp    80106d9f <trap+0x38d>
    }
    else{
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d2a:	e8 42 fb ff ff       	call   80106871 <rcr2>
80106d2f:	89 c2                	mov    %eax,%edx
80106d31:	8b 45 08             	mov    0x8(%ebp),%eax
80106d34:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106d37:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106d3d:	0f b6 00             	movzbl (%eax),%eax
	*((int*)tf->esp)=(uint)tf->eip;
	tf->eip=(uint)proc->sighandle[0];
    }
    else{
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d40:	0f b6 f0             	movzbl %al,%esi
80106d43:	8b 45 08             	mov    0x8(%ebp),%eax
80106d46:	8b 58 34             	mov    0x34(%eax),%ebx
80106d49:	8b 45 08             	mov    0x8(%ebp),%eax
80106d4c:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106d4f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d55:	83 c0 6c             	add    $0x6c,%eax
80106d58:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106d5b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
	*((int*)tf->esp)=(uint)tf->eip;
	tf->eip=(uint)proc->sighandle[0];
    }
    else{
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106d61:	8b 40 10             	mov    0x10(%eax),%eax
80106d64:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80106d68:	89 7c 24 18          	mov    %edi,0x18(%esp)
80106d6c:	89 74 24 14          	mov    %esi,0x14(%esp)
80106d70:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80106d74:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106d78:	8b 75 e4             	mov    -0x1c(%ebp),%esi
80106d7b:	89 74 24 08          	mov    %esi,0x8(%esp)
80106d7f:	89 44 24 04          	mov    %eax,0x4(%esp)
80106d83:	c7 04 24 24 8e 10 80 	movl   $0x80108e24,(%esp)
80106d8a:	e8 11 96 ff ff       	call   801003a0 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
80106d8f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106d95:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
80106d9c:	eb 01                	jmp    80106d9f <trap+0x38d>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
80106d9e:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106d9f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106da5:	85 c0                	test   %eax,%eax
80106da7:	74 24                	je     80106dcd <trap+0x3bb>
80106da9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106daf:	8b 40 24             	mov    0x24(%eax),%eax
80106db2:	85 c0                	test   %eax,%eax
80106db4:	74 17                	je     80106dcd <trap+0x3bb>
80106db6:	8b 45 08             	mov    0x8(%ebp),%eax
80106db9:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106dbd:	0f b7 c0             	movzwl %ax,%eax
80106dc0:	83 e0 03             	and    $0x3,%eax
80106dc3:	83 f8 03             	cmp    $0x3,%eax
80106dc6:	75 05                	jne    80106dcd <trap+0x3bb>
    exit();
80106dc8:	e8 a9 d9 ff ff       	call   80104776 <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
80106dcd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106dd3:	85 c0                	test   %eax,%eax
80106dd5:	74 1e                	je     80106df5 <trap+0x3e3>
80106dd7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106ddd:	8b 40 0c             	mov    0xc(%eax),%eax
80106de0:	83 f8 04             	cmp    $0x4,%eax
80106de3:	75 10                	jne    80106df5 <trap+0x3e3>
80106de5:	8b 45 08             	mov    0x8(%ebp),%eax
80106de8:	8b 40 30             	mov    0x30(%eax),%eax
80106deb:	83 f8 20             	cmp    $0x20,%eax
80106dee:	75 05                	jne    80106df5 <trap+0x3e3>
    yield();
80106df0:	e8 05 dd ff ff       	call   80104afa <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106df5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106dfb:	85 c0                	test   %eax,%eax
80106dfd:	74 24                	je     80106e23 <trap+0x411>
80106dff:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106e05:	8b 40 24             	mov    0x24(%eax),%eax
80106e08:	85 c0                	test   %eax,%eax
80106e0a:	74 17                	je     80106e23 <trap+0x411>
80106e0c:	8b 45 08             	mov    0x8(%ebp),%eax
80106e0f:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106e13:	0f b7 c0             	movzwl %ax,%eax
80106e16:	83 e0 03             	and    $0x3,%eax
80106e19:	83 f8 03             	cmp    $0x3,%eax
80106e1c:	75 05                	jne    80106e23 <trap+0x411>
    exit();
80106e1e:	e8 53 d9 ff ff       	call   80104776 <exit>
}
80106e23:	83 c4 3c             	add    $0x3c,%esp
80106e26:	5b                   	pop    %ebx
80106e27:	5e                   	pop    %esi
80106e28:	5f                   	pop    %edi
80106e29:	5d                   	pop    %ebp
80106e2a:	c3                   	ret    

80106e2b <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106e2b:	55                   	push   %ebp
80106e2c:	89 e5                	mov    %esp,%ebp
80106e2e:	83 ec 14             	sub    $0x14,%esp
80106e31:	8b 45 08             	mov    0x8(%ebp),%eax
80106e34:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106e38:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80106e3c:	89 c2                	mov    %eax,%edx
80106e3e:	ec                   	in     (%dx),%al
80106e3f:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80106e42:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80106e46:	c9                   	leave  
80106e47:	c3                   	ret    

80106e48 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106e48:	55                   	push   %ebp
80106e49:	89 e5                	mov    %esp,%ebp
80106e4b:	83 ec 08             	sub    $0x8,%esp
80106e4e:	8b 55 08             	mov    0x8(%ebp),%edx
80106e51:	8b 45 0c             	mov    0xc(%ebp),%eax
80106e54:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106e58:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106e5b:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106e5f:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106e63:	ee                   	out    %al,(%dx)
}
80106e64:	c9                   	leave  
80106e65:	c3                   	ret    

80106e66 <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80106e66:	55                   	push   %ebp
80106e67:	89 e5                	mov    %esp,%ebp
80106e69:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106e6c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106e73:	00 
80106e74:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106e7b:	e8 c8 ff ff ff       	call   80106e48 <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80106e80:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
80106e87:	00 
80106e88:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106e8f:	e8 b4 ff ff ff       	call   80106e48 <outb>
  outb(COM1+0, 115200/9600);
80106e94:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
80106e9b:	00 
80106e9c:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106ea3:	e8 a0 ff ff ff       	call   80106e48 <outb>
  outb(COM1+1, 0);
80106ea8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106eaf:	00 
80106eb0:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106eb7:	e8 8c ff ff ff       	call   80106e48 <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
80106ebc:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80106ec3:	00 
80106ec4:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106ecb:	e8 78 ff ff ff       	call   80106e48 <outb>
  outb(COM1+4, 0);
80106ed0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106ed7:	00 
80106ed8:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
80106edf:	e8 64 ff ff ff       	call   80106e48 <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80106ee4:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106eeb:	00 
80106eec:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106ef3:	e8 50 ff ff ff       	call   80106e48 <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80106ef8:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106eff:	e8 27 ff ff ff       	call   80106e2b <inb>
80106f04:	3c ff                	cmp    $0xff,%al
80106f06:	75 02                	jne    80106f0a <uartinit+0xa4>
    return;
80106f08:	eb 6a                	jmp    80106f74 <uartinit+0x10e>
  uart = 1;
80106f0a:	c7 05 6c b6 10 80 01 	movl   $0x1,0x8010b66c
80106f11:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106f14:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106f1b:	e8 0b ff ff ff       	call   80106e2b <inb>
  inb(COM1+0);
80106f20:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106f27:	e8 ff fe ff ff       	call   80106e2b <inb>
  picenable(IRQ_COM1);
80106f2c:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106f33:	e8 64 ce ff ff       	call   80103d9c <picenable>
  ioapicenable(IRQ_COM1, 0);
80106f38:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106f3f:	00 
80106f40:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106f47:	e8 11 ba ff ff       	call   8010295d <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106f4c:	c7 45 f4 e8 8e 10 80 	movl   $0x80108ee8,-0xc(%ebp)
80106f53:	eb 15                	jmp    80106f6a <uartinit+0x104>
    uartputc(*p);
80106f55:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106f58:	0f b6 00             	movzbl (%eax),%eax
80106f5b:	0f be c0             	movsbl %al,%eax
80106f5e:	89 04 24             	mov    %eax,(%esp)
80106f61:	e8 10 00 00 00       	call   80106f76 <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106f66:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106f6a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106f6d:	0f b6 00             	movzbl (%eax),%eax
80106f70:	84 c0                	test   %al,%al
80106f72:	75 e1                	jne    80106f55 <uartinit+0xef>
    uartputc(*p);
}
80106f74:	c9                   	leave  
80106f75:	c3                   	ret    

80106f76 <uartputc>:

void
uartputc(int c)
{
80106f76:	55                   	push   %ebp
80106f77:	89 e5                	mov    %esp,%ebp
80106f79:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80106f7c:	a1 6c b6 10 80       	mov    0x8010b66c,%eax
80106f81:	85 c0                	test   %eax,%eax
80106f83:	75 02                	jne    80106f87 <uartputc+0x11>
    return;
80106f85:	eb 4b                	jmp    80106fd2 <uartputc+0x5c>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106f87:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106f8e:	eb 10                	jmp    80106fa0 <uartputc+0x2a>
    microdelay(10);
80106f90:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80106f97:	e8 59 bf ff ff       	call   80102ef5 <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106f9c:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106fa0:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80106fa4:	7f 16                	jg     80106fbc <uartputc+0x46>
80106fa6:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106fad:	e8 79 fe ff ff       	call   80106e2b <inb>
80106fb2:	0f b6 c0             	movzbl %al,%eax
80106fb5:	83 e0 20             	and    $0x20,%eax
80106fb8:	85 c0                	test   %eax,%eax
80106fba:	74 d4                	je     80106f90 <uartputc+0x1a>
    microdelay(10);
  outb(COM1+0, c);
80106fbc:	8b 45 08             	mov    0x8(%ebp),%eax
80106fbf:	0f b6 c0             	movzbl %al,%eax
80106fc2:	89 44 24 04          	mov    %eax,0x4(%esp)
80106fc6:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106fcd:	e8 76 fe ff ff       	call   80106e48 <outb>
}
80106fd2:	c9                   	leave  
80106fd3:	c3                   	ret    

80106fd4 <uartgetc>:

static int
uartgetc(void)
{
80106fd4:	55                   	push   %ebp
80106fd5:	89 e5                	mov    %esp,%ebp
80106fd7:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80106fda:	a1 6c b6 10 80       	mov    0x8010b66c,%eax
80106fdf:	85 c0                	test   %eax,%eax
80106fe1:	75 07                	jne    80106fea <uartgetc+0x16>
    return -1;
80106fe3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106fe8:	eb 2c                	jmp    80107016 <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80106fea:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106ff1:	e8 35 fe ff ff       	call   80106e2b <inb>
80106ff6:	0f b6 c0             	movzbl %al,%eax
80106ff9:	83 e0 01             	and    $0x1,%eax
80106ffc:	85 c0                	test   %eax,%eax
80106ffe:	75 07                	jne    80107007 <uartgetc+0x33>
    return -1;
80107000:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107005:	eb 0f                	jmp    80107016 <uartgetc+0x42>
  return inb(COM1+0);
80107007:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
8010700e:	e8 18 fe ff ff       	call   80106e2b <inb>
80107013:	0f b6 c0             	movzbl %al,%eax
}
80107016:	c9                   	leave  
80107017:	c3                   	ret    

80107018 <uartintr>:

void
uartintr(void)
{
80107018:	55                   	push   %ebp
80107019:	89 e5                	mov    %esp,%ebp
8010701b:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
8010701e:	c7 04 24 d4 6f 10 80 	movl   $0x80106fd4,(%esp)
80107025:	e8 83 97 ff ff       	call   801007ad <consoleintr>
}
8010702a:	c9                   	leave  
8010702b:	c3                   	ret    

8010702c <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
8010702c:	6a 00                	push   $0x0
  pushl $0
8010702e:	6a 00                	push   $0x0
  jmp alltraps
80107030:	e9 e8 f7 ff ff       	jmp    8010681d <alltraps>

80107035 <vector1>:
.globl vector1
vector1:
  pushl $0
80107035:	6a 00                	push   $0x0
  pushl $1
80107037:	6a 01                	push   $0x1
  jmp alltraps
80107039:	e9 df f7 ff ff       	jmp    8010681d <alltraps>

8010703e <vector2>:
.globl vector2
vector2:
  pushl $0
8010703e:	6a 00                	push   $0x0
  pushl $2
80107040:	6a 02                	push   $0x2
  jmp alltraps
80107042:	e9 d6 f7 ff ff       	jmp    8010681d <alltraps>

80107047 <vector3>:
.globl vector3
vector3:
  pushl $0
80107047:	6a 00                	push   $0x0
  pushl $3
80107049:	6a 03                	push   $0x3
  jmp alltraps
8010704b:	e9 cd f7 ff ff       	jmp    8010681d <alltraps>

80107050 <vector4>:
.globl vector4
vector4:
  pushl $0
80107050:	6a 00                	push   $0x0
  pushl $4
80107052:	6a 04                	push   $0x4
  jmp alltraps
80107054:	e9 c4 f7 ff ff       	jmp    8010681d <alltraps>

80107059 <vector5>:
.globl vector5
vector5:
  pushl $0
80107059:	6a 00                	push   $0x0
  pushl $5
8010705b:	6a 05                	push   $0x5
  jmp alltraps
8010705d:	e9 bb f7 ff ff       	jmp    8010681d <alltraps>

80107062 <vector6>:
.globl vector6
vector6:
  pushl $0
80107062:	6a 00                	push   $0x0
  pushl $6
80107064:	6a 06                	push   $0x6
  jmp alltraps
80107066:	e9 b2 f7 ff ff       	jmp    8010681d <alltraps>

8010706b <vector7>:
.globl vector7
vector7:
  pushl $0
8010706b:	6a 00                	push   $0x0
  pushl $7
8010706d:	6a 07                	push   $0x7
  jmp alltraps
8010706f:	e9 a9 f7 ff ff       	jmp    8010681d <alltraps>

80107074 <vector8>:
.globl vector8
vector8:
  pushl $8
80107074:	6a 08                	push   $0x8
  jmp alltraps
80107076:	e9 a2 f7 ff ff       	jmp    8010681d <alltraps>

8010707b <vector9>:
.globl vector9
vector9:
  pushl $0
8010707b:	6a 00                	push   $0x0
  pushl $9
8010707d:	6a 09                	push   $0x9
  jmp alltraps
8010707f:	e9 99 f7 ff ff       	jmp    8010681d <alltraps>

80107084 <vector10>:
.globl vector10
vector10:
  pushl $10
80107084:	6a 0a                	push   $0xa
  jmp alltraps
80107086:	e9 92 f7 ff ff       	jmp    8010681d <alltraps>

8010708b <vector11>:
.globl vector11
vector11:
  pushl $11
8010708b:	6a 0b                	push   $0xb
  jmp alltraps
8010708d:	e9 8b f7 ff ff       	jmp    8010681d <alltraps>

80107092 <vector12>:
.globl vector12
vector12:
  pushl $12
80107092:	6a 0c                	push   $0xc
  jmp alltraps
80107094:	e9 84 f7 ff ff       	jmp    8010681d <alltraps>

80107099 <vector13>:
.globl vector13
vector13:
  pushl $13
80107099:	6a 0d                	push   $0xd
  jmp alltraps
8010709b:	e9 7d f7 ff ff       	jmp    8010681d <alltraps>

801070a0 <vector14>:
.globl vector14
vector14:
  pushl $14
801070a0:	6a 0e                	push   $0xe
  jmp alltraps
801070a2:	e9 76 f7 ff ff       	jmp    8010681d <alltraps>

801070a7 <vector15>:
.globl vector15
vector15:
  pushl $0
801070a7:	6a 00                	push   $0x0
  pushl $15
801070a9:	6a 0f                	push   $0xf
  jmp alltraps
801070ab:	e9 6d f7 ff ff       	jmp    8010681d <alltraps>

801070b0 <vector16>:
.globl vector16
vector16:
  pushl $0
801070b0:	6a 00                	push   $0x0
  pushl $16
801070b2:	6a 10                	push   $0x10
  jmp alltraps
801070b4:	e9 64 f7 ff ff       	jmp    8010681d <alltraps>

801070b9 <vector17>:
.globl vector17
vector17:
  pushl $17
801070b9:	6a 11                	push   $0x11
  jmp alltraps
801070bb:	e9 5d f7 ff ff       	jmp    8010681d <alltraps>

801070c0 <vector18>:
.globl vector18
vector18:
  pushl $0
801070c0:	6a 00                	push   $0x0
  pushl $18
801070c2:	6a 12                	push   $0x12
  jmp alltraps
801070c4:	e9 54 f7 ff ff       	jmp    8010681d <alltraps>

801070c9 <vector19>:
.globl vector19
vector19:
  pushl $0
801070c9:	6a 00                	push   $0x0
  pushl $19
801070cb:	6a 13                	push   $0x13
  jmp alltraps
801070cd:	e9 4b f7 ff ff       	jmp    8010681d <alltraps>

801070d2 <vector20>:
.globl vector20
vector20:
  pushl $0
801070d2:	6a 00                	push   $0x0
  pushl $20
801070d4:	6a 14                	push   $0x14
  jmp alltraps
801070d6:	e9 42 f7 ff ff       	jmp    8010681d <alltraps>

801070db <vector21>:
.globl vector21
vector21:
  pushl $0
801070db:	6a 00                	push   $0x0
  pushl $21
801070dd:	6a 15                	push   $0x15
  jmp alltraps
801070df:	e9 39 f7 ff ff       	jmp    8010681d <alltraps>

801070e4 <vector22>:
.globl vector22
vector22:
  pushl $0
801070e4:	6a 00                	push   $0x0
  pushl $22
801070e6:	6a 16                	push   $0x16
  jmp alltraps
801070e8:	e9 30 f7 ff ff       	jmp    8010681d <alltraps>

801070ed <vector23>:
.globl vector23
vector23:
  pushl $0
801070ed:	6a 00                	push   $0x0
  pushl $23
801070ef:	6a 17                	push   $0x17
  jmp alltraps
801070f1:	e9 27 f7 ff ff       	jmp    8010681d <alltraps>

801070f6 <vector24>:
.globl vector24
vector24:
  pushl $0
801070f6:	6a 00                	push   $0x0
  pushl $24
801070f8:	6a 18                	push   $0x18
  jmp alltraps
801070fa:	e9 1e f7 ff ff       	jmp    8010681d <alltraps>

801070ff <vector25>:
.globl vector25
vector25:
  pushl $0
801070ff:	6a 00                	push   $0x0
  pushl $25
80107101:	6a 19                	push   $0x19
  jmp alltraps
80107103:	e9 15 f7 ff ff       	jmp    8010681d <alltraps>

80107108 <vector26>:
.globl vector26
vector26:
  pushl $0
80107108:	6a 00                	push   $0x0
  pushl $26
8010710a:	6a 1a                	push   $0x1a
  jmp alltraps
8010710c:	e9 0c f7 ff ff       	jmp    8010681d <alltraps>

80107111 <vector27>:
.globl vector27
vector27:
  pushl $0
80107111:	6a 00                	push   $0x0
  pushl $27
80107113:	6a 1b                	push   $0x1b
  jmp alltraps
80107115:	e9 03 f7 ff ff       	jmp    8010681d <alltraps>

8010711a <vector28>:
.globl vector28
vector28:
  pushl $0
8010711a:	6a 00                	push   $0x0
  pushl $28
8010711c:	6a 1c                	push   $0x1c
  jmp alltraps
8010711e:	e9 fa f6 ff ff       	jmp    8010681d <alltraps>

80107123 <vector29>:
.globl vector29
vector29:
  pushl $0
80107123:	6a 00                	push   $0x0
  pushl $29
80107125:	6a 1d                	push   $0x1d
  jmp alltraps
80107127:	e9 f1 f6 ff ff       	jmp    8010681d <alltraps>

8010712c <vector30>:
.globl vector30
vector30:
  pushl $0
8010712c:	6a 00                	push   $0x0
  pushl $30
8010712e:	6a 1e                	push   $0x1e
  jmp alltraps
80107130:	e9 e8 f6 ff ff       	jmp    8010681d <alltraps>

80107135 <vector31>:
.globl vector31
vector31:
  pushl $0
80107135:	6a 00                	push   $0x0
  pushl $31
80107137:	6a 1f                	push   $0x1f
  jmp alltraps
80107139:	e9 df f6 ff ff       	jmp    8010681d <alltraps>

8010713e <vector32>:
.globl vector32
vector32:
  pushl $0
8010713e:	6a 00                	push   $0x0
  pushl $32
80107140:	6a 20                	push   $0x20
  jmp alltraps
80107142:	e9 d6 f6 ff ff       	jmp    8010681d <alltraps>

80107147 <vector33>:
.globl vector33
vector33:
  pushl $0
80107147:	6a 00                	push   $0x0
  pushl $33
80107149:	6a 21                	push   $0x21
  jmp alltraps
8010714b:	e9 cd f6 ff ff       	jmp    8010681d <alltraps>

80107150 <vector34>:
.globl vector34
vector34:
  pushl $0
80107150:	6a 00                	push   $0x0
  pushl $34
80107152:	6a 22                	push   $0x22
  jmp alltraps
80107154:	e9 c4 f6 ff ff       	jmp    8010681d <alltraps>

80107159 <vector35>:
.globl vector35
vector35:
  pushl $0
80107159:	6a 00                	push   $0x0
  pushl $35
8010715b:	6a 23                	push   $0x23
  jmp alltraps
8010715d:	e9 bb f6 ff ff       	jmp    8010681d <alltraps>

80107162 <vector36>:
.globl vector36
vector36:
  pushl $0
80107162:	6a 00                	push   $0x0
  pushl $36
80107164:	6a 24                	push   $0x24
  jmp alltraps
80107166:	e9 b2 f6 ff ff       	jmp    8010681d <alltraps>

8010716b <vector37>:
.globl vector37
vector37:
  pushl $0
8010716b:	6a 00                	push   $0x0
  pushl $37
8010716d:	6a 25                	push   $0x25
  jmp alltraps
8010716f:	e9 a9 f6 ff ff       	jmp    8010681d <alltraps>

80107174 <vector38>:
.globl vector38
vector38:
  pushl $0
80107174:	6a 00                	push   $0x0
  pushl $38
80107176:	6a 26                	push   $0x26
  jmp alltraps
80107178:	e9 a0 f6 ff ff       	jmp    8010681d <alltraps>

8010717d <vector39>:
.globl vector39
vector39:
  pushl $0
8010717d:	6a 00                	push   $0x0
  pushl $39
8010717f:	6a 27                	push   $0x27
  jmp alltraps
80107181:	e9 97 f6 ff ff       	jmp    8010681d <alltraps>

80107186 <vector40>:
.globl vector40
vector40:
  pushl $0
80107186:	6a 00                	push   $0x0
  pushl $40
80107188:	6a 28                	push   $0x28
  jmp alltraps
8010718a:	e9 8e f6 ff ff       	jmp    8010681d <alltraps>

8010718f <vector41>:
.globl vector41
vector41:
  pushl $0
8010718f:	6a 00                	push   $0x0
  pushl $41
80107191:	6a 29                	push   $0x29
  jmp alltraps
80107193:	e9 85 f6 ff ff       	jmp    8010681d <alltraps>

80107198 <vector42>:
.globl vector42
vector42:
  pushl $0
80107198:	6a 00                	push   $0x0
  pushl $42
8010719a:	6a 2a                	push   $0x2a
  jmp alltraps
8010719c:	e9 7c f6 ff ff       	jmp    8010681d <alltraps>

801071a1 <vector43>:
.globl vector43
vector43:
  pushl $0
801071a1:	6a 00                	push   $0x0
  pushl $43
801071a3:	6a 2b                	push   $0x2b
  jmp alltraps
801071a5:	e9 73 f6 ff ff       	jmp    8010681d <alltraps>

801071aa <vector44>:
.globl vector44
vector44:
  pushl $0
801071aa:	6a 00                	push   $0x0
  pushl $44
801071ac:	6a 2c                	push   $0x2c
  jmp alltraps
801071ae:	e9 6a f6 ff ff       	jmp    8010681d <alltraps>

801071b3 <vector45>:
.globl vector45
vector45:
  pushl $0
801071b3:	6a 00                	push   $0x0
  pushl $45
801071b5:	6a 2d                	push   $0x2d
  jmp alltraps
801071b7:	e9 61 f6 ff ff       	jmp    8010681d <alltraps>

801071bc <vector46>:
.globl vector46
vector46:
  pushl $0
801071bc:	6a 00                	push   $0x0
  pushl $46
801071be:	6a 2e                	push   $0x2e
  jmp alltraps
801071c0:	e9 58 f6 ff ff       	jmp    8010681d <alltraps>

801071c5 <vector47>:
.globl vector47
vector47:
  pushl $0
801071c5:	6a 00                	push   $0x0
  pushl $47
801071c7:	6a 2f                	push   $0x2f
  jmp alltraps
801071c9:	e9 4f f6 ff ff       	jmp    8010681d <alltraps>

801071ce <vector48>:
.globl vector48
vector48:
  pushl $0
801071ce:	6a 00                	push   $0x0
  pushl $48
801071d0:	6a 30                	push   $0x30
  jmp alltraps
801071d2:	e9 46 f6 ff ff       	jmp    8010681d <alltraps>

801071d7 <vector49>:
.globl vector49
vector49:
  pushl $0
801071d7:	6a 00                	push   $0x0
  pushl $49
801071d9:	6a 31                	push   $0x31
  jmp alltraps
801071db:	e9 3d f6 ff ff       	jmp    8010681d <alltraps>

801071e0 <vector50>:
.globl vector50
vector50:
  pushl $0
801071e0:	6a 00                	push   $0x0
  pushl $50
801071e2:	6a 32                	push   $0x32
  jmp alltraps
801071e4:	e9 34 f6 ff ff       	jmp    8010681d <alltraps>

801071e9 <vector51>:
.globl vector51
vector51:
  pushl $0
801071e9:	6a 00                	push   $0x0
  pushl $51
801071eb:	6a 33                	push   $0x33
  jmp alltraps
801071ed:	e9 2b f6 ff ff       	jmp    8010681d <alltraps>

801071f2 <vector52>:
.globl vector52
vector52:
  pushl $0
801071f2:	6a 00                	push   $0x0
  pushl $52
801071f4:	6a 34                	push   $0x34
  jmp alltraps
801071f6:	e9 22 f6 ff ff       	jmp    8010681d <alltraps>

801071fb <vector53>:
.globl vector53
vector53:
  pushl $0
801071fb:	6a 00                	push   $0x0
  pushl $53
801071fd:	6a 35                	push   $0x35
  jmp alltraps
801071ff:	e9 19 f6 ff ff       	jmp    8010681d <alltraps>

80107204 <vector54>:
.globl vector54
vector54:
  pushl $0
80107204:	6a 00                	push   $0x0
  pushl $54
80107206:	6a 36                	push   $0x36
  jmp alltraps
80107208:	e9 10 f6 ff ff       	jmp    8010681d <alltraps>

8010720d <vector55>:
.globl vector55
vector55:
  pushl $0
8010720d:	6a 00                	push   $0x0
  pushl $55
8010720f:	6a 37                	push   $0x37
  jmp alltraps
80107211:	e9 07 f6 ff ff       	jmp    8010681d <alltraps>

80107216 <vector56>:
.globl vector56
vector56:
  pushl $0
80107216:	6a 00                	push   $0x0
  pushl $56
80107218:	6a 38                	push   $0x38
  jmp alltraps
8010721a:	e9 fe f5 ff ff       	jmp    8010681d <alltraps>

8010721f <vector57>:
.globl vector57
vector57:
  pushl $0
8010721f:	6a 00                	push   $0x0
  pushl $57
80107221:	6a 39                	push   $0x39
  jmp alltraps
80107223:	e9 f5 f5 ff ff       	jmp    8010681d <alltraps>

80107228 <vector58>:
.globl vector58
vector58:
  pushl $0
80107228:	6a 00                	push   $0x0
  pushl $58
8010722a:	6a 3a                	push   $0x3a
  jmp alltraps
8010722c:	e9 ec f5 ff ff       	jmp    8010681d <alltraps>

80107231 <vector59>:
.globl vector59
vector59:
  pushl $0
80107231:	6a 00                	push   $0x0
  pushl $59
80107233:	6a 3b                	push   $0x3b
  jmp alltraps
80107235:	e9 e3 f5 ff ff       	jmp    8010681d <alltraps>

8010723a <vector60>:
.globl vector60
vector60:
  pushl $0
8010723a:	6a 00                	push   $0x0
  pushl $60
8010723c:	6a 3c                	push   $0x3c
  jmp alltraps
8010723e:	e9 da f5 ff ff       	jmp    8010681d <alltraps>

80107243 <vector61>:
.globl vector61
vector61:
  pushl $0
80107243:	6a 00                	push   $0x0
  pushl $61
80107245:	6a 3d                	push   $0x3d
  jmp alltraps
80107247:	e9 d1 f5 ff ff       	jmp    8010681d <alltraps>

8010724c <vector62>:
.globl vector62
vector62:
  pushl $0
8010724c:	6a 00                	push   $0x0
  pushl $62
8010724e:	6a 3e                	push   $0x3e
  jmp alltraps
80107250:	e9 c8 f5 ff ff       	jmp    8010681d <alltraps>

80107255 <vector63>:
.globl vector63
vector63:
  pushl $0
80107255:	6a 00                	push   $0x0
  pushl $63
80107257:	6a 3f                	push   $0x3f
  jmp alltraps
80107259:	e9 bf f5 ff ff       	jmp    8010681d <alltraps>

8010725e <vector64>:
.globl vector64
vector64:
  pushl $0
8010725e:	6a 00                	push   $0x0
  pushl $64
80107260:	6a 40                	push   $0x40
  jmp alltraps
80107262:	e9 b6 f5 ff ff       	jmp    8010681d <alltraps>

80107267 <vector65>:
.globl vector65
vector65:
  pushl $0
80107267:	6a 00                	push   $0x0
  pushl $65
80107269:	6a 41                	push   $0x41
  jmp alltraps
8010726b:	e9 ad f5 ff ff       	jmp    8010681d <alltraps>

80107270 <vector66>:
.globl vector66
vector66:
  pushl $0
80107270:	6a 00                	push   $0x0
  pushl $66
80107272:	6a 42                	push   $0x42
  jmp alltraps
80107274:	e9 a4 f5 ff ff       	jmp    8010681d <alltraps>

80107279 <vector67>:
.globl vector67
vector67:
  pushl $0
80107279:	6a 00                	push   $0x0
  pushl $67
8010727b:	6a 43                	push   $0x43
  jmp alltraps
8010727d:	e9 9b f5 ff ff       	jmp    8010681d <alltraps>

80107282 <vector68>:
.globl vector68
vector68:
  pushl $0
80107282:	6a 00                	push   $0x0
  pushl $68
80107284:	6a 44                	push   $0x44
  jmp alltraps
80107286:	e9 92 f5 ff ff       	jmp    8010681d <alltraps>

8010728b <vector69>:
.globl vector69
vector69:
  pushl $0
8010728b:	6a 00                	push   $0x0
  pushl $69
8010728d:	6a 45                	push   $0x45
  jmp alltraps
8010728f:	e9 89 f5 ff ff       	jmp    8010681d <alltraps>

80107294 <vector70>:
.globl vector70
vector70:
  pushl $0
80107294:	6a 00                	push   $0x0
  pushl $70
80107296:	6a 46                	push   $0x46
  jmp alltraps
80107298:	e9 80 f5 ff ff       	jmp    8010681d <alltraps>

8010729d <vector71>:
.globl vector71
vector71:
  pushl $0
8010729d:	6a 00                	push   $0x0
  pushl $71
8010729f:	6a 47                	push   $0x47
  jmp alltraps
801072a1:	e9 77 f5 ff ff       	jmp    8010681d <alltraps>

801072a6 <vector72>:
.globl vector72
vector72:
  pushl $0
801072a6:	6a 00                	push   $0x0
  pushl $72
801072a8:	6a 48                	push   $0x48
  jmp alltraps
801072aa:	e9 6e f5 ff ff       	jmp    8010681d <alltraps>

801072af <vector73>:
.globl vector73
vector73:
  pushl $0
801072af:	6a 00                	push   $0x0
  pushl $73
801072b1:	6a 49                	push   $0x49
  jmp alltraps
801072b3:	e9 65 f5 ff ff       	jmp    8010681d <alltraps>

801072b8 <vector74>:
.globl vector74
vector74:
  pushl $0
801072b8:	6a 00                	push   $0x0
  pushl $74
801072ba:	6a 4a                	push   $0x4a
  jmp alltraps
801072bc:	e9 5c f5 ff ff       	jmp    8010681d <alltraps>

801072c1 <vector75>:
.globl vector75
vector75:
  pushl $0
801072c1:	6a 00                	push   $0x0
  pushl $75
801072c3:	6a 4b                	push   $0x4b
  jmp alltraps
801072c5:	e9 53 f5 ff ff       	jmp    8010681d <alltraps>

801072ca <vector76>:
.globl vector76
vector76:
  pushl $0
801072ca:	6a 00                	push   $0x0
  pushl $76
801072cc:	6a 4c                	push   $0x4c
  jmp alltraps
801072ce:	e9 4a f5 ff ff       	jmp    8010681d <alltraps>

801072d3 <vector77>:
.globl vector77
vector77:
  pushl $0
801072d3:	6a 00                	push   $0x0
  pushl $77
801072d5:	6a 4d                	push   $0x4d
  jmp alltraps
801072d7:	e9 41 f5 ff ff       	jmp    8010681d <alltraps>

801072dc <vector78>:
.globl vector78
vector78:
  pushl $0
801072dc:	6a 00                	push   $0x0
  pushl $78
801072de:	6a 4e                	push   $0x4e
  jmp alltraps
801072e0:	e9 38 f5 ff ff       	jmp    8010681d <alltraps>

801072e5 <vector79>:
.globl vector79
vector79:
  pushl $0
801072e5:	6a 00                	push   $0x0
  pushl $79
801072e7:	6a 4f                	push   $0x4f
  jmp alltraps
801072e9:	e9 2f f5 ff ff       	jmp    8010681d <alltraps>

801072ee <vector80>:
.globl vector80
vector80:
  pushl $0
801072ee:	6a 00                	push   $0x0
  pushl $80
801072f0:	6a 50                	push   $0x50
  jmp alltraps
801072f2:	e9 26 f5 ff ff       	jmp    8010681d <alltraps>

801072f7 <vector81>:
.globl vector81
vector81:
  pushl $0
801072f7:	6a 00                	push   $0x0
  pushl $81
801072f9:	6a 51                	push   $0x51
  jmp alltraps
801072fb:	e9 1d f5 ff ff       	jmp    8010681d <alltraps>

80107300 <vector82>:
.globl vector82
vector82:
  pushl $0
80107300:	6a 00                	push   $0x0
  pushl $82
80107302:	6a 52                	push   $0x52
  jmp alltraps
80107304:	e9 14 f5 ff ff       	jmp    8010681d <alltraps>

80107309 <vector83>:
.globl vector83
vector83:
  pushl $0
80107309:	6a 00                	push   $0x0
  pushl $83
8010730b:	6a 53                	push   $0x53
  jmp alltraps
8010730d:	e9 0b f5 ff ff       	jmp    8010681d <alltraps>

80107312 <vector84>:
.globl vector84
vector84:
  pushl $0
80107312:	6a 00                	push   $0x0
  pushl $84
80107314:	6a 54                	push   $0x54
  jmp alltraps
80107316:	e9 02 f5 ff ff       	jmp    8010681d <alltraps>

8010731b <vector85>:
.globl vector85
vector85:
  pushl $0
8010731b:	6a 00                	push   $0x0
  pushl $85
8010731d:	6a 55                	push   $0x55
  jmp alltraps
8010731f:	e9 f9 f4 ff ff       	jmp    8010681d <alltraps>

80107324 <vector86>:
.globl vector86
vector86:
  pushl $0
80107324:	6a 00                	push   $0x0
  pushl $86
80107326:	6a 56                	push   $0x56
  jmp alltraps
80107328:	e9 f0 f4 ff ff       	jmp    8010681d <alltraps>

8010732d <vector87>:
.globl vector87
vector87:
  pushl $0
8010732d:	6a 00                	push   $0x0
  pushl $87
8010732f:	6a 57                	push   $0x57
  jmp alltraps
80107331:	e9 e7 f4 ff ff       	jmp    8010681d <alltraps>

80107336 <vector88>:
.globl vector88
vector88:
  pushl $0
80107336:	6a 00                	push   $0x0
  pushl $88
80107338:	6a 58                	push   $0x58
  jmp alltraps
8010733a:	e9 de f4 ff ff       	jmp    8010681d <alltraps>

8010733f <vector89>:
.globl vector89
vector89:
  pushl $0
8010733f:	6a 00                	push   $0x0
  pushl $89
80107341:	6a 59                	push   $0x59
  jmp alltraps
80107343:	e9 d5 f4 ff ff       	jmp    8010681d <alltraps>

80107348 <vector90>:
.globl vector90
vector90:
  pushl $0
80107348:	6a 00                	push   $0x0
  pushl $90
8010734a:	6a 5a                	push   $0x5a
  jmp alltraps
8010734c:	e9 cc f4 ff ff       	jmp    8010681d <alltraps>

80107351 <vector91>:
.globl vector91
vector91:
  pushl $0
80107351:	6a 00                	push   $0x0
  pushl $91
80107353:	6a 5b                	push   $0x5b
  jmp alltraps
80107355:	e9 c3 f4 ff ff       	jmp    8010681d <alltraps>

8010735a <vector92>:
.globl vector92
vector92:
  pushl $0
8010735a:	6a 00                	push   $0x0
  pushl $92
8010735c:	6a 5c                	push   $0x5c
  jmp alltraps
8010735e:	e9 ba f4 ff ff       	jmp    8010681d <alltraps>

80107363 <vector93>:
.globl vector93
vector93:
  pushl $0
80107363:	6a 00                	push   $0x0
  pushl $93
80107365:	6a 5d                	push   $0x5d
  jmp alltraps
80107367:	e9 b1 f4 ff ff       	jmp    8010681d <alltraps>

8010736c <vector94>:
.globl vector94
vector94:
  pushl $0
8010736c:	6a 00                	push   $0x0
  pushl $94
8010736e:	6a 5e                	push   $0x5e
  jmp alltraps
80107370:	e9 a8 f4 ff ff       	jmp    8010681d <alltraps>

80107375 <vector95>:
.globl vector95
vector95:
  pushl $0
80107375:	6a 00                	push   $0x0
  pushl $95
80107377:	6a 5f                	push   $0x5f
  jmp alltraps
80107379:	e9 9f f4 ff ff       	jmp    8010681d <alltraps>

8010737e <vector96>:
.globl vector96
vector96:
  pushl $0
8010737e:	6a 00                	push   $0x0
  pushl $96
80107380:	6a 60                	push   $0x60
  jmp alltraps
80107382:	e9 96 f4 ff ff       	jmp    8010681d <alltraps>

80107387 <vector97>:
.globl vector97
vector97:
  pushl $0
80107387:	6a 00                	push   $0x0
  pushl $97
80107389:	6a 61                	push   $0x61
  jmp alltraps
8010738b:	e9 8d f4 ff ff       	jmp    8010681d <alltraps>

80107390 <vector98>:
.globl vector98
vector98:
  pushl $0
80107390:	6a 00                	push   $0x0
  pushl $98
80107392:	6a 62                	push   $0x62
  jmp alltraps
80107394:	e9 84 f4 ff ff       	jmp    8010681d <alltraps>

80107399 <vector99>:
.globl vector99
vector99:
  pushl $0
80107399:	6a 00                	push   $0x0
  pushl $99
8010739b:	6a 63                	push   $0x63
  jmp alltraps
8010739d:	e9 7b f4 ff ff       	jmp    8010681d <alltraps>

801073a2 <vector100>:
.globl vector100
vector100:
  pushl $0
801073a2:	6a 00                	push   $0x0
  pushl $100
801073a4:	6a 64                	push   $0x64
  jmp alltraps
801073a6:	e9 72 f4 ff ff       	jmp    8010681d <alltraps>

801073ab <vector101>:
.globl vector101
vector101:
  pushl $0
801073ab:	6a 00                	push   $0x0
  pushl $101
801073ad:	6a 65                	push   $0x65
  jmp alltraps
801073af:	e9 69 f4 ff ff       	jmp    8010681d <alltraps>

801073b4 <vector102>:
.globl vector102
vector102:
  pushl $0
801073b4:	6a 00                	push   $0x0
  pushl $102
801073b6:	6a 66                	push   $0x66
  jmp alltraps
801073b8:	e9 60 f4 ff ff       	jmp    8010681d <alltraps>

801073bd <vector103>:
.globl vector103
vector103:
  pushl $0
801073bd:	6a 00                	push   $0x0
  pushl $103
801073bf:	6a 67                	push   $0x67
  jmp alltraps
801073c1:	e9 57 f4 ff ff       	jmp    8010681d <alltraps>

801073c6 <vector104>:
.globl vector104
vector104:
  pushl $0
801073c6:	6a 00                	push   $0x0
  pushl $104
801073c8:	6a 68                	push   $0x68
  jmp alltraps
801073ca:	e9 4e f4 ff ff       	jmp    8010681d <alltraps>

801073cf <vector105>:
.globl vector105
vector105:
  pushl $0
801073cf:	6a 00                	push   $0x0
  pushl $105
801073d1:	6a 69                	push   $0x69
  jmp alltraps
801073d3:	e9 45 f4 ff ff       	jmp    8010681d <alltraps>

801073d8 <vector106>:
.globl vector106
vector106:
  pushl $0
801073d8:	6a 00                	push   $0x0
  pushl $106
801073da:	6a 6a                	push   $0x6a
  jmp alltraps
801073dc:	e9 3c f4 ff ff       	jmp    8010681d <alltraps>

801073e1 <vector107>:
.globl vector107
vector107:
  pushl $0
801073e1:	6a 00                	push   $0x0
  pushl $107
801073e3:	6a 6b                	push   $0x6b
  jmp alltraps
801073e5:	e9 33 f4 ff ff       	jmp    8010681d <alltraps>

801073ea <vector108>:
.globl vector108
vector108:
  pushl $0
801073ea:	6a 00                	push   $0x0
  pushl $108
801073ec:	6a 6c                	push   $0x6c
  jmp alltraps
801073ee:	e9 2a f4 ff ff       	jmp    8010681d <alltraps>

801073f3 <vector109>:
.globl vector109
vector109:
  pushl $0
801073f3:	6a 00                	push   $0x0
  pushl $109
801073f5:	6a 6d                	push   $0x6d
  jmp alltraps
801073f7:	e9 21 f4 ff ff       	jmp    8010681d <alltraps>

801073fc <vector110>:
.globl vector110
vector110:
  pushl $0
801073fc:	6a 00                	push   $0x0
  pushl $110
801073fe:	6a 6e                	push   $0x6e
  jmp alltraps
80107400:	e9 18 f4 ff ff       	jmp    8010681d <alltraps>

80107405 <vector111>:
.globl vector111
vector111:
  pushl $0
80107405:	6a 00                	push   $0x0
  pushl $111
80107407:	6a 6f                	push   $0x6f
  jmp alltraps
80107409:	e9 0f f4 ff ff       	jmp    8010681d <alltraps>

8010740e <vector112>:
.globl vector112
vector112:
  pushl $0
8010740e:	6a 00                	push   $0x0
  pushl $112
80107410:	6a 70                	push   $0x70
  jmp alltraps
80107412:	e9 06 f4 ff ff       	jmp    8010681d <alltraps>

80107417 <vector113>:
.globl vector113
vector113:
  pushl $0
80107417:	6a 00                	push   $0x0
  pushl $113
80107419:	6a 71                	push   $0x71
  jmp alltraps
8010741b:	e9 fd f3 ff ff       	jmp    8010681d <alltraps>

80107420 <vector114>:
.globl vector114
vector114:
  pushl $0
80107420:	6a 00                	push   $0x0
  pushl $114
80107422:	6a 72                	push   $0x72
  jmp alltraps
80107424:	e9 f4 f3 ff ff       	jmp    8010681d <alltraps>

80107429 <vector115>:
.globl vector115
vector115:
  pushl $0
80107429:	6a 00                	push   $0x0
  pushl $115
8010742b:	6a 73                	push   $0x73
  jmp alltraps
8010742d:	e9 eb f3 ff ff       	jmp    8010681d <alltraps>

80107432 <vector116>:
.globl vector116
vector116:
  pushl $0
80107432:	6a 00                	push   $0x0
  pushl $116
80107434:	6a 74                	push   $0x74
  jmp alltraps
80107436:	e9 e2 f3 ff ff       	jmp    8010681d <alltraps>

8010743b <vector117>:
.globl vector117
vector117:
  pushl $0
8010743b:	6a 00                	push   $0x0
  pushl $117
8010743d:	6a 75                	push   $0x75
  jmp alltraps
8010743f:	e9 d9 f3 ff ff       	jmp    8010681d <alltraps>

80107444 <vector118>:
.globl vector118
vector118:
  pushl $0
80107444:	6a 00                	push   $0x0
  pushl $118
80107446:	6a 76                	push   $0x76
  jmp alltraps
80107448:	e9 d0 f3 ff ff       	jmp    8010681d <alltraps>

8010744d <vector119>:
.globl vector119
vector119:
  pushl $0
8010744d:	6a 00                	push   $0x0
  pushl $119
8010744f:	6a 77                	push   $0x77
  jmp alltraps
80107451:	e9 c7 f3 ff ff       	jmp    8010681d <alltraps>

80107456 <vector120>:
.globl vector120
vector120:
  pushl $0
80107456:	6a 00                	push   $0x0
  pushl $120
80107458:	6a 78                	push   $0x78
  jmp alltraps
8010745a:	e9 be f3 ff ff       	jmp    8010681d <alltraps>

8010745f <vector121>:
.globl vector121
vector121:
  pushl $0
8010745f:	6a 00                	push   $0x0
  pushl $121
80107461:	6a 79                	push   $0x79
  jmp alltraps
80107463:	e9 b5 f3 ff ff       	jmp    8010681d <alltraps>

80107468 <vector122>:
.globl vector122
vector122:
  pushl $0
80107468:	6a 00                	push   $0x0
  pushl $122
8010746a:	6a 7a                	push   $0x7a
  jmp alltraps
8010746c:	e9 ac f3 ff ff       	jmp    8010681d <alltraps>

80107471 <vector123>:
.globl vector123
vector123:
  pushl $0
80107471:	6a 00                	push   $0x0
  pushl $123
80107473:	6a 7b                	push   $0x7b
  jmp alltraps
80107475:	e9 a3 f3 ff ff       	jmp    8010681d <alltraps>

8010747a <vector124>:
.globl vector124
vector124:
  pushl $0
8010747a:	6a 00                	push   $0x0
  pushl $124
8010747c:	6a 7c                	push   $0x7c
  jmp alltraps
8010747e:	e9 9a f3 ff ff       	jmp    8010681d <alltraps>

80107483 <vector125>:
.globl vector125
vector125:
  pushl $0
80107483:	6a 00                	push   $0x0
  pushl $125
80107485:	6a 7d                	push   $0x7d
  jmp alltraps
80107487:	e9 91 f3 ff ff       	jmp    8010681d <alltraps>

8010748c <vector126>:
.globl vector126
vector126:
  pushl $0
8010748c:	6a 00                	push   $0x0
  pushl $126
8010748e:	6a 7e                	push   $0x7e
  jmp alltraps
80107490:	e9 88 f3 ff ff       	jmp    8010681d <alltraps>

80107495 <vector127>:
.globl vector127
vector127:
  pushl $0
80107495:	6a 00                	push   $0x0
  pushl $127
80107497:	6a 7f                	push   $0x7f
  jmp alltraps
80107499:	e9 7f f3 ff ff       	jmp    8010681d <alltraps>

8010749e <vector128>:
.globl vector128
vector128:
  pushl $0
8010749e:	6a 00                	push   $0x0
  pushl $128
801074a0:	68 80 00 00 00       	push   $0x80
  jmp alltraps
801074a5:	e9 73 f3 ff ff       	jmp    8010681d <alltraps>

801074aa <vector129>:
.globl vector129
vector129:
  pushl $0
801074aa:	6a 00                	push   $0x0
  pushl $129
801074ac:	68 81 00 00 00       	push   $0x81
  jmp alltraps
801074b1:	e9 67 f3 ff ff       	jmp    8010681d <alltraps>

801074b6 <vector130>:
.globl vector130
vector130:
  pushl $0
801074b6:	6a 00                	push   $0x0
  pushl $130
801074b8:	68 82 00 00 00       	push   $0x82
  jmp alltraps
801074bd:	e9 5b f3 ff ff       	jmp    8010681d <alltraps>

801074c2 <vector131>:
.globl vector131
vector131:
  pushl $0
801074c2:	6a 00                	push   $0x0
  pushl $131
801074c4:	68 83 00 00 00       	push   $0x83
  jmp alltraps
801074c9:	e9 4f f3 ff ff       	jmp    8010681d <alltraps>

801074ce <vector132>:
.globl vector132
vector132:
  pushl $0
801074ce:	6a 00                	push   $0x0
  pushl $132
801074d0:	68 84 00 00 00       	push   $0x84
  jmp alltraps
801074d5:	e9 43 f3 ff ff       	jmp    8010681d <alltraps>

801074da <vector133>:
.globl vector133
vector133:
  pushl $0
801074da:	6a 00                	push   $0x0
  pushl $133
801074dc:	68 85 00 00 00       	push   $0x85
  jmp alltraps
801074e1:	e9 37 f3 ff ff       	jmp    8010681d <alltraps>

801074e6 <vector134>:
.globl vector134
vector134:
  pushl $0
801074e6:	6a 00                	push   $0x0
  pushl $134
801074e8:	68 86 00 00 00       	push   $0x86
  jmp alltraps
801074ed:	e9 2b f3 ff ff       	jmp    8010681d <alltraps>

801074f2 <vector135>:
.globl vector135
vector135:
  pushl $0
801074f2:	6a 00                	push   $0x0
  pushl $135
801074f4:	68 87 00 00 00       	push   $0x87
  jmp alltraps
801074f9:	e9 1f f3 ff ff       	jmp    8010681d <alltraps>

801074fe <vector136>:
.globl vector136
vector136:
  pushl $0
801074fe:	6a 00                	push   $0x0
  pushl $136
80107500:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80107505:	e9 13 f3 ff ff       	jmp    8010681d <alltraps>

8010750a <vector137>:
.globl vector137
vector137:
  pushl $0
8010750a:	6a 00                	push   $0x0
  pushl $137
8010750c:	68 89 00 00 00       	push   $0x89
  jmp alltraps
80107511:	e9 07 f3 ff ff       	jmp    8010681d <alltraps>

80107516 <vector138>:
.globl vector138
vector138:
  pushl $0
80107516:	6a 00                	push   $0x0
  pushl $138
80107518:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
8010751d:	e9 fb f2 ff ff       	jmp    8010681d <alltraps>

80107522 <vector139>:
.globl vector139
vector139:
  pushl $0
80107522:	6a 00                	push   $0x0
  pushl $139
80107524:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80107529:	e9 ef f2 ff ff       	jmp    8010681d <alltraps>

8010752e <vector140>:
.globl vector140
vector140:
  pushl $0
8010752e:	6a 00                	push   $0x0
  pushl $140
80107530:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80107535:	e9 e3 f2 ff ff       	jmp    8010681d <alltraps>

8010753a <vector141>:
.globl vector141
vector141:
  pushl $0
8010753a:	6a 00                	push   $0x0
  pushl $141
8010753c:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
80107541:	e9 d7 f2 ff ff       	jmp    8010681d <alltraps>

80107546 <vector142>:
.globl vector142
vector142:
  pushl $0
80107546:	6a 00                	push   $0x0
  pushl $142
80107548:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
8010754d:	e9 cb f2 ff ff       	jmp    8010681d <alltraps>

80107552 <vector143>:
.globl vector143
vector143:
  pushl $0
80107552:	6a 00                	push   $0x0
  pushl $143
80107554:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80107559:	e9 bf f2 ff ff       	jmp    8010681d <alltraps>

8010755e <vector144>:
.globl vector144
vector144:
  pushl $0
8010755e:	6a 00                	push   $0x0
  pushl $144
80107560:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80107565:	e9 b3 f2 ff ff       	jmp    8010681d <alltraps>

8010756a <vector145>:
.globl vector145
vector145:
  pushl $0
8010756a:	6a 00                	push   $0x0
  pushl $145
8010756c:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80107571:	e9 a7 f2 ff ff       	jmp    8010681d <alltraps>

80107576 <vector146>:
.globl vector146
vector146:
  pushl $0
80107576:	6a 00                	push   $0x0
  pushl $146
80107578:	68 92 00 00 00       	push   $0x92
  jmp alltraps
8010757d:	e9 9b f2 ff ff       	jmp    8010681d <alltraps>

80107582 <vector147>:
.globl vector147
vector147:
  pushl $0
80107582:	6a 00                	push   $0x0
  pushl $147
80107584:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80107589:	e9 8f f2 ff ff       	jmp    8010681d <alltraps>

8010758e <vector148>:
.globl vector148
vector148:
  pushl $0
8010758e:	6a 00                	push   $0x0
  pushl $148
80107590:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80107595:	e9 83 f2 ff ff       	jmp    8010681d <alltraps>

8010759a <vector149>:
.globl vector149
vector149:
  pushl $0
8010759a:	6a 00                	push   $0x0
  pushl $149
8010759c:	68 95 00 00 00       	push   $0x95
  jmp alltraps
801075a1:	e9 77 f2 ff ff       	jmp    8010681d <alltraps>

801075a6 <vector150>:
.globl vector150
vector150:
  pushl $0
801075a6:	6a 00                	push   $0x0
  pushl $150
801075a8:	68 96 00 00 00       	push   $0x96
  jmp alltraps
801075ad:	e9 6b f2 ff ff       	jmp    8010681d <alltraps>

801075b2 <vector151>:
.globl vector151
vector151:
  pushl $0
801075b2:	6a 00                	push   $0x0
  pushl $151
801075b4:	68 97 00 00 00       	push   $0x97
  jmp alltraps
801075b9:	e9 5f f2 ff ff       	jmp    8010681d <alltraps>

801075be <vector152>:
.globl vector152
vector152:
  pushl $0
801075be:	6a 00                	push   $0x0
  pushl $152
801075c0:	68 98 00 00 00       	push   $0x98
  jmp alltraps
801075c5:	e9 53 f2 ff ff       	jmp    8010681d <alltraps>

801075ca <vector153>:
.globl vector153
vector153:
  pushl $0
801075ca:	6a 00                	push   $0x0
  pushl $153
801075cc:	68 99 00 00 00       	push   $0x99
  jmp alltraps
801075d1:	e9 47 f2 ff ff       	jmp    8010681d <alltraps>

801075d6 <vector154>:
.globl vector154
vector154:
  pushl $0
801075d6:	6a 00                	push   $0x0
  pushl $154
801075d8:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
801075dd:	e9 3b f2 ff ff       	jmp    8010681d <alltraps>

801075e2 <vector155>:
.globl vector155
vector155:
  pushl $0
801075e2:	6a 00                	push   $0x0
  pushl $155
801075e4:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
801075e9:	e9 2f f2 ff ff       	jmp    8010681d <alltraps>

801075ee <vector156>:
.globl vector156
vector156:
  pushl $0
801075ee:	6a 00                	push   $0x0
  pushl $156
801075f0:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
801075f5:	e9 23 f2 ff ff       	jmp    8010681d <alltraps>

801075fa <vector157>:
.globl vector157
vector157:
  pushl $0
801075fa:	6a 00                	push   $0x0
  pushl $157
801075fc:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
80107601:	e9 17 f2 ff ff       	jmp    8010681d <alltraps>

80107606 <vector158>:
.globl vector158
vector158:
  pushl $0
80107606:	6a 00                	push   $0x0
  pushl $158
80107608:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
8010760d:	e9 0b f2 ff ff       	jmp    8010681d <alltraps>

80107612 <vector159>:
.globl vector159
vector159:
  pushl $0
80107612:	6a 00                	push   $0x0
  pushl $159
80107614:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80107619:	e9 ff f1 ff ff       	jmp    8010681d <alltraps>

8010761e <vector160>:
.globl vector160
vector160:
  pushl $0
8010761e:	6a 00                	push   $0x0
  pushl $160
80107620:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80107625:	e9 f3 f1 ff ff       	jmp    8010681d <alltraps>

8010762a <vector161>:
.globl vector161
vector161:
  pushl $0
8010762a:	6a 00                	push   $0x0
  pushl $161
8010762c:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80107631:	e9 e7 f1 ff ff       	jmp    8010681d <alltraps>

80107636 <vector162>:
.globl vector162
vector162:
  pushl $0
80107636:	6a 00                	push   $0x0
  pushl $162
80107638:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
8010763d:	e9 db f1 ff ff       	jmp    8010681d <alltraps>

80107642 <vector163>:
.globl vector163
vector163:
  pushl $0
80107642:	6a 00                	push   $0x0
  pushl $163
80107644:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80107649:	e9 cf f1 ff ff       	jmp    8010681d <alltraps>

8010764e <vector164>:
.globl vector164
vector164:
  pushl $0
8010764e:	6a 00                	push   $0x0
  pushl $164
80107650:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
80107655:	e9 c3 f1 ff ff       	jmp    8010681d <alltraps>

8010765a <vector165>:
.globl vector165
vector165:
  pushl $0
8010765a:	6a 00                	push   $0x0
  pushl $165
8010765c:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80107661:	e9 b7 f1 ff ff       	jmp    8010681d <alltraps>

80107666 <vector166>:
.globl vector166
vector166:
  pushl $0
80107666:	6a 00                	push   $0x0
  pushl $166
80107668:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
8010766d:	e9 ab f1 ff ff       	jmp    8010681d <alltraps>

80107672 <vector167>:
.globl vector167
vector167:
  pushl $0
80107672:	6a 00                	push   $0x0
  pushl $167
80107674:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80107679:	e9 9f f1 ff ff       	jmp    8010681d <alltraps>

8010767e <vector168>:
.globl vector168
vector168:
  pushl $0
8010767e:	6a 00                	push   $0x0
  pushl $168
80107680:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80107685:	e9 93 f1 ff ff       	jmp    8010681d <alltraps>

8010768a <vector169>:
.globl vector169
vector169:
  pushl $0
8010768a:	6a 00                	push   $0x0
  pushl $169
8010768c:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80107691:	e9 87 f1 ff ff       	jmp    8010681d <alltraps>

80107696 <vector170>:
.globl vector170
vector170:
  pushl $0
80107696:	6a 00                	push   $0x0
  pushl $170
80107698:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
8010769d:	e9 7b f1 ff ff       	jmp    8010681d <alltraps>

801076a2 <vector171>:
.globl vector171
vector171:
  pushl $0
801076a2:	6a 00                	push   $0x0
  pushl $171
801076a4:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
801076a9:	e9 6f f1 ff ff       	jmp    8010681d <alltraps>

801076ae <vector172>:
.globl vector172
vector172:
  pushl $0
801076ae:	6a 00                	push   $0x0
  pushl $172
801076b0:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
801076b5:	e9 63 f1 ff ff       	jmp    8010681d <alltraps>

801076ba <vector173>:
.globl vector173
vector173:
  pushl $0
801076ba:	6a 00                	push   $0x0
  pushl $173
801076bc:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
801076c1:	e9 57 f1 ff ff       	jmp    8010681d <alltraps>

801076c6 <vector174>:
.globl vector174
vector174:
  pushl $0
801076c6:	6a 00                	push   $0x0
  pushl $174
801076c8:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
801076cd:	e9 4b f1 ff ff       	jmp    8010681d <alltraps>

801076d2 <vector175>:
.globl vector175
vector175:
  pushl $0
801076d2:	6a 00                	push   $0x0
  pushl $175
801076d4:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
801076d9:	e9 3f f1 ff ff       	jmp    8010681d <alltraps>

801076de <vector176>:
.globl vector176
vector176:
  pushl $0
801076de:	6a 00                	push   $0x0
  pushl $176
801076e0:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
801076e5:	e9 33 f1 ff ff       	jmp    8010681d <alltraps>

801076ea <vector177>:
.globl vector177
vector177:
  pushl $0
801076ea:	6a 00                	push   $0x0
  pushl $177
801076ec:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
801076f1:	e9 27 f1 ff ff       	jmp    8010681d <alltraps>

801076f6 <vector178>:
.globl vector178
vector178:
  pushl $0
801076f6:	6a 00                	push   $0x0
  pushl $178
801076f8:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
801076fd:	e9 1b f1 ff ff       	jmp    8010681d <alltraps>

80107702 <vector179>:
.globl vector179
vector179:
  pushl $0
80107702:	6a 00                	push   $0x0
  pushl $179
80107704:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80107709:	e9 0f f1 ff ff       	jmp    8010681d <alltraps>

8010770e <vector180>:
.globl vector180
vector180:
  pushl $0
8010770e:	6a 00                	push   $0x0
  pushl $180
80107710:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
80107715:	e9 03 f1 ff ff       	jmp    8010681d <alltraps>

8010771a <vector181>:
.globl vector181
vector181:
  pushl $0
8010771a:	6a 00                	push   $0x0
  pushl $181
8010771c:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
80107721:	e9 f7 f0 ff ff       	jmp    8010681d <alltraps>

80107726 <vector182>:
.globl vector182
vector182:
  pushl $0
80107726:	6a 00                	push   $0x0
  pushl $182
80107728:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
8010772d:	e9 eb f0 ff ff       	jmp    8010681d <alltraps>

80107732 <vector183>:
.globl vector183
vector183:
  pushl $0
80107732:	6a 00                	push   $0x0
  pushl $183
80107734:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80107739:	e9 df f0 ff ff       	jmp    8010681d <alltraps>

8010773e <vector184>:
.globl vector184
vector184:
  pushl $0
8010773e:	6a 00                	push   $0x0
  pushl $184
80107740:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
80107745:	e9 d3 f0 ff ff       	jmp    8010681d <alltraps>

8010774a <vector185>:
.globl vector185
vector185:
  pushl $0
8010774a:	6a 00                	push   $0x0
  pushl $185
8010774c:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
80107751:	e9 c7 f0 ff ff       	jmp    8010681d <alltraps>

80107756 <vector186>:
.globl vector186
vector186:
  pushl $0
80107756:	6a 00                	push   $0x0
  pushl $186
80107758:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
8010775d:	e9 bb f0 ff ff       	jmp    8010681d <alltraps>

80107762 <vector187>:
.globl vector187
vector187:
  pushl $0
80107762:	6a 00                	push   $0x0
  pushl $187
80107764:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
80107769:	e9 af f0 ff ff       	jmp    8010681d <alltraps>

8010776e <vector188>:
.globl vector188
vector188:
  pushl $0
8010776e:	6a 00                	push   $0x0
  pushl $188
80107770:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
80107775:	e9 a3 f0 ff ff       	jmp    8010681d <alltraps>

8010777a <vector189>:
.globl vector189
vector189:
  pushl $0
8010777a:	6a 00                	push   $0x0
  pushl $189
8010777c:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
80107781:	e9 97 f0 ff ff       	jmp    8010681d <alltraps>

80107786 <vector190>:
.globl vector190
vector190:
  pushl $0
80107786:	6a 00                	push   $0x0
  pushl $190
80107788:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
8010778d:	e9 8b f0 ff ff       	jmp    8010681d <alltraps>

80107792 <vector191>:
.globl vector191
vector191:
  pushl $0
80107792:	6a 00                	push   $0x0
  pushl $191
80107794:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
80107799:	e9 7f f0 ff ff       	jmp    8010681d <alltraps>

8010779e <vector192>:
.globl vector192
vector192:
  pushl $0
8010779e:	6a 00                	push   $0x0
  pushl $192
801077a0:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
801077a5:	e9 73 f0 ff ff       	jmp    8010681d <alltraps>

801077aa <vector193>:
.globl vector193
vector193:
  pushl $0
801077aa:	6a 00                	push   $0x0
  pushl $193
801077ac:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
801077b1:	e9 67 f0 ff ff       	jmp    8010681d <alltraps>

801077b6 <vector194>:
.globl vector194
vector194:
  pushl $0
801077b6:	6a 00                	push   $0x0
  pushl $194
801077b8:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
801077bd:	e9 5b f0 ff ff       	jmp    8010681d <alltraps>

801077c2 <vector195>:
.globl vector195
vector195:
  pushl $0
801077c2:	6a 00                	push   $0x0
  pushl $195
801077c4:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
801077c9:	e9 4f f0 ff ff       	jmp    8010681d <alltraps>

801077ce <vector196>:
.globl vector196
vector196:
  pushl $0
801077ce:	6a 00                	push   $0x0
  pushl $196
801077d0:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
801077d5:	e9 43 f0 ff ff       	jmp    8010681d <alltraps>

801077da <vector197>:
.globl vector197
vector197:
  pushl $0
801077da:	6a 00                	push   $0x0
  pushl $197
801077dc:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
801077e1:	e9 37 f0 ff ff       	jmp    8010681d <alltraps>

801077e6 <vector198>:
.globl vector198
vector198:
  pushl $0
801077e6:	6a 00                	push   $0x0
  pushl $198
801077e8:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
801077ed:	e9 2b f0 ff ff       	jmp    8010681d <alltraps>

801077f2 <vector199>:
.globl vector199
vector199:
  pushl $0
801077f2:	6a 00                	push   $0x0
  pushl $199
801077f4:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
801077f9:	e9 1f f0 ff ff       	jmp    8010681d <alltraps>

801077fe <vector200>:
.globl vector200
vector200:
  pushl $0
801077fe:	6a 00                	push   $0x0
  pushl $200
80107800:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
80107805:	e9 13 f0 ff ff       	jmp    8010681d <alltraps>

8010780a <vector201>:
.globl vector201
vector201:
  pushl $0
8010780a:	6a 00                	push   $0x0
  pushl $201
8010780c:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
80107811:	e9 07 f0 ff ff       	jmp    8010681d <alltraps>

80107816 <vector202>:
.globl vector202
vector202:
  pushl $0
80107816:	6a 00                	push   $0x0
  pushl $202
80107818:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
8010781d:	e9 fb ef ff ff       	jmp    8010681d <alltraps>

80107822 <vector203>:
.globl vector203
vector203:
  pushl $0
80107822:	6a 00                	push   $0x0
  pushl $203
80107824:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80107829:	e9 ef ef ff ff       	jmp    8010681d <alltraps>

8010782e <vector204>:
.globl vector204
vector204:
  pushl $0
8010782e:	6a 00                	push   $0x0
  pushl $204
80107830:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
80107835:	e9 e3 ef ff ff       	jmp    8010681d <alltraps>

8010783a <vector205>:
.globl vector205
vector205:
  pushl $0
8010783a:	6a 00                	push   $0x0
  pushl $205
8010783c:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
80107841:	e9 d7 ef ff ff       	jmp    8010681d <alltraps>

80107846 <vector206>:
.globl vector206
vector206:
  pushl $0
80107846:	6a 00                	push   $0x0
  pushl $206
80107848:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
8010784d:	e9 cb ef ff ff       	jmp    8010681d <alltraps>

80107852 <vector207>:
.globl vector207
vector207:
  pushl $0
80107852:	6a 00                	push   $0x0
  pushl $207
80107854:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
80107859:	e9 bf ef ff ff       	jmp    8010681d <alltraps>

8010785e <vector208>:
.globl vector208
vector208:
  pushl $0
8010785e:	6a 00                	push   $0x0
  pushl $208
80107860:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
80107865:	e9 b3 ef ff ff       	jmp    8010681d <alltraps>

8010786a <vector209>:
.globl vector209
vector209:
  pushl $0
8010786a:	6a 00                	push   $0x0
  pushl $209
8010786c:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
80107871:	e9 a7 ef ff ff       	jmp    8010681d <alltraps>

80107876 <vector210>:
.globl vector210
vector210:
  pushl $0
80107876:	6a 00                	push   $0x0
  pushl $210
80107878:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
8010787d:	e9 9b ef ff ff       	jmp    8010681d <alltraps>

80107882 <vector211>:
.globl vector211
vector211:
  pushl $0
80107882:	6a 00                	push   $0x0
  pushl $211
80107884:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80107889:	e9 8f ef ff ff       	jmp    8010681d <alltraps>

8010788e <vector212>:
.globl vector212
vector212:
  pushl $0
8010788e:	6a 00                	push   $0x0
  pushl $212
80107890:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
80107895:	e9 83 ef ff ff       	jmp    8010681d <alltraps>

8010789a <vector213>:
.globl vector213
vector213:
  pushl $0
8010789a:	6a 00                	push   $0x0
  pushl $213
8010789c:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
801078a1:	e9 77 ef ff ff       	jmp    8010681d <alltraps>

801078a6 <vector214>:
.globl vector214
vector214:
  pushl $0
801078a6:	6a 00                	push   $0x0
  pushl $214
801078a8:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
801078ad:	e9 6b ef ff ff       	jmp    8010681d <alltraps>

801078b2 <vector215>:
.globl vector215
vector215:
  pushl $0
801078b2:	6a 00                	push   $0x0
  pushl $215
801078b4:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
801078b9:	e9 5f ef ff ff       	jmp    8010681d <alltraps>

801078be <vector216>:
.globl vector216
vector216:
  pushl $0
801078be:	6a 00                	push   $0x0
  pushl $216
801078c0:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
801078c5:	e9 53 ef ff ff       	jmp    8010681d <alltraps>

801078ca <vector217>:
.globl vector217
vector217:
  pushl $0
801078ca:	6a 00                	push   $0x0
  pushl $217
801078cc:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
801078d1:	e9 47 ef ff ff       	jmp    8010681d <alltraps>

801078d6 <vector218>:
.globl vector218
vector218:
  pushl $0
801078d6:	6a 00                	push   $0x0
  pushl $218
801078d8:	68 da 00 00 00       	push   $0xda
  jmp alltraps
801078dd:	e9 3b ef ff ff       	jmp    8010681d <alltraps>

801078e2 <vector219>:
.globl vector219
vector219:
  pushl $0
801078e2:	6a 00                	push   $0x0
  pushl $219
801078e4:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
801078e9:	e9 2f ef ff ff       	jmp    8010681d <alltraps>

801078ee <vector220>:
.globl vector220
vector220:
  pushl $0
801078ee:	6a 00                	push   $0x0
  pushl $220
801078f0:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
801078f5:	e9 23 ef ff ff       	jmp    8010681d <alltraps>

801078fa <vector221>:
.globl vector221
vector221:
  pushl $0
801078fa:	6a 00                	push   $0x0
  pushl $221
801078fc:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
80107901:	e9 17 ef ff ff       	jmp    8010681d <alltraps>

80107906 <vector222>:
.globl vector222
vector222:
  pushl $0
80107906:	6a 00                	push   $0x0
  pushl $222
80107908:	68 de 00 00 00       	push   $0xde
  jmp alltraps
8010790d:	e9 0b ef ff ff       	jmp    8010681d <alltraps>

80107912 <vector223>:
.globl vector223
vector223:
  pushl $0
80107912:	6a 00                	push   $0x0
  pushl $223
80107914:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
80107919:	e9 ff ee ff ff       	jmp    8010681d <alltraps>

8010791e <vector224>:
.globl vector224
vector224:
  pushl $0
8010791e:	6a 00                	push   $0x0
  pushl $224
80107920:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
80107925:	e9 f3 ee ff ff       	jmp    8010681d <alltraps>

8010792a <vector225>:
.globl vector225
vector225:
  pushl $0
8010792a:	6a 00                	push   $0x0
  pushl $225
8010792c:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
80107931:	e9 e7 ee ff ff       	jmp    8010681d <alltraps>

80107936 <vector226>:
.globl vector226
vector226:
  pushl $0
80107936:	6a 00                	push   $0x0
  pushl $226
80107938:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
8010793d:	e9 db ee ff ff       	jmp    8010681d <alltraps>

80107942 <vector227>:
.globl vector227
vector227:
  pushl $0
80107942:	6a 00                	push   $0x0
  pushl $227
80107944:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
80107949:	e9 cf ee ff ff       	jmp    8010681d <alltraps>

8010794e <vector228>:
.globl vector228
vector228:
  pushl $0
8010794e:	6a 00                	push   $0x0
  pushl $228
80107950:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
80107955:	e9 c3 ee ff ff       	jmp    8010681d <alltraps>

8010795a <vector229>:
.globl vector229
vector229:
  pushl $0
8010795a:	6a 00                	push   $0x0
  pushl $229
8010795c:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
80107961:	e9 b7 ee ff ff       	jmp    8010681d <alltraps>

80107966 <vector230>:
.globl vector230
vector230:
  pushl $0
80107966:	6a 00                	push   $0x0
  pushl $230
80107968:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
8010796d:	e9 ab ee ff ff       	jmp    8010681d <alltraps>

80107972 <vector231>:
.globl vector231
vector231:
  pushl $0
80107972:	6a 00                	push   $0x0
  pushl $231
80107974:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
80107979:	e9 9f ee ff ff       	jmp    8010681d <alltraps>

8010797e <vector232>:
.globl vector232
vector232:
  pushl $0
8010797e:	6a 00                	push   $0x0
  pushl $232
80107980:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
80107985:	e9 93 ee ff ff       	jmp    8010681d <alltraps>

8010798a <vector233>:
.globl vector233
vector233:
  pushl $0
8010798a:	6a 00                	push   $0x0
  pushl $233
8010798c:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
80107991:	e9 87 ee ff ff       	jmp    8010681d <alltraps>

80107996 <vector234>:
.globl vector234
vector234:
  pushl $0
80107996:	6a 00                	push   $0x0
  pushl $234
80107998:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
8010799d:	e9 7b ee ff ff       	jmp    8010681d <alltraps>

801079a2 <vector235>:
.globl vector235
vector235:
  pushl $0
801079a2:	6a 00                	push   $0x0
  pushl $235
801079a4:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
801079a9:	e9 6f ee ff ff       	jmp    8010681d <alltraps>

801079ae <vector236>:
.globl vector236
vector236:
  pushl $0
801079ae:	6a 00                	push   $0x0
  pushl $236
801079b0:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
801079b5:	e9 63 ee ff ff       	jmp    8010681d <alltraps>

801079ba <vector237>:
.globl vector237
vector237:
  pushl $0
801079ba:	6a 00                	push   $0x0
  pushl $237
801079bc:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
801079c1:	e9 57 ee ff ff       	jmp    8010681d <alltraps>

801079c6 <vector238>:
.globl vector238
vector238:
  pushl $0
801079c6:	6a 00                	push   $0x0
  pushl $238
801079c8:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
801079cd:	e9 4b ee ff ff       	jmp    8010681d <alltraps>

801079d2 <vector239>:
.globl vector239
vector239:
  pushl $0
801079d2:	6a 00                	push   $0x0
  pushl $239
801079d4:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
801079d9:	e9 3f ee ff ff       	jmp    8010681d <alltraps>

801079de <vector240>:
.globl vector240
vector240:
  pushl $0
801079de:	6a 00                	push   $0x0
  pushl $240
801079e0:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
801079e5:	e9 33 ee ff ff       	jmp    8010681d <alltraps>

801079ea <vector241>:
.globl vector241
vector241:
  pushl $0
801079ea:	6a 00                	push   $0x0
  pushl $241
801079ec:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
801079f1:	e9 27 ee ff ff       	jmp    8010681d <alltraps>

801079f6 <vector242>:
.globl vector242
vector242:
  pushl $0
801079f6:	6a 00                	push   $0x0
  pushl $242
801079f8:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
801079fd:	e9 1b ee ff ff       	jmp    8010681d <alltraps>

80107a02 <vector243>:
.globl vector243
vector243:
  pushl $0
80107a02:	6a 00                	push   $0x0
  pushl $243
80107a04:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
80107a09:	e9 0f ee ff ff       	jmp    8010681d <alltraps>

80107a0e <vector244>:
.globl vector244
vector244:
  pushl $0
80107a0e:	6a 00                	push   $0x0
  pushl $244
80107a10:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
80107a15:	e9 03 ee ff ff       	jmp    8010681d <alltraps>

80107a1a <vector245>:
.globl vector245
vector245:
  pushl $0
80107a1a:	6a 00                	push   $0x0
  pushl $245
80107a1c:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
80107a21:	e9 f7 ed ff ff       	jmp    8010681d <alltraps>

80107a26 <vector246>:
.globl vector246
vector246:
  pushl $0
80107a26:	6a 00                	push   $0x0
  pushl $246
80107a28:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80107a2d:	e9 eb ed ff ff       	jmp    8010681d <alltraps>

80107a32 <vector247>:
.globl vector247
vector247:
  pushl $0
80107a32:	6a 00                	push   $0x0
  pushl $247
80107a34:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80107a39:	e9 df ed ff ff       	jmp    8010681d <alltraps>

80107a3e <vector248>:
.globl vector248
vector248:
  pushl $0
80107a3e:	6a 00                	push   $0x0
  pushl $248
80107a40:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
80107a45:	e9 d3 ed ff ff       	jmp    8010681d <alltraps>

80107a4a <vector249>:
.globl vector249
vector249:
  pushl $0
80107a4a:	6a 00                	push   $0x0
  pushl $249
80107a4c:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
80107a51:	e9 c7 ed ff ff       	jmp    8010681d <alltraps>

80107a56 <vector250>:
.globl vector250
vector250:
  pushl $0
80107a56:	6a 00                	push   $0x0
  pushl $250
80107a58:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
80107a5d:	e9 bb ed ff ff       	jmp    8010681d <alltraps>

80107a62 <vector251>:
.globl vector251
vector251:
  pushl $0
80107a62:	6a 00                	push   $0x0
  pushl $251
80107a64:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80107a69:	e9 af ed ff ff       	jmp    8010681d <alltraps>

80107a6e <vector252>:
.globl vector252
vector252:
  pushl $0
80107a6e:	6a 00                	push   $0x0
  pushl $252
80107a70:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
80107a75:	e9 a3 ed ff ff       	jmp    8010681d <alltraps>

80107a7a <vector253>:
.globl vector253
vector253:
  pushl $0
80107a7a:	6a 00                	push   $0x0
  pushl $253
80107a7c:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
80107a81:	e9 97 ed ff ff       	jmp    8010681d <alltraps>

80107a86 <vector254>:
.globl vector254
vector254:
  pushl $0
80107a86:	6a 00                	push   $0x0
  pushl $254
80107a88:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
80107a8d:	e9 8b ed ff ff       	jmp    8010681d <alltraps>

80107a92 <vector255>:
.globl vector255
vector255:
  pushl $0
80107a92:	6a 00                	push   $0x0
  pushl $255
80107a94:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
80107a99:	e9 7f ed ff ff       	jmp    8010681d <alltraps>

80107a9e <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
80107a9e:	55                   	push   %ebp
80107a9f:	89 e5                	mov    %esp,%ebp
80107aa1:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80107aa4:	8b 45 0c             	mov    0xc(%ebp),%eax
80107aa7:	83 e8 01             	sub    $0x1,%eax
80107aaa:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80107aae:	8b 45 08             	mov    0x8(%ebp),%eax
80107ab1:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80107ab5:	8b 45 08             	mov    0x8(%ebp),%eax
80107ab8:	c1 e8 10             	shr    $0x10,%eax
80107abb:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
80107abf:	8d 45 fa             	lea    -0x6(%ebp),%eax
80107ac2:	0f 01 10             	lgdtl  (%eax)
}
80107ac5:	c9                   	leave  
80107ac6:	c3                   	ret    

80107ac7 <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
80107ac7:	55                   	push   %ebp
80107ac8:	89 e5                	mov    %esp,%ebp
80107aca:	83 ec 04             	sub    $0x4,%esp
80107acd:	8b 45 08             	mov    0x8(%ebp),%eax
80107ad0:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
80107ad4:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107ad8:	0f 00 d8             	ltr    %ax
}
80107adb:	c9                   	leave  
80107adc:	c3                   	ret    

80107add <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
80107add:	55                   	push   %ebp
80107ade:	89 e5                	mov    %esp,%ebp
80107ae0:	83 ec 04             	sub    $0x4,%esp
80107ae3:	8b 45 08             	mov    0x8(%ebp),%eax
80107ae6:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
80107aea:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107aee:	8e e8                	mov    %eax,%gs
}
80107af0:	c9                   	leave  
80107af1:	c3                   	ret    

80107af2 <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
80107af2:	55                   	push   %ebp
80107af3:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
80107af5:	8b 45 08             	mov    0x8(%ebp),%eax
80107af8:	0f 22 d8             	mov    %eax,%cr3
}
80107afb:	5d                   	pop    %ebp
80107afc:	c3                   	ret    

80107afd <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
80107afd:	55                   	push   %ebp
80107afe:	89 e5                	mov    %esp,%ebp
80107b00:	8b 45 08             	mov    0x8(%ebp),%eax
80107b03:	05 00 00 00 80       	add    $0x80000000,%eax
80107b08:	5d                   	pop    %ebp
80107b09:	c3                   	ret    

80107b0a <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
80107b0a:	55                   	push   %ebp
80107b0b:	89 e5                	mov    %esp,%ebp
80107b0d:	8b 45 08             	mov    0x8(%ebp),%eax
80107b10:	05 00 00 00 80       	add    $0x80000000,%eax
80107b15:	5d                   	pop    %ebp
80107b16:	c3                   	ret    

80107b17 <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
80107b17:	55                   	push   %ebp
80107b18:	89 e5                	mov    %esp,%ebp
80107b1a:	53                   	push   %ebx
80107b1b:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
80107b1e:	e8 55 b3 ff ff       	call   80102e78 <cpunum>
80107b23:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80107b29:	05 80 23 11 80       	add    $0x80112380,%eax
80107b2e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
80107b31:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b34:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
80107b3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b3d:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
80107b43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b46:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
80107b4a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b4d:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b51:	83 e2 f0             	and    $0xfffffff0,%edx
80107b54:	83 ca 0a             	or     $0xa,%edx
80107b57:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b5a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b5d:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b61:	83 ca 10             	or     $0x10,%edx
80107b64:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b67:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b6a:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b6e:	83 e2 9f             	and    $0xffffff9f,%edx
80107b71:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b74:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b77:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107b7b:	83 ca 80             	or     $0xffffff80,%edx
80107b7e:	88 50 7d             	mov    %dl,0x7d(%eax)
80107b81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b84:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b88:	83 ca 0f             	or     $0xf,%edx
80107b8b:	88 50 7e             	mov    %dl,0x7e(%eax)
80107b8e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b91:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107b95:	83 e2 ef             	and    $0xffffffef,%edx
80107b98:	88 50 7e             	mov    %dl,0x7e(%eax)
80107b9b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b9e:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107ba2:	83 e2 df             	and    $0xffffffdf,%edx
80107ba5:	88 50 7e             	mov    %dl,0x7e(%eax)
80107ba8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bab:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107baf:	83 ca 40             	or     $0x40,%edx
80107bb2:	88 50 7e             	mov    %dl,0x7e(%eax)
80107bb5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bb8:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107bbc:	83 ca 80             	or     $0xffffff80,%edx
80107bbf:	88 50 7e             	mov    %dl,0x7e(%eax)
80107bc2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bc5:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80107bc9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bcc:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
80107bd3:	ff ff 
80107bd5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bd8:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
80107bdf:	00 00 
80107be1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107be4:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
80107beb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bee:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107bf5:	83 e2 f0             	and    $0xfffffff0,%edx
80107bf8:	83 ca 02             	or     $0x2,%edx
80107bfb:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107c01:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c04:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107c0b:	83 ca 10             	or     $0x10,%edx
80107c0e:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107c14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c17:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107c1e:	83 e2 9f             	and    $0xffffff9f,%edx
80107c21:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107c27:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c2a:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107c31:	83 ca 80             	or     $0xffffff80,%edx
80107c34:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107c3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c3d:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c44:	83 ca 0f             	or     $0xf,%edx
80107c47:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c50:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c57:	83 e2 ef             	and    $0xffffffef,%edx
80107c5a:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c60:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c63:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c6a:	83 e2 df             	and    $0xffffffdf,%edx
80107c6d:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c76:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c7d:	83 ca 40             	or     $0x40,%edx
80107c80:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c86:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c89:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107c90:	83 ca 80             	or     $0xffffff80,%edx
80107c93:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107c99:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c9c:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80107ca3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ca6:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
80107cad:	ff ff 
80107caf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cb2:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
80107cb9:	00 00 
80107cbb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cbe:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
80107cc5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cc8:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107ccf:	83 e2 f0             	and    $0xfffffff0,%edx
80107cd2:	83 ca 0a             	or     $0xa,%edx
80107cd5:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107cdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cde:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107ce5:	83 ca 10             	or     $0x10,%edx
80107ce8:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107cee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cf1:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107cf8:	83 ca 60             	or     $0x60,%edx
80107cfb:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107d01:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d04:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107d0b:	83 ca 80             	or     $0xffffff80,%edx
80107d0e:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107d14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d17:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d1e:	83 ca 0f             	or     $0xf,%edx
80107d21:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d27:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d2a:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d31:	83 e2 ef             	and    $0xffffffef,%edx
80107d34:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d3d:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d44:	83 e2 df             	and    $0xffffffdf,%edx
80107d47:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d50:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d57:	83 ca 40             	or     $0x40,%edx
80107d5a:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d60:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d63:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107d6a:	83 ca 80             	or     $0xffffff80,%edx
80107d6d:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107d73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d76:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80107d7d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d80:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
80107d87:	ff ff 
80107d89:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d8c:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
80107d93:	00 00 
80107d95:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d98:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
80107d9f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107da2:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107da9:	83 e2 f0             	and    $0xfffffff0,%edx
80107dac:	83 ca 02             	or     $0x2,%edx
80107daf:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107db5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107db8:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107dbf:	83 ca 10             	or     $0x10,%edx
80107dc2:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107dc8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dcb:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107dd2:	83 ca 60             	or     $0x60,%edx
80107dd5:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107ddb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dde:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107de5:	83 ca 80             	or     $0xffffff80,%edx
80107de8:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107dee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107df1:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107df8:	83 ca 0f             	or     $0xf,%edx
80107dfb:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e01:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e04:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e0b:	83 e2 ef             	and    $0xffffffef,%edx
80107e0e:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e17:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e1e:	83 e2 df             	and    $0xffffffdf,%edx
80107e21:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e27:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e2a:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e31:	83 ca 40             	or     $0x40,%edx
80107e34:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e3d:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107e44:	83 ca 80             	or     $0xffffff80,%edx
80107e47:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107e4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e50:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80107e57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e5a:	05 b4 00 00 00       	add    $0xb4,%eax
80107e5f:	89 c3                	mov    %eax,%ebx
80107e61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e64:	05 b4 00 00 00       	add    $0xb4,%eax
80107e69:	c1 e8 10             	shr    $0x10,%eax
80107e6c:	89 c1                	mov    %eax,%ecx
80107e6e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e71:	05 b4 00 00 00       	add    $0xb4,%eax
80107e76:	c1 e8 18             	shr    $0x18,%eax
80107e79:	89 c2                	mov    %eax,%edx
80107e7b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e7e:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
80107e85:	00 00 
80107e87:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e8a:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
80107e91:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e94:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
80107e9a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e9d:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107ea4:	83 e1 f0             	and    $0xfffffff0,%ecx
80107ea7:	83 c9 02             	or     $0x2,%ecx
80107eaa:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107eb0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107eb3:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107eba:	83 c9 10             	or     $0x10,%ecx
80107ebd:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107ec3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ec6:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107ecd:	83 e1 9f             	and    $0xffffff9f,%ecx
80107ed0:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107ed6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ed9:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107ee0:	83 c9 80             	or     $0xffffff80,%ecx
80107ee3:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107ee9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107eec:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107ef3:	83 e1 f0             	and    $0xfffffff0,%ecx
80107ef6:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107efc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107eff:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107f06:	83 e1 ef             	and    $0xffffffef,%ecx
80107f09:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f12:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107f19:	83 e1 df             	and    $0xffffffdf,%ecx
80107f1c:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f22:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f25:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107f2c:	83 c9 40             	or     $0x40,%ecx
80107f2f:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f35:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f38:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107f3f:	83 c9 80             	or     $0xffffff80,%ecx
80107f42:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107f48:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f4b:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
80107f51:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f54:	83 c0 70             	add    $0x70,%eax
80107f57:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
80107f5e:	00 
80107f5f:	89 04 24             	mov    %eax,(%esp)
80107f62:	e8 37 fb ff ff       	call   80107a9e <lgdt>
  loadgs(SEG_KCPU << 3);
80107f67:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
80107f6e:	e8 6a fb ff ff       	call   80107add <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
80107f73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f76:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
80107f7c:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80107f83:	00 00 00 00 
}
80107f87:	83 c4 24             	add    $0x24,%esp
80107f8a:	5b                   	pop    %ebx
80107f8b:	5d                   	pop    %ebp
80107f8c:	c3                   	ret    

80107f8d <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80107f8d:	55                   	push   %ebp
80107f8e:	89 e5                	mov    %esp,%ebp
80107f90:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80107f93:	8b 45 0c             	mov    0xc(%ebp),%eax
80107f96:	c1 e8 16             	shr    $0x16,%eax
80107f99:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107fa0:	8b 45 08             	mov    0x8(%ebp),%eax
80107fa3:	01 d0                	add    %edx,%eax
80107fa5:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80107fa8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107fab:	8b 00                	mov    (%eax),%eax
80107fad:	83 e0 01             	and    $0x1,%eax
80107fb0:	85 c0                	test   %eax,%eax
80107fb2:	74 17                	je     80107fcb <walkpgdir+0x3e>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
80107fb4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107fb7:	8b 00                	mov    (%eax),%eax
80107fb9:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107fbe:	89 04 24             	mov    %eax,(%esp)
80107fc1:	e8 44 fb ff ff       	call   80107b0a <p2v>
80107fc6:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107fc9:	eb 4b                	jmp    80108016 <walkpgdir+0x89>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80107fcb:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80107fcf:	74 0e                	je     80107fdf <walkpgdir+0x52>
80107fd1:	e8 0c ab ff ff       	call   80102ae2 <kalloc>
80107fd6:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107fd9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107fdd:	75 07                	jne    80107fe6 <walkpgdir+0x59>
      return 0;
80107fdf:	b8 00 00 00 00       	mov    $0x0,%eax
80107fe4:	eb 47                	jmp    8010802d <walkpgdir+0xa0>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107fe6:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107fed:	00 
80107fee:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107ff5:	00 
80107ff6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ff9:	89 04 24             	mov    %eax,(%esp)
80107ffc:	e8 c6 d1 ff ff       	call   801051c7 <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
80108001:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108004:	89 04 24             	mov    %eax,(%esp)
80108007:	e8 f1 fa ff ff       	call   80107afd <v2p>
8010800c:	83 c8 07             	or     $0x7,%eax
8010800f:	89 c2                	mov    %eax,%edx
80108011:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108014:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80108016:	8b 45 0c             	mov    0xc(%ebp),%eax
80108019:	c1 e8 0c             	shr    $0xc,%eax
8010801c:	25 ff 03 00 00       	and    $0x3ff,%eax
80108021:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80108028:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010802b:	01 d0                	add    %edx,%eax
}
8010802d:	c9                   	leave  
8010802e:	c3                   	ret    

8010802f <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
8010802f:	55                   	push   %ebp
80108030:	89 e5                	mov    %esp,%ebp
80108032:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
80108035:	8b 45 0c             	mov    0xc(%ebp),%eax
80108038:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010803d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80108040:	8b 55 0c             	mov    0xc(%ebp),%edx
80108043:	8b 45 10             	mov    0x10(%ebp),%eax
80108046:	01 d0                	add    %edx,%eax
80108048:	83 e8 01             	sub    $0x1,%eax
8010804b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108050:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80108053:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
8010805a:	00 
8010805b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010805e:	89 44 24 04          	mov    %eax,0x4(%esp)
80108062:	8b 45 08             	mov    0x8(%ebp),%eax
80108065:	89 04 24             	mov    %eax,(%esp)
80108068:	e8 20 ff ff ff       	call   80107f8d <walkpgdir>
8010806d:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108070:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108074:	75 07                	jne    8010807d <mappages+0x4e>
      return -1;
80108076:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010807b:	eb 48                	jmp    801080c5 <mappages+0x96>
    if(*pte & PTE_P)
8010807d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108080:	8b 00                	mov    (%eax),%eax
80108082:	83 e0 01             	and    $0x1,%eax
80108085:	85 c0                	test   %eax,%eax
80108087:	74 0c                	je     80108095 <mappages+0x66>
      panic("remap");
80108089:	c7 04 24 f0 8e 10 80 	movl   $0x80108ef0,(%esp)
80108090:	e8 a5 84 ff ff       	call   8010053a <panic>
    *pte = pa | perm | PTE_P;
80108095:	8b 45 18             	mov    0x18(%ebp),%eax
80108098:	0b 45 14             	or     0x14(%ebp),%eax
8010809b:	83 c8 01             	or     $0x1,%eax
8010809e:	89 c2                	mov    %eax,%edx
801080a0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801080a3:	89 10                	mov    %edx,(%eax)
    if(a == last)
801080a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801080a8:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801080ab:	75 08                	jne    801080b5 <mappages+0x86>
      break;
801080ad:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
801080ae:	b8 00 00 00 00       	mov    $0x0,%eax
801080b3:	eb 10                	jmp    801080c5 <mappages+0x96>
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
    a += PGSIZE;
801080b5:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
801080bc:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
801080c3:	eb 8e                	jmp    80108053 <mappages+0x24>
  return 0;
}
801080c5:	c9                   	leave  
801080c6:	c3                   	ret    

801080c7 <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm(void)
{
801080c7:	55                   	push   %ebp
801080c8:	89 e5                	mov    %esp,%ebp
801080ca:	53                   	push   %ebx
801080cb:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
801080ce:	e8 0f aa ff ff       	call   80102ae2 <kalloc>
801080d3:	89 45 f0             	mov    %eax,-0x10(%ebp)
801080d6:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801080da:	75 0a                	jne    801080e6 <setupkvm+0x1f>
    return 0;
801080dc:	b8 00 00 00 00       	mov    $0x0,%eax
801080e1:	e9 98 00 00 00       	jmp    8010817e <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
801080e6:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801080ed:	00 
801080ee:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801080f5:	00 
801080f6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801080f9:	89 04 24             	mov    %eax,(%esp)
801080fc:	e8 c6 d0 ff ff       	call   801051c7 <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
80108101:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
80108108:	e8 fd f9 ff ff       	call   80107b0a <p2v>
8010810d:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
80108112:	76 0c                	jbe    80108120 <setupkvm+0x59>
    panic("PHYSTOP too high");
80108114:	c7 04 24 f6 8e 10 80 	movl   $0x80108ef6,(%esp)
8010811b:	e8 1a 84 ff ff       	call   8010053a <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80108120:	c7 45 f4 c0 b4 10 80 	movl   $0x8010b4c0,-0xc(%ebp)
80108127:	eb 49                	jmp    80108172 <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80108129:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010812c:	8b 48 0c             	mov    0xc(%eax),%ecx
8010812f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108132:	8b 50 04             	mov    0x4(%eax),%edx
80108135:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108138:	8b 58 08             	mov    0x8(%eax),%ebx
8010813b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010813e:	8b 40 04             	mov    0x4(%eax),%eax
80108141:	29 c3                	sub    %eax,%ebx
80108143:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108146:	8b 00                	mov    (%eax),%eax
80108148:	89 4c 24 10          	mov    %ecx,0x10(%esp)
8010814c:	89 54 24 0c          	mov    %edx,0xc(%esp)
80108150:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80108154:	89 44 24 04          	mov    %eax,0x4(%esp)
80108158:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010815b:	89 04 24             	mov    %eax,(%esp)
8010815e:	e8 cc fe ff ff       	call   8010802f <mappages>
80108163:	85 c0                	test   %eax,%eax
80108165:	79 07                	jns    8010816e <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80108167:	b8 00 00 00 00       	mov    $0x0,%eax
8010816c:	eb 10                	jmp    8010817e <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
8010816e:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80108172:	81 7d f4 00 b5 10 80 	cmpl   $0x8010b500,-0xc(%ebp)
80108179:	72 ae                	jb     80108129 <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
8010817b:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
8010817e:	83 c4 34             	add    $0x34,%esp
80108181:	5b                   	pop    %ebx
80108182:	5d                   	pop    %ebp
80108183:	c3                   	ret    

80108184 <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
80108184:	55                   	push   %ebp
80108185:	89 e5                	mov    %esp,%ebp
80108187:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
8010818a:	e8 38 ff ff ff       	call   801080c7 <setupkvm>
8010818f:	a3 78 56 11 80       	mov    %eax,0x80115678
  switchkvm();
80108194:	e8 02 00 00 00       	call   8010819b <switchkvm>
}
80108199:	c9                   	leave  
8010819a:	c3                   	ret    

8010819b <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
8010819b:	55                   	push   %ebp
8010819c:	89 e5                	mov    %esp,%ebp
8010819e:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
801081a1:	a1 78 56 11 80       	mov    0x80115678,%eax
801081a6:	89 04 24             	mov    %eax,(%esp)
801081a9:	e8 4f f9 ff ff       	call   80107afd <v2p>
801081ae:	89 04 24             	mov    %eax,(%esp)
801081b1:	e8 3c f9 ff ff       	call   80107af2 <lcr3>
}
801081b6:	c9                   	leave  
801081b7:	c3                   	ret    

801081b8 <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
801081b8:	55                   	push   %ebp
801081b9:	89 e5                	mov    %esp,%ebp
801081bb:	53                   	push   %ebx
801081bc:	83 ec 14             	sub    $0x14,%esp
  pushcli();
801081bf:	e8 03 cf ff ff       	call   801050c7 <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
801081c4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801081ca:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801081d1:	83 c2 08             	add    $0x8,%edx
801081d4:	89 d3                	mov    %edx,%ebx
801081d6:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801081dd:	83 c2 08             	add    $0x8,%edx
801081e0:	c1 ea 10             	shr    $0x10,%edx
801081e3:	89 d1                	mov    %edx,%ecx
801081e5:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801081ec:	83 c2 08             	add    $0x8,%edx
801081ef:	c1 ea 18             	shr    $0x18,%edx
801081f2:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
801081f9:	67 00 
801081fb:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
80108202:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
80108208:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
8010820f:	83 e1 f0             	and    $0xfffffff0,%ecx
80108212:	83 c9 09             	or     $0x9,%ecx
80108215:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010821b:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108222:	83 c9 10             	or     $0x10,%ecx
80108225:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010822b:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108232:	83 e1 9f             	and    $0xffffff9f,%ecx
80108235:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010823b:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108242:	83 c9 80             	or     $0xffffff80,%ecx
80108245:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010824b:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108252:	83 e1 f0             	and    $0xfffffff0,%ecx
80108255:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010825b:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108262:	83 e1 ef             	and    $0xffffffef,%ecx
80108265:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010826b:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108272:	83 e1 df             	and    $0xffffffdf,%ecx
80108275:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010827b:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108282:	83 c9 40             	or     $0x40,%ecx
80108285:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010828b:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80108292:	83 e1 7f             	and    $0x7f,%ecx
80108295:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
8010829b:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
801082a1:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801082a7:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
801082ae:	83 e2 ef             	and    $0xffffffef,%edx
801082b1:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
801082b7:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801082bd:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
801082c3:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801082c9:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
801082d0:	8b 52 08             	mov    0x8(%edx),%edx
801082d3:	81 c2 00 10 00 00    	add    $0x1000,%edx
801082d9:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
801082dc:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
801082e3:	e8 df f7 ff ff       	call   80107ac7 <ltr>
  if(p->pgdir == 0)
801082e8:	8b 45 08             	mov    0x8(%ebp),%eax
801082eb:	8b 40 04             	mov    0x4(%eax),%eax
801082ee:	85 c0                	test   %eax,%eax
801082f0:	75 0c                	jne    801082fe <switchuvm+0x146>
    panic("switchuvm: no pgdir");
801082f2:	c7 04 24 07 8f 10 80 	movl   $0x80108f07,(%esp)
801082f9:	e8 3c 82 ff ff       	call   8010053a <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
801082fe:	8b 45 08             	mov    0x8(%ebp),%eax
80108301:	8b 40 04             	mov    0x4(%eax),%eax
80108304:	89 04 24             	mov    %eax,(%esp)
80108307:	e8 f1 f7 ff ff       	call   80107afd <v2p>
8010830c:	89 04 24             	mov    %eax,(%esp)
8010830f:	e8 de f7 ff ff       	call   80107af2 <lcr3>
  popcli();
80108314:	e8 f2 cd ff ff       	call   8010510b <popcli>
}
80108319:	83 c4 14             	add    $0x14,%esp
8010831c:	5b                   	pop    %ebx
8010831d:	5d                   	pop    %ebp
8010831e:	c3                   	ret    

8010831f <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
8010831f:	55                   	push   %ebp
80108320:	89 e5                	mov    %esp,%ebp
80108322:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
80108325:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
8010832c:	76 0c                	jbe    8010833a <inituvm+0x1b>
    panic("inituvm: more than a page");
8010832e:	c7 04 24 1b 8f 10 80 	movl   $0x80108f1b,(%esp)
80108335:	e8 00 82 ff ff       	call   8010053a <panic>
  mem = kalloc();
8010833a:	e8 a3 a7 ff ff       	call   80102ae2 <kalloc>
8010833f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
80108342:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108349:	00 
8010834a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108351:	00 
80108352:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108355:	89 04 24             	mov    %eax,(%esp)
80108358:	e8 6a ce ff ff       	call   801051c7 <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
8010835d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108360:	89 04 24             	mov    %eax,(%esp)
80108363:	e8 95 f7 ff ff       	call   80107afd <v2p>
80108368:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
8010836f:	00 
80108370:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108374:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010837b:	00 
8010837c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108383:	00 
80108384:	8b 45 08             	mov    0x8(%ebp),%eax
80108387:	89 04 24             	mov    %eax,(%esp)
8010838a:	e8 a0 fc ff ff       	call   8010802f <mappages>
  memmove(mem, init, sz);
8010838f:	8b 45 10             	mov    0x10(%ebp),%eax
80108392:	89 44 24 08          	mov    %eax,0x8(%esp)
80108396:	8b 45 0c             	mov    0xc(%ebp),%eax
80108399:	89 44 24 04          	mov    %eax,0x4(%esp)
8010839d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083a0:	89 04 24             	mov    %eax,(%esp)
801083a3:	e8 ee ce ff ff       	call   80105296 <memmove>
}
801083a8:	c9                   	leave  
801083a9:	c3                   	ret    

801083aa <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
801083aa:	55                   	push   %ebp
801083ab:	89 e5                	mov    %esp,%ebp
801083ad:	53                   	push   %ebx
801083ae:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
801083b1:	8b 45 0c             	mov    0xc(%ebp),%eax
801083b4:	25 ff 0f 00 00       	and    $0xfff,%eax
801083b9:	85 c0                	test   %eax,%eax
801083bb:	74 0c                	je     801083c9 <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
801083bd:	c7 04 24 38 8f 10 80 	movl   $0x80108f38,(%esp)
801083c4:	e8 71 81 ff ff       	call   8010053a <panic>
  for(i = 0; i < sz; i += PGSIZE){
801083c9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801083d0:	e9 a9 00 00 00       	jmp    8010847e <loaduvm+0xd4>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
801083d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083d8:	8b 55 0c             	mov    0xc(%ebp),%edx
801083db:	01 d0                	add    %edx,%eax
801083dd:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801083e4:	00 
801083e5:	89 44 24 04          	mov    %eax,0x4(%esp)
801083e9:	8b 45 08             	mov    0x8(%ebp),%eax
801083ec:	89 04 24             	mov    %eax,(%esp)
801083ef:	e8 99 fb ff ff       	call   80107f8d <walkpgdir>
801083f4:	89 45 ec             	mov    %eax,-0x14(%ebp)
801083f7:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801083fb:	75 0c                	jne    80108409 <loaduvm+0x5f>
      panic("loaduvm: address should exist");
801083fd:	c7 04 24 5b 8f 10 80 	movl   $0x80108f5b,(%esp)
80108404:	e8 31 81 ff ff       	call   8010053a <panic>
    pa = PTE_ADDR(*pte);
80108409:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010840c:	8b 00                	mov    (%eax),%eax
8010840e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108413:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
80108416:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108419:	8b 55 18             	mov    0x18(%ebp),%edx
8010841c:	29 c2                	sub    %eax,%edx
8010841e:	89 d0                	mov    %edx,%eax
80108420:	3d ff 0f 00 00       	cmp    $0xfff,%eax
80108425:	77 0f                	ja     80108436 <loaduvm+0x8c>
      n = sz - i;
80108427:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010842a:	8b 55 18             	mov    0x18(%ebp),%edx
8010842d:	29 c2                	sub    %eax,%edx
8010842f:	89 d0                	mov    %edx,%eax
80108431:	89 45 f0             	mov    %eax,-0x10(%ebp)
80108434:	eb 07                	jmp    8010843d <loaduvm+0x93>
    else
      n = PGSIZE;
80108436:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
8010843d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108440:	8b 55 14             	mov    0x14(%ebp),%edx
80108443:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80108446:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108449:	89 04 24             	mov    %eax,(%esp)
8010844c:	e8 b9 f6 ff ff       	call   80107b0a <p2v>
80108451:	8b 55 f0             	mov    -0x10(%ebp),%edx
80108454:	89 54 24 0c          	mov    %edx,0xc(%esp)
80108458:	89 5c 24 08          	mov    %ebx,0x8(%esp)
8010845c:	89 44 24 04          	mov    %eax,0x4(%esp)
80108460:	8b 45 10             	mov    0x10(%ebp),%eax
80108463:	89 04 24             	mov    %eax,(%esp)
80108466:	e8 fd 98 ff ff       	call   80101d68 <readi>
8010846b:	3b 45 f0             	cmp    -0x10(%ebp),%eax
8010846e:	74 07                	je     80108477 <loaduvm+0xcd>
      return -1;
80108470:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80108475:	eb 18                	jmp    8010848f <loaduvm+0xe5>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
80108477:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
8010847e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108481:	3b 45 18             	cmp    0x18(%ebp),%eax
80108484:	0f 82 4b ff ff ff    	jb     801083d5 <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
8010848a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010848f:	83 c4 24             	add    $0x24,%esp
80108492:	5b                   	pop    %ebx
80108493:	5d                   	pop    %ebp
80108494:	c3                   	ret    

80108495 <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80108495:	55                   	push   %ebp
80108496:	89 e5                	mov    %esp,%ebp
80108498:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
8010849b:	8b 45 10             	mov    0x10(%ebp),%eax
8010849e:	85 c0                	test   %eax,%eax
801084a0:	79 0a                	jns    801084ac <allocuvm+0x17>
    return 0;
801084a2:	b8 00 00 00 00       	mov    $0x0,%eax
801084a7:	e9 c1 00 00 00       	jmp    8010856d <allocuvm+0xd8>
  if(newsz < oldsz)
801084ac:	8b 45 10             	mov    0x10(%ebp),%eax
801084af:	3b 45 0c             	cmp    0xc(%ebp),%eax
801084b2:	73 08                	jae    801084bc <allocuvm+0x27>
    return oldsz;
801084b4:	8b 45 0c             	mov    0xc(%ebp),%eax
801084b7:	e9 b1 00 00 00       	jmp    8010856d <allocuvm+0xd8>

  a = PGROUNDUP(oldsz);
801084bc:	8b 45 0c             	mov    0xc(%ebp),%eax
801084bf:	05 ff 0f 00 00       	add    $0xfff,%eax
801084c4:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801084c9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
801084cc:	e9 8d 00 00 00       	jmp    8010855e <allocuvm+0xc9>
    mem = kalloc();
801084d1:	e8 0c a6 ff ff       	call   80102ae2 <kalloc>
801084d6:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
801084d9:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801084dd:	75 2c                	jne    8010850b <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
801084df:	c7 04 24 79 8f 10 80 	movl   $0x80108f79,(%esp)
801084e6:	e8 b5 7e ff ff       	call   801003a0 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
801084eb:	8b 45 0c             	mov    0xc(%ebp),%eax
801084ee:	89 44 24 08          	mov    %eax,0x8(%esp)
801084f2:	8b 45 10             	mov    0x10(%ebp),%eax
801084f5:	89 44 24 04          	mov    %eax,0x4(%esp)
801084f9:	8b 45 08             	mov    0x8(%ebp),%eax
801084fc:	89 04 24             	mov    %eax,(%esp)
801084ff:	e8 6b 00 00 00       	call   8010856f <deallocuvm>
      return 0;
80108504:	b8 00 00 00 00       	mov    $0x0,%eax
80108509:	eb 62                	jmp    8010856d <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
8010850b:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108512:	00 
80108513:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010851a:	00 
8010851b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010851e:	89 04 24             	mov    %eax,(%esp)
80108521:	e8 a1 cc ff ff       	call   801051c7 <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
80108526:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108529:	89 04 24             	mov    %eax,(%esp)
8010852c:	e8 cc f5 ff ff       	call   80107afd <v2p>
80108531:	8b 55 f4             	mov    -0xc(%ebp),%edx
80108534:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
8010853b:	00 
8010853c:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108540:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108547:	00 
80108548:	89 54 24 04          	mov    %edx,0x4(%esp)
8010854c:	8b 45 08             	mov    0x8(%ebp),%eax
8010854f:	89 04 24             	mov    %eax,(%esp)
80108552:	e8 d8 fa ff ff       	call   8010802f <mappages>
    return 0;
  if(newsz < oldsz)
    return oldsz;

  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
80108557:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
8010855e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108561:	3b 45 10             	cmp    0x10(%ebp),%eax
80108564:	0f 82 67 ff ff ff    	jb     801084d1 <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
8010856a:	8b 45 10             	mov    0x10(%ebp),%eax
}
8010856d:	c9                   	leave  
8010856e:	c3                   	ret    

8010856f <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
8010856f:	55                   	push   %ebp
80108570:	89 e5                	mov    %esp,%ebp
80108572:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
80108575:	8b 45 10             	mov    0x10(%ebp),%eax
80108578:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010857b:	72 08                	jb     80108585 <deallocuvm+0x16>
    return oldsz;
8010857d:	8b 45 0c             	mov    0xc(%ebp),%eax
80108580:	e9 a4 00 00 00       	jmp    80108629 <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
80108585:	8b 45 10             	mov    0x10(%ebp),%eax
80108588:	05 ff 0f 00 00       	add    $0xfff,%eax
8010858d:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108592:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80108595:	e9 80 00 00 00       	jmp    8010861a <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
8010859a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010859d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801085a4:	00 
801085a5:	89 44 24 04          	mov    %eax,0x4(%esp)
801085a9:	8b 45 08             	mov    0x8(%ebp),%eax
801085ac:	89 04 24             	mov    %eax,(%esp)
801085af:	e8 d9 f9 ff ff       	call   80107f8d <walkpgdir>
801085b4:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
801085b7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801085bb:	75 09                	jne    801085c6 <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
801085bd:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
801085c4:	eb 4d                	jmp    80108613 <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
801085c6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085c9:	8b 00                	mov    (%eax),%eax
801085cb:	83 e0 01             	and    $0x1,%eax
801085ce:	85 c0                	test   %eax,%eax
801085d0:	74 41                	je     80108613 <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
801085d2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801085d5:	8b 00                	mov    (%eax),%eax
801085d7:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801085dc:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
801085df:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801085e3:	75 0c                	jne    801085f1 <deallocuvm+0x82>
        panic("kfree");
801085e5:	c7 04 24 91 8f 10 80 	movl   $0x80108f91,(%esp)
801085ec:	e8 49 7f ff ff       	call   8010053a <panic>
      char *v = p2v(pa);
801085f1:	8b 45 ec             	mov    -0x14(%ebp),%eax
801085f4:	89 04 24             	mov    %eax,(%esp)
801085f7:	e8 0e f5 ff ff       	call   80107b0a <p2v>
801085fc:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
801085ff:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108602:	89 04 24             	mov    %eax,(%esp)
80108605:	e8 3f a4 ff ff       	call   80102a49 <kfree>
      *pte = 0;
8010860a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010860d:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
80108613:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
8010861a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010861d:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108620:	0f 82 74 ff ff ff    	jb     8010859a <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
80108626:	8b 45 10             	mov    0x10(%ebp),%eax
}
80108629:	c9                   	leave  
8010862a:	c3                   	ret    

8010862b <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
8010862b:	55                   	push   %ebp
8010862c:	89 e5                	mov    %esp,%ebp
8010862e:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
80108631:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80108635:	75 0c                	jne    80108643 <freevm+0x18>
    panic("freevm: no pgdir");
80108637:	c7 04 24 97 8f 10 80 	movl   $0x80108f97,(%esp)
8010863e:	e8 f7 7e ff ff       	call   8010053a <panic>
  deallocuvm(pgdir, KERNBASE, 0);
80108643:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010864a:	00 
8010864b:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
80108652:	80 
80108653:	8b 45 08             	mov    0x8(%ebp),%eax
80108656:	89 04 24             	mov    %eax,(%esp)
80108659:	e8 11 ff ff ff       	call   8010856f <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
8010865e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80108665:	eb 48                	jmp    801086af <freevm+0x84>
    if(pgdir[i] & PTE_P){
80108667:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010866a:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80108671:	8b 45 08             	mov    0x8(%ebp),%eax
80108674:	01 d0                	add    %edx,%eax
80108676:	8b 00                	mov    (%eax),%eax
80108678:	83 e0 01             	and    $0x1,%eax
8010867b:	85 c0                	test   %eax,%eax
8010867d:	74 2c                	je     801086ab <freevm+0x80>
      char * v = p2v(PTE_ADDR(pgdir[i]));
8010867f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108682:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80108689:	8b 45 08             	mov    0x8(%ebp),%eax
8010868c:	01 d0                	add    %edx,%eax
8010868e:	8b 00                	mov    (%eax),%eax
80108690:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108695:	89 04 24             	mov    %eax,(%esp)
80108698:	e8 6d f4 ff ff       	call   80107b0a <p2v>
8010869d:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
801086a0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801086a3:	89 04 24             	mov    %eax,(%esp)
801086a6:	e8 9e a3 ff ff       	call   80102a49 <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
801086ab:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801086af:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
801086b6:	76 af                	jbe    80108667 <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
801086b8:	8b 45 08             	mov    0x8(%ebp),%eax
801086bb:	89 04 24             	mov    %eax,(%esp)
801086be:	e8 86 a3 ff ff       	call   80102a49 <kfree>
}
801086c3:	c9                   	leave  
801086c4:	c3                   	ret    

801086c5 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
801086c5:	55                   	push   %ebp
801086c6:	89 e5                	mov    %esp,%ebp
801086c8:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801086cb:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801086d2:	00 
801086d3:	8b 45 0c             	mov    0xc(%ebp),%eax
801086d6:	89 44 24 04          	mov    %eax,0x4(%esp)
801086da:	8b 45 08             	mov    0x8(%ebp),%eax
801086dd:	89 04 24             	mov    %eax,(%esp)
801086e0:	e8 a8 f8 ff ff       	call   80107f8d <walkpgdir>
801086e5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
801086e8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801086ec:	75 0c                	jne    801086fa <clearpteu+0x35>
    panic("clearpteu");
801086ee:	c7 04 24 a8 8f 10 80 	movl   $0x80108fa8,(%esp)
801086f5:	e8 40 7e ff ff       	call   8010053a <panic>
  *pte &= ~PTE_U;
801086fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801086fd:	8b 00                	mov    (%eax),%eax
801086ff:	83 e0 fb             	and    $0xfffffffb,%eax
80108702:	89 c2                	mov    %eax,%edx
80108704:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108707:	89 10                	mov    %edx,(%eax)
}
80108709:	c9                   	leave  
8010870a:	c3                   	ret    

8010870b <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
8010870b:	55                   	push   %ebp
8010870c:	89 e5                	mov    %esp,%ebp
8010870e:	53                   	push   %ebx
8010870f:	83 ec 44             	sub    $0x44,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80108712:	e8 b0 f9 ff ff       	call   801080c7 <setupkvm>
80108717:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010871a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010871e:	75 0a                	jne    8010872a <copyuvm+0x1f>
    return 0;
80108720:	b8 00 00 00 00       	mov    $0x0,%eax
80108725:	e9 fd 00 00 00       	jmp    80108827 <copyuvm+0x11c>
  for(i = 0; i < sz; i += PGSIZE){
8010872a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80108731:	e9 d0 00 00 00       	jmp    80108806 <copyuvm+0xfb>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
80108736:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108739:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108740:	00 
80108741:	89 44 24 04          	mov    %eax,0x4(%esp)
80108745:	8b 45 08             	mov    0x8(%ebp),%eax
80108748:	89 04 24             	mov    %eax,(%esp)
8010874b:	e8 3d f8 ff ff       	call   80107f8d <walkpgdir>
80108750:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108753:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108757:	75 0c                	jne    80108765 <copyuvm+0x5a>
      panic("copyuvm: pte should exist");
80108759:	c7 04 24 b2 8f 10 80 	movl   $0x80108fb2,(%esp)
80108760:	e8 d5 7d ff ff       	call   8010053a <panic>
    if(!(*pte & PTE_P))
80108765:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108768:	8b 00                	mov    (%eax),%eax
8010876a:	83 e0 01             	and    $0x1,%eax
8010876d:	85 c0                	test   %eax,%eax
8010876f:	75 0c                	jne    8010877d <copyuvm+0x72>
      panic("copyuvm: page not present");
80108771:	c7 04 24 cc 8f 10 80 	movl   $0x80108fcc,(%esp)
80108778:	e8 bd 7d ff ff       	call   8010053a <panic>
    pa = PTE_ADDR(*pte);
8010877d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108780:	8b 00                	mov    (%eax),%eax
80108782:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108787:	89 45 e8             	mov    %eax,-0x18(%ebp)
    flags = PTE_FLAGS(*pte);
8010878a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010878d:	8b 00                	mov    (%eax),%eax
8010878f:	25 ff 0f 00 00       	and    $0xfff,%eax
80108794:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if((mem = kalloc()) == 0)
80108797:	e8 46 a3 ff ff       	call   80102ae2 <kalloc>
8010879c:	89 45 e0             	mov    %eax,-0x20(%ebp)
8010879f:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
801087a3:	75 02                	jne    801087a7 <copyuvm+0x9c>
      goto bad;
801087a5:	eb 70                	jmp    80108817 <copyuvm+0x10c>
    memmove(mem, (char*)p2v(pa), PGSIZE);
801087a7:	8b 45 e8             	mov    -0x18(%ebp),%eax
801087aa:	89 04 24             	mov    %eax,(%esp)
801087ad:	e8 58 f3 ff ff       	call   80107b0a <p2v>
801087b2:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801087b9:	00 
801087ba:	89 44 24 04          	mov    %eax,0x4(%esp)
801087be:	8b 45 e0             	mov    -0x20(%ebp),%eax
801087c1:	89 04 24             	mov    %eax,(%esp)
801087c4:	e8 cd ca ff ff       	call   80105296 <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
801087c9:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
801087cc:	8b 45 e0             	mov    -0x20(%ebp),%eax
801087cf:	89 04 24             	mov    %eax,(%esp)
801087d2:	e8 26 f3 ff ff       	call   80107afd <v2p>
801087d7:	8b 55 f4             	mov    -0xc(%ebp),%edx
801087da:	89 5c 24 10          	mov    %ebx,0x10(%esp)
801087de:	89 44 24 0c          	mov    %eax,0xc(%esp)
801087e2:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801087e9:	00 
801087ea:	89 54 24 04          	mov    %edx,0x4(%esp)
801087ee:	8b 45 f0             	mov    -0x10(%ebp),%eax
801087f1:	89 04 24             	mov    %eax,(%esp)
801087f4:	e8 36 f8 ff ff       	call   8010802f <mappages>
801087f9:	85 c0                	test   %eax,%eax
801087fb:	79 02                	jns    801087ff <copyuvm+0xf4>
      goto bad;
801087fd:	eb 18                	jmp    80108817 <copyuvm+0x10c>
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
801087ff:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108806:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108809:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010880c:	0f 82 24 ff ff ff    	jb     80108736 <copyuvm+0x2b>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
      goto bad;
  }
  return d;
80108812:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108815:	eb 10                	jmp    80108827 <copyuvm+0x11c>

bad:
  freevm(d);
80108817:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010881a:	89 04 24             	mov    %eax,(%esp)
8010881d:	e8 09 fe ff ff       	call   8010862b <freevm>
  return 0;
80108822:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108827:	83 c4 44             	add    $0x44,%esp
8010882a:	5b                   	pop    %ebx
8010882b:	5d                   	pop    %ebp
8010882c:	c3                   	ret    

8010882d <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
8010882d:	55                   	push   %ebp
8010882e:	89 e5                	mov    %esp,%ebp
80108830:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80108833:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010883a:	00 
8010883b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010883e:	89 44 24 04          	mov    %eax,0x4(%esp)
80108842:	8b 45 08             	mov    0x8(%ebp),%eax
80108845:	89 04 24             	mov    %eax,(%esp)
80108848:	e8 40 f7 ff ff       	call   80107f8d <walkpgdir>
8010884d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
80108850:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108853:	8b 00                	mov    (%eax),%eax
80108855:	83 e0 01             	and    $0x1,%eax
80108858:	85 c0                	test   %eax,%eax
8010885a:	75 07                	jne    80108863 <uva2ka+0x36>
    return 0;
8010885c:	b8 00 00 00 00       	mov    $0x0,%eax
80108861:	eb 25                	jmp    80108888 <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
80108863:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108866:	8b 00                	mov    (%eax),%eax
80108868:	83 e0 04             	and    $0x4,%eax
8010886b:	85 c0                	test   %eax,%eax
8010886d:	75 07                	jne    80108876 <uva2ka+0x49>
    return 0;
8010886f:	b8 00 00 00 00       	mov    $0x0,%eax
80108874:	eb 12                	jmp    80108888 <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
80108876:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108879:	8b 00                	mov    (%eax),%eax
8010887b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108880:	89 04 24             	mov    %eax,(%esp)
80108883:	e8 82 f2 ff ff       	call   80107b0a <p2v>
}
80108888:	c9                   	leave  
80108889:	c3                   	ret    

8010888a <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
8010888a:	55                   	push   %ebp
8010888b:	89 e5                	mov    %esp,%ebp
8010888d:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
80108890:	8b 45 10             	mov    0x10(%ebp),%eax
80108893:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
80108896:	e9 87 00 00 00       	jmp    80108922 <copyout+0x98>
    va0 = (uint)PGROUNDDOWN(va);
8010889b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010889e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801088a3:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
801088a6:	8b 45 ec             	mov    -0x14(%ebp),%eax
801088a9:	89 44 24 04          	mov    %eax,0x4(%esp)
801088ad:	8b 45 08             	mov    0x8(%ebp),%eax
801088b0:	89 04 24             	mov    %eax,(%esp)
801088b3:	e8 75 ff ff ff       	call   8010882d <uva2ka>
801088b8:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
801088bb:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801088bf:	75 07                	jne    801088c8 <copyout+0x3e>
      return -1;
801088c1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801088c6:	eb 69                	jmp    80108931 <copyout+0xa7>
    n = PGSIZE - (va - va0);
801088c8:	8b 45 0c             	mov    0xc(%ebp),%eax
801088cb:	8b 55 ec             	mov    -0x14(%ebp),%edx
801088ce:	29 c2                	sub    %eax,%edx
801088d0:	89 d0                	mov    %edx,%eax
801088d2:	05 00 10 00 00       	add    $0x1000,%eax
801088d7:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
801088da:	8b 45 f0             	mov    -0x10(%ebp),%eax
801088dd:	3b 45 14             	cmp    0x14(%ebp),%eax
801088e0:	76 06                	jbe    801088e8 <copyout+0x5e>
      n = len;
801088e2:	8b 45 14             	mov    0x14(%ebp),%eax
801088e5:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
801088e8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801088eb:	8b 55 0c             	mov    0xc(%ebp),%edx
801088ee:	29 c2                	sub    %eax,%edx
801088f0:	8b 45 e8             	mov    -0x18(%ebp),%eax
801088f3:	01 c2                	add    %eax,%edx
801088f5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801088f8:	89 44 24 08          	mov    %eax,0x8(%esp)
801088fc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801088ff:	89 44 24 04          	mov    %eax,0x4(%esp)
80108903:	89 14 24             	mov    %edx,(%esp)
80108906:	e8 8b c9 ff ff       	call   80105296 <memmove>
    len -= n;
8010890b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010890e:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
80108911:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108914:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
80108917:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010891a:	05 00 10 00 00       	add    $0x1000,%eax
8010891f:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
80108922:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80108926:	0f 85 6f ff ff ff    	jne    8010889b <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
8010892c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108931:	c9                   	leave  
80108932:	c3                   	ret    
